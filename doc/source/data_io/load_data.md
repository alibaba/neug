...
| Option       | Type  | Default        | Description |
| ------------ | ----- | -------------- | ----------- |
| `parallel`   | bool  | `true` | Enable parallel reading using multiple threads (max core number). When enabled for Parquet files, row groups are scanned concurrently and row order is **not** preserved. |
