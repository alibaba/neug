...
> **Note:** Most algorithms (except Label Propagation) require a **simple graph**
...
Propagation also allows multiple vertex labels in the projected subgraph
(unlike most other algorithms).
...
| Algorithm | CALL Name | Predicates | Key Options |
|---|---|---|---|
| PageRank | `page_rank` | vertex only | `damping_factor`, `max_iterations`, `directed` |
| BFS | `bfs` | none | `source` (required) |
| SSSP | `sssp` | none | `source` (required), `weight`, `directed` |
| WCC | `wcc` | none | `concurrency` |
| LCC | `lcc` | none | `directed`, `degree_threshold` |
| K-Core | `kcore` | none | `k` |
| Label Propagation | `label_propagation` | vertex + edge | `max_iterations` |
| Personalized PageRank | `personalized_page_rank` | none | `edge_weight`, `node_weight` (not yet registered) |
...
- **Most algorithms:** defaults to the number of CPU cores (`std::thread::hardware_concurrency()`)
- **Label Propagation, Personalized PageRank:** defaults to `1`

## Limitations

- Most algorithms require a **simple graph** subgraph (exactly one vertex label
  and one homogeneous edge triplet `[A, edge, A]`). Label Propagation is the
  exception.
- Edge predicates are only supported by Label Propagation. Other algorithms will
  reject subgraphs that include edge predicates.
