...
try (Session session = driver.session()) {
    ...
    try {
        session.run("CREATE (:Person {id: 1, name: 'Alice'})").close();
        try (ResultSet rs = session.run(
