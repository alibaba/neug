...
Commit the active explicit transaction. Persistent `COPY FROM` statements are
published through one checkpoint; other writes use the ordinary logical-WAL
commit path. A rollback-only transaction must be rolled back instead.
