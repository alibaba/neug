...
✅ available · Planned: a later release · — no client for that mode.

The two mode sections below therefore show different languages.
...
| Rejected | Concretely |
|----------|------------|
| Bulk and file loading | `COPY … FROM`, `COPY … TO`, `COPY TEMP`, `LOAD FROM` |
| Temporary schema | `CREATE TEMP …`, and the temporary table that `COPY TEMP` creates |
...
also exposes a read-only variant of its begin call. For the exact signatures and
the transaction-control methods of each client, see the
...
For the transaction-control methods each service-mode client exposes, see the
[API reference](../../reference).

**Using the session.** Every query issued on the session runs inside its active
transaction, and closing the session tries to roll that transaction back. Never
issue concurrent requests through one `Session` — use one `Session` per client
worker. Schema lookups always show the service's committed schema, never a
...
**Lifetime.** Unlike AP, the service can end a transaction on its own — through
an absolute service-configured lifetime, a failure, or shutdown. The session then
drops its handle and reports no active transaction, and a rollback call fails. So
guard the rollback instead of issuing it unconditionally:

```python
if session.has_active_transaction:  # Python example
    session.rollback()
session.begin_transaction()  # The session is ready for a new transaction.
```

**Ending a rollback-only transaction.** Roll it back while the session still
reports an active transaction, as in the snippet above.
