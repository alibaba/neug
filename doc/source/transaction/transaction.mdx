...
programmatic explicit transactions for AP mode. Python `Session` supports
them for TP service mode, as described below.
...
transactions are rolled back and ended. When the service reports that a
transaction has ended, `Session` clears its local handle so it can begin a new
transaction. Clients must reliably finish a
