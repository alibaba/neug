...
Auto-commit remains the default in every supported interface. The embedded
C++, Node.js, and Python `Connection` APIs support programmatic explicit
transactions for AP mode. Python `Session` supports them for TP service mode,
as described below.

#### Common Explicit Transaction Semantics

An explicit transaction spans multiple queries: begin a transaction, execute
queries within it, then commit or roll it back. A read-only transaction pins
one read view and rejects writes. A read-write transaction owns a private COW
view, reads its own successful writes, and publishes its changes only when
commit succeeds.

An ordinary statement that fails after reaching the database makes the
transaction rollback-only. This includes analysis, syntax or compilation,
execution, read-only writes, and rejected bulk or maintenance operations.
Errors rejected by a client binding before the statement reaches the database
do not change the transaction; for example, Python rejects an invalid
`access_mode` with `ValueError`. In the rollback-only state, call the
client-specific rollback method before issuing another query or beginning
another transaction. The active-transaction property remains true while the
transaction is rollback-only. Nested begin and invalid programmatic control
calls report a transaction-state error without changing an otherwise active
transaction.

Cypher `BEGIN`, `COMMIT`, and `ROLLBACK` are unsupported. Submitting one as a
query is a failed ordinary statement and makes the active transaction
rollback-only; use the client-specific transaction-control methods instead.
...
For a read-only transaction, use `TransactionMode::kReadOnly` in C++,
`beginTransaction({ readOnly: true })` in Node.js, or
`begin_transaction(read_only=True)` in Python. C++ uses
`TransactionMode::kReadWrite` for read-write transactions; that is the default
mode in Node.js and Python.

An AP read-write transaction holds exclusive AP admission for its full
lifetime. Any AP operation that contends for that admission waits until the
transaction is committed, rolled back, or its connection is closed; explicit
transactions currently have no timeout. Applications should therefore finish
them promptly. Embedded READ_WRITE mode currently allows only one public
`Connection` at a time.

The embedded API does not include remote clients, Java bindings, savepoints,
transaction timeouts, or explicit-transaction `COPY`, batch, index,
...
This guide documents that client-level contract; service URIs, headers, status
codes, and request or response payloads are transport details rather than
public transaction semantics.

While a TP read-write transaction owns the TP update lease, another read-write
transaction cannot begin; ordinary TP inserts and updates wait for it to finish,
while TP reads remain concurrent.
A successful TP read-write commit appends and publishes one WAL commit.

Use the client-specific API below to begin, execute within, commit, or roll
back an explicit transaction:
...
Pass `read_only=True` to `begin_transaction()` to pin a TP read view and reject
writes. One `Session` owns at most one explicit transaction. Do not issue
concurrent requests through that session; use one `Session` per client worker.
`execute()` uses that transaction while it is active, and `close()` attempts to
roll it back. `get_schema()` always reflects the service's committed schema; it
does not expose a transaction-private schema view.

TP transactions may have an absolute service-configured lifetime. Expired,
failed, or shutdown transactions are rolled back and ended. When the service
reports that a transaction has ended, `Session` clears its local handle so it
can begin a new transaction. Clients must reliably finish a read-write
transaction: an abandoned transaction can keep the TP update lease and delay
ordinary TP writes until the service releases it.
