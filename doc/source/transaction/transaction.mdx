...
[Python](../../reference/python_api/connection/),
[Node.js](../../reference/nodejs_api/connection/), and
[C++](../../reference/cpp_api/connection/) connection references.
...
[Python Session reference](../../reference/python_api/session/) and
[Java transaction API](../../reference/java_api/transaction/).
...
Since v0.2, committed ordinary writes are appended to the WAL and recovered
automatically after restart. Persistent direct bulk operations, such as `COPY`,
publish a checkpoint before returning success; temporary bulk writes remain
in memory only. An explicit checkpoint materializes the current state and
consolidates WAL, but is not required for ordinary commit durability. See
[Checkpoints](checkpoint.md) for lifecycle and recovery details.
