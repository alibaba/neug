...
class VertexTableTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    property_values_ = {neug::columnar::Value::STRING("Alice"),
                        neug::columnar::Value::INT32(30),
                        neug::columnar::Value::DOUBLE(88.5)};
    default_prop_values_ = {neug::columnar::Value::STRING(""),
                            neug::columnar::Value::INT32(0),
                            neug::columnar::Value::DOUBLE(0.0)};
  ...
  std::vector<std::shared_ptr<neug::columnar::DataChunk>> generate_data_chunks(
      size_t num_vertices) {
  ...
  std::vector<neug::columnar::Value> property_values_;
  std::vector<neug::columnar::Value> default_prop_values_;
...
TEST_F(VertexTableTest, VertexTableBasicOps) {
  ...
  auto oid1 = neug::columnar::Value::INT64(1);
  auto oid2 = neug::columnar::Value::INT64(2);
  auto oid3 = neug::columnar::Value::INT64(3);
...
TEST_F(VertexTableTest, VertexTableAddAndDeleteAndReload) {
  ...
  neug::vid_t lid1, lid2, lid3;
  neug::columnar::Value oid1, oid2, oid3;
  neug::CheckpointManifest desc;
  {
    neug::VertexTable table(schema_.get_vertex_schema(v_label_id_));
    OpenVertexTableLegacy(table, ckp, neug::CheckpointManifest(),
    ...
    oid1 = neug::columnar::Value::INT64(1);
    oid2 = neug::columnar::Value::INT64(2);
    oid3 = neug::columnar::Value::INT64(3);
    EXPECT_TRUE(table.AddVertex(oid1, property_values_, lid1, 1, false));
    EXPECT_TRUE(table.AddVertex(oid2, property_values_, lid2, 2, false));
    EXPECT_TRUE(table.AddVertex(oid3, property_values_, lid3, 3, false));
    LOG(INFO) << "Added vertices with lids: " << lid1 << ", " << lid2 << ", "
...
TEST_F(VertexTableTest, AddVertexBasic) {
  ...
  auto oid1 = neug::columnar::Value::INT64(100);
  auto oid2 = neug::columnar::Value::INT64(200);
  auto oid3 = neug::columnar::Value::INT64(300);
  neug::vid_t lid1, lid2, lid3;
...
TEST_F(VertexTableTest, AddVertex) {
  ...
  neug::vid_t tmp_vid;
  EXPECT_FALSE(table.AddVertex(neug::columnar::Value::INT64(1),
                               property_values_, tmp_vid, 0, false));

  std::vector<neug::columnar::Value> oids;
  std::vector<neug::vid_t> lids;
  table.EnsureCapacity(100);
  lids.resize(100);

  for (int64_t i = 0; i < 100; ++i) {
    auto oid = neug::columnar::Value::INT64(i);
    oids.push_back(oid);
...
TEST_F(VertexTableTest, MultipleDeletesAndReverts) {
  ...
  auto oid = neug::columnar::Value::INT64(42);
  neug::vid_t lid;
...
TEST_F(VertexTableTest, DeleteAlreadyDeletedVertex) {
  ...
  auto oid = neug::columnar::Value::INT64(1);
  neug::vid_t lid;
  EXPECT_TRUE(table.AddVertex(oid, property_values_, lid, 1, false));
