 ...
 protected:
  ...
  void build_indexer(neug::LFIndexer<neug::vid_t>& indexer, neug::vid_t num,
    ...
    for (neug::vid_t i = 0; i < num; ++i) {
      indexer.insert(neug::columnar::Value::INT64(i), i);
    }
  }
  ...
  void BatchInsert(
      std::vector<std::shared_ptr<neug::columnar::DataChunk>>&& chunks) {
  ...
  neug::vid_t GetSrcLid(const neug::columnar::Value& src_oid) {
    neug::vid_t src_lid;
  ...
  neug::vid_t GetDstLid(const neug::columnar::Value& dst_oid) {
    neug::vid_t dst_lid;
...
TEST_F(EdgeTableTest, TestDeleteEdge) {
  ...
  for (size_t i = 0; i < edge_num; ++i) {
    if (i % 10 == 0) {
      neug::vid_t src_lid =
          GetSrcLid(neug::columnar::Value::INT64(src_list[i]));
      neug::vid_t dst_lid =
          GetDstLid(neug::columnar::Value::INT64(dst_list[i]));
...
TEST_F(EdgeTableTest, TestBatchAddEdgesBundled) {
  ...
  std::vector<std::vector<neug::columnar::Value>> edge_data;
  for (size_t i = 0; i < more_edge_num; ++i) {
    edge_data.push_back({neug::columnar::Value::INT32(more_data_list[i])});
  }
...
TEST_F(EdgeTableTest, TestBatchAddEdgesUnbundled) {
  ...
  std::vector<std::vector<neug::columnar::Value>> edge_data;
  for (size_t i = 0; i < more_edge_num; ++i) {
    edge_data.push_back({neug::columnar::Value::STRING(more_data_list0[i]),
                         neug::columnar::Value::INT32(more_data_list1[i])});
  }
...
TEST_F(EdgeTableTest, TestAddEdgeAndDelete) {
  ...
  for (auto src_oid : src_oids) {
    neug::vid_t src_lid =
        this->src_indexer.insert(neug::columnar::Value::INT64(src_oid), true);
    src_lids.push_back(src_lid);
  }
  for (auto dst_oid : dst_oids) {
    neug::vid_t dst_lid =
        this->dst_indexer.insert(neug::columnar::Value::INT64(dst_oid), true);
  ...
  for (size_t i = 0; i < src_lids.size(); ++i) {
    edge_data.push_back({neug::columnar::Value::INT32(static_cast<int>(i))});
  }
...
TEST_F(EdgeTableTest, TestAddEdgeDeleteUnbundled) {
  ...
  std::vector<std::vector<neug::columnar::Value>> edge_data;
  for (size_t i = 0; i < src_lids.size(); ++i) {
    edge_data.push_back({neug::columnar::Value::STRING("edge_data"),
                         neug::columnar::Value::INT32(static_cast<int>(i))});
  }
...
TEST_F(EdgeTableTest, TestUpdateEdgeData) {
  ...
  std::vector<std::vector<neug::columnar::Value>> edge_data;
  for (size_t i = 0; i < src_lids.size(); ++i) {
    edge_data.push_back({neug::columnar::Value::STRING("old_data"),
                         neug::columnar::Value::INT32(static_cast<int>(0))});
  }
  ...
  this->ExpectUnbundledStats(edge_num, 4096);
  std::vector<neug::columnar::Value> new_data = {
      neug::columnar::Value::STRING(std::string("new_data")),
      neug::columnar::Value::INT32(static_cast<int>(1))};
  auto oe_view = this->edge_table->get_outgoing_view(neug::MAX_TIMESTAMP);
  auto ie_view = this->edge_table->get_incoming_view(neug::MAX_TIMESTAMP);
...
TEST_F(EdgeTableTest, TestAddPropertiesTransitionFromEmptyToBundledUnbundled) {
  ...
  schema_.AddEdgeProperties(
      ...
      {neug::columnar::Value::STRING(std::string("new-tag"))});
  this->edge_table->SetEdgeSchema(
      schema_.get_edge_schema(src_label_, dst_label_, edge_label_empty_));
...
TEST_F(EdgeTableTest, TestAddStringPropertyTransitionFromEmptyToUnbundled) {
  ...
  schema_.get_edge_schema(src_label_, dst_label_, edge_label_empty_)
      ->add_properties({"tag"}, {neug::DataTypeId::kVarchar},
                       {neug::columnar::Value::STRING(std::string("seed"))});
  this->edge_table->AddProperties(
      *ckp, {"tag"}, {neug::DataTypeId::kVarchar},
      {neug::columnar::Value::STRING(std::string("seed"))});
  schema_.get_edge_schema(src_label_, dst_label_, edge_label_empty_)
      ->add_properties({"desc"}, {neug::DataTypeId::kVarchar},
                       {neug::columnar::Value::STRING(std::string("unknown"))});
  this->edge_table->AddProperties(
...
TEST_F(EdgeTableTest,
  ...
  for (const auto& [src_oid, dst_oid, data0, data1] : input) {
    auto src_lid = this->GetSrcLid(neug::columnar::Value::INT64(src_oid));
    auto dst_lid = this->GetDstLid(neug::columnar::Value::INT64(dst_oid));
    this->edge_table->AddEdge(
        src_lid, dst_lid,
        {neug::columnar::Value::STRING(std::string(data0)),
         neug::columnar::Value::INT32(data1)},
        0, allocator, false);
  }
  ...
  {
    auto src_lid = this->GetSrcLid(neug::columnar::Value::INT64(3));
    auto dst_lid = this->GetDstLid(neug::columnar::Value::INT64(0));
...
TEST_F(EdgeTableTest, TestDeletePropertiesTransitionFromUnbundledToBundled) {
  ...
  for (const auto& [src_oid, dst_oid, data0, data1] : input) {
    auto src_lid = this->GetSrcLid(neug::columnar::Value::INT64(src_oid));
    auto dst_lid = this->GetDstLid(neug::columnar::Value::INT64(dst_oid));
    this->edge_table->AddEdge(src_lid, dst_lid,
                              {neug::columnar::Value::STRING(data0),
                               neug::columnar::Value::INT32(data1)},
                              0, allocator, false);
  }
  this->ExpectUnbundledStats(input.size(), 4096);
  ...
  this->ExpectBundledStats(input.size());
  {
    ...
    auto dst_lid = this->GetDstLid(neug::columnar::Value::INT64(0));
    this->edge_table->AddEdge(src_lid, dst_lid,
                              {neug::columnar::Value::INT32(44)}, 0, allocator,
                              false);
  }
  this->ExpectBundledStats(input.size() + 1);
}
...
TYPED_TEST(EdgeTableToolsTest, TestBatchAddEdges) {
  ...
  for (uint32_t i = 0; i < 10; i++) {
    auto oid = neug::columnar::Value::UINT32(i);
