    ...
    EXPECT_TRUE(graph_
                    ->CreateVertexType(
                        person_builder.VertexLabel("person")
                            ...
                            .AddProperty("age", columnar::Value::INT32(0))
                            .AddProperty("score", columnar::Value::DOUBLE(0.0))
...
TEST_F(PropertyGraphTest, TestOpenAndBulkInsert) {
  ...
  vid_t vid1, vid2;
  EXPECT_TRUE(graph_
                  ->AddVertex(person_label, columnar::Value::INT64(1),
                              {columnar::Value::STRING("Alice"),
                               columnar::Value::INT32(30),
                               columnar::Value::DOUBLE(88.5)},
                              vid1, 0)
                  .ok());
  EXPECT_TRUE(graph_
                  ->AddVertex(person_label, columnar::Value::INT64(2),
                              {columnar::Value::STRING("Bob"),
                               columnar::Value::INT32(25),
                               columnar::Value::DOUBLE(92.0)},
                              vid2, 0)
                  .ok());
  ...
  for (size_t i = 3; i <= 4096; ++i) {
    vid_t vid;
    graph_->AddVertex(person_label, columnar::Value::INT64(i),
                      {columnar::Value::STRING("User" + std::to_string(i)),
                       columnar::Value::INT32(20 + (i % 10)),
                       columnar::Value::DOUBLE(80.0 + (i % 20))},
                      vid, 0);
  }
  EXPECT_EQ(graph_->VertexNum(person_label), 4096);
  vid_t vid4097;
  EXPECT_FALSE(graph_
                   ->AddVertex(person_label, columnar::Value::INT64(4097),
                               {columnar::Value::STRING("User4097"),
                                columnar::Value::INT32(27),
                                columnar::Value::DOUBLE(85.0)},
                               vid4097, 0)
                   .ok());
  ...
  }
  {
    ...
    EXPECT_FALSE(graph_
                     ->AddEdge(person_label, 4095, person_label, 4096,
                               knows_label, {columnar::Value::DOUBLE(1.0)},
