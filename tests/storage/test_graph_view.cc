  ...
  void SetUp() override {
    ...
    ASSERT_TRUE(graph_
                    ->AddVertex(person_label, columnar::Value::INT64(1),
                                {columnar::Value::STRING("Alice")}, vid1, 0,
                                false)
                    .ok());
    ASSERT_TRUE(graph_
                    ->AddVertex(person_label, columnar::Value::INT64(2),
                                {columnar::Value::STRING("Bob")}, vid2, 0,
                                false)
                    .ok());
    ASSERT_TRUE(graph_
                    ->AddVertex(person_label, columnar::Value::INT64(3),
                                {columnar::Value::STRING("Charlie")}, vid3, 0,
                                false)
                    .ok());
    ...
    ASSERT_TRUE(graph_
                    ->AddEdge(person_label, vid1, person_label, vid2,
                              knows_label, {columnar::Value::DOUBLE(0.5)}, 0,
                              *alloc_, oe_offset, edge_prop, false)
                    .ok());
    ASSERT_TRUE(graph_
                    ->AddEdge(person_label, vid2, person_label, vid3,
                              knows_label, {columnar::Value::DOUBLE(0.7)}, 0,
                              *alloc_, oe_offset, edge_prop, false)
                    .ok());
  }
...
TEST_F(GraphViewTest, GetLid) {
  ...
  vid_t lid;
  EXPECT_TRUE(view.get_lid(person_label, columnar::Value::INT64(1), lid, 0));
  EXPECT_TRUE(view.get_lid(person_label, columnar::Value::INT64(2), lid, 0));
  EXPECT_TRUE(view.get_lid(person_label, columnar::Value::INT64(3), lid, 0));
  EXPECT_FALSE(view.get_lid(person_label, columnar::Value::INT64(999), lid, 0));
}
