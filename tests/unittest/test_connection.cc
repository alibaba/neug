 ...
 protected:
  ...
  void atomicityInit(std::shared_ptr<Connection> conn) {
    EXPECT_TRUE(conn->Query(
        "CREATE NODE TABLE atomic_person (id INT64, id2 INT64, name STRING, "
        "emails STRING, PRIMARY KEY(id));"));
    EXPECT_TRUE(conn->Query(
        "CREATE REL TABLE atomic_knows(FROM atomic_person TO atomic_person, "
        "since INT64);"));

    EXPECT_TRUE(
        conn->Query("CREATE (u: atomic_person { id: 1, id2: 1, name: 'Alice', "
                    "emails: 'alice@example.com' });"));
    EXPECT_TRUE(
        conn->Query("CREATE (u: atomic_person { id: 2, id2: 1, name: 'Bob', "
  ...
  std::pair<int64_t, int64_t> atomicityCheck(std::shared_ptr<Connection> conn) {
    ...
    auto res =
        conn->Query("MATCH (n:atomic_person) RETURN n.id2 ORDER BY n.id2;");
    EXPECT_TRUE(res);
...
TEST_F(ConnectionTest, TestParallelExecutionAtomicity) {
  ...
  queries.push_back("MATCH (n:atomic_person {id: 1}) set n.id2 = n.id2 + 1;");
  queries.push_back(
      "CREATE (n1:atomic_person {id: $TXN_ID + 3, id2: 1, name: "
...
TEST_F(ConnectionTest, TestParameterizedQuery) {
  ...
  auto res = conn->Query(
      "MATCH (n:atomic_person {id: $person_id}) SET n.id2 = n.id2 + "
      "$increment;",
      "update",
