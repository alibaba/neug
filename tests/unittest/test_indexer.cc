 ...
 protected:
  ...
  static void ExpectInt64Values(const LFIndexer<INDEX_T>& indexer,
    ...
    for (size_t i = 0; i < values.size(); ++i) {
      INDEX_T lid;
      ASSERT_TRUE(indexer.get_index(columnar::Value::INT64(values[i]), lid));
  ...
  static void ExpectStringValues(const LFIndexer<INDEX_T>& indexer,
    ...
    for (size_t i = 0; i < values.size(); ++i) {
      INDEX_T lid;
      ASSERT_TRUE(indexer.get_index(columnar::Value::STRING(values[i]), lid));
      EXPECT_EQ(lid, static_cast<INDEX_T>(i));
...
TEST_F(LFIndexerTest, SupportsCoreMutableInterfacesInMemory) {
  ...
  EXPECT_EQ(indexer.insert(columnar::Value::INT64(values[0]), false), 0U);
  for (size_t i = 1; i < values.size(); ++i) {
    EXPECT_EQ(indexer.insert(columnar::Value::INT64(values[i]), true),
  ...
  EXPECT_TRUE(indexer.get_index(columnar::Value::INT64(23), lid));
  EXPECT_EQ(lid, 5U);
  EXPECT_FALSE(indexer.get_index(columnar::Value::INT32(23), lid));
  EXPECT_TRUE(indexer.contains(columnar::Value::INT64(37)));
  EXPECT_FALSE(indexer.contains(columnar::Value::INT64(1001)));
  EXPECT_EQ(indexer.get_index(columnar::Value::INT64(1001)),
...
TEST_F(LFIndexerTest, DumpsAndOpensAcrossBackends) {
  ...
  CheckpointManifest desc;
  {
    ...
    for (const auto& value : values) {
      writable.insert(columnar::Value::INT64(value), true);
    }
...
TEST_F(LFIndexerTest, SupportsBuildEmptySwapAndVarcharKeys) {
  ...
  for (const auto& value : lhs_values) {
    lhs.insert(columnar::Value::STRING(value), true);
  }
  for (const auto& value : rhs_values) {
    rhs.insert(columnar::Value::STRING(value), true);
  }
...
TEST_F(LFIndexerTest, VarcharReserveEnablesNonSafeInsert) {
  ...
  for (const auto& v : values) {
    indexer.insert(columnar::Value::STRING(v), false);
  }
...
TEST_F(LFIndexerTest, VarcharReserveSmallerThanCapacityIsNoop) {
  ...
  indexer.insert(columnar::Value::STRING(std::string("foo")), false);
  indexer.insert(columnar::Value::STRING(std::string("bar")), false);
  indexer.insert(columnar::Value::STRING(std::string("baz")), false);
  indexer.insert(columnar::Value::STRING(std::string("qux")), false);
...
TEST_F(LFIndexerTest, VarcharRehashPreservesData) {
  ...
  std::vector<std::string> values = {"foo",  "bar",   "baz",   "qux",
                                     "quux", "corge", "grault"};
  for (const auto& v : values) {
    indexer.insert(columnar::Value::STRING(v), true);
  }
  ExpectStringValues(indexer, values);
...
TEST_F(LFIndexerTest, VarcharReserveInsertDumpReload) {
  ...
  for (const auto& v : values) {
    writable.insert(columnar::Value::STRING(v), false);
  }
...
TEST_F(LFIndexerTest, VarcharShortDumpReopenReserveThenInsertLong_InMemory) {
  ...
  std::vector<std::string> short_values = {"a", "bb", "ccc"};
  {
    ...
    for (const auto& v : short_values) {
      writer.insert(columnar::Value::STRING(v), true);
    }
...
TEST_F(LFIndexerTest, VarcharShortDumpReopenInsertSafeLong_InMemory) {
  ...
  std::vector<std::string> short_values = {"x", "y", "z", "w"};
  {
    ...
    for (const auto& v : short_values) {
      writer.insert(columnar::Value::STRING(v), false);
    }
    dump_desc = DumpIndexerLegacy(writer, *ckp);
    writer.Close();
  }
...
TEST_F(LFIndexerTest, VarcharStringOverflow) {
  ...
  for (size_t i = 0; i < 2; ++i) {
    ...
    indexer.insert(columnar::Value::STRING(test_string), false);
    valid_strings.push_back(test_string);
  }
  ExpectStringValues(indexer, valid_strings);

  EXPECT_THROW(indexer.insert(columnar::Value::STRING(overflow_string), false),
