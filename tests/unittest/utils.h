...
class GeneratedChunkSupplier : public neug::IDataChunkSupplier {
 public:
  ...
  std::shared_ptr<neug::columnar::DataChunk> GetNextChunk() override {
    if (chunks_.empty()) {
      return nullptr;
    }
 ...
 private:
  std::vector<std::shared_ptr<neug::columnar::DataChunk>> chunks_;
};

template <typename T>
std::shared_ptr<neug::columnar::IColumn> build_value_column_slice(
    const std::vector<T>& data, size_t begin, size_t end) {
  neug::columnar::ValueColumnBuilder<T> builder;
  for (size_t i = begin; i < end && i < data.size(); ++i) {
    builder.push_back_elem(neug::columnar::Value::CreateValue<T>(data[i]));
  }
  return builder.finish();
}

template <typename T>
std::vector<std::shared_ptr<neug::columnar::IColumn>> split_column_to_chunks(
    const std::vector<T>& data, int num_chunks) {
  size_t chunk_size = (data.size() + num_chunks - 1) / num_chunks;
  std::vector<std::shared_ptr<neug::columnar::IColumn>> columns;
...
inline std::vector<std::shared_ptr<neug::columnar::DataChunk>>
convert_to_data_chunks(
    const std::vector<std::vector<std::shared_ptr<neug::columnar::IColumn>>>&
        column_chunks) {
