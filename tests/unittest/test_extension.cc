...
protected:
    ...
    void SetUp() override {
        auto vfs_ = std::make_unique<gs::common::VirtualFileSystem>();
    ...
    void SetupDatabase(const std::string& db_path) {
        ...
        ASSERT_TRUE(res.has_value()) << "Failed to create school table: " << res.error().ToString();
        ...
        ASSERT_TRUE(res.has_value()) << "Failed to create knows table: " << res.error().ToString();
        ...
        ASSERT_TRUE(res.has_value()) << "Failed to create attends table: " << res.error().ToString();
...
TEST_F(TestJsonExtension, LoadAndImportJsonArray) {
    ...
    ASSERT_TRUE(import_res.has_value()) << "Import JSON Array vertex failed: " << import_res.error().ToString();
    ...
    ASSERT_TRUE(import_res.has_value()) << "Import JSON Array school vertex failed: " << import_res.error().ToString();

    // Verify school count
    {
        auto count_res = conn->Query("MATCH (s:school) RETURN count(s);");
        ASSERT_TRUE(count_res.has_value()) << "Count school query failed: " << count_res.error().ToString();
        auto& rs = count_res.value();
        ASSERT_TRUE(rs.hasNext());
        auto row = rs.next();
    ...
    // Verify specific school data
    {
        ...
        ASSERT_TRUE(q1.has_value()) << "Query school name failed: " << q1.error().ToString();
        auto& rs1 = q1.value();
        ASSERT_TRUE(rs1.hasNext());
        auto r1 = rs1.next();
    ...
    }
    {
        ...
        ASSERT_TRUE(q2.has_value()) << "Query school name failed: " << q2.error().ToString();
        auto& rs2 = q2.value();
        ASSERT_TRUE(rs2.hasNext());
        auto r2 = rs2.next();
        EXPECT_NE(r2.ToString().find("Millenium Science School"), std::string::npos);
    }
    {
        auto q3 = conn->Query("MATCH (s:school) WHERE s.id = 6 RETURN s.name;");
        ASSERT_TRUE(q3.has_value()) << "Query school name failed: " << q3.error().ToString();
        auto& rs3 = q3.value();
        ASSERT_TRUE(rs3.hasNext());
        auto r3 = rs3.next();
    ...
    import_query = "COPY knows FROM \"" + knows_jsona + "\" (from=\"student\",to=\"student\");";
    LOG(INFO) << "Executing query = " << import_query;
    import_res = conn->Query(import_query);
    ASSERT_TRUE(import_res.has_value()) << "Import JSON array knows edge failed: " << import_res.error().ToString();

    // Verify knows edge count
    {
        auto count_res = conn->Query("MATCH ()-[k:knows]->() RETURN count(k);");
        ASSERT_TRUE(count_res.has_value()) << "Count knows edge failed: " << count_res.error().ToString();
        auto& rs = count_res.value();
        ASSERT_TRUE(rs.hasNext());
        auto row = rs.next();
    ...
    import_query = "COPY attends FROM \"" + attends_jsona + "\" (from=\"student\",to=\"school\");";
    LOG(INFO) << "Executing query = " << import_query;
    import_res = conn->Query(import_query);
    ASSERT_TRUE(import_res.has_value()) << "Import JSON array attends edge failed: " << import_res.error().ToString();

    // Verify attends edge count
    {
        auto count_res = conn->Query("MATCH ()-[a:attends]->() RETURN count(a);");
        ASSERT_TRUE(count_res.has_value()) << "Count attends edge failed: " << count_res.error().ToString();
        auto& rs = count_res.value();
        ASSERT_TRUE(rs.hasNext());
        auto row = rs.next();
    ...
    {
        auto q1 = conn->Query("MATCH (s:student)-[:attends]->(sc:school) WHERE s.id = 1 RETURN sc.name;");
        ASSERT_TRUE(q1.has_value()) << "Query student 1 school failed: " << q1.error().ToString();
        auto& rs1 = q1.value();
        ASSERT_TRUE(rs1.hasNext());
        auto r1 = rs1.next();
    ...
    {
        auto q2 = conn->Query("MATCH (s:student)-[:attends]->(sc:school) WHERE s.id = 2 RETURN sc.name;");
        ASSERT_TRUE(q2.has_value()) << "Query student 2 school failed: " << q2.error().ToString();
        auto& rs2 = q2.value();
        ASSERT_TRUE(rs2.hasNext());
        auto r2 = rs2.next();
        EXPECT_NE(r2.ToString().find("Millenium Science School"), std::string::npos);
    ...
    {
        auto q3 = conn->Query("MATCH (s:student)-[:attends]->(sc:school) WHERE s.id = 3 RETURN sc.name;");
        ASSERT_TRUE(q3.has_value()) << "Query student 3 school failed: " << q3.error().ToString();
        auto& rs3 = q3.value();
        ASSERT_TRUE(rs3.hasNext());
        auto r3 = rs3.next();
    ...
    {
        auto q4 = conn->Query("MATCH (s:student)-[:attends]->(sc:school) WHERE s.id = 4 RETURN sc.name;");
        ASSERT_TRUE(q4.has_value()) << "Query student 4 school failed: " << q4.error().ToString();
        auto& rs4 = q4.value();
...
TEST_F(TestJsonExtension, LoadAndImportJsonLines) {
    ...
    auto import_res = conn->Query(import_query);
    ASSERT_TRUE(import_res.has_value()) << "Import JSONL student failed: " << import_res.error().ToString();
    ...
    {
        auto q1 = conn->Query("MATCH (s:student) WHERE s.id = 1 RETURN s.name, s.age;");
        ASSERT_TRUE(q1.has_value()) << "Query id=1 failed: " << q1.error().ToString();
        auto& rs1 = q1.value();
        ASSERT_TRUE(rs1.hasNext());
        auto r1 = rs1.next();
        auto result_str = r1.ToString();
        EXPECT_NE(result_str.find("Mika"), std::string::npos) << "Expected Mika in result";
    ...
    {
        auto q2 = conn->Query("MATCH (s:student) WHERE s.id = 16 RETURN s.name, s.height;");
        ASSERT_TRUE(q2.has_value()) << "Query id=16 failed: " << q2.error().ToString();
        auto& rs2 = q2.value();
        ASSERT_TRUE(rs2.hasNext());
        auto r2 = rs2.next();
        auto result_str = r2.ToString();
        EXPECT_NE(result_str.find("Hifumi"), std::string::npos) << "Expected Hifumi in result";
        EXPECT_NE(result_str.find("158.4"), std::string::npos) << "Expected height 158.4";
        LOG(INFO) << "Hifumi query result: " << result_str;
    }

    // Verify Nagisa's data (id=5)
    {
        auto q3 = conn->Query("MATCH (s:student) WHERE s.id = 5 RETURN s.name, s.club;");
        ASSERT_TRUE(q3.has_value()) << "Query id=5 failed: " << q3.error().ToString();
        auto& rs3 = q3.value();
        ASSERT_TRUE(rs3.hasNext());
        auto r3 = rs3.next();
        auto result_str = r3.ToString();
        EXPECT_NE(result_str.find("Nagisa"), std::string::npos) << "Expected Nagisa in result";
        EXPECT_NE(result_str.find("Tea Party"), std::string::npos) << "Expected club Tea Party";
        LOG(INFO) << "Nagisa query result: " << result_str;
    }

    // Verify Airi's data (id=24)
    {
        auto q4 = conn->Query("MATCH (s:student) WHERE s.id = 24 RETURN s.name, s.club;");
        ASSERT_TRUE(q4.has_value()) << "Query id=24s failed: " << q4.error().ToString();
        auto& rs4 = q4.value();
        ASSERT_TRUE(rs4.hasNext());
        auto r4 = rs4.next();
        auto result_str = r4.ToString();
        EXPECT_NE(result_str.find("Airi"), std::string::npos) << "Expected Airi in result";
        EXPECT_NE(result_str.find("After School Sweets Club"), std::string::npos) << "Expected club After School Sweets Club";
        LOG(INFO) << "Airi query result: " << result_str;
    }

    // Verify date field for Yuuka
    {
        auto q_date = conn->Query("MATCH (s:student) WHERE s.birthday = DATE('2005-03-14') RETURN s.name;");
        if (q_date.has_value()) {
            ...
            LOG(WARNING) << "Date comparison query failed: " << q_date.error().ToString();
        }
    }

    // Test age range query
    {
        auto q_range = conn->Query("MATCH (s:student) WHERE s.age >= 18 AND s.age <= 20 RETURN s.name ORDER BY s.age;");
    ...
    import_res = conn->Query(import_query);
    ASSERT_TRUE(import_res.has_value()) << "Import JSONL school vertex failed: " << import_res.error().ToString();
    ...
    }
    {
        ...
        ASSERT_TRUE(rs2.hasNext());
        auto r2 = rs2.next();
        EXPECT_NE(r2.ToString().find("Red Winter Federal Academy"), std::string::npos);
    }

    LOG(INFO) << "=== Testing loading knows edge data (JSONL) ===";

    // Import knows edge data
    import_query = "COPY knows FROM \"" + knows_jsonl + "\" (from=\"student\",to=\"student\");";
    LOG(INFO) << "Executing query = " << import_query;
    import_res = conn->Query(import_query);
    ASSERT_TRUE(import_res.has_value()) << "Import JSONL knows edge failed: " << import_res.error().ToString();
    ...
    // Import attends edge data
    import_query = "COPY attends FROM \"" + attends_jsonl + "\" (from=\"student\",to=\"school\");";
    LOG(INFO) << "Executing query = " << import_query;
    import_res = conn->Query(import_query);
    ASSERT_TRUE(import_res.has_value()) << "Import JSONL attends edge failed: " << import_res.error().ToString();
    ...
    {
        auto q2 = conn->Query("MATCH (s:student)-[:attends]->(sc:school) WHERE s.id = 4 RETURN sc.name;");
        ASSERT_TRUE(q2.has_value()) << "Query student 4 school failed: " << q2.error().ToString();
        auto& rs2 = q2.value();
        ASSERT_TRUE(rs2.hasNext());
        auto r2 = rs2.next();
    ...
    {
        auto q_path = conn->Query("MATCH (s:student)-[:attends]->(sc:school) RETURN s.name, sc.name ORDER BY s.id;");
        if (q_path.has_value()) {
