...
TEST_F(ValueTest, GetStringValueUnified) {
  {
    auto v = columnar::Value::STRING(std::string("hello"));
    EXPECT_EQ(v.GetValue<std::string>(), "hello");
  }
  {
    auto v = columnar::Value::STRING(std::string("world"));
    EXPECT_EQ(v.GetValue<std::string>(), "world");
  }
}

TEST_F(ValueTest, LessThan) {
  auto v1 = columnar::Value::INT32(10);
  auto v2 = columnar::Value::INT32(20);
  EXPECT_TRUE(v1 < v2);
  EXPECT_FALSE(v2 < v1);
}
...
TEST_F(ValueTest, AssignmentOperator) {
  auto v1 = columnar::Value::INT32(42);
  auto v2 = v1;
...
TEST_F(ValueTest, EqualityOperator) {
  auto v1 = columnar::Value::INT32(42);
  auto v2 = columnar::Value::INT32(42);
  auto v3 = columnar::Value::INT32(43);
  auto v4 = columnar::Value::STRING(std::string("same"));
  auto v5 = columnar::Value::STRING(std::string("same"));
  auto v6 = columnar::Value::STRING(std::string("diff"));
