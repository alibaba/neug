...
TEST_F(PBUtilsTest, ProtoToJsonTest) {
  results::CollectiveResults collective;

  auto* result = collective.add_results();
  auto* record = result->mutable_record();

  auto* i32_column = record->add_columns();
  i32_column->mutable_name_or_id()->set_name("i32_property");
  i32_column->mutable_entry()->mutable_element()->mutable_object()->set_i32(25);

  auto* i64_column = record->add_columns();
  i64_column->mutable_name_or_id()->set_name("i64_property");
  i64_column->mutable_entry()->mutable_element()->mutable_object()->set_i64(
      12580);

  auto* f64_column = record->add_columns();
  f64_column->mutable_name_or_id()->set_name("f64_property");
  f64_column->mutable_entry()->mutable_element()->mutable_object()->set_f64(
      3.145);

  auto* str_column = record->add_columns();
  str_column->mutable_name_or_id()->set_id(2);
  str_column->mutable_entry()->mutable_element()->mutable_object()->set_str(
      "Alice");

  auto* str_array_column = record->add_columns();
  str_array_column->mutable_name_or_id()->set_name("str_array_property");
  auto* str_array = str_array_column->mutable_entry()
                        ->mutable_element()
                        ->mutable_object()
                        ->mutable_str_array();
  str_array->add_item("Bob");
  str_array->add_item("Rose");

  auto* bool_column = record->add_columns();
  bool_column->mutable_name_or_id()->set_name("bool_property");
  bool_column->mutable_entry()
      ->mutable_element()
      ->mutable_object()
      ->set_boolean(false);

  auto* i32_array_column = record->add_columns();
  i32_array_column->mutable_name_or_id()->set_name("i32_array_property");
  auto* i32_array = i32_array_column->mutable_entry()
                        ->mutable_element()
                        ->mutable_object()
                        ->mutable_i32_array();
  i32_array->add_item(100);
  i32_array->add_item(200);

  auto* i64_array_column = record->add_columns();
  i64_array_column->mutable_name_or_id()->set_name("i64_array_property");
  auto* i64_array = i64_array_column->mutable_entry()
                        ->mutable_element()
                        ->mutable_object()
                        ->mutable_i64_array();
  i64_array->add_item(1000000000000LL);
  i64_array->add_item(2000000000000LL);

  auto* f64_array_column = record->add_columns();
  f64_array_column->mutable_name_or_id()->set_name("f64_array_property");
  auto* f64_array = f64_array_column->mutable_entry()
                        ->mutable_element()
                        ->mutable_object()
                        ->mutable_f64_array();
  f64_array->add_item(3.14f);
  f64_array->add_item(2.71f);

  auto* vertex_column = record->add_columns();
  vertex_column->mutable_name_or_id()->set_name("vertex");
  auto* vertex =
      vertex_column->mutable_entry()->mutable_element()->mutable_vertex();
  vertex->set_id(1);
  vertex->mutable_label()->set_id(0);
  for (size_t i = 0; i < 8; i++) {
    vertex->add_properties();
  }
  vertex->mutable_properties(0)->mutable_key()->set_id(0);
  vertex->mutable_properties(0)->mutable_value()->set_i32(0);
  vertex->mutable_properties(1)->mutable_key()->set_id(1);
  vertex->mutable_properties(1)->mutable_value()->set_i64(0);
  vertex->mutable_properties(2)->mutable_key()->set_id(2);
  vertex->mutable_properties(2)->mutable_value()->set_f64(3.2814);
  vertex->mutable_properties(3)->mutable_key()->set_id(3);
  vertex->mutable_properties(3)->mutable_value()->set_boolean(false);
  vertex->mutable_properties(4)->mutable_key()->set_id(4);
  vertex->mutable_properties(4)->mutable_value()->set_str("test_str");
  vertex->mutable_properties(5)->mutable_key()->set_id(5);
  auto* v_i32_array =
      vertex->mutable_properties(5)->mutable_value()->mutable_i32_array();
  v_i32_array->add_item(100);
  v_i32_array->add_item(200);
  vertex->mutable_properties(6)->mutable_key()->set_id(6);
  auto* v_i64_array =
      vertex->mutable_properties(6)->mutable_value()->mutable_i64_array();
  v_i64_array->add_item(100LL);
  v_i64_array->add_item(200LL);
  vertex->mutable_properties(7)->mutable_key()->set_name("f64_property");
  auto* v_f64_array =
      vertex->mutable_properties(7)->mutable_value()->mutable_f64_array();
  v_f64_array->add_item(100.439);
  v_f64_array->add_item(200.642);

  auto* edge_column = record->add_columns();
  edge_column->mutable_name_or_id()->set_name("edge");
  auto* edge = edge_column->mutable_entry()->mutable_element()->mutable_edge();
  edge->set_id(0);
  edge->set_src_id(0);
  edge->set_dst_id(1);
  edge->mutable_label()->set_id(1);
  edge->mutable_src_label()->set_id(0);
  edge->mutable_dst_label()->set_id(0);

  auto* path_column = record->add_columns();
  path_column->mutable_name_or_id()->set_name("path");
  auto* path =
      path_column->mutable_entry()->mutable_element()->mutable_graph_path();
  path->add_path();
  auto* path_v = path->mutable_path(0)->mutable_vertex();
  path_v->set_id(1);
  path_v->mutable_label()->set_id(0);

  auto* collection_column = record->add_columns();
  collection_column->mutable_name_or_id()->set_name("collection");
  auto* collection = collection_column->mutable_entry()->mutable_collection();
  auto* collection_ele = collection->add_collection();
  collection_ele->mutable_element()->mutable_object()->set_i32(9);

  auto* map_column = record->add_columns();
  map_column->mutable_name_or_id()->set_name("map");
  auto* map = map_column->mutable_entry()->mutable_map();
  map->add_key_values();
  map->mutable_key_values(0)->mutable_key()->set_str("key");
  map->mutable_key_values(0)
      ->mutable_value()
      ->mutable_element()
      ->mutable_object()
      ->set_i32(9);

  std::string json = proto_to_bolt_response(collective);
  EXPECT_FALSE(json.empty());
}

TEST_F(PBUtilsTest, ProtoToBoltResponse_Empty) {
  results::CollectiveResults result;
  std::string json = proto_to_bolt_response(result);
  EXPECT_FALSE(json.empty());
