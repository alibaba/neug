...
class UpdateTransactionTest : public ::testing::Test {
 protected:
  ...
  void create_new_edge_type(neug::UpdateTransaction& txn,
    ...
    std::vector<std::pair<std::string, neug::columnar::Value>> edge_props = {
        std::make_pair("rating", neug::columnar::Value::DOUBLE(0.0)),
        std::make_pair("year", neug::columnar::Value::INT64(2000))};
    EXPECT_TRUE(interface.CreateEdgeType(BuildCreateEdgeTypeParam(
        "person", "software", "developed", edge_props)));
    std::vector<std::pair<std::string, neug::columnar::Value>> v_props = {
        std::make_pair("id", neug::columnar::Value::INT64(0)),
        std::make_pair("name", neug::columnar::Value::STRING(std::string("")))};
    ...
    EXPECT_TRUE(interface.AddVertex(
        cmp_label, neug::columnar::Value::INT64(1),
        {neug::columnar::Value::STRING(std::string("TechCorp"))}, vid));
    neug::vid_t p1_vid;
    EXPECT_TRUE(txn.GetVertexIndex(person_label,
                                   neug::columnar::Value::INT64(1), p1_vid));
    neug::vid_t software_vid;
    EXPECT_TRUE(txn.GetVertexIndex(
        software_label, neug::columnar::Value::INT64(1), software_vid));
    neug::vid_t cmp_vid;
    EXPECT_TRUE(txn.GetVertexIndex(cmp_label, neug::columnar::Value::INT64(1),
                                   cmp_vid));
    const void* edge_prop = nullptr;
    EXPECT_TRUE(interface.AddEdge(person_label, p1_vid, software_label,
                                  software_vid, dev_label,
                                  {neug::columnar::Value::DOUBLE(4.5),
                                   neug::columnar::Value::INT64(2023)},
                                  edge_prop));
  ...
  std::vector<std::string> create_string_prop_relation(
    ...
    std::vector<std::pair<std::string, neug::columnar::Value>> edge_props = {
        std::make_pair(
            "review", neug::columnar::Value::STRING(std::string("no review")))};
    ...
    EXPECT_TRUE(graph.GetVertexIndex(software_label,
                                     neug::columnar::Value::INT64(1), s1_vid));
    ...
    for (int i = 0; i < num_edges; i++) {
      ...
      EXPECT_TRUE(graph.AddEdge(
          ...
          {neug::columnar::Value::STRING(std::string(full_review))},
          edge_prop));
    }
    return reviews;
  }
  ...
  static neug::CreateVertexTypeParam BuildCreateVertexTypeParam(
      ...
      const std::vector<std::pair<std::string, neug::columnar::Value>>&
          properties,
...
TEST_F(UpdateTransactionTest, AddVertex) {
  ...
  auto svc = std::make_shared<neug::NeugDBService>(db);
  {
    ...
    neug::vid_t vid;
    EXPECT_TRUE(
        gui.AddVertex(person_label, neug::columnar::Value::INT64(3),
...
TEST_F(UpdateTransactionTest, AddVertexBatch) {
  ...
  auto svc = std::make_shared<neug::NeugDBService>(db);
  {
    ...
    for (int i = 4; i <= 10000; i++) {
      neug::vid_t vid;
      EXPECT_TRUE(
          gui.AddVertex(person_label, neug::columnar::Value::INT64(i),
                        {neug::columnar::Value::STRING(std::string("User")),
                         neug::columnar::Value::INT64(20 + i % 10)},
                        vid));
    }
    EXPECT_TRUE(txn.Commit());
  }
...
TEST_F(UpdateTransactionTest, AddEdge) {
  ...
  auto svc = std::make_shared<neug::NeugDBService>(db);
  {
    ...
    const void* edge_prop = nullptr;
    EXPECT_TRUE(gui.AddEdge(person_label, vid, software_label, vid2,
                            created_label,
...
TEST_F(UpdateTransactionTest, AddVertexEdge) {
  ...
  auto svc = std::make_shared<neug::NeugDBService>(db);
  {
    ...
    EXPECT_TRUE(
        gui.AddVertex(person_label, neug::columnar::Value::INT64(4),
                      {neug::columnar::Value::STRING(std::string("David")),
                       neug::columnar::Value::INT64(32)},
                      vid4));
    EXPECT_TRUE(
        gui.AddVertex(software_label, neug::columnar::Value::INT64(3),
                      {neug::columnar::Value::STRING(std::string("NeugDB")),
                       neug::columnar::Value::STRING(std::string("C++"))},
                      vid3));
    const void* edge_prop = nullptr;
    EXPECT_TRUE(gui.AddEdge(person_label, vid4, software_label, vid3,
                            created_label,
                            {neug::columnar::Value::DOUBLE(0.85),
                             neug::columnar::Value::INT64(2023)},
                            edge_prop));
    EXPECT_TRUE(txn.GetVertexIndex(person_label,
                                   neug::columnar::Value::INT64(2), vid2));
    EXPECT_TRUE(gui.AddEdge(person_label, vid2, software_label, vid3,
                            created_label,
                            {neug::columnar::Value::DOUBLE(0.75),
                             neug::columnar::Value::INT64(2021)},
                            edge_prop));
    neug::vid_t vid1;
    EXPECT_TRUE(txn.GetVertexIndex(person_label,
                                   neug::columnar::Value::INT64(1), vid1));
    EXPECT_TRUE(gui.AddEdge(person_label, vid4, person_label, vid1,
                            txn.schema().get_edge_label_id("knows"),
                            {neug::columnar::Value::DOUBLE(0.95)}, edge_prop));
    EXPECT_TRUE(txn.Commit());
  }
  {
    auto sess = svc->AcquireSession();
    ...
    EXPECT_TRUE(gi.GetVertexIndex(person_label, neug::columnar::Value::INT64(4),
                                  david_vid));
    EXPECT_EQ(count_edges_filter_src(gi, person_label, software_label,
                                     created_label, david_vid, true),
              1);
    neug::vid_t neugdb_vid;
    EXPECT_TRUE(gi.GetVertexIndex(software_label,
                                  neug::columnar::Value::INT64(3), neugdb_vid));
...
TEST_F(UpdateTransactionTest, AddVertexEdgeAbort) {
  ...
  auto svc = std::make_shared<neug::NeugDBService>(db);
  {
    ...
    EXPECT_TRUE(
        gui.AddVertex(person_label, neug::columnar::Value::INT64(5),
                      {neug::columnar::Value::STRING(std::string("Frank")),
                       neug::columnar::Value::INT64(27)},
                      vid5));
    EXPECT_TRUE(
        gui.AddVertex(software_label, neug::columnar::Value::INT64(4),
                      {neug::columnar::Value::STRING(std::string("UltraGraph")),
                       neug::columnar::Value::STRING(std::string("Go"))},
                      vid4));
    const void* edge_prop = nullptr;
    EXPECT_TRUE(gui.AddEdge(person_label, vid5, software_label, vid4,
                            created_label,
                            {neug::columnar::Value::DOUBLE(0.65),
                             neug::columnar::Value::INT64(2022)},
                            edge_prop));
    txn.Abort();
  }
...
TEST_F(UpdateTransactionTest, UpdateVertexProperty) {
  ...
  auto svc = std::make_shared<neug::NeugDBService>(db);
  {
    ...
    CHECK(txn.GetVertexIndex(person_label, neug::columnar::Value::INT64(2),
                             vertex_id));
    gui.UpdateVertexProperty(person_label, vertex_id, 1,
                             neug::columnar::Value::INT64(26));
...
TEST_F(UpdateTransactionTest, UpdateEdgeProperty) {
  ...
  auto svc = std::make_shared<neug::NeugDBService>(db);
  {
    ...
    auto created_label = txn.schema().get_edge_label_id("created");
    neug::vid_t vertex_id;
    CHECK(txn.GetVertexIndex(person_label, neug::columnar::Value::INT64(1),
                             vertex_id));
    update_edge_property(
        ...
        [&](neug::vid_t dst_vid, int32_t oe_offset, int32_t ie_offset) {
          gui.UpdateEdgeProperty(person_label, vertex_id, software_label,
                                 ...
                                 0, neug::columnar::Value::DOUBLE(0.99));
        });
...
TEST_F(UpdateTransactionTest, AddVertexAbort) {
  ...
  auto svc = std::make_shared<neug::NeugDBService>(db);
  {
    ...
    EXPECT_TRUE(
        gui.AddVertex(person_label, neug::columnar::Value::INT64(4),
                      {neug::columnar::Value::STRING(std::string("Charlie")),
                       neug::columnar::Value::INT64(29)},
                      vid));
    txn.Abort();
  }
  {
...
TEST_F(UpdateTransactionTest, AddEdgeAbort) {
  ...
  auto svc = std::make_shared<neug::NeugDBService>(db);
  {
    ...
    EXPECT_TRUE(txn.GetVertexIndex(software_label,
                                   neug::columnar::Value::INT64(1), vid1));
    const void* edge_prop = nullptr;
    EXPECT_TRUE(gui.AddEdge(person_label, vid2, software_label, vid1,
                            created_label,
                            {neug::columnar::Value::DOUBLE(0.8),
                             neug::columnar::Value::INT64(2021)},
                            edge_prop));
    txn.Abort();
  }
  ...
  auto svc = std::make_shared<neug::NeugDBService>(db);
  {
    ...
    update_edge_property(
        ...
        [&](neug::vid_t dst_vid, int32_t oe_offset, int32_t ie_offset) {
          gui.UpdateEdgeProperty(person_label, vertex_id, software_label,
                                 dst_vid, created_label, oe_offset, ie_offset,
                                 0, neug::columnar::Value::DOUBLE(0.9));
          gui.UpdateEdgeProperty(person_label, vertex_id, software_label,
                                 dst_vid, created_label, oe_offset, ie_offset,
                                 1, neug::columnar::Value::INT64(2023));
        });
...
TEST_F(UpdateTransactionTest, DeleteVertexPropertiesThenInsertVertex) {
  ...
  auto svc = std::make_shared<neug::NeugDBService>(db);
  {
    ...
    EXPECT_TRUE(interface.AddVertex(person_label,
                                    neug::columnar::Value::INT64(3),
                                    {neug::columnar::Value::INT64(28)}, vid));
    EXPECT_TRUE(txn.Commit());
  }
  ...
  // Verify the new vertex is readable and "name" is gone.
  {
    ...
    neug::vid_t vid;
    ASSERT_TRUE(
        gi.GetVertexIndex(person_label, neug::columnar::Value::INT64(3), vid));
...
TEST_F(UpdateTransactionTest, DeleteVertexPropertiesThenUpdateRemaining) {
  ...
  // First transaction: add two more properties to person.
  {
    ...
    auto txn = sess->GetUpdateTransaction();
    neug::StorageTPUpdateInterface gui(txn);
    std::vector<std::pair<std::string, neug::columnar::Value>> new_props = {
        std::make_pair("email", neug::columnar::Value::STRING(std::string(""))),
        std::make_pair("score", neug::columnar::Value::DOUBLE(0.0))};
    ...
    gui.UpdateVertexProperty(person_label, vid, 3,
                             neug::columnar::Value::DOUBLE(95.5));
    EXPECT_TRUE(txn.Commit());
  }
  ...
  //   4. With fix: columns_detached correctly has 3 entries after deletion
  {
    ...
    interface.UpdateVertexProperty(person_label, vid, 2,
                                   neug::columnar::Value::DOUBLE(88.0));
    ...
    neug::vid_t new_vid;
    EXPECT_TRUE(interface.AddVertex(
        person_label, neug::columnar::Value::INT64(3),
        {neug::columnar::Value::INT64(22),
         neug::columnar::Value::STRING(std::string("charlie@test.com")),
         neug::columnar::Value::DOUBLE(77.0)},
        new_vid));
    EXPECT_TRUE(txn.Commit());
  }
  ...
  // Verify data correctness
  {
    ...
    {
      neug::vid_t vid;
      CHECK(gi.GetVertexIndex(person_label, neug::columnar::Value::INT64(1),
                              vid));
      auto age_col = std::dynamic_pointer_cast<
          neug::StorageReadInterface::vertex_column_t<int64_t>>(
          gi.GetVertexPropColumn(person_label, "age"));
    ...
    {
      neug::vid_t vid;
      CHECK(gi.GetVertexIndex(person_label, neug::columnar::Value::INT64(3),
                              vid));
      auto age_col = std::dynamic_pointer_cast<
          neug::StorageReadInterface::vertex_column_t<int64_t>>(
          gi.GetVertexPropColumn(person_label, "age"));
...
TEST_F(UpdateTransactionTest, DeleteEdgePropertiesThenInsertEdge) {
  ...
  // leaves 2 properties (avoids bundled<->unbundled conversion path).
  {
    ...
    std::vector<std::pair<std::string, neug::columnar::Value>> new_props = {
        std::make_pair("rating", neug::columnar::Value::DOUBLE(0.0))};
    EXPECT_TRUE(gui.AddEdgeProperties(BuildAddEdgePropertiesParam(
  ...
  // Now "created" has weight(0), since(1), rating(2) — 3 columns
  {
    ...
    CHECK(txn.GetVertexIndex(person_label, neug::columnar::Value::INT64(1),
                             p1_vid));
    CHECK(txn.GetVertexIndex(software_label, neug::columnar::Value::INT64(1),
                             s1_vid));
    const void* edge_prop = nullptr;
    EXPECT_TRUE(interface.AddEdge(person_label, p1_vid, software_label, s1_vid,
                                  created_label,
                                  {neug::columnar::Value::DOUBLE(0.9),
                                   neug::columnar::Value::DOUBLE(4.5)},
                                  edge_prop));
...
TEST_F(UpdateTransactionTest, DeleteVertexTypeWithEdgesThenCreateNewTypes) {
...

  {
    ...
    neug::vid_t cmp_vid;
    EXPECT_TRUE(interface.AddVertex(
        company_label, neug::columnar::Value::INT64(1),
        {neug::columnar::Value::STRING(std::string("TechCorp"))}, cmp_vid));
...
TEST_F(UpdateTransactionTest, DeleteEdgeTypeThenCreateNewEdgeType) {
...

  {
    ...
    std::vector<std::pair<std::string, neug::columnar::Value>> e_props = {
        std::make_pair("closeness", neug::columnar::Value::DOUBLE(0.0))};
    EXPECT_TRUE(interface.CreateEdgeType(
    ...
    EXPECT_TRUE(interface.AddEdge(
        ...
        {neug::columnar::Value::DOUBLE(0.75)}, edge_prop));
    EXPECT_TRUE(txn.Commit());
  }
    ...
    update_edge_property(
        ...
        [&](neug::vid_t dst_vid, int32_t oe_offset, int32_t ie_offset) {
          gui.UpdateEdgeProperty(person_label, vertex_id, person_label, dst_vid,
                                 ...
                                 neug::columnar::Value::DOUBLE(0.95));
        });
...
TEST_F(UpdateTransactionTest, AddEdgeAndUpdateAndAbort) {
  ...
  auto svc = std::make_shared<neug::NeugDBService>(db);
  {
    ...
    EXPECT_TRUE(txn.GetVertexIndex(software_label,
                                   neug::columnar::Value::INT64(2), vid2));
    const void* edge_prop = nullptr;
    EXPECT_TRUE(gui.AddEdge(person_label, vid1, software_label, vid2,
                            created_label,
                            {neug::columnar::Value::DOUBLE(0.85),
                             neug::columnar::Value::INT64(2023)},
                            edge_prop));
...
TEST_F(UpdateTransactionTest, DeleteVertex) {
  ...
  }
  {
    ...
    EXPECT_FALSE(gi.GetVertexIndex(person_label,
                                   neug::columnar::Value::INT64(2), vertex_id));
    EXPECT_EQ(
  ...
  }
  {
    ...
    neug::StorageTPUpdateInterface gui(txn);
    auto person_label = txn.schema().get_vertex_label_id("person");
    EXPECT_TRUE(gui.DeleteVertexType("person"));
    neug::vid_t vertex_id;
    EXPECT_THROW(txn.GetVertexIndex(person_label,
                                    neug::columnar::Value::INT64(1), vertex_id),
...
TEST_F(UpdateTransactionTest, DeleteEdgeAbort) {
...

  {
    ...
    neug::vid_t vid2, vid1;
    EXPECT_TRUE(txn.GetVertexIndex(person_label,
                                   neug::columnar::Value::INT64(1), vid2));
    EXPECT_TRUE(txn.GetVertexIndex(software_label,
                                   neug::columnar::Value::INT64(1), vid1));
...
TEST_F(UpdateTransactionTest, AddDeleteVertexAbort) {
  ...
  }
  {
    ...
    EXPECT_EQ(count_vertices(gi, person_label), 2);
    neug::vid_t vertex_id;
    EXPECT_TRUE(gi.GetVertexIndex(person_label, neug::columnar::Value::INT64(2),
                                  vertex_id));
    EXPECT_FALSE(gi.GetVertexIndex(person_label,
                                   neug::columnar::Value::INT64(3), vertex_id));
  }
  {
    // Add again
    auto sess = svc->AcquireSession();
  ...
  }
  {
    ...
    EXPECT_EQ(count_vertices(gi, person_label), 3);
    neug::vid_t vertex_id;
    EXPECT_TRUE(gi.GetVertexIndex(person_label, neug::columnar::Value::INT64(3),
                                  vertex_id));
    EXPECT_EQ(count_vertices(gi, person_label), 3);
  }
  db.Close();
...
TEST_F(UpdateTransactionTest, DeleteEdgeTypeAbort) {
...

  {
    ...
    auto person_label = txn.schema().get_vertex_label_id("person");
    auto software_label = txn.schema().get_vertex_label_id("software");
    EXPECT_TRUE(gui.DeleteEdgeType("person", "software", "created"));
...
TEST_F(UpdateTransactionTest, AddVertexProperties) {
...

  {
    ...
    std::vector<std::pair<std::string, neug::columnar::Value>> new_props = {
        std::make_pair("email", neug::columnar::Value::STRING(std::string(""))),
        std::make_pair("height", neug::columnar::Value::DOUBLE(0.0))};
    EXPECT_TRUE(gui.AddVertexProperties(
        BuildAddVertexPropertiesParam("person", new_props)));
    ...
    gui.UpdateVertexProperty(
        person_label, vid, 2,
        neug::columnar::Value::STRING(std::string("eve@example.com")));
    EXPECT_TRUE(txn.Commit());
  }
  {
    auto sess = svc->AcquireSession();
    ...
    std::vector<std::pair<std::string, neug::columnar::Value>> new_props = {
        std::make_pair("address",
                       neug::columnar::Value::STRING(std::string("")))};
    EXPECT_TRUE(gui.AddVertexProperties(
        BuildAddVertexPropertiesParam("person", new_props)));
    neug::vid_t vid;
    CHECK(
        txn.GetVertexIndex(person_label, neug::columnar::Value::INT64(2), vid));
    gui.UpdateVertexProperty(person_label, vid, 3,
                             neug::columnar::Value::DOUBLE(175.5));
    txn.Abort();
  }
  {
    auto sess = svc->AcquireSession();
    ...
    neug::vid_t vid;
    CHECK(
        gi.GetVertexIndex(person_label, neug::columnar::Value::INT64(1), vid));
    EXPECT_EQ(email_accessor->get_any(vid).GetValue<std::string>(),
              "eve@example.com");
    CHECK(
        gi.GetVertexIndex(person_label, neug::columnar::Value::INT64(2), vid));
...
TEST_F(UpdateTransactionTest, AddEdgeProperties) {
...

  {
    ...
    neug::StorageTPUpdateInterface interface(txn);
    std::vector<std::pair<std::string, neug::columnar::Value>> new_props = {
        std::make_pair("version", neug::columnar::Value::INT64(0)),
  ...
  }
  {
    ...
    neug::StorageTPUpdateInterface interface(txn);
    std::vector<std::pair<std::string, neug::columnar::Value>> new_props = {
        std::make_pair("contributions", neug::columnar::Value::DOUBLE(0.0))};
    EXPECT_TRUE(interface.AddEdgeProperties(BuildAddEdgePropertiesParam(
        "person", "software", "created", new_props)));
    txn.Abort();
  }
...
TEST_F(UpdateTransactionTest, TestReplayWal) {
  ...
  config.checkpoint_after_recovery = true;
  {
    ...
    EXPECT_TRUE(txn.GetVertexIndex(person_label,
                                   neug::columnar::Value::INT64(1), src_p));
    EXPECT_TRUE(txn.GetVertexIndex(person_label,
                                   neug::columnar::Value::INT64(2), dst_p));
    ...
    update_edge_property(
        ...
        [&](neug::vid_t dst_vid, int32_t oe_offset, int32_t ie_offset) {
          interface.UpdateEdgeProperty(person_label, src_p, person_label, dst_p,
                                       ...
                                       neug::columnar::Value::DOUBLE(0.5));
        });
    EXPECT_TRUE(txn.Commit());
    db.Close();
  }
  {
    ...
    EXPECT_EQ(count_vertices(gi, person_label), 3);
    neug::vid_t src_p, dst_p;
    EXPECT_TRUE(gi.GetVertexIndex(person_label, neug::columnar::Value::INT64(1),
                                  src_p));
    EXPECT_TRUE(gi.GetVertexIndex(person_label, neug::columnar::Value::INT64(2),
                                  dst_p));
    auto vprop_accessor = std::dynamic_pointer_cast<
...
TEST_F(UpdateTransactionTest, TestAPIAfterDeleteVertexLabel) {
...

  {
    ...
    EXPECT_THROW(interface.UpdateVertexProperty(
                     person_label, 0, 1, neug::columnar::Value::INT64(30)),
                 neug::exception::Exception);
    EXPECT_THROW(
        interface.AddVertex(person_label, neug::columnar::Value::INT64(3),
                            {neug::columnar::Value::STRING(std::string("Eve")),
  ...
  }
  {
    ...
    std::vector<std::pair<std::string, neug::columnar::Value>> new_props = {
        std::make_pair("age", neug::columnar::Value::INT32(0))};
...
TEST_F(UpdateTransactionTest, TestAPIAfterDeleteEdgeLabel) {
...

  {
    ...
    EXPECT_TRUE(txn.GetVertexIndex(person_label,
                                   neug::columnar::Value::INT64(1), src_vid));
    EXPECT_TRUE(txn.GetVertexIndex(person_label,
                                   neug::columnar::Value::INT64(2), dst_vid));
    ...
    EXPECT_FALSE(interface.UpdateEdgeProperty(
                     ...
                     0, neug::columnar::Value::DOUBLE(0.8)).ok());
  ...
  }
  {
    ...
    auto person_label = txn.schema().get_vertex_label_id("person");
    auto knows_label = txn.schema().get_edge_label_id("knows");
    EXPECT_TRUE(gui.DeleteEdgeProperties(BuildDeleteEdgePropertiesParam(
        "person", "person", "knows", {"closeness"})));
    // Renaming a just-deleted property must fail. PropertyGraph::Rename*
    // returns Status::ERR_SCHEMA_MISMATCH (not throw) for missing properties,
    // so check the Status's bool conversion.
    EXPECT_FALSE(gui.RenameEdgeProperties(BuildRenameEdgePropertiesParam(
        "person", "person", "knows",
...
TEST_F(UpdateTransactionTest, DeleteVertexAbortRestoresEdges) {
  ...
  size_t initial_created_count, initial_knows_count;
  {
    ...
    EXPECT_TRUE(txn.GetVertexIndex(person_label,
                                   neug::columnar::Value::INT64(1), p1_vid));
    EXPECT_TRUE(txn.GetVertexIndex(software_label,
                                   neug::columnar::Value::INT64(1), p2_vid));
...
TEST_F(UpdateTransactionTest, DeleteVertexWithMultipleEdgeTypes) {
...

  {
    ...
    auto person_label = txn.schema().get_vertex_label_id("person");
    std::vector<std::pair<std::string, neug::columnar::Value>> edge_props = {
        std::make_pair("since", neug::columnar::Value::INT64(2020))};
    ...
    EXPECT_TRUE(gui.AddEdge(person_label, p1_vid, person_label, p2_vid,
                            follows_label, {neug::columnar::Value::INT64(2022)},
                            edge_prop_follows));
    EXPECT_TRUE(txn.Commit());
  }
  {
...
TEST_F(UpdateTransactionTest, BatchDeleteVertices) {
...

  {
    ...
    EXPECT_TRUE(txn.GetVertexIndex(person_label,
                                   neug::columnar::Value::INT64(1), alice_vid));
    EXPECT_TRUE(txn.GetVertexIndex(person_label,
                                   neug::columnar::Value::INT64(2), bob_vid));
...
TEST_F(UpdateTransactionTest, BatchDeleteEdges) {
...

  {
    ...
    EXPECT_TRUE(txn.GetVertexIndex(software_label,
                                   neug::columnar::Value::INT64(1), s1_vid));
    EXPECT_TRUE(txn.GetVertexIndex(software_label,
                                   neug::columnar::Value::INT64(2), s2_vid));
    std::vector<std::tuple<neug::vid_t, neug::vid_t>> edges = {
...
TEST_F(UpdateTransactionTest, TestUpdateStringProperty) {
  ...
  auto svc = std::make_shared<neug::NeugDBService>(db);
  {
    ...
    EXPECT_TRUE(interface.GetVertexIndex(
        person_label, neug::columnar::Value::INT64(1), p1_vid));
    EXPECT_TRUE(interface.GetVertexIndex(
        person_label, neug::columnar::Value::INT64(2), p2_vid));
    std::string long_name(neug::STRING_DEFAULT_MAX_LENGTH + 10, 'a');
    interface.UpdateVertexProperty(
        person_label, p1_vid, 0,
        neug::columnar::Value::STRING(std::string(long_name)));
    ...
    EXPECT_NO_THROW(interface.UpdateVertexProperty(
        person_label, p1_vid, 0,
        neug::columnar::Value::STRING(std::string(valid_name))));
    ...
    for (neug::vid_t vid : vertex_set) {
      ...
      for (auto it = edges.begin(); it != edges.end(); ++it) {
        ...
        interface.UpdateEdgeProperty(
            ...
            oe_offset, ie_offset, 0,
            neug::columnar::Value::STRING(std::string(updated_review)));
