...
neug::Value neug_generate_id() {
  return neug::Value::INT64(neug_current_id.fetch_add(1));
}
...
void neug_append_string_to_field(CowGraphStorage& gui, label_t label,
  ...
  gui.UpdateVertexProperty(label, vit, col_id, neug::Value::STRING(cur_str));
}
...
std::shared_ptr<neug::NeugDBService> neug_AtomicityInit(
  ...
  EXPECT_TRUE(gii.AddVertex(
      ...
      {neug::Value::INT64(id1), neug::Value::STRING(std::string(name1)),
       neug::Value::STRING(std::string(email1))},
      vid));
  EXPECT_TRUE(gii.AddVertex(
      person_label_id, neug_generate_id(),
      {neug::Value::INT64(id2), neug::Value::STRING(std::string(name2)),
       neug::Value::STRING(std::string(email2))},
      vid));
  txn.Commit();
...
bool neug_AtomicityC(neug::ExecutionSlot& db, int64_t person2_id,
  ...
  auto txn = db.BeginSnapshotCowWriteTransaction();
  ...
  if (!gui.AddVertex(person_label_id, p2_id,
                     {neug::Value::INT64(person2_id),
                      neug::Value::STRING(std::string(name)),
  ...
  if (!gui.AddEdge(person_label_id, vit, person_label_id, vid, knows_label_id,
                   {neug::Value::INT64(since)}, edge_prop)) {
    txn.Abort();
    return false;
  }
  txn.Commit();
...
bool neug_AtomicityRB(neug::ExecutionSlot& db, int64_t person2_id,
  ...
  neug::vid_t vit2;
  if (gui.GetVertexIndex(person_label_id, neug::Value::INT64(person2_id),
                         vit2)) {
    txn.Abort();
    return false;
  }
...
std::shared_ptr<neug::NeugDBService> G0Init(NeugDB& db,
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    CHECK(gii.AddVertex(person_label_id, p1_id,
                        {neug::Value::INT64(p1_id_property),
                         neug::Value::STRING(std::string(value))},
                        vid0));
    auto p2_id = neug_generate_id();
    int64_t p2_id_property = 2 * i + 2;
    CHECK(gii.AddVertex(person_label_id, p2_id,
                        {neug::Value::INT64(p2_id_property),
                         neug::Value::STRING(std::string(value))},
                        vid1));
    const void* edge_prop = nullptr;
    CHECK(gii.AddEdge(person_label_id, vid0, person_label_id, vid1,
                      knows_label_id, {neug::Value::STRING(std::string(value))},
                      edge_prop));
  }
  txn.Commit();
  return svc;
...
void G0(neug::ExecutionSlot& db, int64_t person1_id, int64_t person2_id,
  ...
  neug::Value new_value = neug::Value::STRING(cur_str);
...
std::shared_ptr<neug::NeugDBService> InitPersonWithVersion(
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    CHECK(gii.AddVertex(
        person_label_id, neug_generate_id(),
        {neug::Value::INT64(i + 1), neug::Value::INT64(initial_version)}, vid));
  }
  txn.Commit();
  return svc;
}
...
int64_t G1B2(neug::ExecutionSlot& db) {
  auto txn = db.BeginSnapshotReadTransaction();
  StorageReadInterface gi(txn.view(), txn.timestamp());
...
void G1A1(neug::ExecutionSlot& db) {
  ...
  gui.UpdateVertexProperty(person_label_id, vit, 1, neug::Value::INT64(2));
  std::this_thread::sleep_for(std::chrono::milliseconds(SLEEP_TIME_MILLI_SEC));
...
std::shared_ptr<neug::NeugDBService> OTVInit(NeugDB& db,
  ...
  for (int j = 1; j <= 100; j++) {
    ...
    for (int i = 1; i <= 4; i++) {
      ...
      CHECK(
          gii.AddVertex(person_label_id, id,
                        {neug::Value::INT64(id_property),
                         neug::Value::STRING(std::string(string_props.back())),
                         neug::Value::INT64(value)},
                        vid));
      vids.push_back(vid);
    }
    for (int i = 0; i < 4; i++) {
...
std::shared_ptr<neug::NeugDBService> LUInit(NeugDB& db,
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    CHECK(gii.AddVertex(
        person_label_id, neug_generate_id(),
        {neug::Value::INT64(id_property), neug::Value::INT64(num_property)},
        vid));
  }
...
std::shared_ptr<neug::NeugDBService> WSInit(NeugDB& db,
  ...
  for (int i = 1; i <= 100; i++) {
    ...
    CHECK(gi.AddVertex(person_label_id, neug_generate_id(),
                       {neug::Value::INT64(id1), neug::Value::INT64(version1)},
                       vid));
    int64_t id2 = 2 * i;
    int64_t version2 = 80;
    CHECK(gi.AddVertex(person_label_id, neug_generate_id(),
                       {neug::Value::INT64(id2), neug::Value::INT64(version2)},
                       vid));
  }
  txn.Commit();
  return svc;
...
std::shared_ptr<NeugDBService> cc_init(NeugDB& db, const std::string& work_dir,
  ...
  for (int i = 1; i <= kSeedVertices; ++i) {
    ...
    vid_t vid;
    CHECK(txn.AddVertex(
        person_label, neug::Value::INT64(i),
        {neug::Value::STRING(std::string(name)), neug::Value::INT64(age)},
        vid));
    vids.push_back(vid);
  }
  std::mt19937 gen(42);
...
int64_t cc_read_age(NeugDBService& svc, int64_t person_id) {
  ...
  vid_t vid;
  if (!gi.GetVertexIndex(person_label, neug::Value::INT64(person_id), vid)) {
    return -1;
  }
...
// Read age via an existing SnapshotReadTransaction's captured snapshot — used
// to verify that a held snapshot remains coherent across concurrent commits.
int64_t cc_read_age_via(const SnapshotReadTransaction& txn, NeugDB& db,
                        int64_t person_id) {
  StorageReadInterface gi(txn.view(), txn.timestamp());
  auto person_label = db.schema().get_vertex_label_id("person");
  vid_t vid;
...
vid_t cc_person_vid(Txn& txn, NeugDB& db, int64_t oid) {
  ...
  vid_t vid;
  CHECK(txn.GetVertexIndex(p_label, neug::Value::INT64(oid), vid));
  return vid;
}
...
//
// Each "*_via" helper reads through a held SnapshotReadTransaction's frozen
// GraphView
...
size_t cc_count_vertices_via(const SnapshotReadTransaction& txn,
                             label_t label) {
  StorageReadInterface gi(txn.view(), txn.timestamp());
  size_t n = 0;
...
size_t cc_count_oe_from_via(const SnapshotReadTransaction& txn,
                            label_t src_label, label_t dst_label,
                            label_t edge_label, vid_t src_vid) {
  StorageReadInterface gi(txn.view(), txn.timestamp());
...
size_t cc_count_all_oe_via(const SnapshotReadTransaction& txn,
                           label_t src_label, label_t dst_label,
                           label_t edge_label) {
  StorageReadInterface gi(txn.view(), txn.timestamp());
  auto view = gi.GetGenericOutgoingGraphView(src_label, dst_label, edge_label);
...
// Same as cc_count_all_oe_via but on a fresh SnapshotReadTransaction (live
// snapshot).
...
double cc_read_knows_weight_via(const SnapshotReadTransaction& txn, NeugDB& db,
  ...
  if (!gi.GetVertexIndex(p_label, neug::Value::INT64(src_oid), src_vid))
    return std::nan("");
...
void cc_setup_unbundled_created(NeugDBService& svc) {
  ...
  ASSERT_TRUE(
      gui.CreateEdgeType(eb.SrcLabel("person")
                             .DstLabel("software")
                             .EdgeLabel("created")
                             .AddProperty("weight", neug::Value::DOUBLE(0.0))
                             .AddProperty("since", neug::Value::INT64(0))
                             .Build())
          .ok());
  ...
  vid_t sw_vid;
  ASSERT_TRUE(gui.AddVertex(sw_label, neug::Value::INT64(1),
                            {neug::Value::STRING(std::string("NeugDB"))},
                            sw_vid));

  vid_t p1_vid;
  ASSERT_TRUE(gui.GetVertexIndex(p_label, neug::Value::INT64(1), p1_vid));
...
int64_t cc_read_created_since_via(const SnapshotReadTransaction& txn) {
  StorageReadInterface gi(txn.view(), txn.timestamp());
  auto p_label = gi.schema().get_vertex_label_id("person");
  auto sw_label = gi.schema().get_vertex_label_id("software");
  auto e_label = gi.schema().get_edge_label_id("created");
  vid_t p1_vid, sw_vid;
  if (!gi.GetVertexIndex(p_label, neug::Value::INT64(1), p1_vid))
    return -1;
  if (!gi.GetVertexIndex(sw_label, neug::Value::INT64(1), sw_vid))
    return -1;
  ...
  for (int i = 0; i < kInserterThreads; ++i) {
    threads.emplace_back([&] {
      ...
      while (!stop.load() && insert_count.load() < kMaxInserts) {
        ...
        auto txn = lease->BeginMvccInsertTransaction();
        vid_t vid;
        if (txn.AddVertex(
                person_label, neug::Value::INT64(id),
                {neug::Value::STRING(std::string("x")), neug::Value::INT64(99)},
                vid) &&
            txn.Commit()) {
...
TEST_F(NeugDBACIDTest, SnapshotIsolationForUpdateAndInsert) {
  ...
  auto txn_r = sess_r->BeginSnapshotReadTransaction();
...

  {
    ...
    auto txn_w = sess_w->BeginMvccInsertTransaction();
    auto person_label = db.schema().get_vertex_label_id("person");
    vid_t vid;
    ASSERT_TRUE(txn_w.AddVertex(
        person_label, neug::Value::INT64(9999),
        {neug::Value::STRING(std::string("late")), neug::Value::INT64(50)},
        vid));
...
// ============================================================================
// Category 3 — SnapshotCowWriteTransaction COW isolation + rollback correctness
// ============================================================================
...
TEST_F(NeugDBACIDTest, UpdateCowCloneDoesNotAffectActiveReaders) {
  ...
  auto txn_u = sess_u->BeginSnapshotCowWriteTransaction();
  auto gui = txn_u.OpenStorage();
  auto person_label = db.schema().get_vertex_label_id("person");
  vid_t vid_u;
  ASSERT_TRUE(gui.GetVertexIndex(person_label, neug::Value::INT64(5), vid_u));
  gui.UpdateVertexProperty(person_label, vid_u, 1, neug::Value::INT64(7777));
  ...
  size_t labels_pre = svc->db().schema().vertex_label_num();
  {
    ...
    auto status =
        gui.CreateVertexType(b.VertexLabel("foo")
                                 .AddProperty("x", neug::Value::INT64(0))
                                 .AddPrimaryKeyName("x")
                                 .Build());
...
// ============================================================================
// Category 3a — SnapshotCowWriteTransaction COW isolation across single-row
// DML.
//
...
TEST_F(NeugDBACIDTest, DMLCommitDoesNotAffectHeldReader) {
  ...
  vid_t r_v1;
  {
    ...
    ASSERT_TRUE(gi.GetVertexIndex(p_label, neug::Value::INT64(1), r_v1));
  }
  size_t oe_v1_pre =
  ...
  cc_run_update(*svc, [&](auto& txn) {
    ...
    ASSERT_TRUE(gui.AddVertex(
        ...
        {neug::Value::STRING(std::string("late")), neug::Value::INT64(77)},
        vid));
  });
  ...
  EXPECT_EQ(cc_count_vertices_via(txn_r, p_label), n_pre);
  {
    StorageReadInterface gi(txn_r.view(), txn_r.timestamp());
    vid_t v;
    EXPECT_FALSE(gi.GetVertexIndex(p_label, neug::Value::INT64(900001), v));
  }
  ...
  cc_run_update(*svc, [&](auto& txn) {
    ...
    EXPECT_TRUE(gui.AddEdge(p_label, cc_person_vid(gui, db, 1), p_label,
                            ...
                            {neug::Value::DOUBLE(0.55)}, edge_prop));
  });
...
TEST_F(NeugDBACIDTest,
  ...
  bool saw_99 = false;
  {
    ...
    auto accessor = gi.GetEdgeDataAccessor(p_label, p_label, e_label, 0);
    vid_t s, d;
    ASSERT_TRUE(gi.GetVertexIndex(p_label, neug::Value::INT64(1), s));
    ASSERT_TRUE(gi.GetVertexIndex(p_label, neug::Value::INT64(2), d));
...
TEST_F(NeugDBACIDTest,
  ...
  cc_run_update(*svc, [&](auto& txn) {
    ...
    vid_t p1, sw1;
    ASSERT_TRUE(txn.GetVertexIndex(p_label, neug::Value::INT64(1), p1));
    ASSERT_TRUE(txn.GetVertexIndex(sw_label, neug::Value::INT64(1), sw1));
    gui.UpdateEdgeProperty(p_label, p1, sw_label, sw1, e_label, 0, 0, 1,
                           neug::Value::INT64(2099));
  });
...
TEST_F(NeugDBACIDTest, EdgePropertyDDLCommitDoesNotAffectHeldReader) {
  ...
  auto txn_r2 = sess_r2->BeginSnapshotReadTransaction();
...
TEST_F(NeugDBACIDTest, SchemaTypeDDLCommitDoesNotAffectHeldReader) {
  ...
  EXPECT_EQ(cc_count_all_oe_via(txn_r, p_label, p_label, e_label), e_pre);
  {
    ...
    vid_t v;
    ASSERT_TRUE(gi.GetVertexIndex(p_label, neug::Value::INT64(5), v));
...
TEST_F(NeugDBACIDTest, MultipleSequentialCommitsEachSnapshotIsolated) {
  ...
  // SnapshotReadTransaction holds reference members and is therefore neither
  // copyable nor movable, so it can't live in a standard container after
  // value-init.
...
TEST_F(NeugDBACIDTest, UpdateStringPropertyCommitDoesNotAffectHeldReader) {
  ...
  auto read_name_via = [&](const SnapshotReadTransaction& txn, int64_t oid) {
    StorageReadInterface gi(txn.view(), txn.timestamp());
    vid_t v;
    if (!gi.GetVertexIndex(p_label, neug::Value::INT64(oid), v))
      return std::string();
    return std::string(
...
// Design validation: BeginSnapshotCowWriteTransaction changes the admission
// state from kOpen to kInsertsBlocked, so only one UpdateTxn can be open at a
// time. InsertTxn requires kOpen, so it is also blocked by an open UpdateTxn.
TEST_F(NeugDBACIDTest, WriteMutexExclusionSemantics) {
  ...
  // Part 1: second UpdateTxn blocks until first commits.
  {
    ...
    std::thread t1([&] {
      auto txn = sess1->BeginSnapshotCowWriteTransaction();
    ...
    std::thread t2([&] {
      ...
      auto txn = sess2->BeginSnapshotCowWriteTransaction();
  ...
  // Part 2: InsertTxn blocks while UpdateTxn is open.
  {
    ...
    std::thread t_insert([&] {
      while (!update_acquired.load())
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
      auto txn = sess2->BeginMvccInsertTransaction();
      ...
      ASSERT_TRUE(txn.AddVertex(
          person_label, neug::Value::INT64(999999),
          {neug::Value::STRING(std::string("blocked")), neug::Value::INT64(42)},
          vid));
      EXPECT_TRUE(txn.Commit());
    });
...
TEST_F(NeugDBACIDTest, UpdateQueryPlansAfterPreviousUpdateCommits) {
  ...
  auto first_update = first_slot->BeginSnapshotCowWriteTransaction();
  auto first_storage = first_update.OpenStorage();
...
// Validates the design: a long-running SnapshotReadTransaction runs lock-free
// (no mutex held during execution). A concurrent Update::Commit must therefore
...
TEST_F(NeugDBACIDTest, LongRunningReadDoesNotBlockUpdateCommit) {
  ...
  std::thread t_reader([&] {
    auto txn = sess_r->BeginSnapshotReadTransaction();
    reader_acquired.store(true);
    StorageReadInterface gi(txn.view(), txn.timestamp());
    auto person_label = db.schema().get_vertex_label_id("person");
    vid_t vid;
    ASSERT_TRUE(gi.GetVertexIndex(person_label, neug::Value::INT64(1), vid));
  ...
  std::thread t_updater([&] {
    ...
    auto txn = sess_u->BeginSnapshotCowWriteTransaction();
    auto gui = txn.OpenStorage();
    auto person_label = db.schema().get_vertex_label_id("person");
    vid_t vid;
    ASSERT_TRUE(gui.GetVertexIndex(person_label, neug::Value::INT64(1), vid));
  ...
  // Post-commit, fresh SnapshotReadTransactions see the new value.
...
// its read_ts (allocated at BeginSnapshotReadTransaction time) is < or >= the
// writer's write_ts (allocated at BeginSnapshotCowWriteTransaction time,
// published at UpdateTimestampLease::finish time).
//
...
TEST_F(NeugDBACIDTest, ConcurrentReadsAndCommitsObserveConsistentValues) {
  ...
  for (int iter = 0; iter < kIters; ++iter) {
    ...
    //
    // The SnapshotCowWriteTransaction must be fully owned by the writer thread:
    // BeginSnapshotCowWriteTransaction changes VersionManager's admission state
    // from kOpen to kInsertsBlocked, and Commit restores it to kOpen.
