...
TEST_F(NeugDBACIDTest, VertexPropertyDDLCommitDoesNotAffectHeldReader) {
  ...
  cc_run_update(*svc, [&](auto& txn) {
    ...
    auto config =
        b.AddProperty("email", neug::Value::STRING(std::string("")))
  ...
  cc_run_update(*svc, [&](auto& txn) {
    ...
    EXPECT_TRUE(
        gui.RenameVertexProperties(gui.schema().get_vertex_label_id("person"),
                                   config)
            .ok());
  });
...
TEST_F(NeugDBACIDTest, EdgePropertyDDLCommitDoesNotAffectHeldReader) {
  ...
  cc_run_update(*svc, [&](auto& txn) {
    ...
    EXPECT_TRUE(gui.AddEdgeProperties(gui.schema().get_vertex_label_id("person"),
                                      gui.schema().get_vertex_label_id("person"),
...
TEST_F(NeugDBACIDTest, SchemaTypeDDLCommitDoesNotAffectHeldReader) {
  ...
  cc_run_update(*svc, [&](auto& txn) {
    StorageTPUpdateInterface gui(txn);
    EXPECT_TRUE(gui.DeleteEdgeType(gui.schema().get_vertex_label_id("person"), gui.schema().get_vertex_label_id("person"), gui.schema().get_edge_label_id("knows")).ok());
    EXPECT_TRUE(gui.DeleteVertexType(gui.schema().get_vertex_label_id("person")).ok());
  });
