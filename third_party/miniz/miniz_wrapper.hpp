...
enum class MiniZStreamType : uint8_t {
  MINIZ_TYPE_NONE,
  MINIZ_TYPE_INFLATE,
  MINIZ_TYPE_DEFLATE
};
...
struct MiniZStream {
  ...
  static constexpr unsigned char GZIP_FLAG_UNSUPPORTED =
 ...
 public:
  ...
  void FormatException(const char* error_msg, int mz_ret) {
    ...
    FormatException(error_msg + std::string(": ") +
  ...
  size_t MaxCompressedLength(size_t input_size) {
    return miniz::mz_compressBound(input_size) + GZIP_HEADER_MINSIZE +
           GZIP_FOOTER_SIZE;
  }
  ...
  void Compress(const char* uncompressed_data, size_t uncompressed_size,
                char* out_data, size_t* out_size) {
    auto mz_ret = mz_deflateInit2(&stream, miniz::MZ_DEFAULT_LEVEL, MZ_DEFLATED,
    ...
    auto crc =
        miniz::mz_crc32(MZ_CRC32_INIT, (const unsigned char*) uncompressed_data,
                        uncompressed_size);
