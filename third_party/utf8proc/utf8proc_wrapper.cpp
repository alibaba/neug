...
static inline UnicodeType UTF8ExtraByteLoop(const int firstPosSeq, int utf8char,
                                            size_t& i, const char* s,
                                            const size_t len,
                                            UnicodeInvalidReason* invalidReason,
                                            size_t* invalidPos) {
  if ((len - i) < (nextraBytes + 1)) {
    assignInvalidUtf8Reason(invalidReason, invalidPos, firstPosSeq,
                            UnicodeInvalidReason::BYTE_MISMATCH);
    return UnicodeType::INVALID;
  }
  for (size_t j = 0; j < nextraBytes; j++) {
    int c = (int) s[++i];
    if ((c & 0xC0) != 0x80) {
      assignInvalidUtf8Reason(invalidReason, invalidPos, i,
                              UnicodeInvalidReason::BYTE_MISMATCH);
      return UnicodeType::INVALID;
    }
  ...
  if ((utf8char & mask) == 0) {
    assignInvalidUtf8Reason(invalidReason, invalidPos, firstPosSeq,
                            UnicodeInvalidReason::INVALID_UNICODE);
    return UnicodeType::INVALID;
  }
...
UnicodeType Utf8Proc::analyze(const char* s, size_t len,
  ...
  for (size_t i = 0; i < len; i++) {
    ...
    if ((c & 0x80) == 0) {
      ...
      if ((c & 0xE0) == 0xC0) {
        int utf8char = c & 0x1F;
        type = UTF8ExtraByteLoop<1, 0x000780>(firstPosSeq, utf8char, i, s, len,
                                              invalidReason, invalidPos);
      } else if ((c & 0xF0) == 0xE0) {
        int utf8char = c & 0x0F;
        type = UTF8ExtraByteLoop<2, 0x00F800>(firstPosSeq, utf8char, i, s, len,
                                              invalidReason, invalidPos);
      } else if ((c & 0xF8) == 0xF0) {
        int utf8char = c & 0x07;
        type = UTF8ExtraByteLoop<3, 0x1F0000>(firstPosSeq, utf8char, i, s, len,
                                              invalidReason, invalidPos);
      } else {
        assignInvalidUtf8Reason(invalidReason, invalidPos, i,
                                UnicodeInvalidReason::BYTE_MISMATCH);
...
size_t Utf8Proc::previousGraphemeCluster(const char* s, size_t len,
                                         size_t charPos) {
...
int Utf8Proc::codepointLength(int cp) { return utf8proc_codepoint_length(cp); }
