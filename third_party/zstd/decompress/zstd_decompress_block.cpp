                                ...
                                size_t srcSize) /* note : srcSize < BLOCKSIZE */
{
...

  {
    ...
    case set_repeat:
      DEBUGLOG(5,
               "set_repeat flag : re-using stats from previous compressed "
               "literals block");
...
static size_t ZSTD_buildSeqTable(
    ...
    size_t srcSize, const U32* baseValue, const U32* nbAdditionalBits,
      ...
      ""); /* minimum possible size: 1 byte for symbol encoding types */
  {
    ...
    /* Build DTables */
    {
      size_t const llhSize = ZSTD_buildSeqTable(
          ...
          iend - ip, ZSTDConstants::LL_base, ZSTDInternalConstants::LL_bits,
...

    {
      size_t const ofhSize = ZSTD_buildSeqTable(
          ...
          iend - ip, ZSTDConstants::OF_base, ZSTDConstants::OF_bits,
...

    {
      size_t const mlhSize = ZSTD_buildSeqTable(
          ...
          iend - ip, ZSTDConstants::ML_base, ZSTDInternalConstants::ML_bits,
 ...
 * WILDCOPY_OVERLENGTH past the input buffer and write up to 16 bytes past
 * oend_w (op >= oend_w is allowed). This function is only called in the
 * uncommon case where the sequence is near the end of the block. It should be
 * fast for a single long sequence, but can be slow for several short sequences.
 *
 ...
 * any distance apart. The src buffer must be before the dst buffer.
 */
static void ZSTD_safecopy(BYTE* op, BYTE* const oend_w, BYTE const* ip,
 ...
 * requires more careful checks to make sure there is no overflow. By separating
 * out these hard and unlikely cases, we can speed up the common cases.
 *
 * NOTE: This function needs to be fast for a single long sequence, but doesn't
 * need to be optimized for many small sequences, since those fall into
 * ZSTD_execSequence().
 */
FORCE_NOINLINE
...
size_t ZSTD_execSequence(BYTE* op, BYTE* const oend, seq_t sequence,
  ...
  BYTE* const oend_w =
      oend -
      WILDCOPY_OVERLENGTH; /* risk : address space underflow on oend=NULL */
  const BYTE* const iLitEnd = *litPtr + sequence.litLength;
  const BYTE* match = oLitEnd - sequence.offset;
  ...
  *litPtr = iLitEnd; /* update for next sequence */
   ...
   * wildcopy without overlap checking.
   */
...
FORCE_INLINE_TEMPLATE void ZSTD_updateFseStateWithDInfo(
    ZSTD_fseState* DStatePtr, BIT_DStream_t* bitD, ZSTD_seqSymbol const DInfo) {
  U32 const nbBits = DInfo.nbBits;
  size_t const lowBits = BIT_readBits(bitD, nbBits);
  DStatePtr->state = DInfo.nextState + lowBits;
}
...
ZSTD_decodeSequence(seqState_t* seqState, const ZSTD_longOffset_e longOffsets,
  ...
  ZSTD_seqSymbol const mlDInfo =
      seqState->stateML.table[seqState->stateML.state];
  ZSTD_seqSymbol const ofDInfo =
  ...
  if (prefetch == ZSTD_p_prefetch) {
    ...
    seq.match =
        matchBase + pos -
        seq.offset; /* note : this operation can overflow when seq.offset is
                     * really too large, which can only happen when input is
                     * corrupted. No consequence though : no memory access will
                     * occur, offset is only used for prefetching */
...
MEM_STATIC void ZSTD_assertValidSequence(ZSTD_DCtx const* dctx, BYTE const* op,
  ...
  if (nbSeq) {
    ...
    for (;;) {
      seq_t const sequence =
          ZSTD_decodeSequence(&seqState, isLongOffset, ZSTD_p_noPrefetch);
...
#define ADVANCED_SEQS 4
    ...
    for (seqNb = 0;
         ...
         seqNb++) {
      sequences[seqNb] =
          ZSTD_decodeSequence(&seqState, isLongOffset, ZSTD_p_prefetch);
      PREFETCH_L1(sequences[seqNb].match);
      PREFETCH_L1(sequences[seqNb].match + sequences[seqNb].matchLength -
                  1); /* note : it's safe to invoke PREFETCH() on any memory
                         address, including invalid ones */
    }
    ...
    for (; seqNb < nbSeq; seqNb++) {
      size_t const oneSeqSize =
          ZSTD_execSequence(op, oend, sequences[seqNb & STORED_SEQS_MASK],
                            &litPtr, litEnd, prefixStart, dictStart, dictEnd);
#if defined(FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION) && \
    defined(FUZZING_ASSERT_VALID_SEQUENCE)
      assert(!ZSTD_isError(oneSeqSize));
 ...
 * considered "long", aka out of cache. note : "long" definition seems
 * overloaded here, sometimes meaning "wider than bitstream register", and
 * sometimes meaning "farther than memory cache distance". This function will
 * try to mitigate main memory latency through the use of prefetching */
...
size_t ZSTD_decompressBlock_internal(
    ...
    size_t srcSize, const int frame) { /* blockType == blockCompressed */
