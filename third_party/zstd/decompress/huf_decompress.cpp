...
size_t HUF_readDTableX1_wksp(HUF_DTable* DTable, const void* src,
  ...
  /* Table header */
  {
    ...
    if (tableLog > (U32) (dtd.maxTableLog + 1))
      return ERROR(
          tableLog_tooLarge); /* DTable too small, Huffman tree cannot fit in */
    dtd.tableType = 0;
...
size_t HUF_decompress1X1_usingDTable(void* dst, size_t dstSize,
...
size_t HUF_decompress4X1_usingDTable(void* dst, size_t dstSize,
                                     const void* cSrc, size_t cSrcSize,
                                     const HUF_DTable* DTable) {
  DTableDesc dtd = HUF_getDTableDesc(DTable);
  if (dtd.tableType != 0)
...
static void HUF_fillDTableX2Level2(HUF_DEltX2* DTable, U32 sizeLog,
...
static void HUF_fillDTableX2(HUF_DEltX2* DTable, const U32 targetLog,
  ...
  for (s = 0; s < sortedListSize; s++) {
    ...
    if (targetLog - nbBits >= minBits) { /* enough room for a second symbol */
      ...
      HUF_fillDTableX2Level2(
          DTable + start, targetLog - nbBits, nbBits, rankValOrigin[nbBits],
          minWeight, sortedList + sortedRank, sortedListSize - sortedRank,
...
FORCE_INLINE_TEMPLATE U32 HUF_decodeLastSymbolX2(void* op,
  ...
  if (dt[val].length == 1)
    ...
    if (DStream->bitsConsumed < (sizeof(DStream->bitContainer) * 8)) {
      BIT_skipBits(DStream, dt[val].nbBits);
      if (DStream->bitsConsumed > (sizeof(DStream->bitContainer) * 8))
        /* ugly hack; works only because it's the last symbol. Note : can't
         * easily extract nbBits from just this symbol */
...
size_t HUF_decompress1X2_usingDTable(void* dst, size_t dstSize,
                                     const void* cSrc, size_t cSrcSize,
                                     const HUF_DTable* DTable) {
  DTableDesc dtd = HUF_getDTableDesc(DTable);
  if (dtd.tableType != 1)
...
size_t HUF_decompress4X2_usingDTable(void* dst, size_t dstSize,
                                     const void* cSrc, size_t cSrcSize,
                                     const HUF_DTable* DTable) {
  DTableDesc dtd = HUF_getDTableDesc(DTable);
  if (dtd.tableType != 1)
...
size_t HUF_decompress(void* dst, size_t dstSize, const void* cSrc,
                      size_t cSrcSize) {
#if !defined(HUF_FORCE_DECOMPRESS_X1) && !defined(HUF_FORCE_DECOMPRESS_X2)
...
size_t HUF_decompress4X_hufOnly(HUF_DTable* dctx, void* dst, size_t dstSize,
                                const void* cSrc, size_t cSrcSize) {
  U32 workSpace[HUF_DECOMPRESS_WORKSPACE_SIZE_U32];
...
size_t HUF_decompress4X_hufOnly_wksp_bmi2(HUF_DTable* dctx, void* dst,
...

  {
    ...
    return HUF_decompress4X2_DCtx_wksp_bmi2(dctx, dst, dstSize, cSrc, cSrcSize,
                                            workSpace, wkspSize, bmi2);
#else
    return algoNb ? HUF_decompress4X2_DCtx_wksp_bmi2(dctx, dst, dstSize, cSrc,
                                                     cSrcSize, workSpace,
                                                     wkspSize, bmi2)
