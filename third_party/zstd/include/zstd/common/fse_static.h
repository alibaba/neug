 ...
 * (`workSpace`). FSE_WKSP_SIZE_U32() provides the minimum size required for
 * `workSpace` as a table of FSE_CTable.
 */
 ...
 * (`workSpace`). `wkspSize` must be >= `(1<<tableLog)`.
 */
...
/*!
   ...
   inlined. Hence their body are included in next section.
*/
typedef struct {
  ptrdiff_t value;
...
reverse direction**. So the first symbol you will encode is the last you will
decode, like a LIFO stack.
...
tested, using FSE_isError(); You can then encode your input data, byte after
byte. FSE_encodeSymbol() outputs a maximum of 'tableLog' bits at a time.
...
register, for speed. Local register size is 64-bits on 64-bits systems, 32-bits
on 32-bits systems (size_t). Writing data to memory is a manual operation,
performed by the flushBits function. BIT_flushBits(&bitStream);
...
FSE_isError()). size_t size = BIT_closeCStream(&bitStream);
*/
...
'tableLog'. Keep in mind that symbols are decoded in reverse order, like a LIFO
stack (last in, first out). unsigned char symbol = FSE_decodeSymbol(&DState,
&DStream);

You can retrieve any bitfield you eventually stored into the bitStream (in
reverse order) Note : maximum allowed nbBits is 25, for 32-bits compatibility
    size_t bitField = BIT_readBits(&DStream, nbBits);

All above operations only read from local register (which size depends on
size_t). Refueling the register from memory is manually performed by the reload
method. endSignal = FSE_reloadDStream(&DStream);

BIT_reloadDStream() result tells if there is still some more data to read from
DStream. BIT_DStream_unfinished : there is still some data left into the
DStream. BIT_DStream_endOfBuffer : Dstream reached end of buffer. Its container
may no longer be completely filled. BIT_DStream_completed : Dstream reached its
exact end, corresponding in general to decompression completed.
BIT_DStream_tooFar : Dstream went too far. Decompression result is corrupted.

When reaching end of buffer (BIT_DStream_endOfBuffer), progress slowly, notably
if you decode multiple symbols per loop, to properly detect the exact end of
stream. After each decoded symbol, check if DStream is fully consumed using this
simple test : BIT_reloadDStream(&DStream) >= BIT_DStream_completed

When it's done, verify decompression is fully completed, by checking both
DStream and the relevant states. Checking if DStream has reached its end is
performed by : BIT_endOfDStream(&DStream); Check also the states. There might be
some symbols left there, if some high probability ones (>50%) are possible.
 ...
 * the last to be read) uses the smallest state value possible, saving the cost
 * of this symbol */
 ...
 * gives the same result as a frequency of 2) note 1 : assume symbolValue is
 * valid (<= maxSymbolValue) note 2 : if freq[symbolValue]==0, @return a fake
 * cost of tableLog+1 bits */
 ...
 * (accuracyLog fractional bits) note 1 : assume symbolValue is valid (<=
 * maxSymbolValue) note 2 : if freq[symbolValue]==0, @return a fake cost of
 * tableLog+1 bits */
MEM_STATIC U32 FSE_bitCost(const void* symbolTTPtr, U32 tableLog,
  ...
  assert(
      accuracyLog <
      31 - tableLog); /* ensure enough room for renormalization double shift */
  {
