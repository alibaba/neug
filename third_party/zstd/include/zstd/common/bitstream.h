 ...
 * performance. Since link-time-optimization is not available for all compilers,
 ...
 * 32-bits systems. Writing data into memory is an explicit operation, performed
 * by the flushBits function. Hence keep track how many bits are potentially
 * stored into local register to avoid register overflow. After a flushBits, a
 * maximum of 7 bits might still be stored into local register.
 *
 ...
 * systems (size_t). You can then retrieve bitFields stored into the local
 * register, **in reverse order**. Local register is explicitly reloaded from
 * memory by the BIT_reloadDStream() method. A reload guarantee a minimum of
 * ((8*sizeof(bitD->bitContainer))-7) bits when its result is
...
MEM_STATIC void BIT_addBits(BIT_CStream_t* bitC, size_t value,
                            unsigned nbBits) {
...
MEM_STATIC size_t BIT_getMiddleBits(size_t bitContainer, U32 const start,
                                    U32 const nbBits) {
...
MEM_STATIC BIT_DStream_status BIT_reloadDStream(BIT_DStream_t* bitD) {
  ...
  /* start < ptr < limitPtr */
  {
    ...
    bitD->bitContainer = MEM_readLEST(
        bitD->ptr); /* reminder : srcSize > sizeof(bitD->bitContainer),
                       otherwise bitD->ptr == bitD->start */
    return result;
  }
}
