...
typedef struct {
  ...
  unsigned*
      matchLengthFreq; /* table of matchLength statistics, of size (MaxML+1) */
...
struct ZSTD_matchState_t {
  ...
  U32 hashLog3; /* dispatch table for matches of len==3 : larger == faster, more
                   memory */
  U32* hashTable;
  U32* hashTable3;
  U32* chainTable;
...
struct ZSTD_CCtx_params_s {
  ...
  size_t
      targetCBlockSize; /* Tries to fit compressed block size to be around
                         ...
                         * 0. There is no guarantee on compressed block size */
  int srcSizeHint;      /* User's best guess of source size.
                         * Hint is not valid when srcSizeHint == 0.
                         * There is no guarantee that hint is close to actual source
                         * size */
...
MEM_STATIC repcodes_t ZSTD_updateRep(U32 const rep[3], U32 const offset,
                                     U32 const ll0) {
  repcodes_t newReps;
...
MEM_STATIC size_t ZSTD_noCompressBlock(void* dst, size_t dstCapacity,
  ...
  RETURN_ERROR_IF(
      srcSize + ZSTDInternalConstants::ZSTD_blockHeaderSize > dstCapacity,
      dstSize_tooSmall, "dst buf too small for uncompressed block");
  MEM_writeLE24(dst, cBlockHeader24);
  memcpy((BYTE*) dst + ZSTDInternalConstants::ZSTD_blockHeaderSize, src,
         srcSize);
...
MEM_STATIC size_t ZSTD_rleCompressBlock(void* dst, size_t dstCapacity, BYTE src,
  ...
  U32 const cBlockHeader =
      lastBlock + (((U32) bt_rle) << 1) + (U32) (srcSize << 3);
...
HINT_INLINE UNUSED_ATTR void ZSTD_storeSeq(seqStore_t* seqStorePtr,
                                           size_t litLength,
                                           const BYTE* literals,
                                           const BYTE* litLimit, U32 offCode,
                                           size_t mlBase) {
  ...
  if (g_start == NULL)
    g_start = (const BYTE*) literals; /* note : index only works for compression
...
static unsigned ZSTD_NbCommonBytes(size_t val) {
  if (MEM_isLittleEndian()) {
    ...
    if (MEM_64bits()) {
      ...
      unsigned r;
      const unsigned n32 =
          sizeof(size_t) * 4; /* calculate this way due to compiler complaining
                                 in 32-bits mode */
      if (!(val >> n32)) {
        r = 4;
      } else {
...
MEM_STATIC U32 ZSTD_window_correctOverflow(ZSTD_window_t* window, U32 cycleLog,
  ...
  U32 const currentCycle1 =
      currentCycle0 == 0 ? (1U << cycleLog) : currentCycle0;
 ...
 */
MEM_STATIC void ZSTD_window_enforceMaxDist(
    ZSTD_window_t* window, const void* blockEnd, U32 maxDist,
...
MEM_STATIC void ZSTD_checkDictValidity(
    const ZSTD_window_t* window, const void* blockEnd, U32 maxDist,
    U32* loadedDictEndPtr, const ZSTD_matchState_t** dictMatchStatePtr) {
  ...
  assert(dictMatchStatePtr != NULL);
  {
    ...
    if (blockEndIdx > loadedDictEnd + maxDist) {
       ...
       * block, the dictionary is invalidated for the full block.
       */
      DEBUGLOG(
          6,
