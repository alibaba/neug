...
MEM_STATIC void ZSTD_cwksp_internal_advance_phase(
  ...
  if (phase > ws->phase) {
    ...
    if (ws->phase < ZSTD_cwksp_alloc_aligned &&
      ...
      ws->allocStart = (BYTE*) ws->allocStart -
                       ((size_t) ws->allocStart & (sizeof(U32) - 1));
...
MEM_STATIC void* ZSTD_cwksp_reserve_internal(ZSTD_cwksp* ws, size_t bytes,
  ...
  DEBUGLOG(5, "cwksp: reserving %p %zd bytes, %zd bytes remaining", alloc,
...
MEM_STATIC void* ZSTD_cwksp_reserve_aligned(ZSTD_cwksp* ws, size_t bytes) {
  ...
  return ZSTD_cwksp_reserve_internal(ws, ZSTD_cwksp_align(bytes, sizeof(U32)),
                                     ZSTD_cwksp_alloc_aligned);
}
...
MEM_STATIC void ZSTD_cwksp_clean_tables(ZSTD_cwksp* ws) {
  ...
  assert(ws->tableValidEnd <= ws->allocStart);
  if (ws->tableValidEnd < ws->tableEnd) {
    memset(ws->tableValidEnd, 0,
           (BYTE*) ws->tableEnd - (BYTE*) ws->tableValidEnd);
  }
...
MEM_STATIC void ZSTD_cwksp_init(ZSTD_cwksp* ws, void* start, size_t size) {
  ...
  assert(((size_t) start & (sizeof(void*) - 1)) ==
