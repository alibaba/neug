...
#define ZSTD_PREDEF_THRESHOLD                                                  \
  1024 /* if srcSize < ZSTD_PREDEF_THRESHOLD, symbols' cost is assumed static, \
 ...
 * symbols otherwise downscale existing stats, to be used as seed for next
 * block.
 */
    ...
    if (optPtr->symbolCosts->huf.repeatMode == HUF_repeat_valid) {
...

      {
        ...
        for (ml = 0; ml <= MaxML; ml++) {
          ...
          assert(bitCost < scaleLog);
          optPtr->matchLengthFreq[ml] = bitCost
                                            ? 1 << (scaleLog - bitCost)
                                            : 1 /*minimum to calculate cost*/;
...
static U32 ZSTD_rawLiteralsCost(const BYTE* const literals, U32 const litLength,
  ...
  if (!ZSTD_compressedLiterals(optPtr))
    return (litLength << 3) *
           BITCOST_MULTIPLIER; /* Uncompressed - 8 bytes per literal. */
  ...
  /* dynamic statistics */
  {
    ...
    for (u = 0; u < litLength; u++) {
      assert(
          WEIGHT(optPtr->litFreq[literals[u]], optLevel) <=
 ...
 * sequence. optLevel: when <2, favors small offset for decompression speed
...
FORCE_INLINE_TEMPLATE U32 ZSTD_getMatchPrice(U32 const offset,
  ...
  if ((optLevel < 2) /*static*/ && offCode >= 20)
    price += (offCode - 19) * 2 *
             BITCOST_MULTIPLIER; /* handicap for long distance offsets, favor
                                    decompression speed */

  /* match Length */
  {
    U32 const mlCode = ZSTD_MLcode(mlBase);
    price += (ZSTDInternalConstants::ML_bits[mlCode] * BITCOST_MULTIPLIER) +
...
static void ZSTD_updateStats(optState_t* const optPtr, U32 litLength,
                             const BYTE* literals, U32 offsetCode,
                             U32 matchLength) {
  /* literals */
  if (ZSTD_compressedLiterals(optPtr)) {
    U32 u;
...
static U32 ZSTD_insertBt1(ZSTD_matchState_t* ms, const BYTE* const ip,
                          const BYTE* const iend, U32 const mls,
                          const int extDict) {
...
U32 ZSTD_insertBtAndGetAllMatches(
    ...
    ZSTD_matchState_t* ms, U32* nextToUpdate3, const BYTE* const ip,
    const BYTE* const iLimit, const ZSTD_dictMode_e dictMode,
    const U32 rep[ZSTD_REP_NUM],
    U32 const ll0, /* tells if associated literal length is 0 or not. This value
                      must be 0 or 1 */
    const U32 lengthToBeat, U32 const mls /* template */) {
  const ZSTD_compressionParameters* const cParams = &ms->cParams;
  ...
  assert(ll0 <= 1); /* necessarily 1 or 0 */
  {
    ...
    for (repCode = ll0; repCode < lastR; repCode++) {
      ...
      if (repOffset - 1 /* intentional overflow, discards 0 and -1 */ <
          current -
              dictLimit) { /* equivalent to `current > repIndex >= dictLimit` */
        /* We must validate the repcode offset because when we're using a
         * dictionary the valid offset range shrinks when the dictionary goes
         * out of bounds.
         */
        ...
        if ( dictMode == ZSTD_extDict
          ...
          repLen =
              (U32) ZSTD_count_2segments(ip + minMatch, repMatch + minMatch,
        ...
        if (dictMode == ZSTD_dictMatchState &&
            (((repOffset - 1) /*intentional overflow*/ <
              current -
                  (dmsLowLimit + dmsIndexDelta)) /* equivalent to `current >
                                                    repIndex >= dmsLowLimit` */
             & ((U32) ((dictLimit - 1) - repIndex) >=
                3)) /* intentional overflow : do not test positions overlapping
                       2 memory segments */
            && (ZSTD_readMINMATCH(ip, minMatch) ==
...
FORCE_INLINE_TEMPLATE U32 ZSTD_BtGetAllMatches(
    ZSTD_match_t*
        matches, /* store result (match found, increasing size) in this table */
    ZSTD_matchState_t* ms, U32* nextToUpdate3, const BYTE* ip,
    const BYTE* const iHighLimit, const ZSTD_dictMode_e dictMode,
...
static U32 ZSTD_totalLen(ZSTD_optimal_t sol) { return sol.litlen + sol.mlen; }
...
FORCE_INLINE_TEMPLATE size_t ZSTD_compressBlock_opt_generic(
    ...
    const void* src, size_t srcSize, const int optLevel,
    const ZSTD_dictMode_e dictMode) {
  ...
  while (ip < ilimit) {
    ...
    /* find first match */
    {
      ...
      U32 const nbMatches = ZSTD_BtGetAllMatches(
          matches, ms, &nextToUpdate3, ip, iend, dictMode, rep, ll0, minMatch);
      if (!nbMatches) {
        ip++;
        continue;
      }
      ...
      /* large match -> immediate encoding */
      {
        ...
        DEBUGLOG(6,
                 "found %u matches of maxLength=%u and maxOffCode=%u at "
        ...
        for (matchNb = 0; matchNb < nbMatches; matchNb++) {
          ...
          for (; pos <= end; pos++) {
            U32 const matchPrice =
                ZSTD_getMatchPrice(offset, pos, optStatePtr, optLevel);
    ...
    for (cur = 1; cur <= last_pos; cur++) {
      ...
      /* Fix current position with one literal if cheaper */
      {
        ...
        if (price <= opt[cur].price) {
          DEBUGLOG(7,
                   "cPos:%zi==rPos:%u : better price (%.2f<=%.2f) using "
                   "literal (ll==%u) (hist:%u,%u,%u)",
          ...
          DEBUGLOG(7,
                   ...
                   "(hist:%u,%u,%u)",
                   inr - istart, cur, ZSTD_fCost(price),
                   ZSTD_fCost(opt[cur].price), opt[cur].rep[0], opt[cur].rep[1],
                   opt[cur].rep[2]);
        }
      }
      ...
      if (opt[cur].mlen != 0) {
        ...
        repcodes_t newReps =
            ZSTD_updateRep(opt[prev].rep, opt[cur].off, opt[cur].litlen == 0);
...

      {
        ...
        U32 const nbMatches =
            ZSTD_BtGetAllMatches(matches, ms, &nextToUpdate3, inr, iend,
...

        {
          ...
          if ((maxML > sufficient_len) || (cur + maxML >= ZSTD_OPT_NUM)) {
            ...
            cur -= (opt[cur].mlen == 0)
                       ? opt[cur].litlen
                       : 0; /* last sequence is actually only literals, fix cur
                               to last match - note : may underflow, in which
                               case, it's first sequence, and it's okay */
  ...
  _shortestPath: /* cur, last_pos, best_mlen, best_off have to be set */
    ...
    if (lastSequence.mlen != 0) {
      repcodes_t reps = ZSTD_updateRep(opt[cur].rep, lastSequence.off,
...

    {
      ...
      DEBUGLOG(6, "sending selected sequences into seqStore") {
        ...
        for (storePos = storeStart; storePos <= storeEnd; storePos++) {
          ...
          if (mlen == 0) { /* only literals => must be last "sequence", actually
                              starting a new stream of sequences */
 ...
 * values. only works on first block, with no dictionary and no ldm. this
 * function cannot error, hence its contract must be respected.
 */
static void ZSTD_initStats_ultra(ZSTD_matchState_t* ms, seqStore_t* seqStore,
                                 U32 rep[ZSTD_REP_NUM], const void* src,
                                 size_t srcSize) {
  ...
  ZSTD_compressBlock_opt_generic(ms, seqStore, tmpRep, src, srcSize,
                                 2 /*optLevel*/,
...
size_t ZSTD_compressBlock_btultra2(ZSTD_matchState_t* ms, seqStore_t* seqStore,
   ...
   * tables, aka, no dictionary, no prefix, no ldm preprocessing. The
   * compression ratio gain is generally small (~0.5% on first block), the cost
   * is 2x cpu time on first block. */
