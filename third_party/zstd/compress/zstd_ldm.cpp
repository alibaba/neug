...
static U64 ZSTD_ldm_fillLdmHashTable(ldmState_t* state, U64 lastHash,
...
void ZSTD_ldm_fillHashTable(ldmState_t* state, const BYTE* ip, const BYTE* iend,
  ...
  if ((size_t) (iend - ip) >= params->minMatchLength) {
    ...
    ZSTD_ldm_fillLdmHashTable(state, startingHash, ip,
                              iend - params->minMatchLength, state->window.base,
...
static void ZSTD_ldm_limitTableUpdate(ZSTD_matchState_t* ms,
  ...
  if (current > ms->nextToUpdate + 1024) {
    ms->nextToUpdate = current - MIN(512, current - ms->nextToUpdate - 1024);
  }
}
...
static size_t ZSTD_ldm_generateSequences_internal(ldmState_t* ldmState,
  ...
  while (ip <= ilimit) {
    ...
    if (ip != istart) {
      rollingHash = ZSTD_rollingHash_rotate(
          rollingHash, lastHashed[0], lastHashed[minMatchLength], hashPower);
    } else {
      ...
      for (cur = bucket; cur < bucket + ldmBucketSize; ++cur) {
        ...
        if (extDict) {
          ...
          curForwardMatchLength =
              ZSTD_count_2segments(ip, pMatch, iend, matchEnd, lowPrefixPtr);
          ...
          curBackwardMatchLength =
              ZSTD_ldm_countBackwardsMatch(ip, anchor, pMatch, lowPrefixPtr);
          curTotalMatchLength = curForwardMatchLength + curBackwardMatchLength;
        }
    ...
    if (bestEntry == NULL) {
      ZSTD_ldm_makeEntryAndInsertByTag(ldmState, rollingHash, hBits, current,
                                       *params);
      ip++;
      continue;
    }
