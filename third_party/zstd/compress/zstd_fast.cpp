...
#pragma vector always
  ...
  while (ip1 < ilimit) { /* < instead of <=, because check at ip0+2 */
    ...
    {
      size_t const step =
          ((size_t) (ip0 - anchor) >> (kSearchStrength - 1)) + stepSize;
      assert(step >= 2);
      ip0 += step;
      ip1 += step;
      continue;
    ...
    if (ip0 <= ilimit) {
      ...
      if (offset_2 > 0) { /* offset_2==0 means offset_2 is invalidated */
        while ((ip0 <= ilimit) &&
          ...
          ip0 += rLength;
          ZSTD_storeSeq(seqStore, 0 /*litLen*/, anchor, iend, 0 /*offCode*/,
                        rLength - MINMATCH);
          anchor = ip0;
...
size_t ZSTD_compressBlock_fast_dictMatchState_generic(
  ...
  while (ip < ilimit) { /* < instead of <=, because repcode check at (ip+1) */
    ...
    if (((U32) ((prefixStartIndex - 1) - repIndex) >=
         3) /* intentional underflow : ensure repIndex isn't overlapping dict +
               prefix */
...
size_t ZSTD_compressBlock_fast_dictMatchState(ZSTD_matchState_t* ms,
  ...
  default: /* includes case 3 */
  case 4:
    return ZSTD_compressBlock_fast_dictMatchState_generic(ms, seqStore, rep,
                                                          src, srcSize, 4);
  case 5:
    return ZSTD_compressBlock_fast_dictMatchState_generic(ms, seqStore, rep,
                                                          src, srcSize, 5);
  ...
  while (ip < ilimit) { /* < instead of <=, because (ip+1) */
    ...
    if ((((U32) ((prefixStartIndex - 1) - repIndex) >=
      ...
      const BYTE* const repMatchEnd =
          repIndex < prefixStartIndex ? dictEnd : iend;
      size_t const rLength =
          ZSTD_count_2segments(ip + 1 + 4, repMatch + 4, iend, repMatchEnd,
                               prefixStart) +
          4;
      ip++;
      ZSTD_storeSeq(seqStore, (size_t) (ip - anchor), anchor, iend, 0,
