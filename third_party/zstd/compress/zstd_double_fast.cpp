...
size_t ZSTD_compressBlock_doubleFast_generic(ZSTD_matchState_t* ms,
                                             seqStore_t* seqStore,
  ...
  if (dictMode == ZSTD_noDict) {
    U32 const current = (U32) (ip - base);
    U32 const windowLow =
        ZSTD_getLowestPrefixIndex(ms, current, cParams->windowLog);
...

  {
    ...
    if (dictMode == ZSTD_dictMatchState && matchIndexS < prefixLowestIndex) {
      mLength =
          ZSTD_count_2segments(ip + 4, match + 4, iend, dictEnd, prefixLowest) +
          4;
  ...
  _match_stored:
    ...
    if (ip <= ilimit) {
      ...
      if (dictMode == ZSTD_dictMatchState) {
        while (ip <= ilimit) {
          ...
          const BYTE* repMatch2 =
              dictMode == ZSTD_dictMatchState && repIndex2 < prefixLowestIndex
      ...
      if (dictMode == ZSTD_noDict) {
        while (
            (ip <= ilimit) &&
            ((offset_2 > 0) & (MEM_read32(ip) == MEM_read32(ip - offset_2)))) {
          /* store sequence */
...
static size_t ZSTD_compressBlock_doubleFast_extDict_generic(
  ...
  while (ip < ilimit) { /* < instead of <=, because (ip+1) */
    ...
    if ((((U32) ((prefixStartIndex - 1) - repIndex) >=
          3) /* intentional underflow : ensure repIndex doesn't overlap dict +
                prefix */
    ...
    if (ip <= ilimit) {
      ...
      while (ip <= ilimit) {
        ...
        if ((((U32) ((prefixStartIndex - 1) - repIndex2) >=
              3) /* intentional overflow : ensure repIndex2 doesn't overlap dict
                    + prefix */
