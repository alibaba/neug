 ...
 * (`workSpace`). wkspSize should be sized to handle worst case situation, which
 * is `1<<max_tableLog * sizeof(FSE_FUNCTION_TYPE)` workSpace must also be
 * properly aligned with FSE_FUNCTION_TYPE requirements
 */
size_t FSE_buildCTable_wksp(FSE_CTable* ct, const short* normalizedCounter,
  ...
  /* Build table */
  {
    ...
    for (u = 0; u < tableSize; u++) {
      ...
      tableU16[cumul[s]++] =
          (U16) (tableSize + u); /* TableU16 : sorted by symbol order; gives
                                    next state value */
    }
  }
...
size_t FSE_NCountWriteBound(unsigned maxSymbolValue, unsigned tableLog) {
  ...
  return maxSymbolValue ? maxHeaderSize
...
static size_t FSE_writeNCount_generic(void* header, size_t headerBufferSize,
  ...
  while ((symbol < alphabetSize) && (remaining > 1)) { /* stops at 1 */
    ...
    }
    {
      ...
      if (count >= threshold)
        count += max; /* [0..max[ [max..threshold[ (...) [threshold+max
                         2*threshold[ */
...
size_t FSE_normalizeCount(short* normalizedCounter, unsigned tableLog,
  ...
  if (tableLog < FSE_minTableLog(total, maxSymbolValue))
    return ERROR(
        GENERIC); /* Too small tableLog, compression potentially impossible */

  {
    ...
    if (-stillToDistribute >= (normalizedCounter[largest] >> 1)) {
      ...
      size_t const errorCode = FSE_normalizeM2(normalizedCounter, tableLog,
                                               count, total, maxSymbolValue);
 ...
 * (`workSpace`). `wkspSize` size must be `(1<<tableLog)`.
 */
size_t FSE_compress_wksp(void* dst, size_t dstSize, const void* src,
  ...
  {
    CHECK_V_F(nc_err,
              FSE_writeNCount(op, oend - op, norm, maxSymbolValue, tableLog));
    op += nc_err;
  }
  ...
  {
    CHECK_V_F(cSize,
              FSE_compress_usingCTable(op, oend - op, src, srcSize, CTable));
    if (cSize == 0)
...
size_t FSE_compress2(void* dst, size_t dstCapacity, const void* src,
                     size_t srcSize, unsigned maxSymbolValue,
                     unsigned tableLog) {
  ...
  DEBUG_STATIC_ASSERT(
      ...
      FSE_WKSP_SIZE_U32(
          FSE_MAX_TABLELOG,
          FSE_MAX_SYMBOL_VALUE)); /* compilation failures here means
...
size_t FSE_compress(void* dst, size_t dstCapacity, const void* src,
                    size_t srcSize) {
