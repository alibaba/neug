...
static size_t ZSTD_buildSuperBlockEntropy_literal(
    ...
    const int disableLiteralsCompression, void* workspace, size_t wkspSize) {
...
static size_t ZSTD_buildSuperBlockEntropy_sequences(
    seqStore_t* seqStorePtr, const ZSTD_fseCTables_t* prevEntropy,
  ...
  /* build CTable for Literal Lengths */
  {
    ...
    size_t const mostFrequent =
        HIST_countFast_wksp(countWksp, &max, llCodeTable, nbSeq, workspace,
                            wkspSize); /* can't fail */
                 ...
                 FSE_repeat_none)); /* We don't copy tables */
    {
      size_t const countSize = ZSTD_buildCTable(
          ...
          (symbolEncodingType_e) LLtype, countWksp, max, llCodeTable, nbSeq,
          ZSTDInternalConstants::LL_defaultNorm,
  ...
  /* build CTable for Offsets */
  {
    ...
    unsigned max = MaxOff;
    size_t const mostFrequent =
        HIST_countFast_wksp(countWksp, &max, ofCodeTable, nbSeq, workspace,
                            wkspSize); /* can't fail */
    ...
    Offtype = ZSTD_selectEncodingType(
        &nextEntropy->offcode_repeatMode, countWksp, max, mostFrequent, nbSeq,
                 ...
                 FSE_repeat_none)); /* We don't copy tables */
    {
      size_t const countSize = ZSTD_buildCTable(
          ...
          (symbolEncodingType_e) Offtype, countWksp, max, ofCodeTable, nbSeq,
          ZSTDInternalConstants::OF_defaultNorm,
  ...
  /* build CTable for MatchLengths */
  {
    ...
    unsigned max = MaxML;
    size_t const mostFrequent =
        HIST_countFast_wksp(countWksp, &max, mlCodeTable, nbSeq, workspace,
                            wkspSize); /* can't fail */
                 ...
                 FSE_repeat_none)); /* We don't copy tables */
    {
      size_t const countSize = ZSTD_buildCTable(
          ...
          (symbolEncodingType_e) MLtype, countWksp, max, mlCodeTable, nbSeq,
          ZSTDInternalConstants::ML_defaultNorm,
          ZSTDInternalConstants::ML_defaultNormLog, MaxML,
          prevEntropy->matchlengthCTable,
          sizeof(prevEntropy->matchlengthCTable), cTableWksp, cTableWkspSize);
...
static size_t ZSTD_buildSuperBlockEntropy(
  ...
  entropyMetadata->hufMetadata.hufDesSize = ZSTD_buildSuperBlockEntropy_literal(
      ...
      ZSTD_disableLiteralsCompression(cctxParams), workspace, wkspSize);
 ...
 *  table size. If we guessed incorrectly, we fall back to uncompressed
 * literals.
 *
 *  We write the header when writeEntropy=1 and set entropyWrriten=1 when we
 * succeeded in writing the header, otherwise it is set to 0.
 *
 *  hufMetadata->hType has literals block type info.
 *      If it is set_basic, all sub-blocks literals section will be
 * Raw_Literals_Block. If it is set_rle, all sub-blocks literals section will be
 * RLE_Literals_Block. If it is set_compressed, first sub-block's literals
 * section will be Compressed_Literals_Block If it is set_compressed, first
 * sub-block's literals section will be Treeless_Literals_Block and the
 * following sub-blocks' literals sections will be Treeless_Literals_Block.
...
static size_t ZSTD_compressSubBlock_literal(
  ...
  *entropyWritten = 0;
  ...
  /* TODO bmi2 */
  {
    const size_t cSize = singleStream
                             ? HUF_compress1X_usingCTable(
                                   op, oend - op, literals, litSize, hufTable)
    ...
    if (!writeEntropy && cLitSize >= litSize) {
      DEBUGLOG(5,
               "ZSTD_compressSubBlock_literal using raw literal because "
               "uncompressible");
      return ZSTD_noCompressLiterals(dst, dstSize, literals, litSize);
    }
  ...
  case 3: /* 2 - 2 - 10 - 10 */
  {
    U32 const lhc = hType + ((!singleStream) << 2) + ((U32) litSize << 4) +
                    ((U32) cLitSize << 14);
  ...
  *entropyWritten = 0;
...

  {
    size_t const bitstreamSize = ZSTD_encodeSequences(
        op, oend - op, fseTables->matchlengthCTable, mlCode,
...
static size_t ZSTD_compressSubBlock(
           ...
           litSize, nbSeq, writeLitEntropy, writeSeqEntropy, lastBlock);
  {
    size_t cLitSize = ZSTD_compressSubBlock_literal(
        ...
        literals, litSize, op, oend - op, bmi2, writeLitEntropy,
        litEntropyWritten);
  ...
  }
  {
    size_t cSeqSize = ZSTD_compressSubBlock_sequences(
        ...
        mlCode, ofCode, cctxParams, op, oend - op, bmi2, writeSeqEntropy,
        seqEntropyWritten);
...
static size_t ZSTD_estimateSubBlockSize_symbolType(
    ...
    size_t nbSeq, const FSE_CTable* fseCTable, const U32* additionalBits,
    short const* defaultNorm, U32 defaultNormLog, void* workspace,
    size_t wkspSize) {
  unsigned* const countWksp = (unsigned*) workspace;
  ...
  HIST_countFast_wksp(countWksp, &max, codeTable, nbSeq, workspace,
                      wkspSize); /* can't fail */
  if (type == set_basic) {
  ...
  while (ctp < ctEnd) {
    if (additionalBits)
      ...
      cSymbolTypeSizeEstimateInBits += *ctp; /* for offset, offset code is also
                                                the number of additional bits */
    ctp++;
  }
...
static size_t ZSTD_estimateSubBlockSize_sequences(
  ...
  cSeqSizeEstimate += ZSTD_estimateSubBlockSize_symbolType(
      ...
      NULL, ZSTDInternalConstants::OF_defaultNorm,
      ZSTDInternalConstants::OF_defaultNormLog, workspace, wkspSize);
  cSeqSizeEstimate += ZSTD_estimateSubBlockSize_symbolType(
      fseMetadata->llType, llCodeTable, MaxLL, nbSeq,
      fseTables->litlengthCTable, ZSTDInternalConstants::LL_bits,
      ZSTDInternalConstants::LL_defaultNorm,
      ZSTDInternalConstants::LL_defaultNormLog, workspace, wkspSize);
  cSeqSizeEstimate += ZSTD_estimateSubBlockSize_symbolType(
      fseMetadata->mlType, mlCodeTable, MaxML, nbSeq,
      fseTables->matchlengthCTable, ZSTDInternalConstants::ML_bits,
      ZSTDInternalConstants::ML_defaultNorm,
      ZSTDInternalConstants::ML_defaultNormLog, workspace, wkspSize);
  if (writeEntropy)
    cSeqSizeEstimate += fseMetadata->fseTablesSize;
  return cSeqSizeEstimate + sequencesSectionHeaderSize;
}

static size_t ZSTD_estimateSubBlockSize(
...
static int ZSTD_needSequenceEntropyTables(
    ZSTD_fseCTablesMetadata_t const* fseMetadata) {
...
static size_t ZSTD_compressSubBlock_multi(
  ...
  do {
    ...
    cBlockSizeEstimate = ZSTD_estimateSubBlockSize(
        lp, litSize, ofCodePtr, llCodePtr, mlCodePtr, seqCount,
        &nextCBlock->entropy, entropyMetadata, workspace, wkspSize,
        writeLitEntropy, writeSeqEntropy);
    if (cBlockSizeEstimate > targetCBlockSize || lastSequence) {
      int litEntropyWritten = 0;
      int seqEntropyWritten = 0;
      const size_t decompressedSize = ZSTD_seqDecompressedSize(
          seqStorePtr, sp, seqCount, litSize, lastSequence);
      const size_t cSize = ZSTD_compressSubBlock(
          &nextCBlock->entropy, entropyMetadata, sp, seqCount, lp, litSize,
          llCodePtr, mlCodePtr, ofCodePtr, cctxParams, op, oend - op, bmi2,
          writeLitEntropy, writeSeqEntropy, &litEntropyWritten,
          &seqEntropyWritten, lastBlock && lastSequence);
  ...
  if (writeLitEntropy) {
    DEBUGLOG(
        5, "ZSTD_compressSubBlock_multi has literal entropy tables unwritten");
    memcpy(&nextCBlock->entropy.huf, &prevCBlock->entropy.huf,
  ...
  if (writeSeqEntropy &&
     ...
     */
    DEBUGLOG(
        5, "ZSTD_compressSubBlock_multi has sequence entropy tables unwritten");
    return 0;
  }
  if (ip < iend) {
    size_t const cSize =
        ZSTD_noCompressBlock(op, oend - op, ip, iend - ip, lastBlock);
    DEBUGLOG(
        5, "ZSTD_compressSubBlock_multi last sub-block uncompressed, %zu bytes",
        (size_t) (iend - ip));
...
size_t ZSTD_compressSuperBlock(ZSTD_CCtx* zc, void* dst, size_t dstCapacity,
  ...
  return ZSTD_compressSubBlock_multi(
      ...
      zc->bmi2, lastBlock, zc->entropyWorkspace,
      HUF_WORKSPACE_SIZE /* statically allocated in resetCCtx */);
}
