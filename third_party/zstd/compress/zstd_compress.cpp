...
struct ZSTD_CDict_s {
  ...
  U32 dictID;
  int compressionLevel; /* 0 indicates that advanced API was used to select
                           CDict params */
...
ZSTD_CCtx* ZSTD_initStaticCCtx(void* workspace, size_t workspaceSize) {
  ...
  cctx->entropyWorkspace =
      (U32*) ZSTD_cwksp_reserve_object(&cctx->workspace, HUF_WORKSPACE_SIZE);
  cctx->bmi2 = 0;
  return cctx;
}
...
static ZSTD_CCtx_params* ZSTD_createCCtxParams_advanced(
  ...
  if (!customMem.customAlloc ^ !customMem.customFree)
    return NULL;
  params = (ZSTD_CCtx_params*) ZSTD_calloc(sizeof(ZSTD_CCtx_params), customMem);
  if (!params) {
    return NULL;
  }
...
ZSTD_CCtx_params* ZSTD_createCCtxParams(void) {
  return ZSTD_createCCtxParams_advanced(
      ZSTDInternalConstants::ZSTD_defaultCMem);
}
...
ZSTD_bounds ZSTD_cParam_getBounds(ZSTD_cParameter param) {
  ...
  case ZSTD_c_format:
    ...
    bounds.upperBound =
        ZSTD_f_zstd1_magicless; /* note : how to ensure at compile time that
  ...
  case ZSTD_c_forceAttachDict:
    ...
    bounds.upperBound =
        ZSTD_dictForceLoad; /* note : how to ensure at compile time that this is
                               the highest value enum ? */
    return bounds;

  case ZSTD_c_literalCompressionMode:
    ZSTD_STATIC_ASSERT(ZSTD_lcm_auto < ZSTD_lcm_huffman &&
                       ZSTD_lcm_huffman < ZSTD_lcm_uncompressed);
...
size_t ZSTD_CCtxParams_setParameter(ZSTD_CCtx_params* CCtxParams,
  ...
  case ZSTD_c_checksumFlag:
    /* A 32-bits content checksum will be calculated and written at end of frame
     * (default:0) */
    CCtxParams->fParams.checksumFlag = value != 0;
    return CCtxParams->fParams.checksumFlag;

  case ZSTD_c_dictIDFlag: /* When applicable, dictionary's dictID is provided in
  ...
  case ZSTD_c_literalCompressionMode: {
    const ZSTD_literalCompressionMode_e lcm =
        (ZSTD_literalCompressionMode_e) value;
...
ZSTDLIB_API size_t ZSTD_CCtx_setPledgedSrcSize(
    ZSTD_CCtx* cctx, unsigned long long pledgedSrcSize) {
...
static size_t ZSTD_initLocalDict(ZSTD_CCtx* cctx) {
  ...
  dl->cdict =
      ZSTD_createCDict_advanced(dl->dict, dl->dictSize, ZSTD_dlm_byRef,
                                dl->dictContentType, cParams, cctx->customMem);
...
static ZSTD_compressionParameters ZSTD_clampCParams(
    ZSTD_compressionParameters cParams) {
 ...
 *  condition : cPar is presumed validated (can be checked using
 * ZSTD_checkCParams()). */
static ZSTD_compressionParameters ZSTD_adjustCParams_internal(
    ZSTD_compressionParameters cPar, unsigned long long srcSize,
    size_t dictSize) {
  ...
  if (cPar.windowLog < ZSTD_WINDOWLOG_ABSOLUTEMIN)
    cPar.windowLog = ZSTD_WINDOWLOG_ABSOLUTEMIN; /* minimum wlog required for
                                                    valid frame header */
...
ZSTD_compressionParameters ZSTD_adjustCParams(ZSTD_compressionParameters cPar,
                                              ...
                                              size_t dictSize) {
  cPar = ZSTD_clampCParams(cPar); /* resulting cPar is necessarily valid (all
...
static ZSTD_compressionParameters ZSTD_getCParams_internal(
    int compressionLevel, unsigned long long srcSizeHint, size_t dictSize);
...
static size_t ZSTD_sizeof_matchState(
    const ZSTD_compressionParameters* const cParams, const U32 forCCtx) {
...
size_t ZSTD_estimateCCtxSize_usingCCtxParams(const ZSTD_CCtx_params* params) {
      ...
      "Estimate CCtx size is supported for single-threaded compression only.");
  {
    ...
    size_t const bufferSpace =
        ZSTD_cwksp_alloc_size(0) + ZSTD_cwksp_alloc_size(0);

    size_t const cctxSpace = ZSTD_cwksp_alloc_size(sizeof(ZSTD_CCtx));

    size_t const neededSpace = cctxSpace + entropySpace + blockStateSpace +
...
static size_t ZSTD_estimateCCtxSize_internal(int compressionLevel) {
  ZSTD_compressionParameters const cParams =
      ZSTD_getCParams_internal(compressionLevel, ZSTD_CONTENTSIZE_UNKNOWN, 0);
...
size_t ZSTD_estimateCStreamSize_usingCCtxParams(
      ...
      "Estimate CCtx size is supported for single-threaded compression only.");
  {
    ...
    size_t const streamingSize =
        ZSTD_cwksp_alloc_size(inBuffSize) + ZSTD_cwksp_alloc_size(outBuffSize);
 ...
 * current frame. able to count progression inside worker threads (non-blocking
 * mode).
 */
ZSTD_frameProgression ZSTD_getFrameProgression(const ZSTD_CCtx* cctx) {
...
#endif
  {
    ...
    fp.flushed = cctx->producedCSize; /* simplified; some data might still be
                                         left within streaming output buffer */
    fp.currentJobID = 0;
...
static size_t ZSTD_reset_matchState(
    ZSTD_matchState_t* ms, ZSTD_cwksp* ws,
    const ZSTD_compressionParameters* cParams, const ZSTD_compResetPolicy_e crp,
  ...
  assert(!ZSTD_cwksp_reserve_failed(
      ws)); /* check that allocation hasn't already failed */
  ...
  if ((forWho == ZSTD_resetTarget_CCtx) && (cParams->strategy >= ZSTD_btopt)) {
    ...
    ms->opt.litFreq = (unsigned*) ZSTD_cwksp_reserve_aligned(
        ws, (1 << Litbits) * sizeof(unsigned));
    ms->opt.litLengthFreq = (unsigned*) ZSTD_cwksp_reserve_aligned(
        ws, (MaxLL + 1) * sizeof(unsigned));
    ms->opt.matchLengthFreq = (unsigned*) ZSTD_cwksp_reserve_aligned(
        ws, (MaxML + 1) * sizeof(unsigned));
    ms->opt.offCodeFreq = (unsigned*) ZSTD_cwksp_reserve_aligned(
...
static size_t ZSTD_resetCCtx_internal(ZSTD_CCtx* zc, ZSTD_CCtx_params params,
...

  {
    ...
    ZSTD_indexResetPolicy_e needsIndexReset =
        zc->initialized ? ZSTDirp_continue : ZSTDirp_reset;
    ...
    /* Check if workspace is large enough, alloc a new one if needed */
    {
      size_t const cctxSpace =
          zc->staticSize ? ZSTD_cwksp_alloc_size(sizeof(ZSTD_CCtx)) : 0;
      size_t const entropySpace = ZSTD_cwksp_alloc_size(HUF_WORKSPACE_SIZE);
      size_t const blockStateSpace =
          2 * ZSTD_cwksp_alloc_size(sizeof(ZSTD_compressedBlockState_t));
      ...
      size_t const ldmSeqSpace =
          ZSTD_cwksp_alloc_size(maxNbLdmSeq * sizeof(rawSeq));
      ...
      DEBUGLOG(4,
               "Need %zuKB workspace, including %zuKB for match state, and "
               "%zuKB for buffers",
      ...
      if (workspaceTooSmall || workspaceWasteful) {
        ...
        zc->entropyWorkspace =
            (U32*) ZSTD_cwksp_reserve_object(ws, HUF_WORKSPACE_SIZE);
        RETURN_ERROR_IF(zc->blockState.nextCBlock == NULL, memory_allocation,
    ...
    FORWARD_IF_ERROR(
        ZSTD_reset_matchState(&zc->blockState.matchState, ws, &params.cParams,
                              crp, needsIndexReset, ZSTD_resetTarget_CCtx),
        "");
...
static size_t ZSTD_resetCCtx_byCopyingCDict(ZSTD_CCtx* cctx,
  ...
  /* copy tables */
  {
    ...
    memcpy(cctx->blockState.matchState.hashTable, cdict->matchState.hashTable,
           hSize * sizeof(U32));
    memcpy(cctx->blockState.matchState.chainTable, cdict->matchState.chainTable,
 ...
 * call to ZSTD_compressContinue()). The "context", in this case, refers to the
 * hash and chain tables, entropy tables, and dictionary references. `windowLog`
...
static size_t ZSTD_copyCCtx_internal(ZSTD_CCtx* dstCCtx,
  ...
  memcpy(&dstCCtx->customMem, &srcCCtx->customMem, sizeof(ZSTD_customMem));
  {
    ...
    ZSTD_resetCCtx_internal(dstCCtx, params, pledgedSrcSize, ZSTDcrp_leaveDirty,
                            zbuff);
 ...
 *  Duplicate an existing context `srcCCtx` into another one `dstCCtx`.
 *  Only works during stage ZSTDcs_init (i.e. after creation, but before first
 * call to ZSTD_compressContinue()). pledgedSrcSize==0 means "unknown".
 *   @return : 0, or an error code */
...
FORCE_INLINE_TEMPLATE void ZSTD_reduceTable_internal(U32* const table,
  ...
  for (rowNb = 0; rowNb < nbRows; rowNb++) {
    ...
    for (column = 0; column < ZSTD_ROWSIZE; column++) {
      if (preserveMark) {
        U32 const adder =
            (table[cellNb] == ZSTD_DUBT_UNSORTED_MARK) ? reducerValue : 0;
        table[cellNb] += adder;
      }
 ...
 * If used, compression will do best effort to make a compressed block size to
 * be around targetCBlockSize. Returns 1 if true, 0 otherwise. */
...
MEM_STATIC size_t ZSTD_compressSequences_internal(
  ...
  /* build CTable for Literal Lengths */
  {
    ...
    size_t const mostFrequent =
        HIST_countFast_wksp(count, &max, llCodeTable, nbSeq, entropyWorkspace,
                            entropyWkspSize); /* can't fail */
    DEBUGLOG(5, "Building LL table");
    nextEntropy->fse.litlength_repeatMode =
        prevEntropy->fse.litlength_repeatMode;
    LLtype = ZSTD_selectEncodingType(&nextEntropy->fse.litlength_repeatMode,
                                     count, max, mostFrequent, nbSeq, LLFSELog,
                                     prevEntropy->fse.litlengthCTable,
                                     ZSTDInternalConstants::LL_defaultNorm,
                                     ZSTDInternalConstants::LL_defaultNormLog,
                 ...
                 FSE_repeat_none)); /* We don't copy tables */
    {
      size_t const countSize = ZSTD_buildCTable(
          ...
          ZSTDInternalConstants::LL_defaultNorm,
          ZSTDInternalConstants::LL_defaultNormLog, MaxLL,
          prevEntropy->fse.litlengthCTable,
          sizeof(prevEntropy->fse.litlengthCTable), entropyWorkspace,
          entropyWkspSize);
  ...
  /* build CTable for Offsets */
  {
    unsigned max = MaxOff;
    size_t const mostFrequent =
        HIST_countFast_wksp(count, &max, ofCodeTable, nbSeq, entropyWorkspace,
                            entropyWkspSize); /* can't fail */
    /* We can only use the basic table if max <= DefaultMaxOff, otherwise the
    ...
    Offtype = ZSTD_selectEncodingType(
        &nextEntropy->fse.offcode_repeatMode, count, max, mostFrequent, nbSeq,
        OffFSELog, prevEntropy->fse.offcodeCTable,
        ZSTDInternalConstants::OF_defaultNorm,
        ZSTDInternalConstants::OF_defaultNormLog, defaultPolicy, strategy);
                 ...
                 FSE_repeat_none)); /* We don't copy tables */
    {
      size_t const countSize = ZSTD_buildCTable(
          ...
          ZSTDInternalConstants::OF_defaultNorm,
          ZSTDInternalConstants::OF_defaultNormLog, DefaultMaxOff,
          prevEntropy->fse.offcodeCTable,
          sizeof(prevEntropy->fse.offcodeCTable), entropyWorkspace,
          entropyWkspSize);
  ...
  /* build CTable for MatchLengths */
  {
    unsigned max = MaxML;
    size_t const mostFrequent =
        HIST_countFast_wksp(count, &max, mlCodeTable, nbSeq, entropyWorkspace,
                            entropyWkspSize); /* can't fail */
    DEBUGLOG(5, "Building ML table (remaining space : %i)", (int) (oend - op));
    nextEntropy->fse.matchlength_repeatMode =
        prevEntropy->fse.matchlength_repeatMode;
    MLtype = ZSTD_selectEncodingType(&nextEntropy->fse.matchlength_repeatMode,
                                     count, max, mostFrequent, nbSeq, MLFSELog,
                                     prevEntropy->fse.matchlengthCTable,
                                     ZSTDInternalConstants::ML_defaultNorm,
                                     ZSTDInternalConstants::ML_defaultNormLog,
                                     ZSTD_defaultAllowed, strategy);
                 ...
                 FSE_repeat_none)); /* We don't copy tables */
    {
      size_t const countSize = ZSTD_buildCTable(
          ...
          ZSTDInternalConstants::ML_defaultNorm,
          ZSTDInternalConstants::ML_defaultNormLog, MaxML,
          prevEntropy->fse.matchlengthCTable,
          sizeof(prevEntropy->fse.matchlengthCTable), entropyWorkspace,
          entropyWkspSize);
...
MEM_STATIC size_t ZSTD_compressSequences(
    seqStore_t* seqStorePtr, const ZSTD_entropyCTables_t* prevEntropy,
    ZSTD_entropyCTables_t* nextEntropy, const ZSTD_CCtx_params* cctxParams,
...
static size_t ZSTD_buildSeqStore(ZSTD_CCtx* zc, const void* src,
  ...
  if (srcSize <
      MIN_CBLOCK_SIZE + ZSTDInternalConstants::ZSTD_blockHeaderSize + 1) {
    ZSTD_ldm_skipSequences(&zc->externSeqStore, srcSize,
                           zc->appliedParams.cParams.minMatch);
    return ZSTDbss_noCompress; /* don't even attempt compression below a certain
                                  srcSize */
  }
  ...
  /* select and store sequences */
  {
    ...
    if (zc->externSeqStore.pos < zc->externSeqStore.size) {
      ...
      lastLLSize =
          ZSTD_ldm_blockCompress(&zc->externSeqStore, ms, &zc->seqStore,
      ...
      /* Updates ldmSeqStore.pos */
      lastLLSize =
          ZSTD_ldm_blockCompress(&ldmSeqStore, ms, &zc->seqStore,
                                 zc->blockState.nextCBlock->rep, src, srcSize);
...
static void ZSTD_copyBlockSequences(ZSTD_CCtx* zc) {
  ...
  ZSTD_Sequence* outSeqs =
      &zc->seqCollector.seqStart[zc->seqCollector.seqIndex];
  size_t i;
  size_t position;
  int repIdx;
  ...
  for (i = 0, position = 0; i < seqsSize; ++i) {
    ...
    if (outSeqs[i].offset <= ZSTD_REP_NUM) {
      ...
      assert(repIdx >= -3);
      outSeqs[i].offset =
          repIdx >= 0 ? outSeqs[repIdx].offset
                      : ZSTDInternalConstants::repStartValue[-repIdx - 1];
      if (outSeqs[i].rep == 4) {
        --outSeqs[i].offset;
      }
    } else {
...
static size_t ZSTD_compressBlock_targetCBlockSize_body(
  ...
  if (bss == ZSTDbss_compress) {
    if (/* We don't want to emit our first block as a RLE even if it qualifies
         * because doing so will cause the decoder (cli only) to throw a "should
         * consume all input error." This is only an issue for zstd <= v1.4.3
         */
     ...
     *
     * Note that compressed size of ZSTD_compressSuperBlock() is not bound by
     * the standard ZSTD_compressBound(). This is a problem, because even if we
     * have space now, taking an extra byte now could cause us to run out of
     * space later and violate ZSTD_compressBound().
     *
     ...
     *   * cSize == dstSize_tooSmall: We may have expanded beyond
     * blockBound(srcSize). ZSTD_noCompressBlock() will return dstSize_tooSmall
     * if we are really out of output space.
     ...
     */
    {
      ...
      if (cSize != ERROR(dstSize_tooSmall)) {
        ...
        if (cSize != 0 &&
            cSize < maxCSize + ZSTDInternalConstants::ZSTD_blockHeaderSize) {
          ZSTD_confirmRepcodesAndEntropyTables(zc);
          return cSize;
        }
      }
  ...
  while (remaining) {
...

    {
      ...
      if (ZSTD_useTargetCBlockSize(&cctx->appliedParams)) {
        ...
        assert(cSize > 0);
        assert(cSize <=
               blockSize + ZSTDInternalConstants::ZSTD_blockHeaderSize);
      } else {
        cSize = ZSTD_compressBlock_internal(
            cctx, op + ZSTDInternalConstants::ZSTD_blockHeaderSize,
            dstCapacity - ZSTDInternalConstants::ZSTD_blockHeaderSize, ip,
            blockSize, 1 /* frame */);
...
static size_t ZSTD_loadDictionaryContent(ZSTD_matchState_t* ms, ldmState_t* ls,
  ...
  if (params->ldmParams.enableLdm && ls != NULL) {
    ...
    ls->loadedDictEnd =
        params->forceWindow ? 0 : (U32) (iend - ls->window.base);
  }
  ...
  while (iend - ip > HASH_READ_SIZE) {
    ...
    if (params->ldmParams.enableLdm && ls != NULL)
      ZSTD_ldm_fillHashTable(ls, (const BYTE*) src, (const BYTE*) src + srcSize,
                             &params->ldmParams);
...
size_t ZSTD_loadCEntropy(ZSTD_compressedBlockState_t* bs, void* workspace,
...

  {
    ...
    size_t const hufHeaderSize =
        HUF_readCTable((HUF_CElt*) bs->entropy.huf.CTable, &maxSymbolValue,
                       dictPtr, dictEnd - dictPtr, &hasZeroWeights);
...

  {
    ...
    /* Defer checking offcodeMaxValue because we need to know the size of the
     * dictionary content */
...

  {
    ...
    FORWARD_IF_ERROR(
        ZSTD_checkDictNCount(matchlengthNCount, matchlengthMaxValue, MaxML),
        "");
    RETURN_ERROR_IF(FSE_isError(FSE_buildCTable_wksp(
...
static size_t ZSTD_loadZstdDictionary(ZSTD_compressedBlockState_t* bs,
  ...
  dictID = params->fParams.noDictIDFlag
               ? 0
               : MEM_readLE32(dictPtr + 4 /* skip magic number */);
  eSize = ZSTD_loadCEntropy(bs, workspace, offcodeNCount, &offcodeMaxValue,
                            dict, dictSize);
...

  {
    ...
    if (dictContentSize <= ((U32) -1) - 128 KB) {
      ...
      offcodeMax =
          ZSTD_highbit32(maxOffset); /* Calculate minimum offset code required
                                        to represent maxOffset */
    }
    /* All offset values <= dictContentSize + 128 KB must be representable */
    FORWARD_IF_ERROR(ZSTD_checkDictNCount(offcodeNCount, offcodeMaxValue,
...
static size_t ZSTD_compress_insertDictionary(
  ...
  return ZSTD_loadZstdDictionary(bs, ms, ws, params, dict, dictSize, dtlm,
                                 workspace);
}
                   ...
                   "");
  {
    size_t const dictID =
        cdict
            ...
            : ZSTD_compress_insertDictionary(
                  cctx->blockState.prevCBlock, &cctx->blockState.matchState,
                  &cctx->ldmState, &cctx->workspace, &cctx->appliedParams, dict,
...
size_t ZSTD_compressBegin_usingDict(ZSTD_CCtx* cctx, const void* dict,
  ...
  ZSTD_parameters const params = ZSTD_getParams_internal(
      compressionLevel, ZSTD_CONTENTSIZE_UNKNOWN, dictSize);
  ZSTD_CCtx_params const cctxParams =
      ZSTD_assignParamsToCCtxParams(&cctx->requestedParams, &params);
...
size_t ZSTD_compress_usingDict(ZSTD_CCtx* cctx, void* dst, size_t dstCapacity,
  ...
  ZSTD_parameters const params =
      ZSTD_getParams_internal(compressionLevel, srcSize, dict ? dictSize : 0);
  ZSTD_CCtx_params cctxParams =
      ZSTD_assignParamsToCCtxParams(&cctx->requestedParams, &params);
...
static size_t ZSTD_initCDict_internal(ZSTD_CDict* cdict, const void* dictBuffer,
  ...
  FORWARD_IF_ERROR(ZSTD_reset_matchState(&cdict->matchState, &cdict->workspace,
                                         &cParams, ZSTDcrp_makeClean,
   ...
   */
  {
    ...
    params.cParams = cParams;
    {
      size_t const dictID = ZSTD_compress_insertDictionary(
          ...
          ZSTD_dtlm_full, cdict->entropyWorkspace);
      FORWARD_IF_ERROR(dictID, "ZSTD_compress_insertDictionary failed");
...
ZSTD_CDict* ZSTD_createCDict(const void* dict, size_t dictSize,
  ...
  ZSTD_CDict* cdict = ZSTD_createCDict_advanced(
      dict, dictSize, ZSTD_dlm_byCopy, ZSTD_dct_auto, cParams,
      ZSTDInternalConstants::ZSTD_defaultCMem);
  if (cdict)
...
const ZSTD_CDict* ZSTD_initStaticCDict(void* workspace, size_t workspaceSize,
  ...
  if (ZSTD_isError(ZSTD_initCDict_internal(
          cdict, dict, dictSize, dictLoadMethod, dictContentType, cParams)))
    return NULL;
...
size_t ZSTD_compress_usingCDict_advanced(ZSTD_CCtx* cctx, void* dst,
                                         ...
                                         ZSTD_frameParameters fParams) {
  FORWARD_IF_ERROR(
      ZSTD_compressBegin_usingCDict_advanced(cctx, cdict, fParams, srcSize),
 ...
 * dictionary is used multiple times. Note that compression parameters are
 * decided at CDict creation time while frame parameters are hardcoded */
size_t ZSTD_compress_usingCDict(ZSTD_CCtx* cctx, void* dst, size_t dstCapacity,
                                const void* src, size_t srcSize,
                                const ZSTD_CDict* cdict) {
  ZSTD_frameParameters const fParams = {1 /*content*/, 0 /*checksum*/,
                                        0 /*noDictID*/};
  return ZSTD_compress_usingCDict_advanced(cctx, dst, dstCapacity, src, srcSize,
                                           cdict, fParams);
}
...
ZSTD_CStream* ZSTD_createCStream_advanced(
    ZSTD_customMem customMem) { /* CStream and CCtx are now same object */
...
size_t ZSTD_CStreamOutSize(void) {
  return ZSTD_compressBound(ZSTD_BLOCKSIZE_MAX) +
         ZSTDInternalConstants::ZSTD_blockHeaderSize + 4 /* 32-bits hash */;
}

static size_t ZSTD_resetCStream_internal(
    ZSTD_CStream* cctx, const void* const dict, size_t const dictSize,
    ZSTD_dictContentType_e const dictContentType, const ZSTD_CDict* const cdict,
  ...
  FORWARD_IF_ERROR(ZSTD_compressBegin_internal(
                       cctx, dict, dictSize, dictContentType, ZSTD_dtlm_fast,
                       cdict, &params, pledgedSrcSize, ZSTDb_buffered),
                   "");
  ...
  cctx->inBuffTarget =
      cctx->blockSize +
      (cctx->blockSize ==
       pledgedSrcSize); /* for small input: avoid automatic flush on reaching
                           end of block, since it would require to add a 3-bytes
                           null block to end frame */
...
static size_t ZSTD_compressStream_generic(ZSTD_CStream* zcs,
  ...
  while (someMoreWork) {
    ...
    case zcss_load:
      ...
      /* compress current block (note : this stage cannot be stopped in the
       * middle) */
...
size_t ZSTD_compressStream2(ZSTD_CCtx* cctx, ZSTD_outBuffer* output,
  ...
  if (cctx->streamStage == zcss_init) {
    ...
    if (params.nbWorkers > 0) {
      ...
      if (cctx->mtctx == NULL) {
        DEBUGLOG(4, "ZSTD_compressStream2: creating new mtctx for nbWorkers=%u",
                 params.nbWorkers);
        cctx->mtctx =
            ZSTDMT_createCCtx_advanced((U32) params.nbWorkers, cctx->customMem);
    ...
    {
      FORWARD_IF_ERROR(
          ZSTD_resetCStream_internal(cctx, prefixDict.dict, prefixDict.dictSize,
  ...
  if (cctx->appliedParams.nbWorkers > 0) {
    ...
    size_t flushMin;
    assert(forceMaxProgress ||
           endOp == ZSTD_e_continue /* Protection for a new flush type */);
 ...
 * level, srcSize and dictSize. Note: srcSizeHint 0 means 0, use
 * ZSTD_CONTENTSIZE_UNKNOWN for unknown. Use dictSize == 0 for unknown or
 * unused. */
static ZSTD_compressionParameters ZSTD_getCParams_internal(
    int compressionLevel, unsigned long long srcSizeHint, size_t dictSize) {
  int const unknown = srcSizeHint == ZSTD_CONTENTSIZE_UNKNOWN;
  size_t const addedSize = unknown && dictSize > 0 ? 500 : 0;
  U64 const rSize = unknown && dictSize == 0
                        ? ZSTD_CONTENTSIZE_UNKNOWN
...
/*! ZSTD_getCParams() :
 * @return ZSTD_compressionParameters structure for a selected compression
 * level, srcSize and dictSize. Size values are optional, provide 0 if not known
 * or unused */
 ...
 * @return a `ZSTD_parameters` structure (instead of
 * `ZSTD_compressionParameters`). Fields of `ZSTD_frameParameters` are set to
 * default values */
static ZSTD_parameters ZSTD_getParams_internal(int compressionLevel,
                                               unsigned long long srcSizeHint,
                                               size_t dictSize) {
  ZSTD_parameters params;
  ZSTD_compressionParameters const cParams =
      ZSTD_getCParams_internal(compressionLevel, srcSizeHint, dictSize);
