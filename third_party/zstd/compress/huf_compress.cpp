...
static size_t HUF_compressWeights(void* dst, size_t dstSize,
  ...
  {
    CHECK_V_F(hSize, FSE_writeNCount(op, (size_t) (oend - op), norm,
  ...
  {
    CHECK_V_F(cSize, FSE_compress_usingCTable(op, (size_t) (oend - op),
                                              weightTable, wtSize, CTable));
    if (cSize == 0)
...
size_t HUF_writeCTable(void* dst, size_t maxDstSize, const HUF_CElt* CTable,
  ...
  if (maxSymbolValue > (256 - 128))
    return ERROR(GENERIC); /* should not happen : likely means source cannot be
                              compressed */
...
size_t HUF_readCTable(HUF_CElt* CTable, unsigned* maxSymbolValuePtr,
  ...
  BYTE huffWeight[HUF_SYMBOLVALUE_MAX +
                  1]; /* init not required, even though some static analyzer may
                         complain */
...
static U32 HUF_setMaxHeight(nodeElt* huffNode, U32 lastNonNull, U32 maxNbBits) {
  ...
  /* there are several too large elements (at least >= 2) */
  {
    ...
    /* renorm totalCost */
    totalCost >>= (largestBits - maxNbBits); /* note : totalCost is necessarily
                                                a multiple of baseCost */
      ...
      while (totalCost > 0) {
        ...
        if (rankLast[nBitsToDecrease - 1] == noSymbol)
          rankLast[nBitsToDecrease - 1] =
              rankLast[nBitsToDecrease]; /* this rank is no longer empty */
...
static void HUF_sort(nodeElt* huffNode, const unsigned* count,
                     U32 maxSymbolValue, rankPos* rankPosition) {
  U32 n;
  ...
  for (n = 0; n < 32; n++)
    rankPosition[n].current = rankPosition[n].base;
  for (n = 0; n <= maxSymbolValue; n++) {
    U32 const c = count[n];
 ...
 * as sizeof(HUF_buildCTable_wksp_tables).
 */
 ...
 */
size_t HUF_buildCTable(HUF_CElt* tree, const unsigned* count,
                       unsigned maxSymbolValue, unsigned maxNbBits) {
  HUF_buildCTable_wksp_tables workspace;
...
FORCE_INLINE_TEMPLATE void HUF_encodeSymbol(BIT_CStream_t* bitCPtr, U32 symbol,
...
FORCE_INLINE_TEMPLATE size_t HUF_compress1X_usingCTable_internal_body(
...
static size_t HUF_compress1X_usingCTable_internal_default(
    void* dst, size_t dstSize, const void* src, size_t srcSize,
    const HUF_CElt* CTable) {
  return HUF_compress1X_usingCTable_internal_body(dst, dstSize, src, srcSize,
                                                  CTable);
...
static size_t HUF_compress1X_usingCTable_internal(void* dst, size_t dstSize,
                                                  ...
                                                  const int bmi2) {
  if (bmi2) {
    return HUF_compress1X_usingCTable_internal_bmi2(dst, dstSize, src, srcSize,
                                                    CTable);
  }
  return HUF_compress1X_usingCTable_internal_default(dst, dstSize, src, srcSize,
                                                     CTable);
}
...
static size_t HUF_compress4X_usingCTable_internal(void* dst, size_t dstSize,
  ...
  {
    CHECK_V_F(cSize,
              HUF_compress1X_usingCTable_internal(op, (size_t) (oend - op), ip,
  ...
  assert(ip <= iend);
  {
    CHECK_V_F(cSize, HUF_compress1X_usingCTable_internal(
                         op, (size_t) (oend - op), ip, (size_t) (iend - ip),
                         CTable, bmi2));
    if (cSize == 0)
      return 0;
    op += cSize;
...
static size_t HUF_compressCTable_internal(BYTE* const ostart, BYTE* op,
                                          BYTE* const oend, const void* src,
                                          size_t srcSize,
                                          HUF_nbStreams_e nbStreams,
                                          const HUF_CElt* CTable,
                                          const int bmi2) {
  size_t const cSize =
      (nbStreams == HUF_singleStream)
          ? HUF_compress1X_usingCTable_internal(op, (size_t) (oend - op), src,
                                                srcSize, CTable, bmi2)
          : HUF_compress4X_usingCTable_internal(op, (size_t) (oend - op), src,
                                                srcSize, CTable, bmi2);
...
static size_t HUF_compress_internal(void* dst, size_t dstSize, const void* src,
                                    ...
                                    unsigned huffLog, HUF_nbStreams_e nbStreams,
  ...
  {
    CHECK_V_F(largest,
              HIST_count_wksp(table->count, &maxSymbolValue, (const BYTE*) src,
  ...
  /* Write table description header */
  {
    CHECK_V_F(hSize, HUF_writeCTable(op, dstSize, table->CTable, maxSymbolValue,
                                     huffLog));
  ...
  return HUF_compressCTable_internal(ostart, op, oend, src, srcSize, nbStreams,
                                     table->CTable, bmi2);
}

size_t HUF_compress1X_wksp(void* dst, size_t dstSize, const void* src,
                           size_t srcSize, unsigned maxSymbolValue,
                           unsigned huffLog, void* workSpace, size_t wkspSize) {
...
size_t HUF_compress2(void* dst, size_t dstSize, const void* src, size_t srcSize,
                     unsigned maxSymbolValue, unsigned huffLog) {
  unsigned workSpace[HUF_WORKSPACE_SIZE_U32];
...
size_t HUF_compress(void* dst, size_t maxDstSize, const void* src,
                    size_t srcSize) {
