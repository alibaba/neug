 ...
 *  doesn't fail */
static void ZSTD_insertDUBT1(ZSTD_matchState_t* ms, U32 current,
                             const BYTE* inputEnd, U32 nbCompares, U32 btLow,
  ...
  U32 matchIndex =
      *smallerPtr; /* this candidate is unsorted : next sorted candidate is
                      reached through *smallerPtr, while *largerPtr contains
                      previous unsorted candidate (which is already saved and
                      can be overwritten) */
  U32 dummy32;     /* to be nullified at the end */
  U32 const windowValid = ms->window.lowLimit;
  U32 const maxDistance = 1U << cParams->windowLog;
  U32 const windowLow = (current - windowValid > maxDistance)
                            ? current - maxDistance
                            : windowValid;
  ...
  while (nbCompares-- && (matchIndex > windowLow)) {
    ...
    if ((dictMode != ZSTD_extDict) ||
      ...
      if (matchIndex + matchLength >= dictLimit)
        match =
            base +
            matchIndex; /* preparation for next read of match[matchLength] */
    }
    ...
    if (ip + matchLength == iend) { /* equal : no way to know if inf or sup */
      break; /* drop , to guarantee consistency ; miss a bit of compression, but
...
static size_t ZSTD_DUBT_findBetterDictMatch(
  ...
  while (nbCompares-- && (dictMatchIndex > dictLowLimit)) {
    ...
    if (matchLength > bestLength) {
      ...
      if (ip + matchLength ==
          iend) { /* reached end of input : ip[matchLength] is not valid, no way
                     to know if it's larger or smaller than match */
...
static size_t ZSTD_DUBT_findBestMatch(ZSTD_matchState_t* ms,
  ...
  U32 const current = (U32) (ip - base);
  U32 const windowLow =
      ZSTD_getLowestMatchIndex(ms, current, cParams->windowLog);
  ...
  while ((matchIndex > unsortLimit) &&
    ...
    DEBUGLOG(8, "ZSTD_DUBT_findBestMatch: candidate %u is unsorted",
             matchIndex);
    *unsortedMark =
        previousCandidate; /* the unsortedMark becomes a reversed chain, to move
                              up back to original position */
...
FORCE_INLINE_TEMPLATE size_t ZSTD_BtFindBestMatch(
    ...
    size_t* offsetPtr, const U32 mls /* template */,
    const ZSTD_dictMode_e dictMode) {
...
size_t ZSTD_HcFindBestMatch_generic(ZSTD_matchState_t* ms, const BYTE* const ip,
  ...
  const U32 withinMaxDistance = (current - lowestValid > maxDistance)
                                    ? current - maxDistance
                                    : lowestValid;
  ...
  for (; (matchIndex > lowLimit) & (nbAttempts > 0); nbAttempts--) {
    ...
    if ((dictMode != ZSTD_extDict) || matchIndex >= dictLimit) {
      ...
      if (MEM_read32(match) ==
          MEM_read32(ip)) /* assumption : matchIndex <= dictLimit-4 (by table
                             construction) */
...
FORCE_INLINE_TEMPLATE size_t ZSTD_compressBlock_lazy_generic(
  ...
  if (dictMode == ZSTD_noDict) {
    U32 const current = (U32) (ip - base);
    U32 const windowLow =
        ZSTD_getLowestPrefixIndex(ms, current, ms->cParams.windowLog);
   ...
   * code alignment is perturbed. To fix the instability align the loop on
   * 32-bytes.
   */
  __asm__(".p2align 5");
#endif
    ...
    if (depth >= 1)
      while (ip < ilimit) {
        ...
        }
        {
          ...
          int const gain1 =
              (int) (matchLength * 4 - ZSTD_highbit32((U32) offset + 1) + 4);
...
size_t ZSTD_compressBlock_lazy_extDict_generic(
  ...
  typedef size_t (*searchMax_f)(ZSTD_matchState_t* ms, const BYTE* ip,
                                const BYTE* iLimit, size_t* offsetPtr);
  searchMax_f searchMax = searchMethod == search_binaryTree
  ...
  while (ip < ilimit) {
    ...
    {
      const U32 windowLow =
          ZSTD_getLowestMatchIndex(ms, current + 1, windowLog);
    ...
    if (depth >= 1)
      while (ip < ilimit) {
        ...
        if (offset) {
          const U32 windowLow =
              ZSTD_getLowestMatchIndex(ms, current, windowLog);
