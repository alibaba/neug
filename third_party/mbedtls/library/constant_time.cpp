...
/*
 * The following functions are implemented without using comparison operators,
 * as those might be translated to branches by some compilers on some platforms.
 */
 ...
 * */
static int mbedtls_ct_cond_select_sign(unsigned char condition, int if1,
                                       int if0) {
 ...
 */
static void mbedtls_ct_mem_move_to_left(void* start, size_t total,
                                        size_t offset) {
...
#define MD_CHK(func_call) \
  ...
  for (offset = min_data_len; offset <= max_data_len; offset++) {
    ...
    mbedtls_ct_memcpy_if_eq(output, aux_out, hash_size, offset,
                            data_len_secret);
...
int mbedtls_ct_rsaes_pkcs1_v15_unpadding(unsigned char* input, size_t ilen,
  ...
  plaintext_size = mbedtls_ct_uint_if(bad, (unsigned) plaintext_max_size,
  ...
  ret = -(int) mbedtls_ct_uint_if(
      ...
      mbedtls_ct_uint_if(output_too_large, -MBEDTLS_ERR_RSA_OUTPUT_TOO_LARGE,
                         0));
  ...
  plaintext_size =
      mbedtls_ct_uint_if(output_too_large, (unsigned) plaintext_max_size,
