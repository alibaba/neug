...
class TProtocolDecorator : public TProtocol {
 public:
  ...
  uint32_t writeFieldBegin_virt(const char* name, const TType fieldType,
  ...
  uint32_t writeMapBegin_virt(const TType keyType, const TType valType,
  ...
  uint32_t readMessageBegin_virt(std::string& name, TMessageType& messageType,
  ...
  uint32_t readFieldBegin_virt(std::string& name, TType& fieldType,
  ...
  uint32_t readMapBegin_virt(TType& keyType, TType& valType,
  ...
  uint32_t readBool_virt(gs::vector<bool>::reference value) override {
    return protocol->readBool(value);
  }
