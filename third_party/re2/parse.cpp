...
static void AddFoldedRange(CharClassBuilder* cc, Rune lo, Rune hi, int depth) {
  ...
  while (lo <= hi) {
    const CaseFold* f =
        LookupCaseFold(unicode_casefold, num_unicode_casefold, lo);
...
bool Regexp::ParseState::PushRepetition(int min, int max, const StringPiece& s,
                                        bool nongreedy) {
  if ((max != -1 && max < min) || min > maximum_repeat_count ||
...
// Checks whether a particular regexp op is a marker.
bool Regexp::ParseState::IsMarker(RegexpOp op) { return op >= kLeftParen; }
...
class FactorAlternationImpl {
 public:
  static void Round1(Regexp** sub, int nsub, Regexp::ParseFlags flags,
                     std::vector<Splice>* splices);
  static void Round2(Regexp** sub, int nsub, Regexp::ParseFlags flags,
                     std::vector<Splice>* splices);
  static void Round3(Regexp** sub, int nsub, Regexp::ParseFlags flags,
                     std::vector<Splice>* splices);
};
...
static const UGroup* LookupGroup(const StringPiece& name, const UGroup* groups,
                                 int ngroups) {
  // Simple name lookup.
...
static void AddUGroup(CharClassBuilder* cc, const UGroup* g, int sign,
  ...
  if (sign == +1) {
    ...
    if (parse_flags & Regexp::FoldCase) {
      ...
      // it out. Normally AddRangeFlags does this, but we're bypassing
      // AddRangeFlags.
      bool cutnl =
          !(parse_flags & Regexp::ClassNL) || (parse_flags & Regexp::NeverNL);
      if (cutnl) {
...
const UGroup* MaybeParsePerlCCEscape(StringPiece* s,
                                     Regexp::ParseFlags parse_flags) {
