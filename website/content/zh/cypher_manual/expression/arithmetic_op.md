...
- 如果操作数的位宽不同（例如 32 位与 64 位），结果使用较大的位宽
- 如果操作数位宽相同但符号不同，结果会优先提升为下一个更大的有符号类型（如果存在），例如 `INT32` + `UINT32` -> `INT64`；否则提升为相同位宽的无符号类型，例如 `INT64` + `UINT64` -> `UINT64`
- 浮点类型优先于整数类型，DOUBLE 是精度最高的浮点类型
...
1. **浮点类型**：不进行特殊处理；系统依赖标准 [IEEE 754]() 行为，返回 `Infinity`、`-Infinity` 或 `NaN` 值。

2. **整数类型**：行为在 debug 和 release 模式下有所不同：
   - **Debug 模式**：性能要求较低，允许在运行时进行输入验证，并显式抛出异常
   - **Release 模式**：性能要求较高，溢出/下溢将返回未定义值，但除零操作仍可能显式抛出异常
