...
class TestLoadFromDocs:
    ...
    def test_load_from_csv_basic(self):
        ...
        result = self.conn.execute(
            f'LOAD FROM "{Path(csv_path).as_posix()}" (header=true) RETURN name, age;'
        )
        records = list(result)
    ...
    def test_load_from_column_reordering(self):
        ...
        result = self.conn.execute(
            f'LOAD FROM "{Path(csv_path).as_posix()}" (header=true) RETURN age, name, id;'
        )
        records = list(result)
        assert len(records) == 4
        for r in records:
    ...
    def test_load_from_column_aliases(self):
        ...
        result = self.conn.execute(
            f"""
            LOAD FROM "{Path(csv_path).as_posix()}" (header=true)
    ...
    def test_load_from_performance_options(self):
        ...
        result = self.conn.execute(
            f"""
            LOAD FROM "{Path(csv_path).as_posix()}" (
                header = true,
                parallel = true
            )
            RETURN name, age
    ...
    def test_load_from_csv_array_with_explicit_cast(self):
        ...
        rows = list(
            self.conn.execute(
                f'LOAD FROM "{Path(csv_path).as_posix()}" (header=true, delimiter=",") '
                "RETURN id, readings;"
            )
        )
        assert rows == [[1, "[1,2,3]"]]
...
class TestCopyFromDocs:
    ...
    def test_copy_from_node_table(self):
        ...
        csv_path = os.path.join(self.data_path, "person.csv")
        self.conn.execute(
            f'COPY Person FROM "{Path(csv_path).as_posix()}" (header=true);'
        )
    ...
    def test_copy_from_with_column_remapping(self):
        ...
        self.conn.execute(
            f"""
            COPY Person FROM (
                LOAD FROM "{Path(remap_csv).as_posix()}" (header=true, delimiter=",")
                RETURN id, name, age
            )
            """
        )
    ...
    def test_copy_from_parallel(self):
        ...
        csv_path = os.path.join(self.data_path, "person.csv")
        self.conn.execute(
            f'COPY Person FROM "{Path(csv_path).as_posix()}" (header=true, parallel=true);'
        )
        res = self.conn.execute("MATCH (p:Person) RETURN count(p);")
        assert list(res)[0][0] == 4
    ...
    def test_copy_from_batch_read(self):
        """import_data.md: COPY FROM with batch_read and chunk_size options.

        batch_read and chunk_size are only supported in COPY FROM (not LOAD
