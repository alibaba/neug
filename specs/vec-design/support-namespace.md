...
- 指定多个点类型以及各自的属性过滤条件：`'Enitity': 'domain = "user1"'`, `'Product': 'domain = "user1"'`
...
- `bindInsertClause`、`bindMergeClause`、`bindSubqueryExpression` 传 `DISALLOW`；
- 新增 graph-pattern 调用点必须显式选择模式，避免默认参数使新 clause 意外支持 Namespace。
...
谓词绑定不能直接复用创建 Namespace 时生成的 expression，因为其中的 variable unique name 和当前查询不同。正确做法是保存字符串，在当前 Binder scope 中将未限定属性改写为当前变量属性后重新绑定。例如点谓词 `domain = "user1"` 在变量 `n` 上等价于 `n.domain = "user1"`。
...
Namespace 引用的是 label 名称，相关 DDL 必须定义行为：

- 删除仍被 Namespace 引用的点类型或边类型时拒绝 DDL，并列出引用它的 Namespace；
- rename label 时同步更新所有 Namespace 中对应名称，并写入同一 schema transaction/WAL；
- 删除点类型时还要检查 Namespace 边三元组的 src/dst 引用；
- 删除属性时，若属性被 Namespace 谓词引用则拒绝。第一阶段可通过重新绑定所有受影响谓词判断，而不是实现独立的表达式依赖索引。

这样可以保证任何已提交 Namespace 都始终能在当前 Schema 上重新绑定，不把错误延迟到普通查询阶段。
