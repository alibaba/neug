...
1. **Create PR**: Create a new PR from the current branch. If current branch is `main`, checkout to a new branch. Load the template in a temporary file:
    ```
    PR Title: <prefix>: <title>
    Reviewers: <reviewers>
    ...
    [List all changes in this PR in detail.]
    ```
...
4. **Submit PR**: Submit the PR to the project. Note the initial lines are PR settings and the remaining part is the real PR body.