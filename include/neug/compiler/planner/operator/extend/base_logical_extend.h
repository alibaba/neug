...
class BaseLogicalExtend : public LogicalOperator {
 public:
  BaseLogicalExtend(LogicalOperatorType operatorType,
      ...
      : LogicalOperator{operatorType, std::move(child)},
        boundNode{std::move(boundNode)},
        nbrNode{std::move(nbrNode)},
        rel{std::move(rel)},
        direction{direction},
  ...
  std::unique_ptr<OPPrintInfo> getPrintInfo() const override {
    return std::make_unique<BaseLogicalExtendPrintInfo>(boundNode, nbrNode, rel,
                                                        direction);
  }
 ...
 protected:
  ...
  // This is currently not doable due to recursive plan not setting src node
  // correctly.
