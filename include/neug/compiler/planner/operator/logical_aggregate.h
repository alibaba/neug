...
struct LogicalAggregatePrintInfo final : OPPrintInfo {
  ...
  LogicalAggregatePrintInfo(binder::expression_vector keys,
                            binder::expression_vector aggregates)
  ...
  std::unique_ptr<OPPrintInfo> copy() const override {
    return std::unique_ptr<LogicalAggregatePrintInfo>(
        new LogicalAggregatePrintInfo(*this));
  }
...
class LogicalAggregate final : public LogicalOperator {
 ...
 public:
  LogicalAggregate(binder::expression_vector keys,
                   binder::expression_vector aggregates,
  ...
  LogicalAggregate(binder::expression_vector keys,
      ...
      : LogicalOperator{operatorType_, std::move(child), cardinality},
        keys{std::move(keys)},
  ...
  void setKeys(binder::expression_vector expressions) {
    keys = std::move(expressions);
  }
  ...
  std::unique_ptr<LogicalOperator> copy() override {
    return make_unique<LogicalAggregate>(keys, dependentKeys, aggregates,
 ...
 private:
  binder::expression_vector keys;
  // A dependentKeyExpression depend on a keyExpression (e.g. a.age depends on
  // a.ID) and will not be treated as a hash key during hash aggregation.
  binder::expression_vector dependentKeys;
  binder::expression_vector aggregates;
};
