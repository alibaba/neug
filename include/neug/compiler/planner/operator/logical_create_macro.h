...
struct LogicalCreateMacroPrintInfo final : OPPrintInfo {
  ...
  explicit LogicalCreateMacroPrintInfo(std::string macroName)
      : macroName(std::move(macroName)) {}

  std::string toString() const override;

  std::unique_ptr<OPPrintInfo> copy() const override {
    return std::unique_ptr<LogicalCreateMacroPrintInfo>(
        new LogicalCreateMacroPrintInfo(*this));
  }
...
class LogicalCreateMacro final : public LogicalOperator {
 public:
  LogicalCreateMacro(std::shared_ptr<binder::Expression> outputExpression,
                     std::string macroName,
  ...
  inline std::unique_ptr<LogicalOperator> copy() override {
    return std::make_unique<LogicalCreateMacro>(outputExpression, macroName,
                                                macro->copy());
  }
