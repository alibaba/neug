...
class ExpressionEvaluator {
 public:
  explicit ExpressionEvaluator(EvaluatorType type,
                               std::shared_ptr<binder::Expression> expression)
      : type{type}, expression{std::move(expression)} {};
  ExpressionEvaluator(EvaluatorType type,
                      std::shared_ptr<binder::Expression> expression,
                      bool isResultFlat)
      : type{type},
        expression{std::move(expression)},
        isResultFlat_{isResultFlat} {}
  ExpressionEvaluator(EvaluatorType type,
                      std::shared_ptr<binder::Expression> expression,
  ...
  // Evaluate and duplicate result for count times. This is a fast path we
  // implemented for bulk-insert when evaluate default values. A default value
  // should be
 ...
 protected:
  ...
  bool updateSelectedPos(common::SelectionVector& selVector) const {
    ...
    if (resultSelVector.getSelSize() > 1) {
      ...
      // If result state is flat (i.e. all children are flat), we shouldn't try
      // to update selectedPos because we don't know which one is leading, i.e.
      // the one being selected by filter. So we forget about selectedPos and
      // directly return true/false. This doesn't change the correctness,
      // because when all children are flat the check is done on tuple.
