 ...
 public:
  static inline void operation(
    ...
    common::StringVector::addString(&resultValueVector, result,
                                    paddedResult.data(), paddedResult.size());
  }

  static std::pair<uint32_t, uint32_t> padCountChars(const uint32_t count,
                                                     const char* data,
                                                     const uint32_t size) {
    ...
    for (; charCount < count && byteCount < size; charCount++) {
      ...
      auto bytes = utf8proc::utf8proc_iterate(str + byteCount, size - byteCount,
                                              &codepoint);
      byteCount += bytes;
    }
  ...
  static void insertPadding(uint32_t charCount, common::neug_string_t pad,
    ...
    for (auto i = 0u; i < charCount; i++) {
      if (padByteCount >= padSize) {
        result.insert(result.end(), (char*) padData,
                      (char*) (padData + padByteCount));
        padByteCount = 0;
      }
      utf8proc::utf8proc_int32_t codepoint = 0;
