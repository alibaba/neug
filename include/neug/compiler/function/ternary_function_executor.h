...
struct TernaryFunctionWrapper {
  template <typename A_TYPE, typename B_TYPE, typename C_TYPE,
            typename RESULT_TYPE, typename OP>
  static inline void operation(A_TYPE& a, B_TYPE& b, C_TYPE& c,
                               RESULT_TYPE& result, void* /*aValueVector*/,
...
struct TernaryFunctionExecutor {
  template <typename A_TYPE, typename B_TYPE, typename C_TYPE,
            typename RESULT_TYPE, typename FUNC, typename OP_WRAPPER>
  static void executeOnValue(common::ValueVector& a, common::ValueVector& b,
                             common::ValueVector& c,
                             common::ValueVector& result, uint64_t aPos,
                             uint64_t bPos, uint64_t cPos, uint64_t resPos,
                             void* dataPtr) {
    ...
    OP_WRAPPER::template operation<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, FUNC>(
        ...
        ((C_TYPE*) c.getData())[cPos], resValues[resPos], (void*) &a,
        (void*) &result, dataPtr);
  }
  ...
  static void executeAllFlat(
      common::ValueVector& a, common::SelectionVector* aSelVector,
      common::ValueVector& b, common::SelectionVector* bSelVector,
    ...
    if (a.isNull(aPos) || b.isNull(bPos)) {
      ...
      if (cSelVector->isUnfiltered()) {
        for (auto i = 0u; i < cSelVector->getSelSize(); ++i) {
          ...
          if (!result.isNull(i)) {
            executeOnValue<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, FUNC,
                           OP_WRAPPER>(a, b, c, result, aPos, bPos, i, i,
                                       dataPtr);
          }
        }
      } else {
        for (auto i = 0u; i < cSelVector->getSelSize(); ++i) {
          ...
          if (!result.isNull(pos)) {
            executeOnValue<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, FUNC,
                           OP_WRAPPER>(a, b, c, result, aPos, bPos, pos, pos,
                                       dataPtr);
          }
        }
      }
    ...
    if (a.isNull(aPos)) {
      ...
      if (bSelVector->isUnfiltered()) {
        for (auto i = 0u; i < bSelVector->getSelSize(); ++i) {
          ...
          if (!result.isNull(i)) {
            executeOnValue<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, FUNC,
                           OP_WRAPPER>(a, b, c, result, aPos, i, i, i, dataPtr);
          }
        }
      } else {
        for (auto i = 0u; i < bSelVector->getSelSize(); ++i) {
          ...
          if (!result.isNull(pos)) {
            executeOnValue<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, FUNC,
                           OP_WRAPPER>(a, b, c, result, aPos, pos, pos, pos,
                                       dataPtr);
          }
        }
      }
    ...
    if (a.isNull(aPos) || c.isNull(cPos)) {
      ...
      if (bSelVector->isUnfiltered()) {
        for (auto i = 0u; i < bSelVector->getSelSize(); ++i) {
          ...
          if (!result.isNull(i)) {
            executeOnValue<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, FUNC,
                           OP_WRAPPER>(a, b, c, result, aPos, i, cPos, i,
                                       dataPtr);
          }
        }
      } else {
        for (auto i = 0u; i < bSelVector->getSelSize(); ++i) {
          ...
          if (!result.isNull(pos)) {
            executeOnValue<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, FUNC,
                           OP_WRAPPER>(a, b, c, result, aPos, pos, cPos, pos,
                                       dataPtr);
          }
        }
      }
    ...
    if (a.hasNoNullsGuarantee() && b.hasNoNullsGuarantee() &&
      ...
      if (aSelVector->isUnfiltered()) {
        for (uint64_t i = 0; i < aSelVector->getSelSize(); i++) {
          ...
          if (!result.isNull(i)) {
            executeOnValue<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, FUNC,
                           OP_WRAPPER>(a, b, c, result, i, i, i, i, dataPtr);
          }
        }
      } else {
        for (uint64_t i = 0; i < aSelVector->getSelSize(); i++) {
          ...
          if (!result.isNull(pos)) {
            executeOnValue<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, FUNC,
                           OP_WRAPPER>(a, b, c, result, pos, pos, pos, pos,
                                       dataPtr);
          }
        }
      }
    ...
    if (b.isNull(bPos) || c.isNull(cPos)) {
      ...
      if (aSelVector->isUnfiltered()) {
        for (auto i = 0u; i < aSelVector->getSelSize(); ++i) {
          ...
          if (!result.isNull(i)) {
            executeOnValue<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, FUNC,
                           OP_WRAPPER>(a, b, c, result, i, bPos, cPos, i,
                                       dataPtr);
          }
        }
      } else {
        for (auto i = 0u; i < aSelVector->getSelSize(); ++i) {
          ...
          if (!result.isNull(pos)) {
            executeOnValue<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, FUNC,
                           OP_WRAPPER>(a, b, c, result, pos, bPos, cPos, pos,
                                       dataPtr);
          }
        }
      }
    ...
    if (b.isNull(bPos)) {
      ...
      if (aSelVector->isUnfiltered()) {
        for (auto i = 0u; i < aSelVector->getSelSize(); ++i) {
          ...
          if (!result.isNull(i)) {
            executeOnValue<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, FUNC,
                           OP_WRAPPER>(a, b, c, result, i, bPos, i, i, dataPtr);
          }
        }
      } else {
        for (auto i = 0u; i < aSelVector->getSelSize(); ++i) {
          ...
          if (!result.isNull(pos)) {
            executeOnValue<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, FUNC,
                           OP_WRAPPER>(a, b, c, result, pos, bPos, pos, pos,
                                       dataPtr);
          }
        }
      }
    ...
    if (c.isNull(cPos)) {
      ...
      if (aSelVector->isUnfiltered()) {
        for (auto i = 0u; i < aSelVector->getSelSize(); ++i) {
          ...
          if (!result.isNull(i)) {
            executeOnValue<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, FUNC,
                           OP_WRAPPER>(a, b, c, result, i, i, cPos, i, dataPtr);
          }
        }
      } else {
        for (auto i = 0u; i < aSelVector->getSelSize(); ++i) {
          ...
          if (!result.isNull(pos)) {
            executeOnValue<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, FUNC,
                           OP_WRAPPER>(a, b, c, result, pos, pos, cPos, pos,
                                       dataPtr);
          }
        }
      }
  ...
  static void executeSwitch(
    ...
    if (a.state->isFlat() && b.state->isFlat() && c.state->isFlat()) {
      executeAllFlat<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, FUNC, OP_WRAPPER>(
          a, aSelVector, b, bSelVector, c, cSelVector, result, resultSelVector,
          dataPtr);
    } else if (a.state->isFlat() && b.state->isFlat() && !c.state->isFlat()) {
      executeFlatFlatUnflat<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, FUNC,
                            OP_WRAPPER>(a, aSelVector, b, bSelVector, c,
                                        cSelVector, result, resultSelVector,
                                        dataPtr);
    } else if (a.state->isFlat() && !b.state->isFlat() && !c.state->isFlat()) {
      executeFlatUnflatUnflat<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, FUNC,
                              OP_WRAPPER>(a, aSelVector, b, bSelVector, c,
                                          cSelVector, result, resultSelVector,
                                          dataPtr);
    } else if (a.state->isFlat() && !b.state->isFlat() && c.state->isFlat()) {
      executeFlatUnflatFlat<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, FUNC,
                            OP_WRAPPER>(a, aSelVector, b, bSelVector, c,
                                        cSelVector, result, resultSelVector,
                                        dataPtr);
    } else if (!a.state->isFlat() && !b.state->isFlat() && !c.state->isFlat()) {
      ...
      executeUnflatUnFlatFlat<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, FUNC,
                              OP_WRAPPER>(a, aSelVector, b, bSelVector, c,
                                          cSelVector, result, resultSelVector,
                                          dataPtr);
    } else if (!a.state->isFlat() && b.state->isFlat() && c.state->isFlat()) {
      executeUnflatFlatFlat<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, FUNC,
                            OP_WRAPPER>(a, aSelVector, b, bSelVector, c,
                                        cSelVector, result, resultSelVector,
                                        dataPtr);
    } else if (!a.state->isFlat() && b.state->isFlat() && !c.state->isFlat()) {
      executeUnflatFlatUnflat<A_TYPE, B_TYPE, C_TYPE, RESULT_TYPE, FUNC,
                              OP_WRAPPER>(a, aSelVector, b, bSelVector, c,
                                          cSelVector, result, resultSelVector,
                                          dataPtr);
    } else {
