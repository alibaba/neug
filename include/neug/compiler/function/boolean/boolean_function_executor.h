...
/**
 * Binary boolean function requires special executor implementation because it's
 * truth table handles null differently (e.g. NULL OR TRUE = TRUE). Note that
 * unary boolean operation (currently only NOT) does not require special
 * implementation because NOT NULL = NULL.
 */
struct BinaryBooleanFunctionExecutor {
  template <typename FUNC>
  static inline void executeOnValueNoNull(common::ValueVector& left,
    ...
    FUNC::operation(left.getValue<uint8_t>(lPos), right.getValue<uint8_t>(rPos),
                    resValues[resPos], false /* isLeftNull */,
                    false /* isRightNull */);
  ...
  static inline void executeOnValue(common::ValueVector& left,
                                    common::ValueVector& right,
                                    common::ValueVector& result, uint64_t lPos,
  ...
  template <typename FUNC>
  static void executeFlatUnFlat(common::ValueVector& left,
                                common::SelectionVector* leftSelVector,
                                common::ValueVector& right,
                                common::SelectionVector* rightSelVector,
                                common::ValueVector& result,
  ...
  template <typename FUNC>
  static void executeUnFlatFlat(common::ValueVector& left,
                                common::SelectionVector* leftSelVector,
                                common::ValueVector& right,
                                common::SelectionVector* rightSelVector,
                                common::ValueVector& result,
  ...
  static void executeBothUnFlat(
      common::ValueVector& left, common::SelectionVector* leftSelVector,
      common::ValueVector& right,
  ...
  static void execute(common::ValueVector& left,
    ...
    if (left.state->isFlat() && right.state->isFlat()) {
      ...
      executeFlatUnFlat<FUNC>(left, leftSelVector, right, rightSelVector,
                              result, resultSelVector);
    } else if (!left.state->isFlat() && right.state->isFlat()) {
      executeUnFlatFlat<FUNC>(left, leftSelVector, right, rightSelVector,
                              result, resultSelVector);
    } else {
      executeBothUnFlat<FUNC>(left, leftSelVector, right, rightSelVector,
                              result, resultSelVector);
    }
  }
  ...
  static void selectOnValue(common::ValueVector& left,
    ...
    FUNC::operation(left.getValue<uint8_t>(lPos), right.getValue<uint8_t>(rPos),
                    resultValue, left.isNull(lPos), right.isNull(rPos));
    selectedPositionsBuffer[numSelectedValues] = resPos;
    numSelectedValues += (resultValue == true);
  }

  template <typename FUNC>
  static bool selectBothFlat(common::ValueVector& left,
                             common::ValueVector& right) {
    auto lPos = left.state->getSelVector()[0];
    auto rPos = right.state->getSelVector()[0];
    uint8_t resultValue = 0;
    FUNC::operation(left.getValue<bool>(lPos), right.getValue<bool>(rPos),
                    resultValue, (bool) left.isNull(lPos),
...
struct UnaryBooleanOperationExecutor {
  template <typename FUNC>
  static inline void executeOnValue(common::ValueVector& operand,
                                    uint64_t operandPos,
                                    common::ValueVector& result,
                                    uint64_t resultPos) {
    auto resultValues = (uint8_t*) result.getData();
    FUNC::operation(operand.getValue<uint8_t>(operandPos),
                    operand.isNull(operandPos), resultValues[resultPos]);
  ...
  template <typename FUNC>
  static inline void selectOnValue(
      common::ValueVector& operand, uint64_t operandPos,
      uint64_t& numSelectedValues,
      std::span<common::sel_t> selectedPositionsBuffer) {
    uint8_t resultValue = 0;
    FUNC::operation(operand.getValue<uint8_t>(operandPos),
    ...
    if (operand.state->isFlat()) {
      ...
      if (operandSelVector.isUnfiltered()) {
        for (auto i = 0ul; i < operandSelVector.getSelSize(); i++) {
          selectOnValue<FUNC>(operand, i, numSelectedValues,
                              selectedPositionBuffer);
        }
      } else {
        for (auto i = 0ul; i < operandSelVector.getSelSize(); i++) {
          auto pos = operand.state->getSelVector()[i];
          selectOnValue<FUNC>(operand, pos, numSelectedValues,
                              selectedPositionBuffer);
        }
      }
      selVector.setSelSize(numSelectedValues);
