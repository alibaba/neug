...
class NEUG_API Frontier {
 public:
  ...
  virtual void addNodes(const std::vector<common::nodeID_t>& nodeIDs,
                        iteration_t iter) = 0;
...
// Sparse frontier implementation that refers to the data owned by another
// sparse frontier. This should be used only for shortest-path type of
// algorithms where a node is guaranteed to be visited only once. See
// SPFrontierPair for its usage.
...
class NEUG_API DenseFrontier : public Frontier {
 ...
 public:
  ...
  static std::unique_ptr<DenseFrontier> getUnvisitedFrontier(
      processor::ExecutionContext* context, graph::Graph* graph);
  // Get frontier initialized to INITIAL_VISITED.
  static std::unique_ptr<DenseFrontier> getVisitedFrontier(
      processor::ExecutionContext* context, graph::Graph* graph);
  // Init frontier to 0 according to mask
  static std::unique_ptr<DenseFrontier> getVisitedFrontier(
      processor::ExecutionContext* context, graph::Graph* graph,
      common::NodeOffsetMaskMap* maskMap);
...
// Dense frontier implementation that refers to the data owned by another dense
// frontier. Should be used in the same case as SparseFrontierReference
...
class NEUG_API FrontierPair {
 public:
  ...
  virtual std::unordered_set<common::offset_t>
  getActiveNodesOnCurrentFrontier() = 0;
...
// Shortest path (excluding weighted shortest path )frontier implementation.
// Different from other recursive algorithms, shortest path has the guarantee
// that a node will not be visited repeatedly in different iteration. So we make
// current/next frontier reference writes to the same frontier.
class SPFrontierPair : public FrontierPair {
 public:
  ...
  // Get number of active nodes in current frontier. Used for shortest path
  // early termination.
  common::offset_t getNumActiveNodesInCurrentFrontier(
      common::NodeOffsetMaskMap& mask);
