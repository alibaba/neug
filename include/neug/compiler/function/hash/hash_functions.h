...
inline common::hash_t combineHashScalar(const common::hash_t a,
                                        const common::hash_t b) {
...
struct Hash {
  ...
  static void operation(const T& /*key*/, common::hash_t& /*result*/) {
    // LCOV_EXCL_START
    THROW_RUNTIME_ERROR("Hash type: " + std::string(typeid(T).name()) +
                        " is not supported.");
    // LCOV_EXCL_STOP
  }
...
struct CombineHash {
  static inline void operation(const common::hash_t& left,
                               const common::hash_t& right,
...
template <>
inline void Hash::operation(const common::internalID_t& key,
                            common::hash_t& result) {
...
template <>
inline void Hash::operation(const common::int128_t& key,
                            common::hash_t& result) {
...
template <>
inline void Hash::operation(const std::string_view& key,
                            common::hash_t& result) {
  common::hash_t hashValue = 0;
  auto data64 = reinterpret_cast<const uint64_t*>(key.data());
  for (size_t i = 0u; i < key.size() / 8; i++) {
    auto blockHash = gs::function::murmurhash64(*(data64 + i));
    hashValue = gs::function::combineHashScalar(hashValue, blockHash);
  }
  uint64_t last = 0;
  for (size_t i = 0u; i < key.size() % 8; i++) {
    last |= static_cast<uint64_t>(key[key.size() / 8 * 8 + i]) << i * 8;
  }
  hashValue = gs::function::combineHashScalar(hashValue,
                                              gs::function::murmurhash64(last));
  result = hashValue;
}
...
template <>
inline void Hash::operation(const common::interval_t& key,
                            common::hash_t& result) {
  result = combineHashScalar(
      murmurhash64(key.months),
      combineHashScalar(murmurhash64(key.days), murmurhash64(key.micros)));
}

template <>
inline void Hash::operation(const std::unordered_set<std::string>& key,
                            common::hash_t& result) {
  for (auto&& s : key) {
