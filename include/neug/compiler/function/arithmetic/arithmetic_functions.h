...
template <>
inline void Floor::operation(common::int128_t& input,
...
struct Even {
  ...
  static inline void operation(T& input, double& result) {
    ...
    // Note: c++ doesn't support double % integer, so we have to use the
    // following code to check whether result is odd or even.
...
struct Lgamma {
  template <class T>
  static inline void operation(T& input, double& result) {
    result = lgamma(input);  // NOLINT(concurrency-mt-unsafe): We don't use the
                             // thread-unsafe signgam.
  }
};
