...
// A lightweight, immutable view over a SelectionVector, or a subsequence of a
// selection vector SelectionVectors are also SelectionViews so that you can
// pass a SelectionVector to functions which take a SelectionView&
class SelectionView {
 protected:
  // In DYNAMIC mode, selectedPositions points to a mutable buffer that can be
  // modified through getMutableBuffer In STATIC mode, selectedPositions points
  // to somewhere in INCREMENTAL_SELECTED_POS Note that the vector is considered
  // unfiltered only if it is both STATIC and the first selected position is 0
  enum class State {
    DYNAMIC,
    STATIC,
  };
 ...
 protected:
  static SelectionView slice(std::span<const sel_t> selectedPositions,
                             State state) {
    return SelectionView(selectedPositions, state);
  }

  // Intended to be used only as a subsequence of a SelectionVector in
...
class SelectionVector : public SelectionView {
 public:
  ...
  // This View should be considered invalid if the SelectionVector it was
  // created from has been modified
  SelectionView slice(sel_t startIndex, sel_t selectedSize) const {
    return SelectionView::slice(
        getSelectedPositions().subspan(startIndex, selectedSize), state);
  }
  ...
  // Set to filtered is not very accurate. It sets selectedPositions to a
  // mutable array.
  void setToFiltered() {
    selectedPositions = selectedPositionsBuffer.get();
    state = State::DYNAMIC;
  ...
  void makeDynamic() {
    memcpy(selectedPositionsBuffer.get(), selectedPositions,
