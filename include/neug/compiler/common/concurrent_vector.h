...
// The initial size is fixed, and new elements are added in fixed sized blocks
// which are indexed and the indices are chained in a linked list. Currently
// only one thread can write concurrently, but any number of threads can read,
// even when the vector is being written to.
//
// Accessing elements which existed when the vector was created is as fast as
// possible, requiring just one comparison and one pointer reads, and accessing
// new elements is still reasonably fast, usually requiring reading just two
// pointers, with a small amount of arithmetic, or maybe more if an extremely
// large number of elements has been added (access cost increases every
// BLOCK_SIZE * INDEX_SIZE elements).
class ConcurrentVector {
 public:
  explicit ConcurrentVector(uint64_t initialNumElements,
                            uint64_t initialBlockSize)
      : numElements{initialNumElements},
        initialBlock{std::make_unique<T[]>(initialBlockSize)},
        initialBlockSize{initialBlockSize},
        firstIndex{nullptr} {}
  // resize never deallocates memory
  // Not thread-safe
  // It could be made to be thread-safe by storing the size atomically and doing
  // compare and swap when adding new indices and blocks
  void resize(uint64_t newSize) {
    while (newSize > initialBlockSize + blocks.size() * BLOCK_SIZE) {
      ...
      if (indices.empty()) {
        ...
        NEUG_ASSERT(indices.back()->numBlocks == INDEX_SIZE);
        auto index = std::make_unique<BlockIndex>();
        index->blocks[0] = newBlock.get();
        index->numBlocks = 1;
  ...
  T& operator[](uint64_t elemPos) {
    if (elemPos < initialBlockSize) {
      ...
      NEUG_ASSERT(index != nullptr);
      while (indexNum > 0) {
        NEUG_ASSERT(index->nextIndex != nullptr);
        index = index->nextIndex;
        indexNum--;
      }
      NEUG_ASSERT(index->blocks[blockNum % INDEX_SIZE] != nullptr);
