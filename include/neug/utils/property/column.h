...
class ColumnBase : public Module {
 public:
  ...
  // insert_safe is true when the column needs to be resized to accommodate
  // the new value, which can happen when the value is not fixed length. If
  // the value is fixed length, we should already have enough space allocated,
  // so insert_safe can be false.
