...
template <>
struct GHash<columnar::Value> {
  size_t operator()(const columnar::Value& val) const {
    if (val.IsNull()) {
      return 0;
    }
    switch (val.type().id()) {
...
class LFIndexer {
 ...
 public:
  ...
  INDEX_T insert(const columnar::Value& oid, bool insert_safe) {
  ...
  INDEX_T get_index(const columnar::Value& oid) const {
    assert(oid.type().id() == get_type());
  ...
  bool get_index(const columnar::Value& oid, INDEX_T& ret) const {
    if (indices_->size() == 0) {
      return false;
    }
  ...
  bool contains(const columnar::Value& oid) const {
    assert(oid.type().id() == get_type());
    auto* indices_ptr = indices_->data();
    size_t index =
        hash_policy_.index_for_hash(hasher_(oid), num_slots_minus_one_);
  ...
  columnar::Value get_key(const INDEX_T& index) const {
...
class IdIndexerBase {
 public:
  ...
  virtual void _add(const columnar::Value& oid) = 0;
  virtual bool add(const columnar::Value& oid, INDEX_T& lid) = 0;
  virtual bool get_key(const INDEX_T& lid, columnar::Value& oid) const = 0;
  virtual bool get_index(const columnar::Value& oid, INDEX_T& lid) const = 0;
 ...
 public:
  ...
  DataTypeId get_type() const override {
    if constexpr (std::is_same_v<KEY_T, std::string_view>) {
      ...
      return columnar::ValueConverter<KEY_T>::type().id();
    }
  }

  void _add(const columnar::Value& oid) override {
    assert(get_type() == oid.type().id());
    KEY_T oid_ = oid.GetValue<KEY_T>();
    _add(oid_);
  }

  bool add(const columnar::Value& oid, INDEX_T& lid) override {
    assert(get_type() == oid.type().id());
    KEY_T oid_ = oid.GetValue<KEY_T>();
    return add(oid_, lid);
  }

  bool get_key(const INDEX_T& lid, columnar::Value& oid) const override {
    KEY_T oid_;
    bool flag = get_key(lid, oid_);
    if (flag) {
      oid = columnar::Value::CreateValue<KEY_T>(oid_);
    }
    return flag;
  }

  bool get_index(const columnar::Value& oid, INDEX_T& lid) const override {
    assert(get_type() == oid.type().id());
    KEY_T oid_ = oid.GetValue<KEY_T>();
