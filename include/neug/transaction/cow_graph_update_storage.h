...
/**
 * @brief Storage mutation facade over a private COW graph write set.
 *
 * The facade has no transaction-owner or execution-mode dependency. Its
...
class CowGraphUpdateStorage : public StorageUpdateInterface {
 public:
  ...
  ~CowGraphUpdateStorage() = default;

  const CowDetachLedger& detach_ledger() const { return cow_state_; }
