 ...
 *     // Get property as generic columnar::Value
 *     columnar::Value val = accessor.get_value(it);
 *
...
struct EdgeDataAccessor {
  ...
  /**
   * @brief Get property value for current edge as columnar::Value.
   * @param it Iterator pointing to the edge
   * @return columnar::Value containing the edge data
   */
  inline columnar::Value get_data(const NbrIterator& it) const {
    return data_column_ == nullptr
  ...
  inline columnar::Value get_data_from_ptr(const void* data_ptr) const {
    return data_column_ == nullptr
  ...
  inline void set_data(const NbrIterator& it, const columnar::Value& value,
                       timestamp_t ts) {
...
#define TYPE_DISPATCHER(enum_val, type)             \
  case DataTypeId::enum_val: {                      \
    return columnar::Value::CreateValue<type>(      \
