...
inline CsrPrefetchPolicy create_csr_prefetch_policy(
...
inline uint64_t percentile_rank(uint64_t n, double percentile) {
  if (n == 0) {
    return 0;
  }
...
inline CsrDegreeDistributionStats compute_csr_degree_distribution(
