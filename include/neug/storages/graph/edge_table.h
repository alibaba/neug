...
std::pair<int32_t, const void*> insert_edge_into_csr_internal(
    ...
    vid_t dst_lid, const std::vector<columnar::Value>& properties,
  ...
  if (meta.is_bundled()) {
    ...
    columnar::Value bundled_data =
        properties.empty() ? columnar::Value(DataType::EMPTY) : properties[0];
    ...
    columnar::Value prop = columnar::Value::UINT64(row_id);
