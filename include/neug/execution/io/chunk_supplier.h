...
/// Iterator-like source of columnar::DataChunk batches for file readers and
/// loaders.
class IDataChunkSupplier {
 public:
  virtual ~IDataChunkSupplier() = default;
  virtual std::shared_ptr<columnar::DataChunk> GetNextChunk() = 0;
...
class MultiDataChunkSupplier : public IDataChunkSupplier {
 ...
 private:
  std::vector<std::shared_ptr<columnar::DataChunk>> chunks_;
  size_t index_ = 0;
};
