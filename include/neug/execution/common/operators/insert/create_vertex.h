...
class CreateVertex {
 public:
  static gs::result<Context> insert_vertex(
      StorageInsertInterface& graph, Context&& ctx, const std::vector<label_t>& labels,
    ...
    for (size_t i = 0; i < labels.size(); ++i) {
      ...
      if (properties_name.size() != properties_type.size()) {
        THROW_RUNTIME_ERROR(
            ...
            " has different number of properties: " + std::to_string(properties_name.size()) +
            " vs " + std::to_string(properties_type.size()));
      }
      const auto& pks = schema.get_vertex_primary_key(label);
      if (pks.size() != 1) {
        THROW_RUNTIME_ERROR(
            "Vertex label " + std::to_string(label) +
            " must have exactly one primary key, but found: " + std::to_string(pks.size()));
      }
      const auto& pk = pks[0];
      if (properties.size() != properties_name.size() + 1) {
        THROW_RUNTIME_ERROR(
            "Provided properties size " + std::to_string(properties.size()) +
            " does not match schema size: " + std::to_string(properties_name.size() + 1));
      }
      ...
      for (size_t i = 0; i < ctx.row_num(); ++i) {
        for (size_t j = 0; j < properties.size(); ++j) {
          ...
          if (prop_name == std::get<1>(pk)) {
            ...
            auto it = std::find(properties_name.begin(), properties_name.end(), prop_name);
            if (it == properties_name.end()) {
              THROW_RUNTIME_ERROR("Property " + prop_name + " not found in vertex label " +
                                  std::to_string(label));
            }
        ...
        if (!graph.AddVertex(label, pk_value, property_values, vid)) {
          LOG(ERROR) << "Failed to add vertex with label " << (int32_t) label << " and primary key "
                     << pk_value.to_string();
          RETURN_STATUS_ERROR(gs::StatusCode::ERR_INTERNAL_ERROR,
                              "Failed to add vertex with label " + std::to_string(label) +
