...
class CreateEdge {
 public:
  static gs::result<Context> insert_edge(
      StorageInsertInterface& graph, Context&& ctx, std::vector<LabelTriplet> labels,
    ...
    for (size_t i = 0; i < labels.size(); ++i) {
      ...
      if (properties.size() != properties_name.size()) {
        THROW_RUNTIME_ERROR(
            ...
            " does not match schema size: " + std::to_string(properties_name.size()));
      }
      std::shared_ptr<Arena> arena = std::make_shared<Arena>();
      const auto& src_vertex_col =
          dynamic_cast<const IVertexColumn&>(*ctx.get(src_dst_tags[i].first).get());
      const auto& dst_vertex_col =
          dynamic_cast<const IVertexColumn&>(*ctx.get(src_dst_tags[i].second).get());
      for (size_t i = 0; i < ctx.row_num(); ++i) {
        ...
        for (size_t j = 0; j < properties.size(); ++j) {
          ...
          if (it == properties_name.end()) {
            THROW_RUNTIME_ERROR("Property " + prop_name + " not found in schema for edge (" +
