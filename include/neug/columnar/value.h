...
#include "neug/columnar/numeric_cast.h"
...
using vertex_t = neug::columnar::VertexRecord;
using edge_t = neug::columnar::EdgeRecord;
...
struct ValueConverter<int32_t> {
  ...
  static bool cast(const T& input, int32_t& output) {
    if constexpr (std::is_same_v<T, std::string>) {
      auto [data, len] = neug::columnar::removeWhiteSpaces(input);
      auto [ptr, ec] = std::from_chars(data, data + len, output);
      return ec == std::errc() && ptr == data + len;
    } else {
      return neug::columnar::TryCastWithOverflowCheck(input, output);
    }
  }
};
...
struct ValueConverter<double> {
  ...
  static bool cast(const T& input, double& output) {
    if constexpr (std::is_same_v<T, std::string>) {
      return neug::columnar::tryDoubleCast(input, output);
    } else {
      return neug::columnar::TryCastWithOverflowCheck(input, output);
    }
  }
