...
class IEdgeColumn : public IColumn {
 public:
  IEdgeColumn() : type_(DataType(DataTypeId::kEdge)) {}
  virtual ~IEdgeColumn() = default;

  ColumnKind column_type() const override { return ColumnKind::kEdge; }
...
class SDSLEdgeColumnBuilder : public IColumnBuilder {
 public:
...
class MSEdgeColumnBuilder : public IColumnBuilder {
 public:
  ...
  inline std::shared_ptr<IColumn> finish() override {
    if (!cur_edges_.empty()) {
      edges_.emplace_back(cur_label_idx_, cur_dir_, std::move(cur_edges_));
    }
    if (edges_.empty()) {
...
class BDSLEdgeColumnBuilder : public IColumnBuilder {
 public:
...
class SDMLEdgeColumnBuilder : public IColumnBuilder {
 public:
...
class BDMLEdgeColumnBuilder : public IColumnBuilder {
 public:
