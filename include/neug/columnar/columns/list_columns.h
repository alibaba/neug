...
class ListColumn : public IColumn {
 public:
  ...
  ColumnKind column_type() const override { return ColumnKind::kValue; }
  ...
  std::pair<std::shared_ptr<IColumn>, sel_vec_t> unfold() const;

  std::shared_ptr<IColumn> data_column() const { return datas_; }

  const vector_t<list_item>& items() const { return items_; }

  std::shared_ptr<IColumn> reorder() const {
 ...
 private:
  template <typename T>
  std::pair<std::shared_ptr<IColumn>, sel_vec_t> unfold_impl() const {
    sel_vec_t offsets;
  ...
  std::shared_ptr<IColumn> datas_;
};

class ListColumnBuilder : public IColumnBuilder {
 public:
 ...
 private:
  ...
  vector_t<list_item> items_;
  std::shared_ptr<IColumnBuilder> child_builder_;
};
