...
class IColumn {
 public:
  ...
  virtual ColumnKind column_type() const = 0;

  virtual const DataType& elem_type() const = 0;

  virtual std::shared_ptr<IColumn> shuffle(const sel_vec_t& offsets) const {
    LOG(FATAL) << "shuffle not implemented for " << this->column_info();
    return nullptr;
  }

  virtual std::shared_ptr<IColumn> optional_shuffle(
  ...
  virtual std::shared_ptr<IColumn> union_col(
  ...
  virtual std::pair<std::shared_ptr<IColumn>, vector_t<sel_vec_t>>
  generate_aggregate_offset() const {
    ...
    std::shared_ptr<IColumn> col(nullptr);
...
class IColumnBuilder {
 public:
  ...
  virtual std::shared_ptr<IColumn> finish() = 0;
};
