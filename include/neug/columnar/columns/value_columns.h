...
template <typename T>
class ValueColumn : public IColumn {
 public:
  ...
  inline ColumnKind column_type() const override { return ColumnKind::kValue; }

  std::shared_ptr<IColumn> shuffle(const sel_vec_t& offsets) const override;

  std::shared_ptr<IColumn> optional_shuffle(
  ...
  std::shared_ptr<IColumn> union_col(
      std::shared_ptr<IColumn> other) const override;
...
template <typename T>
class ValueColumnBuilder : public IColumnBuilder {
 public:
  ...
  std::shared_ptr<IColumn> finish() override {
    if (is_optional_) {
...
template <typename T>
std::shared_ptr<IColumn> ValueColumn<T>::shuffle(
...
template <typename T>
std::shared_ptr<IColumn> ValueColumn<T>::optional_shuffle(
    const sel_vec_t& offsets) const {
...
template <typename T>
std::shared_ptr<IColumn> ValueColumn<T>::union_col(
