...
class StructColumn : public IColumn {
 public:
  ...
  const std::vector<std::shared_ptr<IColumn>>& children() const {
    return children_;
  }
 ...
 private:
  ...
  std::vector<std::shared_ptr<IColumn>> children_;
};

class StructColumnBuilder : public IColumnBuilder {
 public:
 ...
 private:
  ...
  bool is_optional_;
  vector_t<bool> valids_;
  std::vector<std::shared_ptr<IColumnBuilder>> child_builders_;
};
