...
inline columnar::Path build_path_from_chain(const std::vector<vid_t>& chain,
                                            label_t vertex_label,
                                            label_t edge_label, bool directed,
                                            const StorageReadInterface& graph) {
  if (chain.size() <= 1) {
    return columnar::Path(vertex_label, chain[0]);
  }
  ...
  std::vector<std::pair<columnar::Direction, const void*>> edge_datas;
  ...
  for (size_t i = 0; i + 1 < chain.size(); ++i) {
    ...
    columnar::Direction dir = columnar::Direction::kOut;
  ...
  return columnar::Path(vertex_label, edge_label, chain, edge_datas);
}
...
inline columnar::Path reconstruct_path(vid_t target, vid_t source,
