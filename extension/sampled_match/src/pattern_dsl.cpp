...
// ============================================================================
// Pattern DSL → sampled_match JSON translator.
//
...
// SampledMatchTest.InOperatorIsNoOp), so we don't expose them through the
...
std::string TranslatePatternDslToJson(std::string_view dsl) {
  try {
    ...
    LOG(WARNING) << "[SAMPLED_MATCH_DSL] parse error at " << e.line << ":"
                 << e.col << " — " << e.what();
    return "";
  } catch (const std::exception& e) {
    LOG(WARNING) << "[SAMPLED_MATCH_DSL] " << e.what();
    return "";
  } catch (...) {
    LOG(WARNING) << "[SAMPLED_MATCH_DSL] unknown error during translation";
    return "";
  }
}
