...
JsonStreamingState::JsonStreamingState(
    std::unique_ptr<JsonVFSReader> reader,
    const std::vector<DataTypeId>& columnTypes)
    : reader_(std::move(reader)),
      columnTypes_(columnTypes),
      buffer_(JsonReaderConstants::BUFFER_SIZE +
...
bool JsonStreamingState::readNextBuffer() {
  ...
  std::memset(buffer_.data() + bufferSize_, 0,
              JsonReaderConstants::PADDING_SIZE);
...
bool JsonStreamingState::parseAndAppendJsonObject(const rapidjson::Value& obj) {
  ...
  if (fieldNames_.empty()) {
    ...
    if (fieldNames_.size() != columnTypes_.size()) {
      THROW_EXTENSION_EXCEPTION("JSONL object field count (" +
                                std::to_string(fieldNames_.size()) +
...
size_t JsonStreamingState::parseCurrentBuffer() {
  ...
  while (bufferOffset_ < bufferSize_) {
    ...
    if (lineEnd == nullptr) {
      ...
      // Save the remaining content to reconstructBuffer, which will be
      // concatenated on the next read
