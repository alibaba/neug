...
class Value {
 public:
  ...
  static Value BOOLEAN(bool value) { return Value(static_cast<int32_t>(value ? 1 : 0)); }
  ...
  std::string ToString() const {
    if (std::holds_alternative<std::monostate>(value_)) return "";
    if (std::holds_alternative<int32_t>(value_)) return std::to_string(std::get<int32_t>(value_));
  ...
  bool operator==(const Value& other) const {
    ...
    double lhs = 0;
    double rhs = 0;
    if (TryDouble(lhs) && other.TryDouble(rhs)) return lhs == rhs;
...
struct ExprNode {
  using PropertyResolver = std::function<Value(const std::string&, const std::string&)>;
...
class PropCons {
 public:
  ...
  PropCons(std::string prop_name, CompType comp_type, Value value)
      : _prop_name(std::move(prop_name)), _comp_type(comp_type), _value(std::move(value)) {}
  ...
  bool CheckExpr(const ExprNode::PropertyResolver& resolver) const {
    if (!_use_expr || !_left_expr) return false;
    Value lhs = _left_expr->Evaluate(resolver);
    if (_comp_type == CompType::COMP_IS_NULL) return lhs.IsNull() || lhs.is_orgininally_null;
    if (_comp_type == CompType::COMP_IS_NOT_NULL) return !lhs.IsNull() && !lhs.is_orgininally_null;
    if (!_right_expr) return false;
...
struct SchemaGraph {
  ...
  std::unordered_map<EIdType, std::unordered_map<std::string, ValueType>> vertex_value_type_map;
  std::unordered_map<int, std::map<std::pair<int, int>, EIdType>> label_edge;
  std::unordered_map<EIdType, std::unordered_map<std::string, ValueType>> edge_value_type_map;
};
