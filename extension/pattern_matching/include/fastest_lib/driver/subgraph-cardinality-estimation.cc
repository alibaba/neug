...
std::set<std::string> double_type_results = {
    "Truth",           "Est",           "logQError",
    "CSBuildTime",     "TreeCountTime", "TreeSampleTime",
    "GraphSampleTime", "QueryTime"};
std::set<std::string> longlong_type_results = {};
std::vector<std::string> print_order = {
    "#CSVertex",     "#CSEdge",        "#CandTree",
    "#TreeTrials",   "#TreeSuccess",   "Truth",
    "Est",           "logQError",      "CSBuildTime",
    "TreeCountTime", "TreeSampleTime", "GraphSampleTime",
    "QueryTime"};
...
void read_ans(const std::string& dataset) {
  ...
  LOG(INFO) << "Reading answers from: " << ans_file_name;
  std::ifstream ans_in(ans_file_name);
  while (!ans_in.eof()) {
    std::string name, t, c;
    ans_in >> name >> t >> c;
    if (name.empty() || c.empty())
      continue;
...
void read_filter_option(const std::string& opt, const std::string& filter,
                        CardinalityEstimation::CardEstOption& option) {
...
int32_t main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < query_names.size(); i++) {
    ...
    opt.MAX_QUERY_VERTEX =
        std::max(opt.MAX_QUERY_VERTEX, pattern_graphs[i]->GetNumVertices());
    opt.MAX_QUERY_EDGE =
        std::max(opt.MAX_QUERY_EDGE, pattern_graphs[i]->GetNumEdges());
  }
  ...
  for (int i = 0; i < pattern_graphs.size(); i++) {
    ...
    std::string query_name = query_names[i];
    LOG(INFO) << "Start processing " << query_name;
    ...
    if (true_cnt.find(query_name) != true_cnt.end()) {
      ...
      query_result["logQError"] =
          std::any(logQError(true_cnt[query_name] * 1.0, est));
    }
    for (auto& key : print_order) {
      if (query_result.find(key) == query_result.end())
        continue;
      ...
      else if (scientific_type_results.find(key) !=
               scientific_type_results.end())
  ...
  fprintf(stdout, "Total Time: %.2fms\n", Total(results, "QueryTime"));
  fprintf(stdout, "Average Abs log Q-Error: %.2f\n",
          Average(results, "logQError", absolute_value));
}
