  ...
  for (int i = 0; i < GetNumVertices() - 2; i++) {
    ...
    for (int e : inc_edge_list[u]) {
      ...
      for (int eprime : GetAllIncidentEdges(v)) {
        ...
        if (nbr_edge_id[w] != -1) {
          local_triangles[e].emplace_back(
              std::tuple(w, nbr_edge_id[w], eprime));
          local_triangles[oe].emplace_back(
              std::tuple(w, eprime, nbr_edge_id[w]));
          num_triangles += 2;
          if (e > nbr_edge_id[w]) {
            local_triangles[eprime].emplace_back(
                std::tuple(u, oe, GetOppositeEdge(nbr_edge_id[w])));
            local_triangles[GetOppositeEdge(eprime)].emplace_back(
                std::tuple(u, GetOppositeEdge(nbr_edge_id[w]), oe));
            num_triangles += 2;
          }
          if (num_triangles >= MAX_NUM_TRIANGLES) {
            local_triangles.clear();
            LOG(WARNING) << "Too many triangles during local triangle "
                            "enumeration.";
            return false;
          }
        }
    ...
    for (int e : inc_edge_list[u]) {
      ...
      if (inc_edge_list[v].size() == 1) {
        ...
        std::swap(inc_edge_list[v][inc_edge_list[v].size() - 1],
                  inc_edge_list[v][oe_idx_from_v]);
...
bool Graph::EnumerateLocalFourCycles() {
  Timer timer;
  timer.Start();
          ...
          for (int& third_edge_opp : GetAllIncidentEdges(fourth_vertex)) {
            ...
            if (snd_edge != -1) {
              ...
              local_four_cycles[i].emplace_back(
                  FourMotif({i, snd_edge, third_edge, fourth_edge},
                            {fst_diag, snd_diag}));
              num_four_cycles++;
              if (num_four_cycles >= MAX_NUM_FOUR_CYCLES) {
                local_four_cycles.clear();
                LOG(WARNING) << "Too many four cycles during local four-cycle "
                                "enumeration.";
                return false;
              }
            }
