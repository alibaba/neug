...
int BipartiteMaximumMatching::FindSCC(int v) {
  S[stkright++] = v;
  int ret = dfsn[v] = ++ord;
...
bool BipartiteMaximumMatching::FindUnmatchableEdges(int required) {
  ord = 0;
  found_scc = 0;
...
BipartiteMaximumMatching::~BipartiteMaximumMatching() {
  delete[] left;
  delete[] right;
  delete[] used;
...
void BipartiteMaximumMatching::Initialize(int max_left, int max_right,
                                          int max_query_vertex) {
  Q = new int[max_right]();
  S = new int[max_right]();
...
void BipartiteMaximumMatching::Reset(bool reset_edges) {
...
bool BipartiteMaximumMatching::dfs(int r) {
  if (used[r])
    return false;
  used[r] = true;
