...
class QueryGraph : public Graph {
 public:
  ...
  inline const std::vector<gbi::PropCons>& GetVertexPropertyConstraints(Vertex v) const;
  inline const std::vector<gbi::PropCons>& GetEdgePropertyConstraints(Size edge_idx) const;
  void SetVertexPropertyConstraints(Vertex v, const std::vector<gbi::PropCons>& constraints);
  void SetEdgePropertyConstraints(Size edge_idx, const std::vector<gbi::PropCons>& constraints);
  bool IsLeafVertex(Vertex u) const;

 public:
  void ExtractResidualStructure();

  std::vector<std::vector<std::pair<Vertex, Label>>> no_edge_pairs;  // 记录不存在的（终点，边类型）对
  ...
  std::vector<std::vector<gbi::PropCons>> vertex_property_constraints_;  // per vertex
  std::vector<std::vector<gbi::PropCons>> edge_property_constraints_;    // per edge
};
...
inline const std::vector<gbi::PropCons>& QueryGraph::GetVertexPropertyConstraints(Vertex v) const {
  return vertex_property_constraints_[v];
}

inline const std::vector<gbi::PropCons>& QueryGraph::GetEdgePropertyConstraints(Size edge_idx) const {
