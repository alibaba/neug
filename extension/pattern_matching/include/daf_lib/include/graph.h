...
#include "../../../third_party/FaSTest/lib/DataStructure/json.hpp"
...
class Graph {
 public:
  ...
  inline const std::vector<std::string>& GetVertexPropertyNames(Label vertex_label) const;
  inline const std::vector<std::string>& GetEdgePropertyNames(Label edge_label) const;
  inline Size GetNumEdgeLabels() const;
  
  // Public method to load properties (for DataGraph and QueryGraph to use)
  void LoadGraphProperties(const std::string& vertex_property_filename, const std::string& edge_property_filename, 
                          std::unordered_map<std::string, int>& label2id_mapping, std::shared_ptr<gbi::SchemaGraph> schema_graph);
                          
  // Property matching methods (for DATA GRAPH)
  bool CheckVertexPropertyConstraints(const std::vector<gbi::PropCons>& constraints, Vertex v) const;
  bool CheckEdgePropertyConstraints(const std::vector<gbi::PropCons>& constraints, Size edge_idx) const;
 ...
 public:
  ...
  void LoadRoughGraph(std::vector<std::vector<Vertex>>* graph, std::unordered_map<std::string, int>& label2id_mapping);
  void LoadProperty(const std::string& vertex_property_filename, const std::string& edge_property_filename, 
                    std::unordered_map<std::string, int>& label2id_mapping, std::shared_ptr<gbi::SchemaGraph> schema_graph);
  void computeCoreNum();
  ...
  std::vector<std::vector<std::string>> vertex_property_names_;  // Property names for each vertex label
  std::vector<std::vector<std::string>> edge_property_names_;    // Property names for each edge label
  std::vector<std::vector<gbi::Value>> vertex_properties_;      // Property values for each vertex
  std::vector<std::vector<gbi::Value>> edge_properties_;        // Property values for each edge
  std::vector<std::pair<Vertex, Vertex>> edge_list_;            // List of edges as vertex pairs
  std::vector<std::unordered_map<int, std::unordered_map<int, int>>> edge_index_map_;  // Map from (u,v,label) to edge index
...
inline const std::vector<gbi::Value>& Graph::GetVertexProperties(Vertex v) const {
  return vertex_properties_[v];
}

inline const std::vector<gbi::Value>& Graph::GetEdgeProperties(Size edge_idx) const {
  return edge_properties_[edge_idx];
}

inline const std::vector<std::string>& Graph::GetVertexPropertyNames(Label vertex_label) const {
  return vertex_property_names_[vertex_label];
}

inline const std::vector<std::string>& Graph::GetEdgePropertyNames(Label edge_label) const {
