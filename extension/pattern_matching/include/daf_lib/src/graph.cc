...
void Graph::LoadRoughGraph(std::vector<std::vector<Vertex>> *graph, std::unordered_map<std::string, int>& label2id_mapping) {
  if (!fin_.is_open()) {
    std::cerr << "Graph file " << filename_ << " not found!\n";
    exit(EXIT_FAILURE);
  }
...
void Graph::LoadProperty(const std::string& vertex_property_filename, const std::string& edge_property_filename,
  ...
  if (vertex_file.is_open()) {
    ...
    while (std::getline(vertex_file, line)) {
      ...
      try {
        ...
        vertex_properties_[vertex_id].resize(vertex_property_names_[label].size());
        ...
        for (size_t i = 0; i < vertex_property_names_[label].size(); i++) {
          ...
          if (j.contains(prop_name)) {
            ...
            if (val.is_number_integer()) {
              ...
              vertex_properties_[vertex_id][i] = gbi::Value((int32_t)(val ? 1 : 0));
            } else if (val.is_null()) {
              ...
              vertex_properties_[vertex_id][i] = gbi::GetDefaultValue(value_type);
            }
          }
        }
  ...
  if (edge_file.is_open()) {
    ...
    while (std::getline(edge_file, line) && edge_idx < num_edge_ / 2) {
      ...
      try {
        ...
        edge_properties_[edge_idx * 2].resize(edge_property_names_[label].size());
        edge_properties_[edge_idx * 2 + 1].resize(edge_property_names_[label].size());
  ...
  std::cout << "Loaded properties for " << num_vertex_ << " vertices and " << num_edge_ / 2 << " edges." << std::endl;
}

void Graph::LoadGraphProperties(const std::string& vertex_property_filename, const std::string& edge_property_filename,
                               std::unordered_map<std::string, int>& label2id_mapping, std::shared_ptr<gbi::SchemaGraph> schema_graph) {
  LoadProperty(vertex_property_filename, edge_property_filename, label2id_mapping, schema_graph);
}

bool Graph::CheckVertexPropertyConstraints(const std::vector<gbi::PropCons>& constraints, Vertex v) const {
  if (constraints.empty()) return true;
  if (!HasVertexAttributes()) return constraints.empty();
  ...
  for (const auto& constraint : constraints) {
    if (constraint._use_expr) {
      ...
      auto resolver = [&prop_names, &prop_values](const std::string& alias, const std::string& prop_name) -> gbi::Value {
        for (size_t i = 0; i < prop_names.size(); i++) {
          if (prop_names[i] == prop_name) {
            if (i < prop_values.size()) {
              if (prop_values[i].IsNull() || prop_values[i].is_orgininally_null) {
                return gbi::Value();
              }
      ...
      switch (constraint._comp_type) {
        ...
        case gbi::CompType::COMP_IS_NULL:
          constraint_satisfied = (actual_value.IsNull() || actual_value.is_orgininally_null);
          break;
...
bool Graph::CheckEdgePropertyConstraints(const std::vector<gbi::PropCons>& constraints, Size edge_idx) const {
  if (constraints.empty()) return true;
  if (!HasEdgeAttributes()) return constraints.empty();
