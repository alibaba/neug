...
TEST_F(ParquetTest, TestTypeMapping_StringToLargeUtf8) {
  ...
  EXPECT_TRUE(arrowType1->Equals(arrow::large_utf8()))
      << "Extension should convert Arrow utf8 to large_utf8 for Neug STRING "
         "type. "
...
TEST_F(ParquetTest, TestIntegration_ColumnPruning) {
  ...
  // Project only id, score, grade (exclude "name")
  ...
  EXPECT_EQ(ctx.col_num(), 3)
      << "Extension should project only specified columns";
  EXPECT_EQ(sharedState->columnNum(), 3)
      << "columnNum should reflect projected column count";
}
