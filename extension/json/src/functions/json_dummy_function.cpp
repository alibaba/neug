...
function_set JsonDummyFunction::getFunctionSet() {
    ...
    functionSet.emplace_back(std::make_unique<ScalarFunction>(
        ...
        nullptr,
        JsonDummyFunction::Exec
    ));
    return functionSet;
}

RTAny JsonDummyFunction::Exec(size_t idx,
                                      Arena& arena,
                                      const std::vector<RTAny>& args) {
    if (args.size() != 1) {
        THROW_EXCEPTION_WITH_FILE_LINE("JSON_DUMMY: expect exactly 1 argument, got " + std::to_string(args.size()));
    }
