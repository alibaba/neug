...
Timing methodology:
  - NeuG:  reports both "algo time" (C++ GDSAlgoOpr elapsed) and
           "total time" (algo + result materialization in Python)
  - NetworkX: total algorithm time (Python, single-threaded)
"""
...
DB_PATH = "/tmp/gds_bench_v2_db"
BFS_SOURCE = 6  # datagen-8_0-fb vertex IDs start at 6
NUM_EDGES = 107507376
...
def neug_algo_time(conn, query):
    """Run query, return (total_wall_time, rows_count).
    We capture C++ algo time from log output via a separate mechanism."""
    t0 = time.time()
...
def main():
    ...
    print(f"  NeuG:  {t_n:.3f}s  |  NetworkX: {t_nx:.3f}s  |  "
          f"Speedup: {t_nx/t_n:.1f}x  ({len(comps)} components)")
    ...
    t_n, n = neug_algo_time(
        conn,
        f"CALL bfs('{GRAPH_NAME}', {{source: {bfs_src}}}) "
        f"YIELD node, distance RETURN node.id, distance;",
    )
    t0 = time.time()
    lengths = nx.single_source_shortest_path_length(G, bfs_src)
    nx_bfs_count = sum(1 for _ in lengths)  # consume generator
    t_nx = time.time() - t0
    results.append(("BFS", t_n, t_nx, f"source={bfs_src}"))
    print(f"  NeuG:  {t_n:.3f}s  |  NetworkX: {t_nx:.3f}s  |  "
    ...
    t_n, n = neug_algo_time(
        conn,
        f"CALL page_rank('{GRAPH_NAME}', {{max_iterations: 10, damping_factor: 0.85}}) "
        f"YIELD node, rank RETURN node.id, rank;",
    )
    t0 = time.time()
    pr = nx.pagerank(G, max_iter=10, tol=1e-6, alpha=0.85)
    t_nx = time.time() - t0
    results.append(("PageRank", t_n, t_nx, "iters=10"))
    print(f"  NeuG:  {t_n:.3f}s  |  NetworkX: {t_nx:.3f}s  |  "
          f"Speedup: {t_nx/t_n:.1f}x")
    ...
    t_nx = time.time() - t0
    results.append(("CDLP", t_n, t_nx, f"{len(nx_lp)} communities"))
    print(f"  NeuG:  {t_n:.3f}s  |  NetworkX: {t_nx:.3f}s  |  "
          f"Speedup: {t_nx/t_n:.1f}x  ({len(nx_lp)} communities)")

    # ── Leiden (NeuG only, no NetworkX built-in) ──
    print("\n── Leiden (NeuG only) ──")
    t_n, n = neug_algo_time(
        conn,
    ...
    # ── Louvain (NeuG vs NetworkX community.louvain_communities) ──
    print("\n── Louvain ──")
    t_n, n = neug_algo_time(
        conn,
    ...
    t0 = time.time()
    try:
        nx_louvain = nx.community.louvain_communities(G, resolution=1.0, seed=42)
        t_nx = time.time() - t0
        nx_comm_count = len(nx_louvain)
    except Exception as e:
        t_nx = -1
        nx_comm_count = 0
        print(f"  NetworkX Louvain error: {e}")
    results.append(("Louvain", t_n, t_nx, f"{n} vertices"))
    if t_nx > 0:
        print(f"  NeuG:  {t_n:.3f}s  |  NetworkX: {t_nx:.3f}s  |  "
              f"Speedup: {t_nx/t_n:.1f}x  ({nx_comm_count} communities)")
    else:
        print(f"  NeuG:  {t_n:.3f}s  |  NetworkX: failed")
    ...
    print(f"  NeuG: C++ parallel (concurrency=4), CSR storage, project_graph view")
    print(f"  NetworkX: Python single-threaded, dict-based adjacency")
    print(f"  LadybugDB: N/A (no GDS algorithm support)")
    print()
    print(f"  {'Algorithm':<15} {'NeuG(s)':<12} {'NetworkX(s)':<14} {'Speedup':<10} {'Notes'}")
    print(f"  {'─' * 65}")
