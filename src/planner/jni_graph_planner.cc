...
JavaVM* CreateJavaVM(const std::string& jvm_options) {
  ...
  if (jvm_opts == NULL) {
    LOG(ERROR) << "Expect FLEX_JVM_OPTS set before initiate jvm";
    return NULL;
  }
  LOG(INFO) << "Jvm opts str: " << jvm_opts;
  ...
  if (status == JNI_OK) {
    ...
    LOG(INFO) << "JNI evn already exists.";
  } else {
...
std::string JavaGraphPlanner::expand_directory(const std::string& path) {
...
std::string JavaGraphPlanner::generate_jvm_options(
    const std::string java_path) {
  auto expanded_java_path = expand_directory(java_path);
  LOG(INFO) << "Expanded java path: " << expanded_java_path;
...
Plan compilePlanJNI(jclass graph_planner_clz_,
  ...
  LOG(INFO) << "Compiling plan with JNI: ";
  jni::GetJavaVM()->AttachCurrentThread(reinterpret_cast<void**>(&env),
                                        nullptr);
  Plan plan;
  if (graph_planner_clz_ == NULL || graph_planner_method_id_ == NULL) {
    LOG(ERROR) << "Invalid JavaGraphPlanner.";
    return plan;
  }
  VLOG(10) << "Compiling plan with JNI: " << compiler_config_path << " "
           << cypher_query_string << " " << graph_schema_yaml << " "
  ...
  jobject jni_plan = (jobject) env->CallStaticObjectMethod(
      graph_planner_clz_, graph_planner_method_id_, param1, param2, param3,
      param4);
  ...
  jstring error_code =
      (jstring) env->CallObjectMethod(jni_plan, get_error_code);

  if (error_code == NULL) {
    LOG(ERROR) << "Fail to get error code from compiled plan.";
    return plan;
  }
  ...
  jmethodID method1 = env->GetMethodID(env->GetObjectClass(jni_plan),
                                       "getPhysicalBytes", "()[B");
  jmethodID method2 =
      env->GetMethodID(env->GetObjectClass(jni_plan), "getResultSchemaYaml",
                       "()Ljava/lang/String;");
...
physical::PhysicalPlan readPhysicalPlan(const std::string& plan_str) {
  LOG(INFO) << "plan str size: " << plan_str.size();
...
physical::PhysicalPlan compilePlanSubprocess(
  ...
  LOG(INFO) << "dst_query_path: " << dst_query_path
  ...
  if (pid == 0) {
    const char* const command_string_array[] = {
        ...
        JavaGraphPlanner::GRAPH_PLANNER_FULL_NAME,
 ...
 */
Plan JavaGraphPlanner::compilePlan(const std::string& cypher_query_string,
                                   const std::string& graph_schema_yaml,
                                   const std::string& graph_statistic_json) {
#if (GRAPH_PLANNER_JNI_INVOKER)
  return compilePlanJNI(
      graph_planner_clz_, graph_planner_method_id_, jni_wrapper_.env(),
      IGraphPlanner::compiler_config_path_, cypher_query_string,
      graph_schema_yaml, graph_statistic_json);
#else
  return compilePlanSubprocess(class_path_, jna_path_, graph_schema_yaml_,
                               graph_statistic_json_, compiler_config_path_,
                               cypher_query_string);
#endif
}
