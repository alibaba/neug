...
void Table::initColumns(const std::vector<std::string>& col_name,
  ...
  strategies.resize(col_num, MemoryLevel::kInMemory);
...
void Table::open(const std::string& name, const std::string& work_dir,
                 ...
                 const std::vector<DataType>& property_types,
                 const std::vector<MemoryLevel>& strategies_) {
  LOG(INFO) << "open table " << name << " from work dir " << work_dir;
  name_ = name;
  work_dir_ = work_dir;
  ...
  for (size_t i = 0; i < columns_.size(); ++i) {
    columns_[i]->open(name + ".col_" + std::to_string(i), snapshot_dir_,
                      tmp_dir(work_dir));
    LOG(INFO) << "open column file "
              << snapshot_dir_ + "/" + name + ".col_" + std::to_string(i)
              << ", size: " << columns_[i]->size();
  }
  touched_ = false;
  buildColumnPtrs();
}
