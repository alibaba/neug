...
RowExpressionFilter::RowExpressionFilter(
  ...
  auto apply_operator = [&](const ::common::ExprOpr& opr) {
    ...
    if (opr.logical() == ::common::Logical::NOT) {
      ...
      value_stack.pop();
      value_stack.push([operand](const columnar::DataChunk& chunk, size_t row) {
  ...
  for (int i = 0; i < expr.operators_size(); ++i) {
    ...
    case ::common::ExprOpr::kVar: {
      ...
      value_stack.push([col_idx](const columnar::DataChunk& chunk, size_t row) {
...
bool RowExpressionFilter::eval(const columnar::DataChunk& chunk,
                               size_t row) const {
  if (!evaluator_) {
    return true;
  }
