...
#include "neug/main/app/cypher_insert_app.h"
...
bool CypherInsertApp::Query(NeugDBSession& graph, Decoder& input,
                            Encoder& output) {
...
AppWrapper CypherInsertAppFactory::CreateApp(const NeugDB& db) {
  return AppWrapper(new CypherInsertApp(db), NULL);
}
}  // namespace gs