...
#include <span>
#include "src/include/common/copy_constructors.h"
#include "src/include/common/data_chunk/sel_vector.h"
...
struct NbrTableInfo {
  ...
  NbrTableInfo(catalog::TableCatalogEntry* nodeEntry,
               catalog::TableCatalogEntry* relEntry)
 ...
 public:
  struct Chunk {
    ...
    std::span<const T> getProperties(size_t propertyIndex) const {
      return std::span(
          reinterpret_cast<const T*>(propertyVectors[propertyIndex]->getData()),
          nodeIDs.size());
    }
 ...
 *
 * Instances of Graph are not expected to be thread-safe. Therefore, if Graph is
 * intended to be used in a parallel manner, the user should first copy() an
 * instance and give each thread a separate copy. It is the responsibility of
 * the implementing Graph class that the copy() is a lightweight operation that
 * does not copy large amounts of data between instances.
 */
class Graph {
 public:
  class EdgeIterator {
   public:
    explicit constexpr EdgeIterator(NbrScanState* scanState)
        : scanState{scanState} {}
    ...
    bool operator==(const EdgeIterator& other) const {
      // Only needed for comparing to the end, so they are equal if and only if
      // both are null
      return scanState == nullptr && other.scanState == nullptr;
    }
    // Counts and consumes the iterator
    uint64_t count() const {
      // TODO(bmwinger): avoid scanning if all that's necessary is to count the
      // results
      uint64_t result = 0;
      do {
    ...
    std::vector<common::nodeID_t> collectNbrNodes() {
      ...
      for (const auto chunk : *this) {
        ...
        chunk.forEach([&](auto neighbors, auto, auto i) {
          nbrNodes.push_back(neighbors[i]);
        });
      }
      return nbrNodes;
    }
    ...
    static constexpr EdgeIterator end() noexcept {
  ...
  // Prepares scan on the specified relationship table (works for backwards and
  // forwards scans)
  virtual std::unique_ptr<NbrScanState> prepareRelScan(
      catalog::TableCatalogEntry* relEntry,
  ...
  virtual EdgeIterator scanFwd(common::nodeID_t nodeID,
                               NbrScanState& state) = 0;

  // Get dst nodeIDs for given src nodeID tables using backward adjList.
  virtual EdgeIterator scanBwd(common::nodeID_t nodeID,
                               NbrScanState& state) = 0;

  class VertexIterator {
   public:
    explicit constexpr VertexIterator(VertexScanState* scanState)
        : scanState{scanState} {}
    ...
    static constexpr VertexIterator end() noexcept {
  ...
  virtual VertexIterator scanVertices(common::offset_t startNodeOffset,
                                      common::offset_t endNodeOffsetExclusive,
                                      VertexScanState& scanState) = 0;
