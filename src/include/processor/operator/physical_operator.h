...
struct OperatorMetrics {
  ...
  OperatorMetrics(common::TimeMetric& executionTime,
                  common::NumericMetric& numOutputTuple)
...
class KUZU_API PhysicalOperator {
 public:
  ...
  PhysicalOperator(PhysicalOperatorType operatorType,
                   ...
                   std::unique_ptr<OPPrintInfo> printInfo);
  PhysicalOperator(PhysicalOperatorType operatorType,
                   physical_op_vector_t children, physical_op_id id,
                   std::unique_ptr<OPPrintInfo> printInfo);
  ...
  std::vector<std::string> getProfilerAttributes(
      common::Profiler& profiler) const;
 ...
 protected:
  ...
  virtual void initLocalStateInternal(ResultSet* /*resultSet_*/,
  ...
  std::string getNumTupleMetricKey() const {
    return "numTuple-" + std::to_string(id);
  }
