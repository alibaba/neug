...
struct SubStr {
 public:
  static inline void operation(common::ku_string_t& src, int64_t start,
    ...
    // 1 character more than length has to be scanned for diatrics case: y + ˘ =
    // ў.
    ...
    if (isAscii) {
      copySubstr(src, start, len, result, resultValueVector,
                 true /* isAscii */);
    } else {
      ...
      // In this case, the function gets the EXACT byte location to start
      // copying from.
      copySubstr(src, startBytePos, endBytePos - startBytePos, result,
                 resultValueVector, false /* isAscii */);
    }
  }
  ...
  static inline void copySubstr(common::ku_string_t& src, int64_t start,
    ...
    if (isAscii) {
      // For normal ASCII char case, we get to the proper byte position to copy
      // from by doing a -1 (since it is guaranteed each char is 1 byte).
      common::StringVector::addString(&resultValueVector, result,
                                      (const char*) (src.getData() + start - 1),
                                      length);
    } else {
      // For utf8 char copy, the function gets the exact starting byte position
      // to copy from.
      common::StringVector::addString(&resultValueVector, result,
                                      (const char*) (src.getData() + start),
                                      length);
    }
  }
};
