...
class ParentList {
 public:
  void setNbrInfo(common::nodeID_t nodeID_, common::relID_t edgeID_,
                  bool isFwd_) {
    nodeID = nodeID_;
    edgeID = edgeID_;
    isFwd = isFwd_;
...
class BaseBFSGraph {
 ...
 public:
  ...
  virtual void addParent(uint16_t iter, common::nodeID_t boundNodeID,
                         common::relID_t edgeID, common::nodeID_t nbrNodeID,
                         bool fwdEdge, ObjectBlock<ParentList>* block) = 0;
  // Used to track path for single shortest path. Assume each offset has at most
  // one parent.
  virtual void addSingleParent(uint16_t iter, common::nodeID_t boundNodeID,
                               common::relID_t edgeID,
                               common::nodeID_t nbrNodeID, bool fwdEdge,
                               ObjectBlock<ParentList>* block) = 0;
  // Used to track path for all weighted shortest path.
  virtual bool tryAddParentWithWeight(common::nodeID_t boundNodeID,
                                      common::relID_t edgeID,
                                      common::nodeID_t nbrNodeID, bool fwdEdge,
                                      double weight,
                                      ObjectBlock<ParentList>* block) = 0;
  // Used to track path for single weighted shortest path. Assume each offset
  // has at most one parent.
  virtual bool tryAddSingleParentWithWeight(common::nodeID_t boundNodeID,
                                            common::relID_t edgeID,
  ...
  virtual void setParentList(common::offset_t offset,
...
class DenseBFSGraph : public BaseBFSGraph {
 ...
 public:
  DenseBFSGraph(storage::MemoryManager* mm,
  ...
  void addParent(uint16_t iter, common::nodeID_t boundNodeID,
                 common::relID_t edgeID, common::nodeID_t nbrNodeID,
                 bool fwdEdge, ObjectBlock<ParentList>* block) override;
  void addSingleParent(uint16_t iter, common::nodeID_t boundNodeID,
                       common::relID_t edgeID, common::nodeID_t nbrNodeID,
                       bool fwdEdge, ObjectBlock<ParentList>* block) override;
  bool tryAddParentWithWeight(common::nodeID_t boundNodeID,
                              common::relID_t edgeID,
                              common::nodeID_t nbrNodeID, bool fwdEdge,
                              double weight,
                              ObjectBlock<ParentList>* block) override;
  bool tryAddSingleParentWithWeight(common::nodeID_t boundNodeID,
                                    common::relID_t edgeID,
                                    common::nodeID_t nbrNodeID, bool fwdEdge,
                                    double weight,
                                    ObjectBlock<ParentList>* block) override;
...
class SparseBFSGraph : public BaseBFSGraph {
 ...
 public:
  ...
  const std::unordered_map<common::offset_t, ParentList*>& getCurrentData()
      const {
    return *curData;
  }
