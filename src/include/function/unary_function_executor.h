...
#include "src/include/function/cast/cast_function_bind_data.h"

namespace gs {
namespace function {

/**
 * Unary operator assumes operation with null returns null. This does NOT
 * applies to IS_NULL and IS_NOT_NULL operation.
 */

struct UnaryFunctionWrapper {
  template <typename OPERAND_TYPE, typename RESULT_TYPE, typename FUNC>
  static inline void operation(void* inputVector, uint64_t inputPos,
                               void* resultVector, uint64_t resultPos,
                               void* /*dataPtr*/) {
...
struct UnaryStringFunctionWrapper {
  template <typename OPERAND_TYPE, typename RESULT_TYPE, typename FUNC>
  static void operation(void* inputVector, uint64_t inputPos,
                        void* resultVector, uint64_t resultPos,
                        void* /*dataPtr*/) {
    auto& inputVector_ = *(common::ValueVector*) inputVector;
    auto& resultVector_ = *(common::ValueVector*) resultVector;
...
struct UnaryStructFunctionWrapper {
  ...
  static void operation(void* /*inputVector*/, uint64_t /*inputPos*/,
    ...
    auto& resultVector_ = *(common::ValueVector*) resultVector;
    FUNC::operation(resultVector_.getValue<RESULT_TYPE>(resultPos),
                    resultVector_, dataPtr);
  }
};
...
struct UnaryFunctionExecutor {
  template <typename OPERAND_TYPE, typename RESULT_TYPE, typename FUNC,
            typename OP_WRAPPER>
  static void executeOnValue(common::ValueVector& inputVector,
                             uint64_t inputPos,
                             common::ValueVector& resultVector,
                             uint64_t resultPos, void* dataPtr) {
    OP_WRAPPER::template operation<OPERAND_TYPE, RESULT_TYPE, FUNC>(
        (void*) &inputVector, inputPos, (void*) &resultVector, resultPos,
        dataPtr);
  }

  static std::pair<common::sel_t, common::sel_t> getSelectedPos(
      common::idx_t selIdx, common::SelectionVector* operandSelVector,
      common::SelectionVector* resultSelVector, bool operandIsUnfiltered,
      bool resultIsUnfiltered) {
    common::sel_t operandPos =
        operandIsUnfiltered ? selIdx : (*operandSelVector)[selIdx];
    common::sel_t resultPos =
        resultIsUnfiltered ? selIdx : (*resultSelVector)[selIdx];
  ...
  static void executeOnSelectedValues(common::ValueVector& operand,
    ...
    for (auto i = 0u; i < operandSelVector->getSelSize(); i++) {
      const auto [operandPos, resultPos] =
          getSelectedPos(i, operandSelVector, resultSelVector,
                         operandIsUnfiltered, resultIsUnfiltered);
      if (noNullsGuaranteed) {
        executeOnValue<OPERAND_TYPE, RESULT_TYPE, FUNC, OP_WRAPPER>(
            operand, operandPos, result, resultPos, dataPtr);
      } else {
  ...
  static void executeSwitch(common::ValueVector& operand,
    ...
    if (operand.state->isFlat()) {
      ...
      executeOnSelectedValues<OPERAND_TYPE, RESULT_TYPE, FUNC, OP_WRAPPER>(
          operand, operandSelVector, result, resultSelVector, dataPtr);
    }
  }
  ...
  static void execute(common::ValueVector& operand,
    ...
    executeSwitch<OPERAND_TYPE, RESULT_TYPE, FUNC, UnaryFunctionWrapper>(
        operand, operandSelVector, result, resultSelVector,
        nullptr /* dataPtr */);
  }
  ...
  static void executeSequence(common::ValueVector& operand,
    ...
    executeOnValue<OPERAND_TYPE, RESULT_TYPE, FUNC,
                   UnarySequenceFunctionWrapper>(operand, inputPos, result,
                                                 resultPos, dataPtr);
  }
};
