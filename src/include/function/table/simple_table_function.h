...
struct TableFuncMorsel {
  ...
  bool isInvalid() const {
    return startOffset == common::INVALID_OFFSET &&
           endOffset == common::INVALID_OFFSET;
  }
};
...
class KUZU_API SimpleTableFuncSharedState : public TableFuncSharedState {
 public:
  ...
  explicit SimpleTableFuncSharedState(
...
struct KUZU_API SimpleTableFunc {
  ...
  static table_func_t getTableFunc(
...
// Note this is only used for internal purpose, and only supports node tables
// for now.
