...
// Different from vanilla dp algorithm where one optimal plan is kept per
// subgraph, we keep multiple plans each with a different factorization
// structure. The following example will explain our rationale. Given a triangle
// with an outgoing edge MATCH (a)->(b)->(c), (a)->(c), (c)->(d) At level 3
// (assume level is based on num of nodes) for subgraph "abc", if we ignore
// factorization structure, the 3 plans that intersects on "a", "b", or "c" are
// considered homogenous and one of them will be picked. Then at level 4 for
// subgraph "abcd", we know the plan that intersect on "c" will be worse because
...
class SubgraphPlans {
 ...
 private:
  // To balance computation time, we encode plan by only considering the flat
  // information of the nodes that are involved in current subgraph.
  std::bitset<binder::MAX_NUM_QUERY_VARIABLES> encodePlan(
 ...
 private:
  ...
  std::unordered_map<std::bitset<binder::MAX_NUM_QUERY_VARIABLES>,
                     common::idx_t>
      encodedPlan2PlanIdx;
};

// A DPLevel is a collection of plans per subgraph. All subgraph should have the
...
class DPLevel {
 public:
  ...
  inline SubgraphPlans* getSubgraphPlans(
  ...
  void addPlan(const binder::SubqueryGraph& subqueryGraph,
