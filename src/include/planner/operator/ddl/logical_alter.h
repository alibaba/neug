...
#include "src/include/planner/operator/simple/logical_simple.h"
...
struct LogicalAlterPrintInfo final : OPPrintInfo {
  ...
  explicit LogicalAlterPrintInfo(binder::BoundAlterInfo info)
      : info{std::move(info)} {}

  std::string toString() const override { return info.toString(); }

  std::unique_ptr<OPPrintInfo> copy() const override {
    return std::unique_ptr<LogicalAlterPrintInfo>(
        new LogicalAlterPrintInfo(*this));
  }
...
class LogicalAlter final : public LogicalSimple {
 ...
 public:
  LogicalAlter(binder::BoundAlterInfo info,
