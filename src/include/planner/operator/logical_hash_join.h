...
#include "src/include/common/enums/join_type.h"
#include "src/include/planner/operator/sip/side_way_info_passing.h"
...
class KUZU_API LogicalHashJoin final : public LogicalOperator {
 ...
 public:
  ...
  // 2. if the build side contains more than one group or the build side has
  // projected out data chunks, which may increase the multiplicity of data
  // chunks in the build side. The key is to keep probe side key unflat only
  // when we know that there is only 0 or 1 match for each key.
  // TODO(Guodong): when the build side has only flat payloads, we should
  // consider getting rid of flattening probe key, instead duplicating keys as
  // in vectorized processing if necessary.
