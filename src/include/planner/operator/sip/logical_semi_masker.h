 ...
 *
 * PATH
 * offsets are collected from value vector of type PATH (LIST[INTERNAL_ID]).
 * This is a fast-path code used when scanning properties along the path.
 *
 * */
...
struct ExtraPathKeyInfo final : ExtraKeyInfo {
  ...
  explicit ExtraPathKeyInfo(common::ExtendDirection direction)
      : direction{direction} {}
...
class LogicalSemiMasker final : public LogicalOperator {
 ...
 public:
  ...
  std::vector<const LogicalOperator*> getTargetOperators() const {
    return targetOps;
  }
  ...
  std::unique_ptr<LogicalOperator> copy() override {
    ...
    auto result = std::make_unique<LogicalSemiMasker>(
        keyType, targetType, key, nodeTableIDs, children[0]->copy());
