...
#include "src/include/binder/query/updating_clause/bound_set_info.h"
...
class LogicalMerge final : public LogicalOperator {
 ...
 public:
  ...
  const std::vector<LogicalInsertInfo>& getInsertNodeInfos() const {
  ...
  const std::vector<LogicalInsertInfo>& getInsertRelInfos() const {
  ...
  const std::vector<binder::BoundSetPropertyInfo>& getOnCreateSetNodeInfos()
      const {
  ...
  const std::vector<binder::BoundSetPropertyInfo>& getOnCreateSetRelInfos()
      const {
  ...
  const std::vector<binder::BoundSetPropertyInfo>& getOnMatchSetNodeInfos()
      const {
  ...
  const std::vector<binder::BoundSetPropertyInfo>& getOnMatchSetRelInfos()
      const {
 ...
 private:
  ...
  // Since we don't re-evaluate the existence of n for each x, we need to create
  // n only for distinct x, i.e. 1 & 3. So there is a notion of key in MERGE.
  binder::expression_vector keys;
};
