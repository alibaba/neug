...
#include "src/include/binder/copy/index_look_up_info.h"
...
// relationship copy. So it is not a source operator. I would suggest move this
// logic into rel copy instead of maintaining an operator.
class LogicalPrimaryKeyLookup final : public LogicalOperator {
  static constexpr LogicalOperatorType type_ =
      LogicalOperatorType::INDEX_LOOK_UP;
 ...
 public:
  ...
  const binder::IndexLookupInfo& getInfo(uint32_t idx) const {
    return infos[idx];
  }
