...
class KUZU_API StringUtils {
 public:
  ...
  static std::vector<std::string_view> smartSplit(
      std::string_view input, char splitChar,
  ...
  static std::string extractStringBetween(const std::string& input,
                                          char delimiterStart,
                                          char delimiterEnd,
  ...
  static bool caseInsensitiveEquals(std::string_view left,
  ...
  static std::string join(const C& input, S count, const std::string& separator,
                          Func f);

  static constexpr uint8_t asciiToLowerCaseMap[] = {
  ...
  // container hash function for strings which lets you hash both string_view
