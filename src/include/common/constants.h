...
#include "src/include/common/array_utils.h"
...
// which won't work for values other than 2. If this is changed, those will need
// to be updated
...
struct BufferPoolConstants {
  // If a user does not specify a max size for BM, we by default set the max
  // size of BM to maxPhyMemSize * DEFAULT_PHY_MEM_SIZE_RATIO_FOR_BM.
  static constexpr double DEFAULT_PHY_MEM_SIZE_RATIO_FOR_BM = 0.8;
// The default max size for a VMRegion.
#ifdef __32BIT__
  static constexpr uint64_t DEFAULT_VM_REGION_MAX_SIZE = (uint64_t) 1
                                                         << 30;  // (1GB)
...
struct StorageConstants {
  ...
  static constexpr uint64_t PAGE_GROUP_SIZE = static_cast<uint64_t>(1)
  ...
  static constexpr uint64_t MAX_NUM_ROWS_IN_TABLE = static_cast<uint64_t>(1)
                                                    << 62;
...
struct CopyConstants {
  ...
  // This means that we will usually read the entirety of the contents of the
  // file we need for a block in one read request. It is also very small, which
  // means we can parallelize small files efficiently.
  ...
  static constexpr const char* BOOL_CSV_PARSING_OPTIONS[] = {
      "HEADER",     "PARALLEL",    "LIST_UNBRACED",
      "AUTODETECT", "AUTO_DETECT", CopyConstants::IGNORE_ERRORS_OPTION_NAME};
  ...
  static constexpr std::array CSV_WARNING_DATA_COLUMN_TYPES = arrayConcat(
      SHARED_WARNING_DATA_COLUMN_TYPES, CSV_SPECIFIC_WARNING_DATA_COLUMN_TYPES);
  static constexpr column_id_t CSV_WARNING_DATA_NUM_COLUMNS =
      CSV_WARNING_DATA_COLUMN_NAMES.size();
  static_assert(CSV_WARNING_DATA_NUM_COLUMNS ==
                CSV_WARNING_DATA_COLUMN_TYPES.size());
...
struct PlannerKnobs {
  ...
  // probe side is not much bigger than build.
...
struct ParquetConstants {
  ...
  // Dictionary pages must be below 2GB. Unlike data pages, there's only one
  // dictionary page. For this reason we go with a much higher, but still a
  // conservative upper bound of 1GB.
...
struct WarningConstants {
  ...
  static_assert(WARNING_TABLE_COLUMN_DATA_TYPES.size() ==
