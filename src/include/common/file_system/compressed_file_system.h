...
#include "src/include/common/file_system/file_system.h"
...
class CompressedFileSystem : public FileSystem {
 public:
  virtual std::unique_ptr<FileInfo> openCompressedFile(
      std::unique_ptr<FileInfo> fileInfo) = 0;
 ...
 protected:
  ...
  void readFromFile(FileInfo& /*fileInfo*/, void* /*buffer*/,
                    uint64_t /*numBytes*/,
                    uint64_t /*position*/) const override;

  int64_t readFile(FileInfo& fileInfo, void* buf,
                   size_t numBytes) const override;

  void writeFile(FileInfo& /*fileInfo*/, const uint8_t* /*buffer*/,
                 uint64_t /*numBytes*/, uint64_t /*offset*/) const override {
    KU_UNREACHABLE;
  }

  void reset(FileInfo& fileInfo) override;

  int64_t seek(FileInfo& /*fileInfo*/, uint64_t /*offset*/,
...
struct CompressedFileInfo : public FileInfo {
  ...
  CompressedFileInfo(CompressedFileSystem& compressedFS,
                     std::unique_ptr<FileInfo> childFileInfo)
      : FileInfo{childFileInfo->path, &compressedFS},
        compressedFS{compressedFS},
