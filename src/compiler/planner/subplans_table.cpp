#include "src/include/planner/subplans_table.h"
  ...
  for (auto i = 0u; i < subqueryGraph.queryGraph.getNumQueryNodes(); ++i) {
    if (subqueryGraph.queryNodesSelector[i]) {
      nodeIDsToEncode.push_back(
          subqueryGraph.queryGraph.getQueryNode(i)->getInternalID());
    }
  }
  maxCost = UINT64_MAX;
}
...
std::bitset<MAX_NUM_QUERY_VARIABLES> SubgraphPlans::encodePlan(
  ...
  for (auto i = 0u; i < nodeIDsToEncode.size(); ++i) {
    result[i] =
        schema->getGroup(schema->getGroupPos(*nodeIDsToEncode[i]))->isFlat();
  }
  return result;
}
...
void DPLevel::addPlan(const gs::binder::SubqueryGraph& subqueryGraph,
  ...
  if (!contains(subqueryGraph)) {
    subgraph2Plans.insert(
        {subqueryGraph, std::make_unique<SubgraphPlans>(subqueryGraph)});
  }
...
uint64_t SubPlansTable::getMaxCost(const SubqueryGraph& subqueryGraph) const {
  return containSubgraphPlans(subqueryGraph)
             ? getDPLevel(subqueryGraph)
                   ->getSubgraphPlans(subqueryGraph)
                   ->getMaxCost()
             : UINT64_MAX;
}

bool SubPlansTable::containSubgraphPlans(
...
void SubPlansTable::addPlan(const SubqueryGraph& subqueryGraph,
                            std::unique_ptr<LogicalPlan> plan) {
  auto dpLevel = getDPLevel(subqueryGraph);
