...
void Planner::planInsertClause(const BoundUpdatingClause* updatingClause,
                               LogicalPlan& plan) {
...
void Planner::planMergeClause(const BoundUpdatingClause* updatingClause,
                              LogicalPlan& plan) {
  ...
  planOptionalMatch(*mergeClause.getQueryGraphCollection(), predicates,
                    existenceMark, plan, nullptr /* hint */);
  auto merge = std::make_shared<LogicalMerge>(existenceMark, keys,
