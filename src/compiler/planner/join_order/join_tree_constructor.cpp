...
static std::vector<std::shared_ptr<NodeExpression>> getJoinNodes(
    const SubqueryGraph& subgraph, const SubqueryGraph& otherSubgraph) {
  NEUG_ASSERT(&subgraph.queryGraph == &otherSubgraph.queryGraph);
...
std::shared_ptr<NodeExpression> getIntersectNode(
  ...
  if (candidates.size() != 1) {
    THROW_BINDER_EXCEPTION("Cannot resolve join condition for multi-way join.");
  }
...
JoinTreeConstructor::IntermediateResult JoinTreeConstructor::constructTreeNode(
  ...
  if (hintNode->isLeaf()) {
    if (ExpressionUtil::isNodePattern(*hintNode->nodeOrRel)) {
      ...
      NEUG_ASSERT(ExpressionUtil::isRelPattern(*hintNode->nodeOrRel) ||
  ...
  if (hintNode->isBinary()) {
    ...
    auto predicates =
        Planner::getNewlyMatchedExprs(left.subqueryGraph, right.subqueryGraph,
                                      newSubgraph, queryGraphPredicates);
    // First try to construct as index nested loop join.
    auto nestedLoopTreeNode = tryConstructNestedLoopJoin(
        joinNodes, *left.treeNode, *right.treeNode, predicates);
    ...
    auto treeNode = std::make_shared<JoinTreeNode>(TreeNodeType::BINARY_JOIN,
                                                   std::move(extraInfo));
  ...
  NEUG_ASSERT(hintNode->isMultiWay());
  ...
  extraInfo->predicates = predicates;
  auto treeNode = std::make_shared<JoinTreeNode>(TreeNodeType::MULTIWAY_JOIN,
                                                 std::move(extraInfo));
...
JoinTreeConstructor::IntermediateResult JoinTreeConstructor::constructNodeScan(
  ...
  // See Planner::planBaseTableScans for how we plan unnest correlated
  // subqueries.
  if (planningInfo.subqueryType == SubqueryPlanningType::UNNEST_CORRELATED &&
      planningInfo.containsCorrExpr(*node.getInternalID())) {
    extraInfo->nodeInfo =
        std::make_unique<NodeRelScanInfo>(expr, expression_vector{});
    ;
    auto treeNode = std::make_shared<JoinTreeNode>(TreeNodeType::NODE_SCAN,
                                                   std::move(extraInfo));
    return {treeNode, newSubgraph};
  }
  auto properties = propertyCollection.getProperties(*expr);
  auto predicates = Planner::getNewlyMatchedExprs(emptySubgraph, newSubgraph,
                                                  queryGraphPredicates);
...
JoinTreeConstructor::IntermediateResult JoinTreeConstructor::constructRelScan(
  ...
  auto treeNode = std::make_shared<JoinTreeNode>(TreeNodeType::REL_SCAN,
                                                 std::move(extraInfo));
  return {treeNode, newSubgraph};
}
...
std::shared_ptr<JoinTreeNode> JoinTreeConstructor::tryConstructNestedLoopJoin(
  ...
  if (leftRoot.type == TreeNodeType::REL_SCAN &&
      rightRoot.type == TreeNodeType::NODE_SCAN) {
    return tryConstructNestedLoopJoin(joinNodes, rightRoot, leftRoot,
                                      predicates);
  }
  ...
  auto& rightExtraInfo =
      rightRoot.extraInfo->constCast<ExtraScanTreeNodeInfo>();
  ...
  return std::make_shared<JoinTreeNode>(TreeNodeType::NODE_SCAN,
                                        std::move(newExtraInfo));
}
