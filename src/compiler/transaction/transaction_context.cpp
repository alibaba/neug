...
TransactionContext::~TransactionContext() {
  if (activeTransaction) {
    clientContext.getDatabase()->transactionManager->rollback(
        clientContext, activeTransaction.get());
  }
}
...
void TransactionContext::beginAutoTransaction(bool readOnlyStatement) {
  ...
  beginTransactionInternal(readOnlyStatement ? TransactionType::READ_ONLY
...
void TransactionContext::beginTransactionInternal(
    TransactionType transactionType) {
  KU_ASSERT(!activeTransaction);
  activeTransaction =
      clientContext.getDatabase()->transactionManager->beginTransaction(
