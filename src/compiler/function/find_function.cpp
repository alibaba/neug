#include "src/include/function/string/functions/find_function.h"
...
int64_t Find::unalignedNeedleSizeFind(const uint8_t* haystack,
  ...
  // We perform unsigned integer comparisons to check for equality of the entire
  // needle in a single comparison. This implementation is inspired by the
  // memmem implementation of freebsd.
  ...
  for (auto offset = needleLen; offset < haystackLen; offset++) {
    ...
    // this shift is only necessary if the needle size is not aligned with the
    // unsigned integer size (e.g. needle size 3, unsigned integer size 4, we
    // need to shift by 1).
    haystackEntry =
        (haystackEntry << 8) | ((UNSIGNED(haystack[offset])) << shift);
  }
  if (haystackEntry == needleEntry) {
...
template <class UNSIGNED>
int64_t Find::alignedNeedleSizeFind(const uint8_t* haystack,
                                    uint32_t haystackLen, const uint8_t* needle,
                                    uint32_t firstMatchCharOffset) {
...
int64_t Find::genericFind(const uint8_t* haystack, uint32_t haystackLen,
  ...
  // (https://github.com/RaphaelJ/fast_strstr) generic contains; note that we
  // can't use strstr because we don't have null-terminated strings anymore we
  // keep track of a shifting window sum of all characters with window size
  // equal to needle_size this shifting sum is used to avoid calling into
  // memcmp; we only need to call into memcmp when the window sum is equal to
  // the needle sum when that happens, the characters are potentially the same
  // and we call into memcmp to check if they are.
  auto sumsDiff = 0u;
...
int64_t Find::find(const uint8_t* haystack, uint32_t haystackLen,
                   const uint8_t* needle, uint32_t needleLen) {
  ...
  case 2:
    return alignedNeedleSizeFind<uint16_t>(firstMatchCharPos, numCharsToMatch,
                                           needle, firstMatchCharOffset);
  case 3:
    return unalignedNeedleSizeFind<uint32_t>(firstMatchCharPos, numCharsToMatch,
                                             needle, 3, firstMatchCharOffset);
  case 4:
    return alignedNeedleSizeFind<uint32_t>(firstMatchCharPos, numCharsToMatch,
                                           needle, firstMatchCharOffset);
  case 5:
  case 6:
  case 7:
    return unalignedNeedleSizeFind<uint64_t>(firstMatchCharPos, numCharsToMatch,
                                             needle, needleLen,
                                             firstMatchCharOffset);
  case 8:
    return alignedNeedleSizeFind<uint64_t>(firstMatchCharPos, numCharsToMatch,
                                           needle, firstMatchCharOffset);
  default:
