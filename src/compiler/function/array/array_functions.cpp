...
#include "src/include/function/array/functions/array_cosine_similarity.h"
#include "src/include/function/array/functions/array_cross_product.h"
#include "src/include/function/array/functions/array_distance.h"
#include "src/include/function/array/functions/array_inner_product.h"
#include "src/include/function/array/functions/array_squared_distance.h"
...
static LogicalType interpretLogicalType(const binder::Expression* expr) {
  if (expr->expressionType == ExpressionType::LITERAL &&
    ...
    auto numChildren = expr->constPtrCast<binder::LiteralExpression>()
                           ->getValue()
                           .getChildrenSize();
    return LogicalType::ARRAY(ListType::getChildType(expr->dataType).copy(),
                              numChildren);
  }
...
static void validateChildType(const LogicalType& type,
...
static LogicalType validateArrayFunctionParameters(
    const LogicalType& leftType, const LogicalType& rightType,
    const std::string& functionName) {
...
std::unique_ptr<FunctionBindData> arrayTemplateBindFunc(
    std::string functionName, ScalarBindFuncInput input) {
  auto leftType = interpretLogicalType(input.arguments[0].get());
  auto rightType = interpretLogicalType(input.arguments[1].get());
  auto paramType =
  ...
  auto bindData = std::make_unique<FunctionBindData>(
      ArrayType::getChildType(paramType).copy());
...
function_set templateGetFunctionSet(const std::string& functionName) {
  ...
  auto function = std::make_unique<ScalarFunction>(functionName,
                                                   ...
                                                   LogicalTypeID::ANY);
  function->bindFunc = std::bind(arrayTemplateBindFunc<OPERATION>, functionName,
                                 std::placeholders::_1);
