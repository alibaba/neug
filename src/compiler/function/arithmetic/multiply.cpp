...
static inline bool tryMultiplyWithOverflowCheck(SRC_TYPE left, SRC_TYPE right,
                                                SRC_TYPE& result) {
  DST_TYPE uresult;
...
template <>
bool inline TryMultiply::operation(uint8_t& left, uint8_t& right,
                                   uint8_t& result) {
  return tryMultiplyWithOverflowCheck<uint8_t, uint16_t>(left, right, result);
}

template <>
bool inline TryMultiply::operation(uint16_t& left, uint16_t& right,
                                   uint16_t& result) {
  return tryMultiplyWithOverflowCheck<uint16_t, uint32_t>(left, right, result);
}

template <>
bool inline TryMultiply::operation(uint32_t& left, uint32_t& right,
                                   uint32_t& result) {
...
template <>
bool inline TryMultiply::operation(int8_t& left, int8_t& right,
                                   int8_t& result) {
  return tryMultiplyWithOverflowCheck<int8_t, int16_t>(left, right, result);
}

template <>
bool inline TryMultiply::operation(int16_t& left, int16_t& right,
                                   int16_t& result) {
  return tryMultiplyWithOverflowCheck<int16_t, int32_t>(left, right, result);
}

template <>
bool inline TryMultiply::operation(int32_t& left, int32_t& right,
                                   int32_t& result) {
...
bool TryMultiply::operation(int64_t& left, int64_t& right, int64_t& result) {
  ...
  // now we know that there is no overflow, we can just perform the
  // multiplication
  result = left * right;
#endif
  return true;
