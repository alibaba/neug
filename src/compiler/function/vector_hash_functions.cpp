...
#include "src/include/function/hash/hash_functions.h"
...
static void executeOnValue(const ValueVector& operand, sel_t operandPos,
                           ValueVector& result, sel_t resultPos) {
  Hash::operation(operand.getValue<OPERAND_TYPE>(operandPos),
                  result.getValue<RESULT_TYPE>(resultPos));
}

template <typename OPERAND_TYPE, typename RESULT_TYPE>
void UnaryHashFunctionExecutor::execute(
    const ValueVector& operand, const SelectionVector& operandSelectVec,
    ValueVector& result, const SelectionVector& resultSelectVec) {
  ...
  if (operand.hasNoNullsGuarantee()) {
    if (operandSelectVec.isUnfiltered()) {
      for (auto i = 0u; i < operandSelectVec.getSelSize(); i++) {
        ...
        executeOnValue<OPERAND_TYPE, RESULT_TYPE>(operand, i, result,
                                                  resultPos);
      }
    } else {
      for (auto i = 0u; i < operandSelectVec.getSelSize(); i++) {
    ...
    if (operandSelectVec.isUnfiltered()) {
      for (auto i = 0u; i < operandSelectVec.getSelSize(); i++) {
        auto resultPos = resultSelectVec[i];
        if (!operand.isNull(i)) {
          Hash::operation(operand.getValue<OPERAND_TYPE>(i),
                          resultValues[resultPos]);
        } else {
...
template <typename LEFT_TYPE, typename RIGHT_TYPE, typename RESULT_TYPE,
          typename FUNC>
static void executeOnValue(const common::ValueVector& left,
                           common::sel_t leftPos,
                           const common::ValueVector& right,
                           common::sel_t rightPos, common::ValueVector& result,
                           common::sel_t resultPos) {
  FUNC::operation(left.getValue<LEFT_TYPE>(leftPos),
                  right.getValue<RIGHT_TYPE>(rightPos),
                  result.getValue<RESULT_TYPE>(resultPos));
}
...
template <typename LEFT_TYPE, typename RIGHT_TYPE, typename RESULT_TYPE,
          typename FUNC>
void BinaryHashFunctionExecutor::execute(
    const common::ValueVector& left, const common::SelectionVector& leftSelVec,
    const common::ValueVector& right,
  ...
  if (leftSelVec.getSelSize() != 1 && rightSelVec.getSelSize() != 1) {
    for (auto i = 0u; i < leftSelVec.getSelSize(); i++) {
      ...
      executeOnValue<LEFT_TYPE, RIGHT_TYPE, RESULT_TYPE, FUNC>(
          left, leftPos, right, rightPos, result, resultPos);
    }
...
static std::unique_ptr<ValueVector> computeDataVecHash(
    const ValueVector& operand) {
  auto hashVector =
      std::make_unique<ValueVector>(LogicalType::LIST(LogicalType::HASH()));
  ...
  // TODO(Ziyi): Allow selection size to be greater than default vector
  // capacity, so we don't have to chunk the selectionVector.
  ...
  while (numValuesComputed < numValuesInDataVec) {
    numValuesToComputeHash = std::min(DEFAULT_VECTOR_CAPACITY,
                                      numValuesInDataVec - numValuesComputed);
    ...
    VectorHashFunction::computeHash(
        *ListVector::getDataVector(&operand), selectionVector,
        *ListVector::getDataVector(hashVector.get()), selectionVector);
  }
  return hashVector;
}

static void finalizeDataVecHash(const ValueVector& operand,
                                const SelectionVector& operandSelVec,
                                ValueVector& result,
                                const SelectionVector& resultSelVec,
                                ValueVector& tmpHashVec) {
      ...
      for (auto j = 0u; j < entry.size; j++) {
        hashValue = combineHashScalar(hashValue,
                                      ListVector::getDataVector(&tmpHashVec)
                                          ->getValue<hash_t>(entry.offset + j));
      }
...
static void computeListVectorHash(const ValueVector& operand,
                                  const SelectionVector& operandSelectVec,
                                  ValueVector& result,
                                  const SelectionVector& resultSelectVec) {
  auto dataVecHash = computeDataVecHash(operand);
  finalizeDataVecHash(operand, operandSelectVec, result, resultSelectVec,
                      *dataVecHash);
}

static void computeStructVecHash(const ValueVector& operand,
                                 const SelectionVector& operandSelVec,
                                 ValueVector& result,
                                 const SelectionVector& resultSelVec) {
  ...
  case LogicalTypeID::STRUCT: {
    VectorHashFunction::computeHash(
        *StructVector::getFieldVector(&operand, 0 /* idx */), operandSelVec,
        result, resultSelVec);
    ...
    for (auto i = 1u; i < StructType::getNumFields(operand.dataType); i++) {
      ...
      VectorHashFunction::computeHash(*fieldVector, operandSelVec,
                                      *tmpHashVector, resultSelVec);
      VectorHashFunction::combineHash(*tmpHashVector, resultSelVec, result,
                                      resultSelVec, result, resultSelVec);
    }
  } break;
  default:
    KU_UNREACHABLE;
...
void VectorHashFunction::computeHash(const ValueVector& operand,
  ...
  TypeUtils::visit(
      ...
      [&]<HashableNonNestedTypes T>(T) {
        UnaryHashFunctionExecutor::execute<T, hash_t>(operand, operandSelectVec,
                                                      result, resultSelectVec);
      },
      [&](struct_entry_t) {
        computeStructVecHash(operand, operandSelectVec, result,
                             resultSelectVec);
      },
      [&](list_entry_t) {
        computeListVectorHash(operand, operandSelectVec, result,
                              resultSelectVec);
      },
      [&operand](auto) {
        // LCOV_EXCL_START
        throw RuntimeException("Cannot hash data type " +
                               operand.dataType.toString());
        // LCOV_EXCL_STOP
      });
}

void VectorHashFunction::combineHash(const ValueVector& left,
                                     const SelectionVector& leftSelVec,
                                     const ValueVector& right,
