...
static void executeNestedOperation(uint8_t& result, ValueVector* leftVector,
  ...
  case PhysicalTypeID::INT128: {
    OP::operation(leftVector->getValue<int128_t>(leftPos),
                  rightVector->getValue<int128_t>(rightPos), result,
                  nullptr /* left */, nullptr /* right */);
  } break;
  case PhysicalTypeID::DOUBLE: {
    OP::operation(leftVector->getValue<double>(leftPos),
                  rightVector->getValue<double>(rightPos), result,
                  nullptr /* left */, nullptr /* right */);
  } break;
  case PhysicalTypeID::FLOAT: {
    OP::operation(leftVector->getValue<float>(leftPos),
                  rightVector->getValue<float>(rightPos), result,
                  nullptr /* left */, nullptr /* right */);
  } break;
  case PhysicalTypeID::STRING: {
    OP::operation(leftVector->getValue<neug_string_t>(leftPos),
                  rightVector->getValue<neug_string_t>(rightPos), result,
                  nullptr /* left */, nullptr /* right */);
  } break;
  ...
  case PhysicalTypeID::INTERNAL_ID: {
    OP::operation(leftVector->getValue<internalID_t>(leftPos),
                  rightVector->getValue<internalID_t>(rightPos), result,
                  nullptr /* left */, nullptr /* right */);
  } break;
  case PhysicalTypeID::ARRAY:
  case PhysicalTypeID::LIST: {
    OP::operation(leftVector->getValue<list_entry_t>(leftPos),
                  rightVector->getValue<list_entry_t>(rightPos), result,
                  leftVector, rightVector);
  } break;
  case PhysicalTypeID::STRUCT: {
    OP::operation(leftVector->getValue<struct_entry_t>(leftPos),
                  rightVector->getValue<struct_entry_t>(rightPos), result,
                  leftVector, rightVector);
  } break;
  default: {
    NEUG_UNREACHABLE;
...
static void executeNestedEqual(uint8_t& result, ValueVector* leftVector,
                               ValueVector* rightVector, uint64_t leftPos,
                               uint64_t rightPos) {
  if (leftVector->isNull(leftPos) && rightVector->isNull(rightPos)) {
    ...
    executeNestedOperation<Equals>(result, leftVector, rightVector, leftPos,
                                   rightPos);
  }
}

template <>
void Equals::operation(const list_entry_t& left, const list_entry_t& right,
                       uint8_t& result, ValueVector* leftVector,
  ...
  for (auto i = 0u; i < left.size; i++) {
    ...
    executeNestedEqual(result, leftDataVector, rightDataVector, leftPos,
                       rightPos);
    if (!result) {
      return;
    }
...
template <>
void Equals::operation(const struct_entry_t& left, const struct_entry_t& right,
                       uint8_t& result, ValueVector* leftVector,
                       ValueVector* rightVector) {
  if (leftVector->dataType != rightVector->dataType) {
    result = false;
...
static void executeNestedGreaterThan(uint8_t& isGreaterThan, uint8_t& isEqual,
                                     ValueVector* leftDataVector,
                                     ValueVector* rightDataVector,
  ...
  if (isLeftNull || isRightNull) {
    ...
    executeNestedOperation<GreaterThan>(isGreaterThan, leftDataVector,
                                        rightDataVector, leftPos, rightPos);
    if (!isGreaterThan) {
      executeNestedOperation<Equals>(isEqual, leftDataVector, rightDataVector,
                                     leftPos, rightPos);
    } else {
      isEqual = false;
    }
...
template <>
void GreaterThan::operation(const list_entry_t& left, const list_entry_t& right,
                            uint8_t& result, ValueVector* leftVector,
                            ValueVector* rightVector) {
  NEUG_ASSERT(leftVector->dataType == rightVector->dataType);
  auto leftDataVector = ListVector::getDataVector(leftVector);
  auto rightDataVector = ListVector::getDataVector(rightVector);
  ...
  for (auto i = 0u; i < commonLength; i++) {
    auto leftPos = left.offset + i;
    auto rightPos = right.offset + i;
    executeNestedGreaterThan(result, isEqual, leftDataVector, rightDataVector,
                             leftPos, rightPos);
...
template <>
void GreaterThan::operation(const struct_entry_t& left,
                            const struct_entry_t& right, uint8_t& result,
  ...
  for (auto i = 0u; i < leftFields.size(); i++) {
    auto leftField = leftFields[i].get();
    auto rightField = rightFields[i].get();
    executeNestedGreaterThan(result, isEqual, leftField, rightField, left.pos,
                             right.pos);
    if (result || (!result && !isEqual)) {
      return;
    }
