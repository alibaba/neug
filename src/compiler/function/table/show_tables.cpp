...
struct TableInfo {
  ...
  TableInfo(std::string name, table_id_t id, std::string type,
            std::string databaseName, std::string comment)
      : name{std::move(name)},
        id{id},
        type{std::move(type)},
...
struct ShowTablesBindData final : TableFuncBindData {
  ...
  ShowTablesBindData(std::vector<TableInfo> tables,
                     binder::expression_vector columns, row_idx_t numRows)
...
static offset_t internalTableFunc(const TableFuncMorsel& morsel,
                                  ...
                                  DataChunk& output) {
  const auto tables =
      input.bindData->constPtrCast<ShowTablesBindData>()->tables;
...
static std::unique_ptr<TableFuncBindData> bindFunc(
  ...
  if (!context->hasDefaultDatabase()) {
    ...
    for (auto& entry : catalog->getTableEntries(
             transaction, context->useInternalCatalogEntry())) {
      if (entry->getType() == CatalogEntryType::REL_TABLE_ENTRY &&
          entry->constCast<RelTableCatalogEntry>().hasParentRelGroup(
              catalog, transaction)) {
        continue;
      }
    ...
    for (auto& entry : catalog->getRelGroupEntries(transaction)) {
      tableInfos.emplace_back(entry->getName(), entry->getOID(), "REL",
                              LOCAL_DB_NAME, entry->getComment());
    }
  }
  ...
  return std::make_unique<ShowTablesBindData>(
      std::move(tableInfos), std::move(columns), tableInfos.size());
}
