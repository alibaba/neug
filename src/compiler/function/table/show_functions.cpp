...
struct ShowFunctionsBindData final : TableFuncBindData {
  ...
  ShowFunctionsBindData(std::vector<FunctionInfo> sequences,
...
static offset_t internalTableFunc(const TableFuncMorsel& morsel,
                                  ...
                                  DataChunk& output) {
  auto sequences =
      input.bindData->constPtrCast<ShowFunctionsBindData>()->sequences;
...
static std::unique_ptr<TableFuncBindData> bindFunc(
  ...
  for (const auto& entry :
       context->getCatalog()->getFunctionEntries(context->getTransaction())) {
  ...
  return std::make_unique<ShowFunctionsBindData>(std::move(FunctionInfos),
                                                 columns, FunctionInfos.size());
}
