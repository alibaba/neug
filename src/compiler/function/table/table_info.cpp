...
#include "src/include/main/database_manager.h"
...
struct ExtraNodePropertyInfo : ExtraPropertyInfo {
  ...
  explicit ExtraNodePropertyInfo(bool isPrimaryKey)
...
struct TableInfoBindData final : TableFuncBindData {
  ...
  TableInfoBindData(CatalogEntryType type, std::vector<PropertyInfo> infos,
      ...
      : TableFuncBindData{std::move(columns), infos.size()},
        type{type},
        infos{std::move(infos)} {}

  std::unique_ptr<TableFuncBindData> copy() const override {
    return std::make_unique<TableInfoBindData>(type, copyVector(infos),
                                               columns);
  }
};
...
static std::vector<PropertyInfo> getForeignPropertyInfos(
...
static std::vector<PropertyInfo> getNodePropertyInfos(
  ...
  for (auto& def : entry->getProperties()) {
    auto info = getInfo(def);
    info.propertyID = entry->getPropertyID(def.getName());
    info.extraInfo = std::make_unique<ExtraNodePropertyInfo>(primaryKeyName ==
                                                             def.getName());
    infos.push_back(std::move(info));
  }
  return infos;
...
static std::vector<PropertyInfo> getRelPropertyInfos(
    RelTableCatalogEntry* entry) {
  std::vector<PropertyInfo> infos;
  for (auto& def : entry->getProperties()) {
...
static std::unique_ptr<TableFuncBindData> bindFunc(
  ...
  columnTypes.push_back(LogicalType::STRING());
  auto name =
      common::StringUtils::split(input->getLiteralVal<std::string>(0), ".");
