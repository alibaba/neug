...
struct SequenceInfo {
  ...
  SequenceInfo(std::string name, std::string databaseName, int64_t startValue,
...
static offset_t internalTableFunc(const TableFuncMorsel& morsel,
  ...
  const auto sequences =
      input.bindData->constPtrCast<ShowSequencesBindData>()->sequences;
...
static std::unique_ptr<TableFuncBindData> bindFunc(
  ...
  for (const auto& entry :
       context->getCatalog()->getSequenceEntries(context->getTransaction())) {
    const auto sequenceData = entry->getSequenceData();
    auto sequenceInfo = SequenceInfo{
        entry->getName(),       LOCAL_DB_NAME,         sequenceData.startValue,
  ...
  //     for (auto& entry :
  //     attachedDatabase->getCatalog()->getSequenceEntries(context->getTx()))
  //     {
  //         auto sequenceData = entry->getSequenceData();
  //         auto sequenceInfo =
  //             SequenceInfo{entry->getName(), stringFormat("{}({})",
  //             databaseName, databaseType),
  //                 sequenceData.startValue, sequenceData.increment,
  ...
  return std::make_unique<ShowSequencesBindData>(std::move(sequenceInfos),
                                                 columns, sequenceInfos.size());
}
