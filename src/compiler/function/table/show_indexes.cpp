...
struct IndexInfo {
  ...
  IndexInfo(std::string tableName, std::string indexName, std::string indexType,
            std::vector<std::string> properties, bool dependencyLoaded,
            std::string indexDefinition)
      : tableName{std::move(tableName)},
        ...
        indexDefinition{std::move(indexDefinition)} {}
};

struct ShowIndexesBindData final : TableFuncBindData {
  std::vector<IndexInfo> indexesInfo;

  ShowIndexesBindData(std::vector<IndexInfo> indexesInfo,
...
static offset_t internalTableFunc(const TableFuncMorsel& morsel,
                                  ...
                                  DataChunk& output) {
  auto& indexesInfo =
      input.bindData->constPtrCast<ShowIndexesBindData>()->indexesInfo;
  ...
  for (auto i = 0u; i < numTuplesToOutput; i++) {
    ...
    auto listEntry =
        ListVector::addList(&propertyVector, indexInfo.properties.size());
...
static std::unique_ptr<TableFuncBindData> bindFunc(
  ...
  for (auto indexEntry : indexEntries) {
    auto tableEntry = catalog->getTableCatalogEntry(context->getTransaction(),
    ...
    indexesInfo.emplace_back(std::move(tableName), std::move(indexName),
                             std::move(indexType), std::move(propertyNames),
                             dependencyLoaded, std::move(indexDefinition));
  }
