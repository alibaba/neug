...
struct ListSlice {
  // Note: this function takes in a 1-based begin/end index (The index of the
  // first value in the listEntry is 1).
  static void operation(common::list_entry_t& listEntry, int64_t& begin,
                        int64_t& end, common::list_entry_t& result,
  ...
  static void operation(common::ku_string_t& str, int64_t& begin, int64_t& end,
    ...
    SubStr::operation(str, startIdx, endIdx - startIdx + 1, result,
                      resultValueVector);
  }
};
...
static std::unique_ptr<FunctionBindData> bindFunc(
  ...
  return std::make_unique<FunctionBindData>(
      std::move(paramTypes), input.arguments[0]->getDataType().copy());
}
...
function_set ListSliceFunction::getFunctionSet() {
  ...
  func = std::make_unique<ScalarFunction>(
      ...
      ScalarFunction::TernaryExecListStructFunction<
          list_entry_t, int64_t, int64_t, list_entry_t, ListSlice>);
