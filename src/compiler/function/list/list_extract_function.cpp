...
static int checkAndGetIndex(const columnar::Value& value) {
...
static columnar::Value execFunc(const std::vector<columnar::Value>& args) {
  if (args.size() != 2) {
    THROW_RUNTIME_ERROR(
  ...
  case neug::DataTypeId::kStruct:
    return columnar::StructValue::GetChildren(arg0).at(index);
  case neug::DataTypeId::kList:
    return columnar::ListValue::GetChildren(arg0).at(index);
  default:
    THROW_RUNTIME_ERROR(
