#include "src/include/function/gds/gds_frontier.h"

#include "src/include/function/gds/gds_utils.h"
...
void SparseFrontierReference::addNodes(const std::vector<nodeID_t>& nodeIDs,
                                       iteration_t iter) {
  KU_ASSERT(curData);
  for (auto nodeID : nodeIDs) {
    addNode(nodeID.offset, iter);
...
class DenseFrontierInitVertexCompute : public VertexCompute {
 public:
  ...
  void vertexCompute(offset_t startOffset, offset_t endOffset,
                     table_id_t) override {
...
void DenseFrontier::init(ExecutionContext* context, Graph* graph,
                         iteration_t val) {}

void DenseFrontier::resetValue(ExecutionContext* context, Graph* graph,
                               iteration_t val) {
...
void DenseFrontier::addNodes(const std::vector<nodeID_t>& nodeIDs,
                             iteration_t iter) {
  KU_ASSERT(curData);
  for (auto nodeID : nodeIDs) {
    curData[nodeID.offset].store(iter, std::memory_order_relaxed);
...
std::unique_ptr<DenseFrontier> DenseFrontier::getUninitializedFrontier(
    ExecutionContext* context, Graph* graph) {
  return nullptr;
}

std::unique_ptr<DenseFrontier> DenseFrontier::getUnvisitedFrontier(
    ExecutionContext* context, Graph* graph) {
  return nullptr;
}

std::unique_ptr<DenseFrontier> DenseFrontier::getVisitedFrontier(
    ExecutionContext* context, Graph* graph) {
  return nullptr;
}

std::unique_ptr<DenseFrontier> DenseFrontier::getVisitedFrontier(
    ExecutionContext* context, Graph* graph, NodeOffsetMaskMap* maskMap) {
  return nullptr;
}
...
void FrontierPair::addNodesToNextFrontier(
...
SPFrontierPair::SPFrontierPair(std::unique_ptr<DenseFrontier> denseFrontier)
  ...
  curDenseFrontier =
      std::make_unique<DenseFrontierReference>(*this->denseFrontier);
  nextDenseFrontier =
      std::make_unique<DenseFrontierReference>(*this->denseFrontier);
  sparseFrontier =
      std::make_unique<SparseFrontier>(this->denseFrontier->nodeMaxOffsetMap);
  curSparseFrontier =
      std::make_unique<SparseFrontierReference>(*this->sparseFrontier);
  nextSparseFrontier =
      std::make_unique<SparseFrontierReference>(*this->sparseFrontier);
...
void SPFrontierPair::switchToDense(ExecutionContext* context,
                                   graph::Graph* graph) {
  KU_ASSERT(state == GDSDensityState::SPARSE);
...
DenseSparseDynamicFrontierPair::DenseSparseDynamicFrontierPair(
  ...
  curSparseFrontier = std::make_unique<SparseFrontier>(
      this->curDenseFrontier->nodeMaxOffsetMap);
  nextSparseFrontier = std::make_unique<SparseFrontier>(
      this->nextDenseFrontier->nodeMaxOffsetMap);
  currentFrontier = curSparseFrontier.get();
  nextFrontier = nextSparseFrontier.get();
}
...
std::unordered_set<offset_t>
DenseSparseDynamicFrontierPair::getActiveNodesOnCurrentFrontier() {
  KU_ASSERT(state == GDSDensityState::SPARSE);
  std::unordered_set<offset_t> result;
...
void DenseSparseDynamicFrontierPair::switchToDense(ExecutionContext* context,
                                                   Graph* graph) {
  KU_ASSERT(state == GDSDensityState::SPARSE);
  state = GDSDensityState::DENSE;
...
DenseFrontierPair::DenseFrontierPair(
    std::unique_ptr<DenseFrontier> curDenseFrontier,
    std::unique_ptr<DenseFrontier> nextDenseFrontier)
    : curDenseFrontier{std::move(curDenseFrontier)},
      nextDenseFrontier{std::move(nextDenseFrontier)} {
...
void DenseFrontierPair::resetValue(ExecutionContext* context, Graph* graph,
                                   iteration_t val) {
