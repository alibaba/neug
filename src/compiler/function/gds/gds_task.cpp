...
#include "src/include/function/gds/frontier_morsel.h"
...
void FrontierTask::run() {
  ...
  auto scanState = graph->prepareRelScan(info.relEntry, info.nbrEntry,
                                         info.propertiesToScan);
  auto ec = info.edgeCompute.copy();
  switch (info.direction) {
  case ExtendDirection::FWD: {
    while (sharedState->morselDispatcher.getNextRangeMorsel(morsel)) {
      for (auto offset = morsel.getBeginOffset();
           offset < morsel.getEndOffset(); ++offset) {
...
void FrontierTask::runSparse() {
  ...
  case ExtendDirection::FWD: {
    for (const auto offset :
         sharedState->frontierPair.getActiveNodesOnCurrentFrontier()) {
...
void VertexComputeTask::run() {
  ...
  if (info.hasPropertiesToScan()) {
    auto scanState =
        graph->prepareVertexScan(info.tableEntry, info.propertiesToScan);
    while (sharedState->morselDispatcher.getNextRangeMorsel(morsel)) {
      for (auto chunk : graph->scanVertices(
               morsel.getBeginOffset(), morsel.getEndOffset(), *scanState)) {
