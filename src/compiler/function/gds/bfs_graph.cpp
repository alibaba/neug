#include "src/include/function/gds/bfs_graph.h"
...
class BFSGraphInitVertexCompute : public VertexCompute {
 public:
  explicit BFSGraphInitVertexCompute(DenseBFSGraph& bfsGraph)
      : bfsGraph{bfsGraph} {}
...
static ParentList* reserveParent(nodeID_t boundNodeID, relID_t edgeID,
...
void DenseBFSGraph::addParent(uint16_t iter, nodeID_t boundNodeID,
                              relID_t edgeID, nodeID_t nbrNodeID, bool fwdEdge,
...
void DenseBFSGraph::addSingleParent(uint16_t iter, nodeID_t boundNodeID,
                                    relID_t edgeID, nodeID_t nbrNodeID,
                                    bool fwdEdge,
                                    ObjectBlock<ParentList>* block) {
  auto parent = reserveParent(boundNodeID, edgeID, fwdEdge, block);
  parent->setIter(iter);
...
static double getCost(ParentList* parentList) {
  return parentList == nullptr ? std::numeric_limits<double>::max()
                               : parentList->getCost();
}

bool DenseBFSGraph::tryAddParentWithWeight(nodeID_t boundNodeID, relID_t edgeID,
                                           nodeID_t nbrNodeID, bool fwdEdge,
                                           double weight,
                                           ObjectBlock<ParentList>* block) {
...
bool DenseBFSGraph::tryAddSingleParentWithWeight(
    nodeID_t boundNodeID, relID_t edgeID, nodeID_t nbrNodeID, bool fwdEdge,
...
ParentList* DenseBFSGraph::getParentListHead(nodeID_t nodeID) {
  return denseObjects.getData(nodeID.tableID)[nodeID.offset].load(
...
void SparseBFSGraph::addParent(uint16_t iter, nodeID_t boundNodeID,
                               relID_t edgeID, nodeID_t nbrNodeID, bool fwdEdge,
                               ObjectBlock<ParentList>* block) {
  auto parent = reserveParent(boundNodeID, edgeID, fwdEdge, block);
  parent->setIter(iter);
...
void SparseBFSGraph::addSingleParent(uint16_t iter, nodeID_t boundNodeID,
                                     relID_t edgeID, nodeID_t nbrNodeID,
                                     bool fwdEdge,
                                     ObjectBlock<ParentList>* block) {
  if (curData->contains(nbrNodeID.offset)) {
...
bool SparseBFSGraph::tryAddParentWithWeight(nodeID_t boundNodeID,
                                            relID_t edgeID, nodeID_t nbrNodeID,
                                            bool fwdEdge, double weight,
                                            ObjectBlock<ParentList>* block) {
...
bool SparseBFSGraph::tryAddSingleParentWithWeight(
    nodeID_t boundNodeID, relID_t edgeID, nodeID_t nbrNodeID, bool fwdEdge,
    double weight, ObjectBlock<ParentList>* block) {
