...
#include "src/include/common/task_system/task_scheduler.h"
#include "src/include/function/gds/gds_task.h"
...
static void scheduleFrontierTask(ExecutionContext* context,
  ...
  auto task = getFrontierTask(clientContext, fromEntry, toEntry, relEntry,
                              graph, extendDirection, computeState,
  ...
  // GDSUtils::runFrontiersUntilConvergence is called from a GDSCall operator,
  // which is already executed by a worker thread Tm of the task scheduler. So
  // this function is executed by Tm. Because this function will monitor the
  // task and wait for it to complete, running GDS algorithms effectively
  // "loses" Tm. This can even lead to the query processor to halt, e.g., if
  // there is a single worker thread in the system, and more generally decrease
  // the number of worker threads by 1. Therefore, we instruct
  // scheduleTaskAndWaitOrError to start a new thread by passing true as the
  // last argument.
  clientContext->getTaskScheduler()->scheduleTaskAndWaitOrError(
      task, context, true /* launchNewWorkerThread */);
}
...
static void runVertexComputeInternal(const TableCatalogEntry* currentEntry,
  ...
  auto maxOffset = graph->getMaxOffset(context->clientContext->getTransaction(),
                                       currentEntry->getTableID());
  auto sharedState = task->getSharedState();
  sharedState->morselDispatcher.init(maxOffset);
  context->clientContext->getTaskScheduler()->scheduleTaskAndWaitOrError(
      task, context, true /* launchNewWorkerThread */);
}
