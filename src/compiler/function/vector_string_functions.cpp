...
columnar::Value UpperFunction::Exec(const std::vector<columnar::Value>& args) {
  if (args.size() != 1) {
  ...
  std::string str(columnar::StringValue::Get(val));
  std::transform(str.begin(), str.end(), str.begin(), ::toupper);
  return columnar::Value::STRING(str);
}
...
columnar::Value LowerFunction::Exec(const std::vector<columnar::Value>& args) {
  if (args.size() != 1) {
...
columnar::Value ReverseFunction::Exec(
    const std::vector<columnar::Value>& args) {
  if (args.size() != 1) {
