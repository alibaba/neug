...
static std::shared_ptr<LogicalOperator> appendAccumulate(
  ...
  auto accumulate = std::make_shared<LogicalAccumulate>(
      AccumulateType::REGULAR, expression_vector{}, nullptr /* mark */,
      std::move(child));
...
static std::shared_ptr<LogicalSemiMasker> appendSemiMasker(
    SemiMaskKeyType keyType, SemiMaskTargetType targetType,
    std::shared_ptr<Expression> key, std::vector<LogicalOperator*> candidates,
    std::shared_ptr<LogicalOperator> child) {
  auto tableIDs = getTableIDs(candidates[0], targetType);
  auto semiMasker = std::make_shared<LogicalSemiMasker>(keyType, targetType,
                                                        key, tableIDs, child);
...
static bool subPlanContainsFilter(LogicalOperator* root) {
  ...
  if (!filterCollector.hasOperators() &&
      !indexScanNodeCollector.hasOperators()) {
    return false;
  }
  return true;
}
...
static bool isProbeSideQualified(LogicalOperator* probeRoot) {
  if (probeRoot->getOperatorType() == LogicalOperatorType::ACCUMULATE) {
    // No Acc hash join if probe side has already been accumulated. This can be
    // solved.
    return false;
  }
...
// Find all ScanNodeIDs under root which scans parameter nodeID. Note that there
// might be multiple ScanNodeIDs matches because both node and rel table scans
// will trigger scanNodeIDs.
static std::vector<LogicalOperator*> getScanNodeCandidates(
...
static std::vector<LogicalOperator*> getRecursiveExtendInputNodeCandidates(
  ...
  for (auto& op : collector.getOperators()) {
    ...
    if (nodeID ==
        *bindData.nodeInput->constCast<NodeExpression>().getInternalID()) {
      result.push_back(op);
    }
  }
  return result;
...
static std::vector<LogicalOperator*> getRecursiveExtendOutputNodeCandidates(
  ...
  for (auto& op : collector.getOperators()) {
    ...
    auto& bindData = recursiveExtend.getBindData();
    if (nodeID ==
        *bindData.nodeOutput->constCast<NodeExpression>().getInternalID()) {
      result.push_back(op);
    }
  }
  return result;
...
static std::shared_ptr<LogicalOperator> tryApplySemiMask(
    std::shared_ptr<Expression> nodeID,
    std::shared_ptr<LogicalOperator> fromRoot, LogicalOperator* toRoot) {
  // TODO(Xiyang): Check if a semi mask can/need to be applied to ScanNodeTable,
  // RecursiveJoin & GDS at the same time
  ...
  auto recursiveExtendNodeCandidates =
      getRecursiveExtendOutputNodeCandidates(*nodeID, toRoot);
  ...
  if (!scanNodeCandidates.empty()) {
    return appendSemiMasker(SemiMaskKeyType::NODE,
                            SemiMaskTargetType::SCAN_NODE, std::move(nodeID),
                            scanNodeCandidates, std::move(fromRoot));
  }
  return nullptr;
}
...
static bool isBuildSideQualified(LogicalOperator* buildRoot) {
  ...
  // TODO(Xiyang): this may not be the best solution. Most of the time we will
  // pass a semi mask to GDS (recursive join) operator and make it generate
  // small result. Though there are also exceptions. In such case we will pay a
  // bit overhead.
  auto op = buildRoot;
...
void HashJoinSIPOptimizer::visitHashJoin(LogicalOperator* op) {
  ...
  if (hashJoin.getSIPInfo().position ==
...
void HashJoinSIPOptimizer::visitIntersect(LogicalOperator* op) {
  ...
  for (auto& nodeID : intersect.getKeyNodeIDs()) {
    ...
    if (!ops.empty()) {
      probeRoot =
          appendSemiMasker(SemiMaskKeyType::NODE, SemiMaskTargetType::SCAN_NODE,
...
void HashJoinSIPOptimizer::visitPathPropertyProbe(LogicalOperator* op) {
  ...
  auto semiMasker = appendSemiMasker(
      ...
      opsToApplySemiMask, pathPropertyProbe.getChild(0));
  auto srcNodeID = recursiveRel->getSrcNode()->getInternalID();
  auto dstNodeID = recursiveRel->getDstNode()->getInternalID();
  semiMasker->setExtraKeyInfo(
      std::make_unique<ExtraNodeIDListKeyInfo>(srcNodeID, dstNodeID));
