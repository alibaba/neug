...
// TODO(Xiyang): we should probably remove the projection between ORDER BY and
// MULTIPLICITY REDUCER We search for pattern ORDER BY -> PROJECTION ->
// MULTIPLICITY REDUCER -> LIMIT ORDER BY -> MULTIPLICITY REDUCER -> LIMIT and
// rewrite as TOP_K
std::shared_ptr<LogicalOperator> TopKOptimizer::visitLimitReplace(
  ...
  KU_ASSERT(multiplicityReducer->getOperatorType() ==
            LogicalOperatorType::MULTIPLICITY_REDUCER);

  auto projectionOrOrderBy = multiplicityReducer->getChild(0);
  std::shared_ptr<LogicalOrderBy> orderBy;
  if (projectionOrOrderBy->getOperatorType() ==
      LogicalOperatorType::PROJECTION) {
    if (projectionOrOrderBy->getChild(0)->getOperatorType() !=
        LogicalOperatorType::ORDER_BY) {
      return op;
    }
    orderBy = std::static_pointer_cast<LogicalOrderBy>(
        projectionOrOrderBy->getChild(0));
  } else if (projectionOrOrderBy->getOperatorType() ==
             LogicalOperatorType::ORDER_BY) {
