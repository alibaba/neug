#include "src/include/optimizer/factorization_rewriter.h"
...
#include "src/include/planner/operator/logical_flatten.h"
...
#include "src/include/planner/operator/logical_union.h"
#include "src/include/planner/operator/logical_unwind.h"
...
#include "src/include/planner/operator/persistent/logical_merge.h"
...
void FactorizationRewriter::visitHashJoin(planner::LogicalOperator* op) {
  ...
  hashJoin.setChild(
      0, appendFlattens(hashJoin.getChild(0), groupsPosToFlattenOnProbeSide));
  auto groupsPosToFlattenOnBuildSide =
      hashJoin.getGroupsPosToFlattenOnBuildSide();
  hashJoin.setChild(
      1, appendFlattens(hashJoin.getChild(1), groupsPosToFlattenOnBuildSide));
}
...
void FactorizationRewriter::visitIntersect(planner::LogicalOperator* op) {
  ...
  intersect.setChild(
      0, appendFlattens(intersect.getChild(0), groupsPosToFlattenOnProbeSide));
  for (auto i = 0u; i < intersect.getNumBuilds(); ++i) {
    ...
    intersect.setChild(childIdx, appendFlattens(intersect.getChild(childIdx),
                                                groupPosToFlatten));
  }
}
...
void FactorizationRewriter::visitProjection(planner::LogicalOperator* op) {
  ...
  if (hasRandomFunction) {
    ...
    auto groupsPosToFlatten = FlattenAll::getGroupsPosToFlatten(
        groupsPos, *op->getChild(0)->getSchema());
    projection.setChild(
        0, appendFlattens(projection.getChild(0), groupsPosToFlatten));
  } else {
...
void FactorizationRewriter::visitAccumulate(planner::LogicalOperator* op) {
  ...
  accumulate.setChild(
      0, appendFlattens(accumulate.getChild(0), groupsPosToFlatten));
}
...
void FactorizationRewriter::visitAggregate(planner::LogicalOperator* op) {
  ...
  aggregate.setChild(0,
                     appendFlattens(aggregate.getChild(0), groupsPosToFlatten));
}
...
void FactorizationRewriter::visitDistinct(planner::LogicalOperator* op) {
  ...
  distinct.setChild(0,
                    appendFlattens(distinct.getChild(0), groupsPosToFlatten));
}
...
FactorizationRewriter::appendFlattenIfNecessary(
    std::shared_ptr<planner::LogicalOperator> op,
