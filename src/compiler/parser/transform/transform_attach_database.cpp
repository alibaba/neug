...
std::unique_ptr<Statement> Transformer::transformAttachDatabase(
  ...
  auto dbAlias =
      ctx.oC_SchemaName() ? transformSchemaName(*ctx.oC_SchemaName()) : "";
  auto dbType = transformSymbolicName(*ctx.oC_SymbolicName());
  auto attachOption =
      ctx.kU_Options() ? transformOptions(*ctx.kU_Options()) : options_t{};
  AttachInfo attachInfo{std::move(dbPath), std::move(dbAlias),
                        std::move(dbType), std::move(attachOption)};
