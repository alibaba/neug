#include "src/include/binder/query/query_graph.h"
...
std::size_t SubqueryGraphHasher::operator()(const SubqueryGraph& key) const {
  if (0 == key.queryRelsSelector.count()) {
    return std::hash<std::bitset<MAX_NUM_QUERY_VARIABLES>>{}(
        key.queryNodesSelector);
  }
  return std::hash<std::bitset<MAX_NUM_QUERY_VARIABLES>>{}(
      key.queryRelsSelector);
}

bool SubqueryGraph::containAllVariables(
...
std::unordered_set<uint32_t> SubqueryGraph::getRelNbrPositions() const {
  std::unordered_set<uint32_t> result;
  for (auto relPos = 0u; relPos < queryGraph.getNumQueryRels(); ++relPos) {
    if (queryRelsSelector[relPos]) {  // rel already in subgraph, cannot be rel
                                      // neighbour
      continue;
    }
    auto rel = queryGraph.getQueryRel(relPos);
...
std::vector<uint32_t> SubqueryGraph::getConnectedNodePos(
    const SubqueryGraph& nbr) const {
...
SubqueryGraph::getNodePositionsIgnoringNodeSelector() const {
  std::unordered_set<uint32_t> result;
...
subquery_graph_set_t SubqueryGraph::getNextNbrSubgraphs(
...
std::vector<std::shared_ptr<NodeOrRelExpression>> QueryGraph::getAllPatterns()
    const {
...
void QueryGraph::addQueryNode(std::shared_ptr<NodeExpression> queryNode) {
  // Note that a node may be added multiple times. We should only keep one of
  // it. E.g. MATCH (a:person)-[:knows]->(b:person), (a)-[:knows]->(c:person) a
  // will be added twice during binding
...
bool QueryGraph::canProjectExpression(
...
void QueryGraphCollection::finalize() {
  ...
  while (true) {
    ...
    if (queryGraphs.size() == prevNumGraphs ||
        baseGraphIdx == queryGraphs.size()) {
      return;
    }
  }
}

std::vector<QueryGraph> QueryGraphCollection::mergeGraphs(
    common::idx_t baseGraphIdx) {
  ...
  while (true) {
    ...
    if (graphToMergeIdx ==
        common::INVALID_IDX) {  // No graph can be merged. Terminate.
      break;
    }
    // Perform merge
...
bool QueryGraphCollection::contains(const std::string& name) const {
  for (auto& queryGraph : queryGraphs) {
    if (queryGraph.containsQueryNode(name) ||
        queryGraph.containsQueryRel(name)) {
      return true;
    }
  }
  return false;
...
QueryGraphCollection::getQueryNodes() const {
...
std::vector<std::shared_ptr<RelExpression>> QueryGraphCollection::getQueryRels()
    const {
