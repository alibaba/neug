...
void validateColumnNamesAreUnique(const std::vector<std::string>& columnNames) {
  ...
  for (auto& name : columnNames) {
    if (existColumnNames.contains(name)) {
      THROW_BINDER_EXCEPTION("Multiple result column with the same name " +
...
std::vector<std::string> getColumnNames(
    const expression_vector& exprs, const std::vector<std::string>& aliases) {
...
BoundWithClause Binder::bindWithClause(const WithClause& withClause) {
  ...
  for (auto& alias : aliases) {
    if (alias.empty()) {
      THROW_BINDER_EXCEPTION("Expression in WITH must be aliased (use AS).");
    }
  }
  ...
  if (withClause.hasWhereExpression()) {
    boundWithClause.setWhereExpression(
...
BoundReturnClause Binder::bindReturnClause(const ReturnClause& returnClause) {
  ...
  NEUG_ASSERT(columnNames.size() == projectionExprs.size());
  for (auto i = 0u; i < columnNames.size(); ++i) {
    statementResult.addColumn(columnNames[i], projectionExprs[i]);
  }
  return BoundReturnClause(std::move(boundProjectionBody),
...
std::pair<expression_vector, std::vector<std::string>>
Binder::bindProjectionList(const ProjectionBody& projectionBody) {
  ...
  for (auto& parsedExpr : projectionBody.getProjectionExpressions()) {
    if (parsedExpr->getExpressionType() == ExpressionType::STAR) {
      ...
      if (propExpr.isStar()) {
        ...
        for (auto& expr :
             expressionBinder.bindPropertyStarExpression(*parsedExpr)) {
          projectionExprs.push_back(expr);
          aliases.push_back("");
        }
      } else {
      ...
      auto expr = expressionBinder.bindExpression(*parsedExpr);
      projectionExprs.push_back(expr);
      aliases.push_back(parsedExpr->hasAlias() ? parsedExpr->getAlias()
                                               : expr->getAlias());
    }
  }
 ...
 protected:
  ...
  void visitChildren(const Expression& expr) override {
    ...
    case ExpressionType::AGGREGATE_FUNCTION: {
      // We do not traverse the child or aggregate because nested agg is
      // validated recursively e.g.  WITH SUM(1) AS x WITH SUM(x) AS y RETURN
      // SUM(y) when validating SUM(y) we only need to check y and not x.
    } break;
    default: {
...
static void validateNestedAggregate(const Expression& expr,
                                    const BinderScope& scope) {
  NEUG_ASSERT(expr.expressionType == ExpressionType::AGGREGATE_FUNCTION);
...
BoundProjectionBody Binder::bindProjectionBody(
  ...
  NEUG_ASSERT(projectionExprs.size() == aliases.size());
  for (auto i = 0u; i < projectionExprs.size(); ++i) {
  ...
  if (!aggregateExprs.empty()) {
    ...
    if (!groupByExprs.empty()) {
      // TODO(Xiyang): we can remove augment group by. But make sure we test
  ...
  if (projectionBody.hasOrderByExpressions()) {
    // Cypher rule of ORDER BY expression scope: if projection contains
    // aggregation, only expressions in projection are available. Otherwise,
    // expressions before projection are also available
    expression_vector orderByExprs;
    if (boundProjectionBody.hasAggregateExpressions() ||
        boundProjectionBody.isDistinct()) {
      scope.clear();
      NEUG_ASSERT(projectionBody.getProjectionExpressions().size() ==
                  projectionExprs.size());
      std::vector<std::string> tmpAliases;
      for (auto& expr : projectionBody.getProjectionExpressions()) {
        tmpAliases.push_back(expr->hasAlias() ? expr->getAlias()
                                              : expr->toString());
      }
