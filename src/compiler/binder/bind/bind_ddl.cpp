...
#include "src/include/catalog/catalog_entry/index_catalog_entry.h"
...
#include "src/include/catalog/catalog_entry/sequence_catalog_entry.h"
#include "src/include/common/enums/extend_direction_util.h"
...
#include "src/include/parser/ddl/alter.h"
#include "src/include/parser/ddl/create_sequence.h"
#include "src/include/parser/ddl/create_table.h"
#include "src/include/parser/ddl/create_table_info.h"
#include "src/include/parser/ddl/create_type.h"
#include "src/include/parser/ddl/drop.h"
...
static void validatePropertyName(
  ...
  for (auto& definition : definitions) {
    ...
    if (Binder::reservedInColumnName(definition.getName())) {
      throw BinderException(stringFormat("{} is a reserved property name.",
                                         definition.getName()));
    }
...
void Binder::validateColumnExistence(const TableCatalogEntry* entry,
  ...
  if (!entry->containsProperty(columnName)) {
    throw BinderException{stringFormat("Column {} does not exist in table {}.",
...
std::unique_ptr<BoundStatement> Binder::bindRenameTable(
  ...
  auto extraInfo =
      ku_dynamic_cast<ExtraRenameTableInfo*>(info->extraInfo.get());
