...
static bool isAutoGeneratedColumnName(const std::string& name) {
...
static bool isEndpointAlias(const std::string& expected,
                            const std::string& actual) {
  static const case_insensitve_set_t srcAliases = {"from", "src", "source"};
  static const case_insensitve_set_t dstAliases = {"to", "dst", "destination"};
  if (srcAliases.contains(expected) && srcAliases.contains(actual)) return true;
  if (dstAliases.contains(expected) && dstAliases.contains(actual)) return true;
  return false;
}

static void validateColumnNames(
    const std::vector<std::string>& expectedNames,
    const std::vector<std::string>& sniffedNames) {
  if (!sniffedNames.empty() && isAutoGeneratedColumnName(sniffedNames[0])) {
    return;
  }
  CaseInsensitiveStringEquality caseInsensitiveEqual;
  for (size_t i = 0; i < expectedNames.size(); ++i) {
    if (caseInsensitiveEqual(expectedNames[i], sniffedNames[i])) continue;
    if (isEndpointAlias(expectedNames[i], sniffedNames[i])) continue;
    THROW_SCHEMA_MISMATCH(stringFormat(
        "Column name mismatch at position {}: expected '{}' but got '{}'.", i,
        expectedNames[i], sniffedNames[i]));
  }
}
...
std::unique_ptr<BoundBaseScanSource> Binder::bindFileScanSource(
  ...
  if (columnNames.empty()) {
    ...
    validateColumnNames(columnNames, sniffSchema->columnNames);
    expectedColumnNames = sniffSchema->columnNames;
