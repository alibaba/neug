...
#include "src/include/binder/query/updating_clause/bound_delete_clause.h"
#include "src/include/binder/query/updating_clause/bound_insert_clause.h"
#include "src/include/binder/query/updating_clause/bound_merge_clause.h"
...
#include "src/include/parser/query/updating_clause/delete_clause.h"
#include "src/include/parser/query/updating_clause/insert_clause.h"
#include "src/include/parser/query/updating_clause/merge_clause.h"
#include "src/include/parser/query/updating_clause/set_clause.h"
...
void Binder::bindInsertNode(std::shared_ptr<NodeExpression> node,
  ...
  if (node->isMultiLabeled()) {
    throw BinderException("Create node " + node->toString() +
...
void Binder::bindInsertRel(std::shared_ptr<RelExpression> rel,
  ...
  insertInfo.columnDataExprs = bindInsertColumnDataExprs(
      rel->getPropertyDataExprRef(), entry->getProperties());
  infos.push_back(std::move(insertInfo));
}
...
BoundSetPropertyInfo Binder::bindSetPropertyInfo(
  ...
  if (isNode) {
    auto info = BoundSetPropertyInfo(TableType::NODE, expr, boundColumn,
                                     boundColumnData);
  ...
  return BoundSetPropertyInfo(TableType::REL, expr, boundColumn,
                              boundColumnData);
}
...
std::unique_ptr<BoundUpdatingClause> Binder::bindDeleteClause(
  ...
  for (auto i = 0u; i < deleteClause.getNumExpressions(); ++i) {
    ...
    if (ExpressionUtil::isNodePattern(*pattern)) {
      auto deleteNodeInfo =
          BoundDeleteInfo(deleteType, TableType::NODE, pattern);
