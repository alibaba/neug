...
static void rewrite(std::shared_ptr<Expression> expr,
                    ...
                    const std::unordered_map<std::string, expression_vector>&
                        varNameToProperties) {
  std::string varName;
...
static expression_vector rewrite(
...
void WithClauseProjectionRewriter::visitSingleQueryUnsafe(
  ...
  for (auto& expr : propertyCollector.getProperties()) {
    ...
    if (!varNameToProperties.contains(property.getVariableName())) {
      varNameToProperties.insert(
          {property.getVariableName(), expression_vector{}});
    }
  ...
  for (auto i = 0u; i < singleQuery.getNumQueryParts() - 1; ++i) {
    ...
    auto newProjectionExprs = rewrite(
        projectionBody->getProjectionExpressions(), varNameToProperties);
  ...
  for (auto i = 0u; i < singleQuery.getNumQueryParts() - 1; ++i) {
    auto queryPart = singleQuery.getQueryPartUnsafe(i);
    auto projectionBody = queryPart->getProjectionBodyUnsafe();
    // Avoid rewrite in the case of ORDER BY 1 or aggregate by constant. Because
    // operator implementation replies on expressions to be projected first.
    if (projectionBody->hasOrderByExpressions() ||
        projectionBody->hasAggregateExpressions()) {
      continue;
    }
