...
OpProfileTree::OpProfileTree(const LogicalOperator* op) {
  ...
  this->opProfileBoxWidth =
      std::max<uint32_t>(maxFieldWidth + 2 * (INDENT_WIDTH + BOX_FRAME_WIDTH),
                         MIN_LOGICAL_BOX_WIDTH);
}
...
void OpProfileTree::calculateNumRowsAndColsForOp(const LogicalOperator* op,
                                                 uint32_t& numRows,
                                                 uint32_t& numCols) {
  ...
  for (auto i = 0u; i < op->getNumChildren(); i++) {
    ...
    calculateNumRowsAndColsForOp(op->getChild(i).get(), numRowsInChild,
                                 numColsInChild);
...
uint32_t OpProfileTree::fillOpProfileBoxes(const LogicalOperator* op,
                                           uint32_t rowIdx, uint32_t colIdx,
                                           uint32_t& maxFieldWidth) {
  auto opProfileBox = std::make_unique<OpProfileBox>(
      PlanPrinter::getOperatorName(op), PlanPrinter::getOperatorParams(op),
      std::vector<std::string>{"Cardinality: " +
                               std::to_string(op->getCardinality())});
  ...
  for (auto i = 0u; i < op->getNumChildren(); i++) {
    colOffset += fillOpProfileBoxes(op->getChild(i).get(), rowIdx + 1,
                                    colIdx + colOffset, maxFieldWidth);
  }
  return colOffset;
}

void OpProfileTree::printOpProfileBoxUpperFrame(uint32_t rowIdx,
  ...
  for (auto i = 0u; i < opProfileBoxes[rowIdx].size(); i++) {
    ...
    if (getOpProfileBox(rowIdx, i)) {
      if (hasOpProfileBoxOnUpperLeft(rowIdx, i)) {
        auto leftFrameLength =
            (opProfileBoxWidth - 2 * BOX_FRAME_WIDTH - 1) / 2;
        oss << "┌" << genHorizLine(leftFrameLength) << "┴"
            << genHorizLine(opProfileBoxWidth - 2 * BOX_FRAME_WIDTH - 1 -
                            leftFrameLength)
            << "┐";
      } else {
...
void OpProfileTree::printOpProfileBoxes(uint32_t rowIdx,
                                        std::ostringstream& oss) const {
    ...
    for (auto j = 0u; j < opProfileBoxes[rowIdx].size(); j++) {
      ...
      if (opProfileBox && i < 2 * (opProfileBox->getNumAttributes() + 1) +
        ...
        if (i == 0) {
          ...
          textToPrint =
              dashedLineAccountingForIndex(opProfileBoxWidth, INDENT_WIDTH);
        ...
        auto numLeftSpaces = (opProfileBoxWidth - (1 + INDENT_WIDTH) * 2 -
                              textToPrint.length()) /
                             2;
        auto numRightSpace = opProfileBoxWidth - (1 + INDENT_WIDTH) * 2 -
                             textToPrint.length() - numLeftSpaces;
        oss << "│" << std::string(INDENT_WIDTH + numLeftSpaces, ' ')
            << textToPrint << std::string(INDENT_WIDTH + numRightSpace, ' ')
            << "│";
        ...
        if (hasOpProfileBox(rowIdx + 1, j) && i >= halfWayPoint) {
          ...
          if (i == halfWayPoint) {
            ...
            if (hasOpProfileBox(rowIdx + 1, j + 4) &&
                !hasOpProfileBox(rowIdx, j + 1)) {
              oss << "┬"
                  << genHorizLine(opProfileBoxWidth - 1 - leftHorizLineSize);
            } else {
...
void OpProfileTree::printOpProfileBoxLowerFrame(uint32_t rowIdx,
                                                std::ostringstream& oss) const {
  for (auto i = 0u; i < opProfileBoxes[rowIdx].size(); i++) {
    if (getOpProfileBox(rowIdx, i)) {
      printSpaceIfNecessary(i, oss);
...
void OpProfileTree::prettyPrintPlanTitle(std::ostringstream& oss,
                                         std::string title) const {
  const std::string plan = title;
  oss << "┌" << genHorizLine(opProfileBoxWidth - 2) << "┐" << '\n';
  oss << "│┌" << genHorizLine(opProfileBoxWidth - 4) << "┐│" << '\n';
  auto numLeftSpaces =
      (opProfileBoxWidth - plan.length() - 2 * (2 + INDENT_WIDTH)) / 2;
  auto numRightSpaces = opProfileBoxWidth - plan.length() -
                        2 * (2 + INDENT_WIDTH) - numLeftSpaces;
...
OpProfileBox* OpProfileTree::getOpProfileBox(uint32_t rowIdx,
                                             uint32_t colIdx) const {
  validateRowIdxAndColIdx(rowIdx, colIdx);
  return opProfileBoxes[rowIdx][colIdx].get();
}

bool OpProfileTree::hasOpProfileBoxOnUpperLeft(uint32_t rowIdx,
                                               uint32_t colIdx) const {
  validateRowIdxAndColIdx(rowIdx, colIdx);
...
uint32_t OpProfileTree::calculateRowHeight(uint32_t rowIdx) const {
  ...
  for (auto i = 0u; i < opProfileBoxes[rowIdx].size(); i++) {
    ...
    if (opProfileBox) {
      height = std::max(height, 2 * opProfileBox->getNumAttributes() +
                                    opProfileBox->getNumParams());
    }
  }
  return height + 2;
...
std::ostringstream PlanPrinter::printPlanToOstream(
  ...
  return OpProfileTree(logicalPlan->getLastOperator().get())
      .printLogicalPlanToOstream();
}

std::string PlanPrinter::getOperatorName(
    const LogicalOperator* logicalOperator) {
  return LogicalOperatorUtils::logicalOperatorTypeToString(
      logicalOperator->getOperatorType());
}

std::string PlanPrinter::getOperatorParams(
    const LogicalOperator* logicalOperator) {
...
nlohmann::json PlanPrinter::toJson(const LogicalOperator* logicalOperator) {
  ...
  for (auto i = 0u; i < logicalOperator->getNumChildren(); ++i) {
    json["Child" + std::to_string(i)] =
        toJson(logicalOperator->getChild(i).get());
  }
  return json;
}
