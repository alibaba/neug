...
static std::unique_ptr<FunctionBindData> bindFunc(
  ...
  for (auto i = 0u; i < input.arguments.size(); i++) {
    ...
    fields.emplace_back(input.optionalArguments[i],
                        argument->getDataType().copy());
  }
...
void StructPackFunctions::compileFunc(
    FunctionBindData* /*bindData*/,
    const std::vector<std::shared_ptr<ValueVector>>& parameters,
    std::shared_ptr<ValueVector>& result) {
  // Our goal is to make the state of the resultVector consistent with its
  // children vectors. If the resultVector and inputVector are in different
  // dataChunks, we should create a new child valueVector, which shares the
  // state with the resultVector, instead of reusing the inputVector.
...
void StructPackFunctions::undirectedRelCompileFunc(
    FunctionBindData*,
...
void StructPackFunctions::undirectedRelPackExecFunc(
  ...
  for (auto i = 0u; i < 2; i++) {
    ...
    if (parameter->state->isFlat()) {
      copyParameterValueToStructFieldVector(parameter.get(), fieldVector,
                                            result.state.get());
    } else {
  ...
  for (auto i = 2u; i < parameters.size(); i++) {
    ...
    StructVector::getFieldVector(&result, i)->resetAuxiliaryBuffer();
    copyParameterValueToStructFieldVector(
        parameter.get(), StructVector::getFieldVector(&result, i).get(),
        result.state.get());
  }
}
