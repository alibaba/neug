...
struct ParquetOptions {
  ...
  explicit ParquetOptions(
      case_insensitive_map_t<common::Value> parsingOptions) {
...
struct ExportParquetBindData final : public ExportFuncBindData {
  ...
  std::unique_ptr<ExportFuncBindData> copy() const override {
    auto bindData = std::make_unique<ExportParquetBindData>(
        columnNames, fileName, parquetOptions);
...
struct ExportParquetLocalState final : public ExportFuncLocalState {
  ...
  ExportParquetLocalState(const ExportFuncBindData& bindData,
...
static std::unique_ptr<ExportFuncBindData> bindFunc(
  ...
  return std::make_unique<ExportParquetBindData>(
      bindInput.columnNames, bindInput.filePath, parquetOptions);
}
...
static std::unique_ptr<ExportFuncLocalState> initLocalStateFunc(
    ...
    std::vector<bool> isFlatVec) {
  return std::make_unique<ExportParquetLocalState>(bindData, context,
                                                   isFlatVec);
}
...
static std::vector<ValueVector*> extractSharedPtr(
  ...
  numTuplesToAppend = inputVectors.size() > 0
                          ? inputVectors[0]->state->getSelVector().getSelSize()
                          : 0;
