...
static void writeString(BufferedSerializer* serializer,
                        const ExportFuncBindData& bindData,
                        const uint8_t* strData, uint64_t strLen,
                        bool forceQuote) {
  ...
  if (forceQuote) {
    ...
    std::string strValToWrite =
        std::string(reinterpret_cast<const char*>(strData), strLen);
    strValToWrite =
        addEscapes(exportCSVBindData.exportOption.escapeChar,
                   exportCSVBindData.exportOption.escapeChar, strValToWrite);
    if (exportCSVBindData.exportOption.escapeChar !=
        exportCSVBindData.exportOption.quoteChar) {
      strValToWrite =
          addEscapes(exportCSVBindData.exportOption.quoteChar,
                     exportCSVBindData.exportOption.escapeChar, strValToWrite);
    }
    serializer->writeBufferData(exportCSVBindData.exportOption.quoteChar);
  ...
  void writeHeader(const ExportFuncBindData& bindData) {
    ...
    if (exportCSVBindData.exportOption.hasHeader) {
      for (auto i = 0u; i < exportCSVBindData.columnNames.size(); i++) {
        if (i != 0) {
          bufferedSerializer.writeBufferData(
              exportCSVBindData.exportOption.delimiter);
        }
        auto& name = exportCSVBindData.columnNames[i];
        writeString(&bufferedSerializer, exportCSVBindData,
                    reinterpret_cast<const uint8_t*>(name.c_str()),
                    name.length(), false /* forceQuote */);
      }
      bufferedSerializer.writeBufferData(
          ExportCSVConstants::DEFAULT_CSV_NEWLINE);
...
static void writeRows(const ExportCSVBindData& exportCSVBindData,
                      ExportCSVLocalState& localState,
  ...
  for (auto i = 0u; i < inputVectors.size(); i++) {
    ...
    exportCSVLocalState.castFuncs[i](
        vectorToCast, common::SelectionVector::fromValueVectors(vectorToCast),
        *castVectors[i], castVectors[i]->getSelVectorPtr(),
        nullptr /* dataPtr */);
  }
  ...
  for (auto i = 0u; i < numRowsToWrite; i++) {
    for (auto j = 0u; j < castVectors.size(); j++) {
      ...
      writeString(serializer.get(), exportCSVBindData, strValue.getData(),
                  strValue.len,
                  ExportCSVConstants::DEFAULT_FORCE_QUOTE ||
                      inputVectors[j]->dataType.getLogicalTypeID() ==
                          LogicalTypeID::LIST);
    }
...
static void sinkFunc(ExportFuncSharedState& sharedState,
                     ExportFuncLocalState& localState,
                     const ExportFuncBindData& bindData,
                     std::vector<std::shared_ptr<ValueVector>> inputVectors) {
  auto& exportCSVLocalState = localState.cast<ExportCSVLocalState>();
  auto& exportCSVBindData = bindData.constCast<ExportCSVBindData>();
  ...
  if (serializer->getSize() > ExportCSVConstants::DEFAULT_CSV_FLUSH_SIZE) {
    ...
    exportCSVSharedState.writeRows(serializer->getBlobData(),
                                   serializer->getSize());
    serializer->reset();
  }
}

static void combineFunc(ExportFuncSharedState& sharedState,
                        ExportFuncLocalState& localState) {
