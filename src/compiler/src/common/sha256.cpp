...
SHA256::SHA256() : shaContext{} {
  ...
  // These errors would only occur if there's an issue with shaContext which is
  // wrapped inside SHA256, or with the mbedtls library itself
  if (mbedtls_sha256_starts(&shaContext, false)) {
    throw RuntimeException{"SHA256 Error"};
  }
}

SHA256::~SHA256() { mbedtls_sha256_free(&shaContext); }

void SHA256::addString(const std::string& str) {
  if (mbedtls_sha256_update(&shaContext,
...
void SHA256::finishSHA256(char* out) {
  ...
  if (mbedtls_sha256_finish(&shaContext,
