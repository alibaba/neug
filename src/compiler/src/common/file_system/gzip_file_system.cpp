...
void MiniZStreamWrapper::initialize(CompressedFileInfo& fileInfo) {
  ...
  auto numBytesRead = fileInfo.childFileInfo->readFile(
      gzipHdr, GZipFileSystem::GZIP_HEADER_MINSIZE);
...
bool MiniZStreamWrapper::read(StreamData& sd) {
  if (sd.refresh) {
    ...
    if (gzipHdr[3] & GZipFileSystem::GZIP_FLAG_NAME) {
      ...
      KU_ASSERT(bodyPtr - sd.inputBufStart <
    ...
    auto sta =
        miniz::mz_inflateInit2(mzStreamPtr.get(), -MZ_DEFAULT_WINDOW_BITS);
    // LCOV_EXCL_START
    if (sta != miniz::MZ_OK) {
      throw InternalException("Failed to initialize miniz");
    }
  ...
  mzStreamPtr->avail_out =
      sd.outputBuf.get() + sd.outputBufSize - sd.outputBufEnd;
  auto ret = miniz::mz_inflate(mzStreamPtr.get(), miniz::MZ_NO_FLUSH);
  // LCOV_EXCL_START
  if (ret != miniz::MZ_OK && ret != miniz::MZ_STREAM_END) {
    throw IOException(common::stringFormat("Failed to decode gzip stream: {}",
                                           miniz::mz_error(ret)));
  }
  // LCOV_EXCL_STOP
  ...
  KU_ASSERT(sd.outputBufEnd + mzStreamPtr->avail_out ==
            sd.outputBuf.get() + sd.outputBufSize);
