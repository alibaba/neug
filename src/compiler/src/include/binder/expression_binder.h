...
struct ExpressionBinderConfig {
  // If a property is not in projection list but required in order by after
  // aggregation, we need to bind it as struct extraction because node/rel must
  // have been evaluated as struct during aggregate e.g. RETURN a, COUNT(*)
  // ORDER BY a.ID
  bool bindOrderByAfterAggregate = false;
  // If a node is single labeled, we rewrite its label function as string
  // literal. This however, should be applied to recursive pattern predicate
  // because if path is of length <= 1, there is no intermediate node and thus
  // the predicate should be a noop. If we try to evaluate, it may lead to empty
  // result. e.g. [* (r, n | WHERE label(n)='dummy') ]
...
class ExpressionBinder {
 ...
 public:
  ...
  std::shared_ptr<Expression> bindExpression(
      const parser::ParsedExpression& parsedExpression);

  // TODO(Xiyang): move to an expression rewriter
  std::shared_ptr<Expression> foldExpression(
  ...
  std::shared_ptr<Expression> combineBooleanExpressions(
  ...
  expression_vector bindPropertyStarExpression(
      const parser::ParsedExpression& parsedExpression);
  static expression_vector bindNodeOrRelPropertyStarExpression(
      const Expression& child);
  expression_vector bindStructPropertyStarExpression(
      const std::shared_ptr<Expression>& child);
  std::shared_ptr<Expression> bindPropertyExpression(
      const parser::ParsedExpression& parsedExpression);
  static std::shared_ptr<Expression> bindNodeOrRelPropertyExpression(
  ...
  void bindLambdaExpression(const Expression& lambdaInput,
  ...
  std::shared_ptr<Expression> bindAggregateFunctionExpression(
      const parser::ParsedExpression& parsedExpression,
      const std::string& functionName, bool isDistinct);
