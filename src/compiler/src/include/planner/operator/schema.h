...
class FactorizationGroup {
 ...
 public:
  FactorizationGroup()
      : flat{false}, singleState{false}, cardinalityMultiplier{1} {}
  FactorizationGroup(const FactorizationGroup& other)
      : flat{other.flat},
        singleState{other.singleState},
        cardinalityMultiplier{other.cardinalityMultiplier},
...
class Schema {
 public:
  ...
  void insertToScope(const std::shared_ptr<binder::Expression>& expression,
                     uint32_t groupPos);
  void insertToGroupAndScope(
      const std::shared_ptr<binder::Expression>& expression, uint32_t groupPos);
  // Use these unsafe insert functions only if the operator may work with
  // duplicate expressions. E.g. group by a.age, a.age
  void insertToScopeMayRepeat(
      const std::shared_ptr<binder::Expression>& expression, uint32_t groupPos);
  void insertToGroupAndScopeMayRepeat(
      const std::shared_ptr<binder::Expression>& expression, uint32_t groupPos);

  void insertToGroupAndScope(const binder::expression_vector& expressions,
                             uint32_t groupPos);
  ...
  std::pair<f_group_pos, uint32_t> getExpressionPos(
    ...
    return std::make_pair(groupPos,
                          groups[groupPos]->getExpressionPos(expression));
  }
  ...
  binder::expression_vector getExpressionsInScope() const {
 ...
 private:
  ...
  // Our projection doesn't explicitly remove expressions. Instead, we keep
  // track of what expressions are in scope (i.e. being projected).
  binder::expression_vector expressionsInScope;
};

class SchemaUtils {
 public:
  // Given a set of factorization group, a leading group is selected as the
  // unFlat group (caller should ensure at most one unFlat group which is our
  // general assumption of factorization). If all groups are flat, we select any
  // (the first) group as leading group.
