...
class LogicalDistinct final : public LogicalOperator {
 public:
  LogicalDistinct(binder::expression_vector keys,
                  std::shared_ptr<LogicalOperator> child)
      : LogicalDistinct{LogicalOperatorType::DISTINCT, keys,
                        binder::expression_vector{}, std::move(child)} {}
  ...
  void setPayloads(binder::expression_vector expressions) {
    payloads = std::move(expressions);
  }
  ...
  std::unique_ptr<LogicalOperator> copy() override {
    return make_unique<LogicalDistinct>(operatorType, keys, payloads,
                                        children[0]->copy());
  }
 ...
 protected:
  ...
  // Payloads meaning additional keys that are functional dependent on the keys
  // above.
  binder::expression_vector payloads;
