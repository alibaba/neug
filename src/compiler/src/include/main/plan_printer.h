...
class OpProfileTree {
 ...
 private:
  static void calculateNumRowsAndColsForOp(const planner::LogicalOperator* op,
                                           uint32_t& numRows,
                                           uint32_t& numCols);

  uint32_t fillOpProfileBoxes(const planner::LogicalOperator* op,
                              uint32_t rowIdx, uint32_t colIdx,
                              uint32_t& maxFieldWidth);

  void printOpProfileBoxUpperFrame(uint32_t rowIdx,
                                   std::ostringstream& oss) const;

  void printOpProfileBoxes(uint32_t rowIdx, std::ostringstream& oss) const;

  void printOpProfileBoxLowerFrame(uint32_t rowIdx,
                                   std::ostringstream& oss) const;
  ...
  inline void validateRowIdxAndColIdx(uint32_t rowIdx, uint32_t colIdx) const {
    KU_ASSERT(rowIdx < opProfileBoxes.size() &&
  ...
  bool hasOpProfileBox(uint32_t rowIdx, uint32_t colIdx) const {
    return rowIdx < opProfileBoxes.size() &&
           colIdx < opProfileBoxes[rowIdx].size() &&
