...
/**
 * Task represents a task that can be executed by multiple threads in the
 * TaskScheduler. Task is a virtual class. Users of TaskScheduler need to extend
 * the Task class and implement at least a virtual run() function. Users can
 * assume that before T.run() is called, a worker thread W has grabbed task T
 * from the TaskScheduler's queue and registered itself to T. They can also
 * assume that after run() is called W will deregister itself from T. When
 * deregistering, if W is the last worker to finish on T, i.e., once W finishes,
 * T will be completed, the finalizeIfNecessary() will be called. So the run()
 * and finalizeIfNecessary() calls are separate calls and if there is some state
 * from the run() function execution that will be needed by finalizeIfNecessary,
 * users should save it somewhere that can be accessed in finalizeIfNecessary().
 * See ProcessorTask for an example of this.
 */
class Task {
 ...
 public:
  ...
  //     deRegisterThreadAndFinalizeTaskIfNecessary() only once by the last
  //     registered worker that is completing this task. So the task lock is
