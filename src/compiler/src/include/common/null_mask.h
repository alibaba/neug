...
constexpr uint64_t NULL_BITMASKS_WITH_SINGLE_ONE[64] = {0x1,
                                                        0x2,
                                                        0x4,
                                                        0x8,
                                                        0x10,
...
const uint64_t NULL_LOWER_MASKS[65] = {0x0,
                                       0x1,
                                       0x3,
                                       0x7,
                                       0xf,
                                       ...
                                       0x7fffffffffffffff,
                                       0xffffffffffffffff};
const uint64_t NULL_HIGH_MASKS[65] = {0x0,
                                      0x8000000000000000,
                                      0xc000000000000000,
                                      0xe000000000000000,
                                      0xf000000000000000,
...
class NullMask {
 public:
  ...
  static constexpr uint64_t NUM_BITS_PER_NULL_ENTRY =
      (uint64_t) 1 << NUM_BITS_PER_NULL_ENTRY_LOG2;
  static constexpr uint64_t NUM_BYTES_PER_NULL_ENTRY =
      NUM_BITS_PER_NULL_ENTRY >> 3;
  ...
  // becomes non-null
  // Modifying the underlying data should be done with setNull or
  // copyFromNullData
  ...
  static inline uint64_t getNumNullEntries(uint64_t numNullBits) {
    return (numNullBits >> NUM_BITS_PER_NULL_ENTRY_LOG2) +
           ((numNullBits - (numNullBits << NUM_BITS_PER_NULL_ENTRY_LOG2)) == 0
                ? 0
                : 1);
  }

  // Copies bitpacked null flags from one buffer to another, starting at an
  // arbitrary bit offset and preserving adjacent bits.
  //
  // returns true if we have copied a nullBit with value 1 (indicates a null
  ...
  inline bool copyFrom(const NullMask& nullMask, uint64_t srcOffset,
    ...
    if (nullMask.hasNoNullsGuarantee()) {
      ...
      return copyFromNullBits(nullMask.getData(), srcOffset, dstOffset,
                              numBitsToCopy, invert);
    }
  }
  bool copyFromNullBits(const uint64_t* srcNullEntries, uint64_t srcOffset,
                        uint64_t dstOffset, uint64_t numBitsToCopy,
                        bool invert = false);

  // Sets the given number of bits to null (if isNull is true) or non-null (if
  // isNull is false), starting at the offset
  static void setNullRange(uint64_t* nullEntries, uint64_t offset,
  ...
  static std::pair<bool, bool> getMinMax(const uint64_t* nullEntries,
                                         uint64_t offset, uint64_t numValues);

 private:
  static inline std::pair<uint64_t, uint64_t> getNullEntryAndBitPos(
      uint64_t pos) {
