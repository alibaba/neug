...
class Timestamp {
 public:
  ...
  KUZU_API static bool tryConvertTimestamp(const char* str, uint64_t len,
                                           timestamp_t& result);

  // Extract the date and time from a given timestamp object.
  KUZU_API static void convert(timestamp_t timestamp, date_t& out_date,
                               dtime_t& out_time);
  ...
  KUZU_API static int32_t getTimestampPart(DatePartSpecifier specifier,
                                           timestamp_t timestamp);

  KUZU_API static timestamp_t trunc(DatePartSpecifier specifier,
                                    timestamp_t date);
  ...
  KUZU_API static bool tryParseUTCOffset(const char* str, uint64_t& pos,
