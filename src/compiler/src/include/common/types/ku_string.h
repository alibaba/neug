...
struct KUZU_API ku_string_t {
  ...
  const uint8_t* getData() const {
    return isShortString(len) ? prefix
                              : reinterpret_cast<uint8_t*>(overflowPtr);
  }
  ...
  // These functions do *NOT* allocate/resize the overflow buffer, it only
  ...
  void setLongString(const ku_string_t& value) {
    ...
    memcpy(reinterpret_cast<char*>(overflowPtr),
           reinterpret_cast<char*>(value.overflowPtr), value.len);
  }
