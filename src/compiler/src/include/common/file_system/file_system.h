...
struct FileFlags {
  ...
  // Always create a new file. If a file exists, the file is truncated. Cannot
  // be used together with CREATE_IF_NOT_EXISTS.
...
class KUZU_API FileSystem {
 ...
 public:
  ...
  virtual std::string expandPath(main::ClientContext* context,
                                 const std::string& path) const;
 ...
 protected:
  ...
  virtual int64_t readFile(FileInfo& fileInfo, void* buf,
                           size_t numBytes) const = 0;

  virtual void writeFile(FileInfo& fileInfo, const uint8_t* buffer,
                         uint64_t numBytes, uint64_t offset) const;

  virtual int64_t seek(FileInfo& fileInfo, uint64_t offset,
                       int whence) const = 0;
