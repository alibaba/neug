...
/**
 * Binary operator assumes function with null returns null. This does NOT
 * applies to binary boolean operations (e.g. AND, OR, XOR).
 */

struct BinaryFunctionWrapper {
  template <typename LEFT_TYPE, typename RIGHT_TYPE, typename RESULT_TYPE,
            typename OP>
  static inline void operation(LEFT_TYPE& left, RIGHT_TYPE& right,
                               RESULT_TYPE& result,
...
struct BinarySelectWithBindDataWrapper {
  ...
  static void operation(LEFT_TYPE& left, RIGHT_TYPE& right, uint8_t& result,
                        common::ValueVector* leftValueVector,
                        common::ValueVector* rightValueVector, void* dataPtr) {
    OP::operation(left, right, result, *leftValueVector, *rightValueVector,
                  *leftValueVector, dataPtr);
  }
};

struct BinaryFunctionExecutor {
  template <typename LEFT_TYPE, typename RIGHT_TYPE, typename RESULT_TYPE,
            typename FUNC, typename OP_WRAPPER>
  static inline void executeOnValue(common::ValueVector& left,
                                    common::ValueVector& right,
                                    common::ValueVector& resultValueVector,
                                    uint64_t lPos, uint64_t rPos,
                                    uint64_t resPos, void* dataPtr) {
    OP_WRAPPER::template operation<LEFT_TYPE, RIGHT_TYPE, RESULT_TYPE, FUNC>(
        ((LEFT_TYPE*) left.getData())[lPos],
        ((RIGHT_TYPE*) right.getData())[rPos],
        ((RESULT_TYPE*) resultValueVector.getData())[resPos], &left, &right,
        &resultValueVector, resPos, dataPtr);
  }

  static inline std::tuple<common::sel_t, common::sel_t, common::sel_t>
  getSelectedPositions(common::SelectionVector* leftSelVector,
                       common::SelectionVector* rightSelVector,
                       common::SelectionVector* resultSelVector,
                       common::sel_t selPos, bool leftFlat, bool rightFlat) {
    common::sel_t lPos = (*leftSelVector)[leftFlat ? 0 : selPos];
    common::sel_t rPos = (*rightSelVector)[rightFlat ? 0 : selPos];
    common::sel_t resPos =
        (*resultSelVector)[leftFlat && rightFlat ? 0 : selPos];
  ...
  static void executeOnSelectedValues(common::ValueVector& left,
    ...
    const bool allNullsGuaranteed =
        (rightFlat && right.isNull((*rightSelVector)[0])) ||
    ...
    if (allNullsGuaranteed) {
      ...
      for (common::sel_t selPos = 0; selPos < numSelectedValues; ++selPos) {
        auto [lPos, rPos, resPos] =
            getSelectedPositions(leftSelVector, rightSelVector, resultSelVector,
                                 selPos, leftFlat, rightFlat);
        if (noNullsGuaranteed) {
          ...
          if (!result.isNull(resPos)) {
            executeOnValue<LEFT_TYPE, RIGHT_TYPE, RESULT_TYPE, FUNC,
                           OP_WRAPPER>(left, right, result, lPos, rPos, resPos,
                                       dataPtr);
          }
        }
      }
  ...
  template <typename LEFT_TYPE, typename RIGHT_TYPE, typename RESULT_TYPE,
            typename FUNC, typename OP_WRAPPER>
  static void executeSwitch(common::ValueVector& left,
                            common::SelectionVector* leftSelVector,
                            common::ValueVector& right,
                            common::SelectionVector* rightSelVector,
                            common::ValueVector& result,
    ...
    executeOnSelectedValues<LEFT_TYPE, RIGHT_TYPE, RESULT_TYPE, FUNC,
                            OP_WRAPPER>(left, leftSelVector, right,
                                        rightSelVector, result, resultSelVector,
                                        dataPtr);
  }

  template <typename LEFT_TYPE, typename RIGHT_TYPE, typename RESULT_TYPE,
            typename FUNC>
  static void execute(common::ValueVector& left,
                      common::SelectionVector* leftSelVector,
                      common::ValueVector& right,
                      common::SelectionVector* rightSelVector,
                      common::ValueVector& result,
    ...
    executeSwitch<LEFT_TYPE, RIGHT_TYPE, RESULT_TYPE, FUNC,
        ...
        left, leftSelVector, right, rightSelVector, result, resultSelVector,
        nullptr /* dataPtr */);
  }
  ...
  static void selectOnValue(common::ValueVector& left,
                            common::ValueVector& right, uint64_t lPos,
                            uint64_t rPos, uint64_t resPos,
    ...
    SELECT_WRAPPER::template operation<LEFT_TYPE, RIGHT_TYPE, FUNC>(
        ((LEFT_TYPE*) left.getData())[lPos],
        ((RIGHT_TYPE*) right.getData())[rPos], resultValue, &left, &right,
        dataPtr);
  ...
  template <class LEFT_TYPE, class RIGHT_TYPE, class FUNC,
            typename SELECT_WRAPPER>
  static uint64_t selectBothFlat(common::ValueVector& left,
                                 common::ValueVector& right, void* dataPtr) {
  ...
  template <typename LEFT_TYPE, typename RIGHT_TYPE, typename FUNC,
            typename SELECT_WRAPPER>
  static bool selectFlatUnFlat(common::ValueVector& left,
                               common::ValueVector& right,
    ...
    if (left.isNull(lPos)) {
      ...
      rightSelVector.forEach([&](auto i) {
        if (!right.isNull(i)) {
          selectOnValue<LEFT_TYPE, RIGHT_TYPE, FUNC, SELECT_WRAPPER>(
              left, right, lPos, i, i, numSelectedValues,
              selectedPositionsBuffer, dataPtr);
        }
      });
    }
  ...
  template <typename LEFT_TYPE, typename RIGHT_TYPE, typename FUNC,
            typename SELECT_WRAPPER>
  static bool selectUnFlatFlat(common::ValueVector& left,
                               common::ValueVector& right,
                               common::SelectionVector& selVector,
                               void* dataPtr) {
    auto rPos = right.state->getSelVector()[0];
    ...
    if (right.isNull(rPos)) {
      ...
      leftSelVector.forEach([&](auto i) {
        if (!left.isNull(i)) {
          selectOnValue<LEFT_TYPE, RIGHT_TYPE, FUNC, SELECT_WRAPPER>(
              left, right, i, rPos, i, numSelectedValues,
              selectedPositionsBuffer, dataPtr);
        }
      });
    }
  ...
  template <class LEFT_TYPE, class RIGHT_TYPE, class FUNC,
            typename SELECT_WRAPPER>
  static bool selectBothUnFlat(common::ValueVector& left,
                               common::ValueVector& right,
                               common::SelectionVector& selVector,
                               void* dataPtr) {
    uint64_t numSelectedValues = 0;
  ...
  static bool select(common::ValueVector& left, common::ValueVector& right,
    ...
    if (left.state->isFlat() && right.state->isFlat()) {
      return selectBothFlat<LEFT_TYPE, RIGHT_TYPE, FUNC, OP_WRAPPER>(
          left, right, dataPtr);
    } else if (left.state->isFlat() && !right.state->isFlat()) {
      return selectFlatUnFlat<LEFT_TYPE, RIGHT_TYPE, FUNC, OP_WRAPPER>(
          left, right, selVector, dataPtr);
    } else if (!left.state->isFlat() && right.state->isFlat()) {
      return selectUnFlatFlat<LEFT_TYPE, RIGHT_TYPE, FUNC, OP_WRAPPER>(
          left, right, selVector, dataPtr);
    } else {
      return selectBothUnFlat<LEFT_TYPE, RIGHT_TYPE, FUNC, OP_WRAPPER>(
          left, right, selVector, dataPtr);
    }
  }
  ...
  template <class LEFT_TYPE, class RIGHT_TYPE, class FUNC>
  static bool selectComparison(common::ValueVector& left,
                               common::ValueVector& right,
                               common::SelectionVector& selVector,
                               void* dataPtr) {
    if (left.state->isFlat() && right.state->isFlat()) {
      return selectBothFlat<LEFT_TYPE, RIGHT_TYPE, FUNC,
                            BinaryComparisonSelectWrapper>(left, right,
                                                           dataPtr);
    } else if (left.state->isFlat() && !right.state->isFlat()) {
      return selectFlatUnFlat<LEFT_TYPE, RIGHT_TYPE, FUNC,
                              BinaryComparisonSelectWrapper>(
          left, right, selVector, dataPtr);
    } else if (!left.state->isFlat() && right.state->isFlat()) {
      return selectUnFlatFlat<LEFT_TYPE, RIGHT_TYPE, FUNC,
                              BinaryComparisonSelectWrapper>(
          left, right, selVector, dataPtr);
    } else {
      return selectBothUnFlat<LEFT_TYPE, RIGHT_TYPE, FUNC,
                              BinaryComparisonSelectWrapper>(
          left, right, selVector, dataPtr);
    }
  }
