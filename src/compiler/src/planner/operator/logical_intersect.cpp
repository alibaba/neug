...
f_group_pos_set LogicalIntersect::getGroupsPosToFlattenOnBuildSide(
    uint32_t buildIdx) {
  f_group_pos_set result;
  auto childIdx = buildIdx + 1;  // skip probe
  result.insert(
      children[childIdx]->getSchema()->getGroupPos(*keyNodeIDs[buildIdx]));
  return result;
}
...
void LogicalIntersect::computeFactorizedSchema() {
  ...
  // Write intersect node and rels into a new group regardless of whether rel is
  // n-n.
...
std::unique_ptr<LogicalOperator> LogicalIntersect::copy() {
  ...
  auto op = make_unique<LogicalIntersect>(
      intersectNodeID, keyNodeIDs, children[0]->copy(),
