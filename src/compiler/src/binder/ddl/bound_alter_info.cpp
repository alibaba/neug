...
std::string BoundAlterInfo::toString() const {
  ...
  case common::AlterType::RENAME: {
    auto renameInfo =
        common::ku_dynamic_cast<BoundExtraRenameTableInfo*>(extraInfo.get());
    result += "Rename Table " + tableName + " to " + renameInfo->newName;
    break;
  }
  case common::AlterType::ADD_PROPERTY: {
    auto addPropInfo =
        common::ku_dynamic_cast<BoundExtraAddPropertyInfo*>(extraInfo.get());
  ...
  case common::AlterType::DROP_PROPERTY: {
    auto dropPropInfo =
        common::ku_dynamic_cast<BoundExtraDropPropertyInfo*>(extraInfo.get());
