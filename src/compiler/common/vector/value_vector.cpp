...
#include "src/include/common/vector/auxiliary_buffer.h"
...
void ValueVector::copyFromVectorData(uint8_t* dstData,
  ...
  KU_ASSERT(srcVector->dataType.getPhysicalType() ==
  ...
  case PhysicalTypeID::STRING: {
    StringVector::addString(this, *(ku_string_t*) dstData,
                            *(ku_string_t*) srcVectorData);
  } break;
  default: {
...
void ValueVector::resetAuxiliaryBuffer() {
  switch (dataType.getPhysicalType()) {
  case PhysicalTypeID::STRING: {
    ku_dynamic_cast<StringAuxiliaryBuffer*>(auxiliaryBuffer.get())
        ->resetOverflowBuffer();
    return;
  }
  case PhysicalTypeID::ARRAY:
  case PhysicalTypeID::LIST: {
    auto listAuxiliaryBuffer =
        ku_dynamic_cast<ListAuxiliaryBuffer*>(auxiliaryBuffer.get());
  ...
  case PhysicalTypeID::STRUCT: {
    auto structAuxiliaryBuffer =
        ku_dynamic_cast<StructAuxiliaryBuffer*>(auxiliaryBuffer.get());
...
template KUZU_API void ValueVector::setValue<nodeID_t>(uint32_t pos,
                                                       nodeID_t val);
template KUZU_API void ValueVector::setValue<bool>(uint32_t pos, bool val);
template KUZU_API void ValueVector::setValue<int64_t>(uint32_t pos,
                                                      int64_t val);
template KUZU_API void ValueVector::setValue<int32_t>(uint32_t pos,
                                                      int32_t val);
template KUZU_API void ValueVector::setValue<int16_t>(uint32_t pos,
                                                      int16_t val);
template KUZU_API void ValueVector::setValue<int8_t>(uint32_t pos, int8_t val);
template KUZU_API void ValueVector::setValue<uint64_t>(uint32_t pos,
                                                       uint64_t val);
template KUZU_API void ValueVector::setValue<uint32_t>(uint32_t pos,
                                                       uint32_t val);
template KUZU_API void ValueVector::setValue<uint16_t>(uint32_t pos,
                                                       uint16_t val);
template KUZU_API void ValueVector::setValue<uint8_t>(uint32_t pos,
                                                      uint8_t val);
template KUZU_API void ValueVector::setValue<int128_t>(uint32_t pos,
                                                       int128_t val);
...
template KUZU_API void ValueVector::setValue<timestamp_t>(uint32_t pos,
                                                          timestamp_t val);
template KUZU_API void ValueVector::setValue<timestamp_ns_t>(
    uint32_t pos, timestamp_ns_t val);
template KUZU_API void ValueVector::setValue<timestamp_ms_t>(
    uint32_t pos, timestamp_ms_t val);
template KUZU_API void ValueVector::setValue<timestamp_sec_t>(
    uint32_t pos, timestamp_sec_t val);
template KUZU_API void ValueVector::setValue<timestamp_tz_t>(
    uint32_t pos, timestamp_tz_t val);
template KUZU_API void ValueVector::setValue<interval_t>(uint32_t pos,
                                                         interval_t val);
template KUZU_API void ValueVector::setValue<list_entry_t>(uint32_t pos,
                                                           list_entry_t val);
template KUZU_API void ValueVector::setValue<ku_uuid_t>(uint32_t pos,
                                                        ku_uuid_t val);
...
void StringVector::addString(ValueVector* vector, uint32_t vectorPos,
  ...
  auto stringBuffer =
      ku_dynamic_cast<StringAuxiliaryBuffer*>(vector->auxiliaryBuffer.get());
...
ku_string_t& StringVector::reserveString(ValueVector* vector,
                                         uint32_t vectorPos, uint64_t length) {
  KU_ASSERT(vector->dataType.getPhysicalType() == PhysicalTypeID::STRING);
  auto stringBuffer =
      ku_dynamic_cast<StringAuxiliaryBuffer*>(vector->auxiliaryBuffer.get());
  auto& dstStr = vector->getValue<ku_string_t>(vectorPos);
...
void StringVector::reserveString(ValueVector* vector, ku_string_t& dstStr,
                                 uint64_t length) {
  KU_ASSERT(vector->dataType.getPhysicalType() == PhysicalTypeID::STRING);
  auto stringBuffer =
      ku_dynamic_cast<StringAuxiliaryBuffer*>(vector->auxiliaryBuffer.get());
...
void StringVector::addString(ValueVector* vector, ku_string_t& dstStr,
                             ku_string_t& srcStr) {
  KU_ASSERT(vector->dataType.getPhysicalType() == PhysicalTypeID::STRING);
  auto stringBuffer =
      ku_dynamic_cast<StringAuxiliaryBuffer*>(vector->auxiliaryBuffer.get());
