#include "src/include/common/arrow/arrow_converter.h"
...
template <typename T>
static void scanArrowArrayFixedSizePrimitive(
    const ArrowArray* array, ValueVector& outputVector, ArrowNullMaskTree* mask,
    uint64_t srcOffset, uint64_t dstOffset, uint64_t count) {
  auto arrayBuffer = (const uint8_t*) array->buffers[1];
  mask->copyToValueVector(&outputVector, dstOffset, count);
  memcpy(
      outputVector.getData() + dstOffset * outputVector.getNumBytesPerValue(),
      arrayBuffer + srcOffset * sizeof(T), count * sizeof(T));
}

template <typename SRC, typename DST>
static void scanArrowArrayFixedSizePrimitiveAndCastTo(
    const ArrowArray* array, ValueVector& outputVector, ArrowNullMaskTree* mask,
    uint64_t srcOffset, uint64_t dstOffset, uint64_t count) {
...
template <>
void scanArrowArrayFixedSizePrimitive<bool>(
    const ArrowArray* array, ValueVector& outputVector, ArrowNullMaskTree* mask,
    uint64_t srcOffset, uint64_t dstOffset, uint64_t count) {
  auto arrayBuffer = (const uint8_t*) array->buffers[1];
  mask->copyToValueVector(&outputVector, dstOffset, count);
...
static void scanArrowArrayDurationScaledUp(const ArrowArray* array,
                                           ValueVector& outputVector,
                                           ArrowNullMaskTree* mask,
                                           int64_t scaleFactor,
                                           uint64_t srcOffset,
...
static void scanArrowArrayDurationScaledDown(
    const ArrowArray* array, ValueVector& outputVector, ArrowNullMaskTree* mask,
    int64_t scaleFactor, uint64_t srcOffset, uint64_t dstOffset,
    uint64_t count) {
  auto arrayBuffer = ((const int64_t*) array->buffers[1]) + srcOffset;
  mask->copyToValueVector(&outputVector, dstOffset, count);
  for (uint64_t i = 0; i < count; i++) {
...
static void scanArrowArrayMonthInterval(const ArrowArray* array,
                                        ValueVector& outputVector,
                                        ArrowNullMaskTree* mask,
  ...
  for (uint64_t i = 0; i < count; i++) {
    if (!mask->isNull(i)) {
      ...
      outputVector.setValue<interval_t>(i + dstOffset,
                                        interval_t(curValue, 0, 0));
    }
  }
}

static void scanArrowArrayDayTimeInterval(const ArrowArray* array,
                                          ValueVector& outputVector,
                                          ArrowNullMaskTree* mask,
                                          uint64_t srcOffset,
                                          uint64_t dstOffset, uint64_t count) {
  ...
  for (uint64_t i = 0; i < count; i++) {
    if (!mask->isNull(i)) {
      ...
      outputVector.setValue<interval_t>(i + dstOffset,
                                        interval_t(0, day, micros));
    }
  }
}

static void scanArrowArrayMonthDayNanoInterval(
    const ArrowArray* array, ValueVector& outputVector, ArrowNullMaskTree* mask,
    uint64_t srcOffset, uint64_t dstOffset, uint64_t count) {
  auto arrayBuffer = (const int64_t*) ((const uint8_t*) array->buffers[1] +
                                       srcOffset * 16);  // 16 bits per value
  mask->copyToValueVector(&outputVector, dstOffset, count);
  for (uint64_t i = 0; i < count; i++) {
    if (!mask->isNull(i)) {
      ...
      outputVector.setValue<interval_t>(i + dstOffset,
                                        interval_t(month, day, micros));
    }
  }
}

template <typename offsetsT>
static void scanArrowArrayBLOB(const ArrowArray* array,
                               ValueVector& outputVector,
...
static void scanArrowArrayBLOBView(const ArrowArray* array,
                                   ValueVector& outputVector,
                                   ArrowNullMaskTree* mask, uint64_t srcOffset,
                                   uint64_t dstOffset, uint64_t count) {
  auto arrayBuffer = (const uint8_t*) (array->buffers[1]);
  ...
  for (uint64_t i = 0; i < count; i++) {
    if (!mask->isNull(i)) {
      ...
      if (viewLength <= 12) {
        BlobVector::addBlob(&outputVector, i + dstOffset,
                            (uint8_t*) (curView + 1), viewLength);
      } else {
        auto bufIndex = curView[2];
        auto offset = curView[3];
        BlobVector::addBlob(&outputVector, i + dstOffset,
                            valueBuffs[bufIndex] + offset, viewLength);
      }
    }
  }
}

static void scanArrowArrayFixedBLOB(const ArrowArray* array,
                                    ValueVector& outputVector,
                                    ArrowNullMaskTree* mask, int64_t BLOBsize,
                                    uint64_t srcOffset, uint64_t dstOffset,
                                    uint64_t count) {
  auto arrayBuffer =
      ((const uint8_t*) array->buffers[1]) + srcOffset * BLOBsize;
  mask->copyToValueVector(&outputVector, dstOffset, count);
  for (uint64_t i = 0; i < count; i++) {
    if (!mask->isNull(i)) {
      BlobVector::addBlob(&outputVector, i + dstOffset,
                          arrayBuffer + i * BLOBsize, BLOBsize);
    }
  }
}

template <typename offsetsT>
static void scanArrowArrayList(const ArrowSchema* schema,
  ...
  ArrowConverter::fromArrowArray(schema->children[0], array->children[0],
                                 *auxiliaryBuffer, mask->getChild(0),
                                 offsets[0] + array->children[0]->offset,
                                 auxDstPosition, offsets[count] - offsets[0]);
}

template <typename offsetsT>
static void scanArrowArrayListView(const ArrowSchema* schema,
                                   const ArrowArray* array,
                                   ValueVector& outputVector,
                                   ArrowNullMaskTree* mask, uint64_t srcOffset,
                                   uint64_t dstOffset, uint64_t count) {
  ...
  for (uint64_t i = 0; i < count; i++) {
    if (!mask->isNull(i)) {
      ...
      outputVector.setValue<list_entry_t>(i + dstOffset, newEntry);
      ArrowNullMaskTree childTree(schema->children[0], array->children[0],
                                  srcOffset, count);
      // make our own child here. precomputing through the mask tree is too
      // complicated
      ArrowConverter::fromArrowArray(schema->children[0], array->children[0],
                                     *auxiliaryBuffer, &childTree, curOffset,
                                     newEntry.offset, newEntry.size);
    }
  }
}

static void scanArrowArrayFixedList(const ArrowSchema* schema,
                                    const ArrowArray* array,
                                    ValueVector& outputVector,
                                    ArrowNullMaskTree* mask, uint64_t srcOffset,
                                    uint64_t dstOffset, uint64_t count) {
  ...
  ArrowConverter::fromArrowArray(
      schema->children[0], array->children[0], *auxiliaryBuffer,
      mask->getChild(0), srcOffset * numElements + array->children[0]->offset,
      dstOffset * numElements, count * numElements);
}

static void scanArrowArrayStruct(const ArrowSchema* schema,
                                 const ArrowArray* array,
                                 ValueVector& outputVector,
                                 ArrowNullMaskTree* mask, uint64_t srcOffset,
                                 uint64_t dstOffset, uint64_t count) {
  ...
  for (int64_t j = 0; j < schema->n_children; j++) {
    ArrowConverter::fromArrowArray(
        schema->children[j], array->children[j],
        *StructVector::getFieldVector(&outputVector, j).get(),
        mask->getChild(j), srcOffset + array->children[j]->offset, dstOffset,
        count);
  }
}

static void scanArrowArrayDenseUnion(const ArrowSchema* schema,
                                     const ArrowArray* array,
                                     ValueVector& outputVector,
                                     ArrowNullMaskTree* mask,
                                     uint64_t srcOffset, uint64_t dstOffset,
  ...
  for (auto i = 0u; i < count; i++) {
    ...
    if (!mask->isNull(i)) {
      ...
      ArrowConverter::fromArrowArray(
          schema->children[curType], array->children[curType],
...
static void scanArrowArraySparseUnion(const ArrowSchema* schema,
                                      const ArrowArray* array,
                                      ValueVector& outputVector,
                                      ArrowNullMaskTree* mask,
                                      uint64_t srcOffset, uint64_t dstOffset,
...
template <typename offsetsT>
static void scanArrowArrayDictionaryEncoded(
    const ArrowSchema* schema, const ArrowArray* array,
    ValueVector& outputVector, ArrowNullMaskTree* mask, uint64_t srcOffset,
    uint64_t dstOffset, uint64_t count) {
  ...
  for (uint64_t i = 0; i < count; i++) {
    if (!mask->isNull(i)) {
      ...
      ArrowConverter::fromArrowArray(
          schema->dictionary, array->dictionary, outputVector, &dictOffseted,
...
static void scanArrowArrayRunEndEncoded(const ArrowSchema* schema,
                                        const ArrowArray* array,
                                        ValueVector& outputVector,
                                        ArrowNullMaskTree* mask,
                                        uint64_t srcOffset, uint64_t dstOffset,
  ...
  for (uint64_t i = 0; i < count; i++) {
    ...
    ArrowConverter::fromArrowArray(
        schema->children[1], array->children[1], outputVector, &valuesOffseted,
        runEndIdx, i + dstOffset,
        1);  // there is optimization to be made here...
  }
}

void ArrowConverter::fromArrowArray(const ArrowSchema* schema,
                                    const ArrowArray* array,
                                    ValueVector& outputVector,
                                    ArrowNullMaskTree* mask, uint64_t srcOffset,
                                    uint64_t dstOffset, uint64_t count) {
  ...
  case 'b':
    // BOOL
    return scanArrowArrayFixedSizePrimitive<bool>(array, outputVector, mask,
  ...
  case 'f':
    // FLOAT
    return scanArrowArrayFixedSizePrimitive<float>(array, outputVector, mask,
                                                   srcOffset, dstOffset, count);
  case 'g':
    // DOUBLE
  ...
  case 'z':
    // BLOB
    return scanArrowArrayBLOB<int32_t>(array, outputVector, mask, srcOffset,
                                       dstOffset, count);
  case 'Z':
    // LONG BLOB
    return scanArrowArrayBLOB<int64_t>(array, outputVector, mask, srcOffset,
                                       dstOffset, count);
  case 'u':
    // STRING
    return scanArrowArrayBLOB<int32_t>(array, outputVector, mask, srcOffset,
  ...
  case 'd': {
    ...
    case PhysicalTypeID::INT32:
      return scanArrowArrayFixedSizePrimitiveAndCastTo<int128_t, int32_t>(
          array, outputVector, mask, srcOffset, dstOffset, count);
    case PhysicalTypeID::INT64:
      return scanArrowArrayFixedSizePrimitiveAndCastTo<int128_t, int64_t>(
          array, outputVector, mask, srcOffset, dstOffset, count);
  ...
  case 'w':
    // FIXED BLOB
    return scanArrowArrayFixedBLOB(array, outputVector, mask,
                                   std::stoi(arrowType + 2), srcOffset,
                                   dstOffset, count);
  case 't':
    switch (arrowType[1]) {
    case 'd':
    ...
    case 'D':
      ...
      case 'm':
        return scanArrowArrayDurationScaledUp(array, outputVector, mask, 1000,
                                              srcOffset, dstOffset, count);
      case 'u':
        return scanArrowArrayDurationScaledUp(array, outputVector, mask, 1,
                                              srcOffset, dstOffset, count);
      case 'n':
    ...
    case 'i':
      ...
      switch (arrowType[2]) {
      case 'M':
        return scanArrowArrayMonthInterval(array, outputVector, mask, srcOffset,
                                           dstOffset, count);
      case 'D':
        return scanArrowArrayDayTimeInterval(array, outputVector, mask,
                                             srcOffset, dstOffset, count);
      case 'n':
  ...
  case '+':
    ...
    case 'r':
      // RUN END ENCODED
      return scanArrowArrayRunEndEncoded(schema, array, outputVector, mask,
                                         srcOffset, dstOffset, count);
    case 'l':
      // LIST
      return scanArrowArrayList<int32_t>(schema, array, outputVector, mask,
                                         srcOffset, dstOffset, count);
    case 'L':
      // LONG LIST
      return scanArrowArrayList<int64_t>(schema, array, outputVector, mask,
                                         srcOffset, dstOffset, count);
    case 'w': {
      // ARRAY
      RUNTIME_CHECK({
    ...
    case 's':
      // STRUCT
      return scanArrowArrayStruct(schema, array, outputVector, mask, srcOffset,
                                  dstOffset, count);
    case 'm':
      // MAP
      return scanArrowArrayList<int32_t>(schema, array, outputVector, mask,
    ...
    case 'u':
      if (arrowType[2] == 'd') {
        // DENSE UNION
        return scanArrowArrayDenseUnion(schema, array, outputVector, mask,
                                        srcOffset, dstOffset, count);
      } else {
        // SPARSE UNION
        return scanArrowArraySparseUnion(schema, array, outputVector, mask,
                                         srcOffset, dstOffset, count);
      }
    case 'v':
      switch (arrowType[2]) {
...
void ArrowConverter::fromArrowArray(const ArrowSchema* schema,
  ...
  return fromArrowArray(schema, array, outputVector, &mask, array->offset, 0,
                        array->length);
}
