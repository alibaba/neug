...
#include "src/include/common/arrow/arrow.h"
#include "src/include/common/arrow/arrow_nullmask_tree.h"
...
void ArrowNullMaskTree::copyToValueVector(ValueVector* vec, uint64_t dstOffset,
                                          uint64_t count) {
...
bool ArrowNullMaskTree::copyFromBuffer(const void* buffer, uint64_t srcOffset,
                                       uint64_t count) {
  if (buffer == nullptr) {
    mask->setAllNonNull();
    return false;
...
void ArrowNullMaskTree::scanListPushDown(const ArrowSchema* schema,
  ...
  for (uint64_t i = 0; i < count; i++) {
    pushDownMask.setNullFromRange(offsets[i] - offsets[0],
                                  offsets[i + 1] - offsets[i], isNull(i));
  }
...
void ArrowNullMaskTree::scanArrayPushDown(const ArrowSchema* schema,
                                          const ArrowArray* array,
                                          uint64_t srcOffset, uint64_t count) {
  ...
  children->push_back(
      ArrowNullMaskTree(schema->children[0], array->children[0],
                        srcOffset * numElements + array->children[0]->offset,
                        auxiliaryLength, &pushDownMask));
}

void ArrowNullMaskTree::scanStructPushDown(const ArrowSchema* schema,
                                           const ArrowArray* array,
                                           uint64_t srcOffset, uint64_t count) {
...
ArrowNullMaskTree::ArrowNullMaskTree(const ArrowSchema* schema,
                                     const ArrowArray* array,
                                     uint64_t srcOffset, uint64_t count,
                                     const NullMask* parentBitmap)
    : offset{0},
      mask{std::make_shared<common::NullMask>(count)},
  ...
  if (schema->dictionary != nullptr) {
    ...
    dictionary = std::make_shared<ArrowNullMaskTree>(
        schema->dictionary, array->dictionary, array->dictionary->offset,
  ...
  case '+':
    ...
    case 'u': {
      ...
      if (schema->format[2] == 'd') {
        ...
        for (int64_t i = 0; i < array->n_children; i++) {
          children->push_back(
              ArrowNullMaskTree(schema->children[i], array->children[i],
    ...
    case 'v':
      // list views *suck*, especially when trying to write code that can
      // support parallelization for this, we generate child NullMaskTrees on
      // the fly, rather than attempt any precomputation
      if (array->buffers[0] == nullptr) {
        mask->setAllNonNull();
      } else {
        mask->copyFromNullBits((const uint64_t*) array->buffers[0], srcOffset,
                               0, count, true);
      }
      if (parentBitmap != nullptr) {
        *mask |= *parentBitmap;
      ...
      break;
    case 'r':
      // it's better to resolve validity during the actual scanning for run-end
      // encoded arrays so for this, let's just resolve child validities and
      // move on
      for (int64_t i = 0; i < array->n_children; i++) {
        children->push_back(ArrowNullMaskTree(
            schema->children[i], array->children[i], array->children[i]->offset,
