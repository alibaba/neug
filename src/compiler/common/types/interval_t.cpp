...
#include "src/include/common/exception/overflow.h"
#include "src/include/common/string_utils.h"
#include "src/include/common/types/cast_helpers.h"
#include "src/include/common/types/timestamp_t.h"
#include "src/include/function/arithmetic/add.h"
#include "src/include/function/arithmetic/multiply.h"
...
bool interval_t::operator==(const interval_t& rhs) const {
  return this->days == rhs.days && this->months == rhs.months &&
         this->micros == rhs.micros;
}
...
date_t interval_t::operator+(const date_t& rhs) const { return rhs + *this; }
...
interval_t interval_t::operator/(const uint64_t& rhs) const {
  ...
  int32_t daysRemainder =
      (days + monthsRemainder * Interval::DAYS_PER_MONTH) % rhs;
...
void Interval::addition(interval_t& result, uint64_t number,
                        std::string specifierStr) {
  StringUtils::toLower(specifierStr);
  if (specifierStr == "year" || specifierStr == "years" ||
      specifierStr == "y") {
    result.months += number * MONTHS_PER_YEAR;
  } else if (specifierStr == "month" || specifierStr == "months" ||
             specifierStr == "mon") {
    result.months += number;
  } else if (specifierStr == "day" || specifierStr == "days" ||
             specifierStr == "d") {
    result.days += number;
  } else if (specifierStr == "hour" || specifierStr == "hours" ||
             specifierStr == "h") {
    result.micros += number * MICROS_PER_HOUR;
  } else if (specifierStr == "minute" || specifierStr == "minutes" ||
             specifierStr == "m") {
    result.micros += number * MICROS_PER_MINUTE;
  } else if (specifierStr == "second" || specifierStr == "seconds" ||
             specifierStr == "s") {
  ...
  for (; pos < len; pos++) {
    ...
    if (isdigit(c)) {
      ...
      function::CastString::operation(
          ku_string_t{str + startPos, pos - startPos}, number);
      fraction = 0;
      if (c == '.') {
...
void Interval::normalizeIntervalEntries(interval_t input, int64_t& months,
...
void Interval::tryGetDatePartSpecifier(std::string specifier,
                                       DatePartSpecifier& result) {
  StringUtils::toLower(specifier);
  if (specifier == "year" || specifier == "yr" || specifier == "y" ||
      specifier == "years" || specifier == "yrs") {
    result = DatePartSpecifier::YEAR;
  } else if (specifier == "month" || specifier == "mon" ||
             specifier == "months" || specifier == "mons") {
  ...
  } else if (specifier == "decade" || specifier == "dec" ||
             specifier == "decades" || specifier == "decs") {
    result = DatePartSpecifier::DECADE;
  } else if (specifier == "century" || specifier == "cent" ||
             specifier == "centuries" || specifier == "c") {
    result = DatePartSpecifier::CENTURY;
  } else if (specifier == "millennium" || specifier == "mil" ||
             specifier == "millenniums" || specifier == "millennia" ||
             specifier == "mils" || specifier == "millenium" ||
             specifier == "milleniums") {
    result = DatePartSpecifier::MILLENNIUM;
  } else if (specifier == "microseconds" || specifier == "microsecond" ||
             specifier == "us" || specifier == "usec" || specifier == "usecs" ||
             specifier == "usecond" || specifier == "useconds") {
    result = DatePartSpecifier::MICROSECOND;
  } else if (specifier == "milliseconds" || specifier == "millisecond" ||
             specifier == "ms" || specifier == "msec" || specifier == "msecs" ||
             specifier == "msecond" || specifier == "mseconds") {
    result = DatePartSpecifier::MILLISECOND;
  } else if (specifier == "second" || specifier == "sec" ||
             specifier == "seconds" || specifier == "secs" ||
             specifier == "s") {
    result = DatePartSpecifier::SECOND;
  } else if (specifier == "minute" || specifier == "min" ||
             specifier == "minutes" || specifier == "mins" ||
             specifier == "m") {
...
int32_t Interval::getIntervalPart(DatePartSpecifier specifier,
                                  interval_t interval) {
  switch (specifier) {
  case DatePartSpecifier::YEAR:
  ...
  case DatePartSpecifier::QUARTER:
    return getIntervalPart(DatePartSpecifier::MONTH, interval) /
...
const regex::RE2& Interval::regexPattern1() {
  static regex::RE2 retval(
      "(?i)((0|[1-9]\\d*) "
      "+(YEARS?|YRS?|Y|MONS?|MONTHS?|DAYS?|D|DAYOFMONTH|DECADES?|DECS?|CENTURY|"
      "CENTURIES|CENT|C|"
      "MILLENN?IUMS?|MILS?|MILLENNIA|MICROSECONDS?|US|USECS?|USECONDS?|"
