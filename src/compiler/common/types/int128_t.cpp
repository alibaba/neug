...
#include "src/include/common/numeric_utils.h"
...
int128_t Int128_t::divModPositive(int128_t lhs, uint64_t rhs,
                                  uint64_t& remainder) {
  int128_t result{};
  result.low = 0;
  result.high = 0;
...
bool Int128_t::subInPlace(int128_t& lhs, int128_t rhs) {
  ...
  if (rhs.high >= 0) {
    ...
    if (lhs.high > INT64_MIN &&
        lhs.high - 1 >= INT64_MAX + rhs.high + underflow) {
      return false;
    }
    lhs.high = lhs.high - rhs.high - underflow;
  }
...
bool Int128_t::tryMultiply(int128_t lhs, int128_t rhs, int128_t& result) {
  ...
  uint64_t top[4] = {uint64_t(lhs.high) >> 32, uint64_t(lhs.high) & 0xffffffff,
                     lhs.low >> 32, lhs.low & 0xffffffff};
  uint64_t bottom[4] = {uint64_t(rhs.high) >> 32,
                        uint64_t(rhs.high) & 0xffffffff, rhs.low >> 32,
                        rhs.low & 0xffffffff};
...
int128_t Int128_t::divMod(int128_t lhs, int128_t rhs, int128_t& remainder) {
  ...
  for (uint8_t x = PositiveInt128BitsAmount(lhs); x > 0; x--) {
    ...
    // we get the value of the bit at position X, where position 0 is the
    ...
    if (greaterThanOrEquals(remainder, rhs)) {
      // the remainder has passed the division multiplier: add one to the divide
      // result
...
int128_t Int128_t::RightShift(int128_t lhs, int amount) {
  ...
  return amount >= 64
             ...
             : amount == 0
                   ? lhs
                   : int128_t((lhs.low >> amount) | (lhs.high << (64 - amount)),
                              lhs.high >> amount);
}
...
bool TryCastInt128Template(int128_t input, DST& result) {
  ...
  case -1:
    if constexpr (!SIGNED) {
      throw common::OverflowException("Cast failed. Cannot cast " +
    ...
    if (input.low >= function::NumericLimits<uint64_t>::maximum() -
      ...
      result =
          -DST(function::NumericLimits<uint64_t>::maximum() - input.low) - 1;
      return true;
    }
    break;
  default:
...
bool CastInt128ToFloating(int128_t input, REAL_T& result) {
  ...
  case -1:
    result =
        -REAL_T(function::NumericLimits<uint64_t>::maximum() - input.low) - 1;
    break;
  default:
    result = REAL_T(input.high) *
                 REAL_T(function::NumericLimits<uint64_t>::maximum()) +
             REAL_T(input.low);
    break;
  }
  return true;
...
bool castFloatingToInt128(REAL_T value, int128_t& result) {
  // TODO: Maybe need to add func isFinite in value.h to see if every type is
  // finite.
...
int128_t int128_t::operator-() const { return Int128_t::negate(*this); }
...
int128_t operator~(const int128_t& val) { return Int128_t::BinaryNot(val); }
...
int128_t::operator int64_t() const { return NarrowCast<int64_t>(*this); }

int128_t::operator int32_t() const { return NarrowCast<int32_t>(*this); }

int128_t::operator int16_t() const { return NarrowCast<int16_t>(*this); }

int128_t::operator int8_t() const { return NarrowCast<int8_t>(*this); }

int128_t::operator uint64_t() const { return NarrowCast<uint64_t>(*this); }

int128_t::operator uint32_t() const { return NarrowCast<uint32_t>(*this); }

int128_t::operator uint16_t() const { return NarrowCast<uint16_t>(*this); }

int128_t::operator uint8_t() const { return NarrowCast<uint8_t>(*this); }
