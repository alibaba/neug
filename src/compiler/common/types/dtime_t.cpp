...
dtime_t::operator int64_t() const { return micros; }

dtime_t::operator double() const { return micros; }
...
bool Time::tryConvertInternal(const char* buf, uint64_t len, uint64_t& pos,
                              dtime_t& result) {
...
bool Time::tryConvertInterval(const char* buf, uint64_t len, uint64_t& pos,
                              dtime_t& result) {
...
bool Time::tryConvertTime(const char* buf, uint64_t len, uint64_t& pos,
                          dtime_t& result) {
  if (!Time::tryConvertInternal(buf, len, pos, result)) {
    return false;
  }
...
std::string Time::toString(dtime_t time) {
  ...
  Time::convert(time, time_units[0], time_units[1], time_units[2],
                time_units[3]);
...
bool Time::isValid(int32_t hour, int32_t minute, int32_t second,
                   int32_t microseconds) {
  if (hour > 23 || hour < 0 || minute > 59 || minute < 0 || second > 59 ||
      second < 0 || microseconds > 999999 || microseconds < 0) {
    return false;
  }
  return true;
}
