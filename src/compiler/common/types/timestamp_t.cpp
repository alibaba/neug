...
timestamp_t::operator int64_t() const { return value; }
...
bool timestamp_t::operator==(const date_t& rhs) const { return rhs == *this; }
...
timestamp_t timestamp_t::operator+(const interval_t& interval) const {
  ...
  int64_t diff =
      interval.micros -
      ((interval.micros / Interval::MICROS_PER_DAY) * Interval::MICROS_PER_DAY);
  time.micros += diff;
...
bool Timestamp::tryConvertTimestamp(const char* str, uint64_t len,
                                    timestamp_t& result) {
  uint64_t pos = 0;
  date_t date;
  dtime_t time;
  ...
  if (pos < len) {
    ...
    if (Timestamp::tryParseUTCOffset(str, pos, len, hour_offset,
                                     minute_offset)) {
...
bool Timestamp::tryParseUTCOffset(const char* str, uint64_t& pos, uint64_t len,
...
date_t Timestamp::getDate(timestamp_t timestamp) {
  return date_t((timestamp.value + (timestamp.value < 0)) /
...
int32_t Timestamp::getTimestampPart(DatePartSpecifier specifier,
...
timestamp_t Timestamp::trunc(DatePartSpecifier specifier,
                             timestamp_t timestamp) {
...
int64_t Timestamp::getEpochNanoSeconds(const timestamp_t& timestamp) {
  int64_t result = 0;
  function::Multiply::operation(timestamp.value, Interval::NANOS_PER_MICRO,
                                result);
  return result;
}
