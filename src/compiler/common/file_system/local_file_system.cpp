...
static void validateFileFlags(uint8_t flags) {
  ...
  NEUG_ASSERT(isRead || isWrite);
  // CREATE flags require writing.
  NEUG_ASSERT(isWrite || !(flags & FileFlags::CREATE_IF_NOT_EXISTS));
  NEUG_ASSERT(isWrite || !(flags & FileFlags::CREATE_AND_TRUNCATE_IF_EXISTS));
  // CREATE_IF_NOT_EXISTS and CREATE_AND_TRUNCATE_IF_EXISTS flags cannot be
  // combined.
  NEUG_ASSERT(!(flags & FileFlags::CREATE_IF_NOT_EXISTS &&
...
std::unique_ptr<FileInfo> LocalFileSystem::openFile(
  ...
  if (writeMode) {
    NEUG_ASSERT(fileFlags & FileFlags::WRITE);
  ...
  HANDLE handle =
      CreateFileA(fullPath.c_str(), dwDesiredAccess, dwShareMode, nullptr,
                  dwCreationDisposition, FILE_ATTRIBUTE_NORMAL, nullptr);
  if (handle == INVALID_HANDLE_VALUE) {
    THROW_IO_EXCEPTION(stringFormat(
        "Cannot open file. path: {} - Error {}: {}", fullPath, GetLastError(),
  ...
  if (flags.lockType != FileLockType::NO_LOCK) {
    ...
    overlapped.Offset = 0;
    BOOL rc = LockFileEx(handle, dwFlags, 0 /*reserved*/, 1 /*numBytesLow*/,
                         0 /*numBytesHigh*/, &overlapped);
    if (!rc) {
      THROW_IO_EXCEPTION(
          "Could not set lock on file : " + fullPath + "\n" +
          "See the docs: https://docs.kuzudb.com/concurrency for "
          "more information.");
    }
  }
...
std::vector<std::string> LocalFileSystem::glob(main::ClientContext* context,
  ...
  if (path[0] == '/' || (std::isalpha(path[0]) && path[1] == ':')) {
    ...
    auto homeDirectory =
        context->getCurrentSetting(main::HomeDirectorySetting::name)
            .getValue<std::string>();
    ...
    if (!globbedPaths.empty()) {
      ...
      auto fileSearchPath =
          context->getCurrentSetting(main::FileSearchPathSetting::name)
              .getValue<std::string>();
      if (fileSearchPath != "") {
        auto searchPaths = common::StringUtils::split(fileSearchPath, ",");
        for (auto& searchPath : searchPaths) {
          pathsToGlob.push_back(
              common::stringFormat("{}/{}", searchPath, path));
        }
      }
    }
  }
...
void LocalFileSystem::overwriteFile(const std::string& from,
                                    const std::string& to) {
  ...
  if (!std::filesystem::copy_file(
    ...
    // LCOV_EXCL_START
    THROW_IO_EXCEPTION(
        stringFormat("Error copying file {} to {}.  ErrorMessage: {}", from, to,
                     errorCode.message()));
    // LCOV_EXCL_STOP
  }
}
...
void LocalFileSystem::copyFile(const std::string& from, const std::string& to) {
  if (!fileOrPathExists(from))
    return;
  std::error_code errorCode;
  if (!std::filesystem::copy_file(from, to, std::filesystem::copy_options::none,
                                  errorCode)) {
    // LCOV_EXCL_START
    THROW_IO_EXCEPTION(
        stringFormat("Error copying file {} to {}.  ErrorMessage: {}", from, to,
...
void LocalFileSystem::createDir(const std::string& dir) const {
  try {
    if (std::filesystem::exists(dir)) {
      // LCOV_EXCL_START
      THROW_IO_EXCEPTION(stringFormat("Directory {} already exists.", dir));
      // LCOV_EXCL_STOP
    }
    ...
    if (directoryToCreate.ends_with('/')
      ...
      // https://github.com/llvm/llvm-project/issues/60634). We have to manually
      // remove the last '/' if the path ends with '/'. (Added the second one
      // for windows)
      directoryToCreate =
    ...
    if (errCode) {
      // LCOV_EXCL_START
      THROW_IO_EXCEPTION(
          stringFormat("Failed to create directory: {}, error message: {}.",
                       dir, errCode.message()));
      // LCOV_EXCL_STOP
    }
  } catch (std::exception& e) {
    // LCOV_EXCL_START
    THROW_IO_EXCEPTION(stringFormat("Failed to create directory {} due to: {}",
                                    dir, e.what()));
    // LCOV_EXCL_STOP
  }
}

bool isSubdirectory(const std::filesystem::path& base,
  ...
  try {
    ...
    std::string relative =
        std::filesystem::relative(canonicalSub, canonicalBase).string();
...
void LocalFileSystem::removeFileIfExists(const std::string& path) {
  ...
  if (!isSubdirectory(homeDir, path)) {
    THROW_IO_EXCEPTION(stringFormat(
        "Error: Path {} is not within the allowed home directory {}", path,
        homeDir));
  }
  std::error_code errCode;
  bool success = false;
  ...
  if (!success) {
    // LCOV_EXCL_START
    THROW_IO_EXCEPTION(
        stringFormat("Error removing directory or file {}.  Error Message: {}",
...
void LocalFileSystem::readFromFile(FileInfo& fileInfo, void* buffer,
  ...
  if (!ReadFile((HANDLE) localFileInfo->handle, buffer, numBytes, &numBytesRead,
                &overlapped)) {
    auto error = GetLastError();
    THROW_IO_EXCEPTION(stringFormat(
        "Cannot read from file: {} handle: {} "
        "numBytesRead: {} numBytesToRead: {} position: {}. Error {}: {}",
        fileInfo.path, (intptr_t) localFileInfo->handle, numBytesRead, numBytes,
        position, error, std::system_category().message(error)));
  }
  if (numBytesRead != numBytes &&
      fileInfo.getFileSize() != position + numBytesRead) {
    THROW_IO_EXCEPTION(
        stringFormat("Cannot read from file: {} handle: {} "
                     "numBytesRead: {} numBytesToRead: {} position: {}",
                     fileInfo.path, (intptr_t) localFileInfo->handle,
                     numBytesRead, numBytes, position));
  }
#else
  ...
  if ((uint64_t) numBytesRead != numBytes &&
    ...
    // LCOV_EXCL_START
    THROW_IO_EXCEPTION(stringFormat(
        "Cannot read from file: {} fileDescriptor: {} "
        "numBytesRead: {} numBytesToRead: {} position: {}",
...
int64_t LocalFileSystem::readFile(FileInfo& fileInfo, void* buf,
                                  size_t nbyte) const {
  auto localFileInfo = fileInfo.constPtrCast<LocalFileInfo>();
#if defined(_WIN32)
  DWORD numBytesRead;
...
void LocalFileSystem::writeFile(FileInfo& fileInfo, const uint8_t* buffer,
  ...
  while (remainingNumBytesToWrite > 0) {
    uint64_t numBytesToWrite =
        std::min(remainingNumBytesToWrite, maxBytesToWriteAtOnce);
    ...
    if (!WriteFile((HANDLE) localFileInfo->handle, buffer + bufferOffset,
                   numBytesToWrite, &numBytesWritten, &overlapped)) {
      auto error = GetLastError();
      THROW_IO_EXCEPTION(stringFormat(
          "Cannot write to file. path: {} handle: {} offsetToWrite: {} "
          "numBytesToWrite: {} numBytesWritten: {}. Error {}: {}.",
          fileInfo.path, (intptr_t) localFileInfo->handle, offset,
          numBytesToWrite, numBytesWritten, error,
          std::system_category().message(error)));
    }
#else
    auto numBytesWritten = pwrite(localFileInfo->fd, buffer + bufferOffset,
                                  numBytesToWrite, offset);
    if (numBytesWritten != (int64_t) numBytesToWrite) {
      // LCOV_EXCL_START
      THROW_IO_EXCEPTION(stringFormat(
          "Cannot write to file. path: {} fileDescriptor: {} offsetToWrite: {} "
          "numBytesToWrite: {} numBytesWritten: {}. Error: {}",
          fileInfo.path, localFileInfo->fd, offset, numBytesToWrite,
          numBytesWritten, posixErrMessage()));
      // LCOV_EXCL_STOP
    }
#endif
...
void LocalFileSystem::syncFile(const FileInfo& fileInfo) const {
  auto localFileInfo = fileInfo.constPtrCast<LocalFileInfo>();
#if defined(_WIN32)
  // Note that `FlushFileBuffers` returns 0 when fail, while `fsync` returns 0
  // when succeed.
  if (FlushFileBuffers((HANDLE) localFileInfo->handle) == 0) {
    auto error = GetLastError();
    THROW_IO_EXCEPTION(stringFormat("Failed to sync file {}. Error {}: {}",
                                    fileInfo.path, error,
                                    std::system_category().message(error)));
  }
#else
  if (fsync(localFileInfo->fd) != 0) {
    THROW_IO_EXCEPTION(stringFormat("Failed to sync file {}.", fileInfo.path));
  }
#endif
}

int64_t LocalFileSystem::seek(FileInfo& fileInfo, uint64_t offset,
                              int whence) const {
  auto localFileInfo = fileInfo.constPtrCast<LocalFileInfo>();
#if defined(_WIN32)
...
void LocalFileSystem::truncate(FileInfo& fileInfo, uint64_t size) const {
  ...
  if (SetFilePointer((HANDLE) localFileInfo->handle, size & 0xffffffff,
                     offsetHighPtr, FILE_BEGIN) == INVALID_SET_FILE_POINTER) {
    auto error = GetLastError();
    THROW_IO_EXCEPTION(
        stringFormat("Cannot set file pointer for file: {} handle: {} "
                     "new position: {}. Error {}: {}",
                     fileInfo.path, (intptr_t) localFileInfo->handle, size,
                     error, std::system_category().message(error)));
  }
  if (!SetEndOfFile((HANDLE) localFileInfo->handle)) {
    auto error = GetLastError();
    THROW_IO_EXCEPTION(
        stringFormat("Cannot truncate file: {} handle: {} "
                     "size: {}. Error {}: {}",
                     fileInfo.path, (intptr_t) localFileInfo->handle, size,
                     error, std::system_category().message(error)));
  }
  ...
  if (ftruncate(localFileInfo->fd, size) < 0) {
    // LCOV_EXCL_START
    THROW_IO_EXCEPTION(stringFormat("Failed to truncate file {}: {}",
