#include "src/include/common/file_system/local_file_system.h"

#include "glob/glob.hpp"
#include "src/include/common/assert.h"
#include "src/include/common/exception/io.h"
#include "src/include/common/string_format.h"
#include "src/include/common/string_utils.h"
#include "src/include/common/system_message.h"
...
#include <io.h>
#include <windows.h>
#include "src/include/common/windows_utils.h"
#else
#include <unistd.h>
#include "sys/stat.h"
#endif
...
void LocalFileSystem::createDir(const std::string& dir) const {
  try {
    ...
    // LCOV_EXCL_START
    throw IOException(stringFormat("Failed to create directory {} due to: {}",
                                   dir, e.what()));
    // LCOV_EXCL_STOP
  }
}
...
void LocalFileSystem::syncFile(const FileInfo& fileInfo) const {
  ...
  if (FlushFileBuffers((HANDLE) localFileInfo->handle) == 0) {
    auto error = GetLastError();
    throw IOException(stringFormat("Failed to sync file {}. Error {}: {}",
                                   fileInfo.path, error,
                                   std::system_category().message(error)));
  }
#else
...
void LocalFileSystem::truncate(FileInfo& fileInfo, uint64_t size) const {
  ...
  if (ftruncate(localFileInfo->fd, size) < 0) {
    // LCOV_EXCL_START
    throw IOException(stringFormat("Failed to truncate file {}: {}",
