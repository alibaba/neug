...
bool NullMask::copyNullMask(const uint64_t* srcNullEntries, uint64_t srcOffset,
                            uint64_t* dstNullEntries, uint64_t dstOffset,
  ...
  // If both offsets are aligned relative to each other then copy up to the
  // first byte using the non-aligned method, then copy aligned, then copy the
  // end unaligned again.
  if (!invert && (srcOffset % 8 == dstOffset % 8) && numBitsToCopy >= 8 &&
    ...
    auto* src = reinterpret_cast<const uint8_t*>(srcNullEntries) +
                (srcOffset + numBitsInFirstByte) / 8;
    auto* dst = reinterpret_cast<uint8_t*>(dstNullEntries) +
                (dstOffset + numBitsInFirstByte) / 8;
    auto numBytesForAlignedCopy = (numBitsToCopy - numBitsInFirstByte) / 8;
    memcpy(dst, src, numBytesForAlignedCopy);
    if (std::any_of(src, src + numBytesForAlignedCopy,
                    [&](uint8_t val) { return val != 0; })) {
      hasNullInSrcNullMask = true;
    }
    auto lastByteStart = numBitsInFirstByte + numBytesForAlignedCopy * 8;
    auto numBitsInLastByte =
        numBitsToCopy - numBitsInFirstByte - numBytesForAlignedCopy * 8;
    if (numBitsInLastByte > 0) {
      return copyUnaligned(srcNullEntries, srcOffset + lastByteStart,
                           dstNullEntries, dstOffset + lastByteStart,
                           numBitsInLastByte, false) ||
...
bool NullMask::copyUnaligned(const uint64_t* srcNullEntries, uint64_t srcOffset,
  ...
  uint64_t bitPos = 0;
  bool hasNullInSrcNullMask = false;
  auto [srcNullEntryPos, srcNullBitPos] =
      getNullEntryAndBitPos(srcOffset + bitPos);
  auto [dstNullEntryPos, dstNullBitPos] =
      getNullEntryAndBitPos(dstOffset + bitPos);
  while (bitPos < numBitsToCopy) {
    ...
    uint64_t srcNullMaskEntry = invert ? ~srcNullEntries[srcNullEntryPos]
                                       : srcNullEntries[srcNullEntryPos];
    if (dstNullBitPos < srcNullBitPos) {
      numBitsToReadInCurrentEntry =
          std::min(NullMask::NUM_BITS_PER_NULL_ENTRY - srcNullBitPos,
                   numBitsToCopy - bitPos);
      ...
      numBitsToReadInCurrentEntry =
          std::min(NullMask::NUM_BITS_PER_NULL_ENTRY - dstNullBitPos,
                   numBitsToCopy - bitPos);
      // Mask lower bits out of current read range to 0.
...
bool NullMask::copyFromNullBits(const uint64_t* srcNullEntries,
                                uint64_t srcOffset, uint64_t dstOffset,
                                uint64_t numBitsToCopy, bool invert) {
  NEUG_ASSERT(dstOffset + numBitsToCopy <= getNumNullBits(data));
  if (copyNullMask(srcNullEntries, srcOffset, this->data.data(), dstOffset,
                   numBitsToCopy, invert)) {
...
void NullMask::setNullFromRange(uint64_t offset, uint64_t numBitsToSet,
                                bool isNull) {
  if (isNull) {
    this->mayContainNulls = true;
  }
  NEUG_ASSERT(offset + numBitsToSet <= getNumNullBits(data));
  setNullRange(data.data(), offset, numBitsToSet, isNull);
}

void NullMask::setNullRange(uint64_t* nullEntries, uint64_t offset,
                            uint64_t numBitsToSet, bool isNull) {
  if (numBitsToSet == 0) {
    return;
  }

  auto [firstEntryPos, firstBitPos] = getNullEntryAndBitPos(offset);
  auto [lastEntryPos, lastBitPos] =
      getNullEntryAndBitPos(offset + numBitsToSet);

  // If the range spans multiple entries, set the entries in the middle to the
  // appropriate value with std::fill
  ...
  if (firstEntryPos == lastEntryPos) {
    if (isNull) {
      ...
      nullEntries[firstEntryPos] |=
          (~NULL_LOWER_MASKS[firstBitPos] &
...
void NullMask::operator|=(const NullMask& other) {
  NEUG_ASSERT(other.data.size() == data.size());
  for (size_t i = 0; i < data.size(); i++) {
    data[i] |= other.getData()[i];
  }
}

std::pair<bool, bool> NullMask::getMinMax(const uint64_t* nullEntries,
                                          uint64_t offset, uint64_t numValues) {
  nullEntries += offset / NUM_BITS_PER_NULL_ENTRY;
  offset = offset % NUM_BITS_PER_NULL_ENTRY;

  // If the offset+numValues are both within a word, just combine the
  // appropriate masks and compare to 0/mask to determine if they are all 1s/all
  // 0s (else a mix)
  ...
  // If the range spans multiple entries, check the first one by masking the
  // start
  ...
  // Check central full bytes, which can be compared in a single operation since
  // we don't ignore any bits If there was no offset, then we calculate the
  // baseline based on the first bit, and compare that to the actual entry
...
uint64_t NullMask::countNulls() const {
  // If capacity % 64 != 0 then there may be unused bits at the end of the last
  // entry, but these should always be 0.
  uint64_t sum = 0;
  for (auto entry : data) {
