...
void FunctionSignatureRegistry::registerScalar(
    const std::string& signature, const runtime::neug_func_exec_t& func) {
  auto& inst = instance();
  std::unique_lock lk(inst.mutex_);
  auto it = inst.sig_2_scalar_func_.find(signature);
  if (it != inst.sig_2_scalar_func_.end()) {
    LOG(WARNING) << "registerScalar: signature [" << signature
...
runtime::neug_func_exec_t FunctionSignatureRegistry::lookup(
