...
#include "src/include/common/serializer/buffered_serializer.h"
...
void IndexCatalogEntry::serialize(common::Serializer& serializer) const {
  ...
  if (isLoaded()) {
    ...
    serializer.write(bufferedWriter->getData().data.get(),
...
std::unique_ptr<IndexCatalogEntry> IndexCatalogEntry::deserialize(
  ...
  auto indexEntry = std::make_unique<IndexCatalogEntry>(
      type, tableID, std::move(indexName), std::move(propertyIDs),
      nullptr /* auxInfo */);
...
std::unique_ptr<common::BufferReader> IndexCatalogEntry::getAuxBufferReader()
    const {
  // LCOV_EXCL_START
  if (!auxBuffer) {
