...
struct UnaryUDFExecutor {
  ...
  static inline void operation(OPERAND_TYPE& input, RESULT_TYPE& result,
                               void* udfFunc) {
...
struct BinaryUDFExecutor {
  ...
  static inline void operation(LEFT_TYPE& left, RIGHT_TYPE& right,
                               RESULT_TYPE& result, void* udfFunc) {
...
struct UDF {
  ...
  static bool templateValidateType(const common::LogicalTypeID& type) {
    ...
    auto physicalTypeMatch = common::TypeUtils::visit(
        physicalType,
        []<typename T1>(T1) { return std::is_same<T, T1>::value; });
    auto logicalTypeMatch = common::TypeUtils::visit(
        logicalType,
        []<typename T1>(T1) { return std::is_same<T, T1>::value; });
    ...
    function::scalar_func_exec_t execFunc =
      ...
      BinaryFunctionExecutor::executeSwitch<LEFT_TYPE, RIGHT_TYPE, RESULT_TYPE,
          ...
          *params[0], paramSelVectors[0], *params[1], paramSelVectors[1],
          result, resultSelVector, (void*) udfFunc);
    };
    return execFunc;
  }
  ...
  static function::scalar_func_exec_t createTernaryExecFunc(
    ...
    function::scalar_func_exec_t execFunc =
      ...
      TernaryFunctionExecutor::executeSwitch<A_TYPE, B_TYPE, C_TYPE,
          ...
          *params[0], paramSelVectors[0], *params[1], paramSelVectors[1],
          *params[2], paramSelVectors[2], result, resultSelVector,
          (void*) udfFunc);
    };
    return execFunc;
  }
  ...
  static scalar_func_exec_t getScalarExecFunc(
    ...
    switch (numArgs) {
    case 0:
      return createEmptyParameterExecFunc<TR, Args...>(
          udfFunc, std::move(parameterTypes));
    case 1:
    ...
    case 3:
      return createTernaryExecFunc<TR, Args...>(udfFunc,
                                                std::move(parameterTypes));
    default:
      THROW_BINDER_EXCEPTION("UDF function only supported until ternary!");
    }
  }
  ...
  static void getParameterTypesRecursive(
      std::vector<common::LogicalTypeID>& arguments) {
  ...
  template <typename TR, typename... Args>
  static function_set getFunction(
      std::string name, TR (*udfFunc)(Args...),
      std::vector<common::LogicalTypeID> parameterTypes,
    ...
    scalar_func_exec_t scalarExecFunc =
        getScalarExecFunc<TR, Args...>(udfFunc, parameterTypes);
    definitions.push_back(std::make_unique<function::ScalarFunction>(
        std::move(name), std::move(parameterTypes), returnType,
        std::move(scalarExecFunc)));
    return definitions;
  }

  template <typename TR, typename... Args>
  static function_set getFunction(std::string name, TR (*udfFunc)(Args...)) {
    return getFunction<TR, Args...>(std::move(name), udfFunc,
                                    getParameterTypes<Args...>(),
                                    getParameterType<TR>());
  }

  template <typename TR, typename... Args>
  static function_set getVectorizedFunction(std::string name,
                                            scalar_func_exec_t execFunc) {
    function_set definitions;
    definitions.push_back(std::make_unique<function::ScalarFunction>(
        std::move(name), getParameterTypes<Args...>(), getParameterType<TR>(),
        std::move(execFunc)));
    return definitions;
  }

  static function_set getVectorizedFunction(
      std::string name, scalar_func_exec_t execFunc,
      std::vector<common::LogicalTypeID> parameterTypes,
      common::LogicalTypeID returnType) {
    function_set definitions;
    definitions.push_back(std::make_unique<function::ScalarFunction>(
