...
std::vector<std::string> StringUtils::splitComma(const std::string& input) {
  ...
  result.push_back(input.substr(currentPos == input.length() ? input.length()
                                                             : currentPos + 1));
  return result;
}
...
std::vector<std::string_view> StringUtils::smartSplit(std::string_view input,
                                                      char splitChar,
                                                      uint64_t maxNumEle) {
  if (input.size() == 0) {
    return {};
...
uint64_t findDelim(const std::string& input, const std::string& delimiter,
                   uint64_t prevPos) {
  if (delimiter != "") {
...
static void changeCase(std::string& input) {
  if (!utf8proc::Utf8Proc::isValid(input.c_str(), input.length())) {
    THROW_RUNTIME_ERROR("Invalid UTF8-encoded string.");
  }
  ...
  function::BaseLowerUpperFunction::convertCase(
      (char*) result.data(), input.length(), input.data(), toUpper);
  input = result;
}

void StringUtils::toLower(std::string& input) { changeCase<false>(input); }

void StringUtils::toUpper(std::string& input) { changeCase<true>(input); }
...
std::string StringUtils::extractStringBetween(const std::string& input,
                                              char delimiterStart,
                                              char delimiterEnd,
...
std::string StringUtils::removeEscapedCharacters(const std::string& input) {
  ...
  for (auto i = 1u; i < input.length() - 1; i++) {
    // Antlr4 already guarantees that the character followed by the escaped
    // character is valid. So we can safely skip the escaped character.
    if (input[i] == '\\') {
      i++;
    }
    resultStr += input[i];
...
bool StringUtils::caseInsensitiveEquals(std::string_view left,
                                        std::string_view right) {
  if (left.size() != right.size()) {
    return false;
  }
  for (auto c = 0u; c < left.size(); c++) {
    if (asciiToLowerCaseMap[(uint8_t) left[c]] !=
        asciiToLowerCaseMap[(uint8_t) right[c]]) {
      return false;
    }
  }
  return true;
...
std::string StringUtils::join(const std::span<const std::string_view> input,
  ...
  return StringUtils::join(
      input, input.size(), separator,
      [](const std::string_view s) { return std::string{s}; });
}

template <typename C, typename S, typename Func>
std::string StringUtils::join(const C& input, S count,
                              const std::string& separator, Func f) {
  std::string result;
...
std::string StringUtils::rtrimNewlines(const std::string& input) {
  ...
  s.erase(find_if(s.rbegin(), s.rend(),
                  [](unsigned char ch) { return !characterIsNewLine(ch); })
              .base(),
          s.end());
  return s;
}
