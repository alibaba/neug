...
using subquery_graph_set_t =
    std::unordered_set<SubqueryGraph, SubqueryGraphHasher>;
template <typename T>
using subquery_graph_V_map_t =
    std::unordered_map<SubqueryGraph, T, SubqueryGraphHasher>;
...
struct SubqueryGraph {
  ...
  explicit SubqueryGraph(const QueryGraph& queryGraph)
      : queryGraph{queryGraph} {}

  void addQueryNode(common::idx_t nodePos) {
  ...
  // E.g. query graph (a)-[e1]->(b) and subgraph (a)-[e1], although (b) is not
  // in subgraph, we return both (a) and (b) regardless of node selector. See
  // needPruneJoin() in join_order_enumerator.cpp for its use case.
  std::unordered_set<common::idx_t> getNodePositionsIgnoringNodeSelector()
      const;
...
class QueryGraph {
 public:
  ...
  std::shared_ptr<NodeExpression> getQueryNode(
      const std::string& queryNodeName) const {
  ...
  std::shared_ptr<RelExpression> getQueryRel(
      const std::string& queryRelName) const {
...
// QueryGraphCollection represents a pattern (a set of connected components)
// specified in MATCH clause.
class QueryGraphCollection {
 public:
  ...
  QueryGraph* getQueryGraphUnsafe(common::idx_t idx) {
    return &queryGraphs[idx];
  }
  const QueryGraph* getQueryGraph(common::idx_t idx) const {
    return &queryGraphs[idx];
  }
