...
std::pair<f_group_pos, f_group_pos_set> FlattenAllButOne::getGroupsPosToFlatten(
  ...
  for (auto expr : exprs) {
    auto analyzer =
        GroupDependencyAnalyzer(false /* collectDependentExpr */, schema);
    analyzer.visit(expr);
...
f_group_pos_set FlattenAllButOne::getGroupsPosToFlatten(
    std::shared_ptr<Expression> expr, const Schema& schema) {
  auto analyzer =
      GroupDependencyAnalyzer(false /* collectDependentExpr */, schema);
  analyzer.visit(expr);
...
f_group_pos_set FlattenAllButOne::getGroupsPosToFlatten(
    const std::unordered_set<f_group_pos>& dependentGroups,
    const Schema& schema) {
  f_group_pos_set result;
  std::vector<f_group_pos> candidates;
...
void GroupDependencyAnalyzer::visitFunction(
    std::shared_ptr<binder::Expression> expr) {
  auto& funcExpr = expr->constCast<ScalarFunctionExpression>();
  for (auto& child : expr->getChildren()) {
    visit(child);
  }
  // For list lambda we need to flatten all dependent expressions in lambda
  // function E.g. MATCH (a)->(b) RETURN list_filter(a.list, x -> x>b.age)
  if (funcExpr.getFunction().isListLambda) {
    auto lambdaFunctionAnalyzer =
        GroupDependencyAnalyzer(collectDependentExpr, schema);
    lambdaFunctionAnalyzer.visit(funcExpr.getChild(1));
    requiredFlatGroups = lambdaFunctionAnalyzer.getDependentGroups();
  }
}

void GroupDependencyAnalyzer::visitCase(
    std::shared_ptr<binder::Expression> expr) {
...
void GroupDependencyAnalyzer::visitNodeOrRel(
    std::shared_ptr<binder::Expression> expr) {
...
void GroupDependencyAnalyzer::visitSubquery(
    std::shared_ptr<binder::Expression> expr) {
