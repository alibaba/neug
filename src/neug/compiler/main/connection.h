...
#include "database.h"
#include "neug/compiler/function/udf_function.h"

namespace gs {
namespace main {

/**
 * @brief Connection is used to interact with a Database instance. Each
 * Connection is thread-safe. Multiple connections can connect to the same
 * Database instance in a multi-threaded environment.
 */
class Connection {
 ...
 public:
   ...
   * @param database A pointer to the database instance that this connection
  ...
  KUZU_API ~Connection();
  /**
   * @brief Sets the maximum number of threads to use for execution in the
   * current connection.
   * @param numThreads The number of threads to use for execution in the current
   * connection.
   */
  KUZU_API void setMaxNumThreadForExec(uint64_t numThreads);
  /**
   * @brief Returns the maximum number of threads to use for execution in the
   * current connection.
   * @return the maximum number of threads to use for execution in the current
   * connection.
   */
  ...
  /**
   * @brief Executes the given prepared statement with args and returns the
   * result.
   * @param preparedStatement The prepared statement to execute.
   * @param args The parameter pack where each arg is a std::pair with the first
   * element being parameter name and second element being parameter value.
   * @return the result of the query.
   */
  ...
  inline std::unique_ptr<QueryResult> execute(
    ...
    std::unordered_map<std::string, std::unique_ptr<common::Value>>
        inputParameters;
    return executeWithParams(preparedStatement, std::move(inputParameters),
                             args...);
  }
  /**
   * @brief Executes the given prepared statement with inputParams and returns
   * the result.
   * @param preparedStatement The prepared statement to execute.
   * @param inputParams The parameter pack where each arg is a std::pair with
   * the first element being parameter name and second element being parameter
   * value.
   * @return the result of the query.
   */
  KUZU_API std::unique_ptr<QueryResult> executeWithParams(
      PreparedStatement* preparedStatement,
      std::unordered_map<std::string, std::unique_ptr<common::Value>>
          inputParams);
  /**
  ...
  /**
   * @brief sets the query timeout value of the current connection. A value of
  ...
  // Note: this function throws exception if creating scalar function fails.
  template <typename TR, typename... Args>
  void createScalarFunction(std::string name,
                            std::vector<common::LogicalTypeID> parameterTypes,
                            common::LogicalTypeID returnType,
                            TR (*udfFunc)(Args...)) {
    addScalarFunction(
        name, function::UDF::getFunction<TR, Args...>(
  ...
  void createVectorizedFunction(std::string name,
    ...
    addScalarFunction(name, function::UDF::getVectorizedFunction<TR, Args...>(
                                name, std::move(scalarFunc)));
  }

  void createVectorizedFunction(
      std::string name, std::vector<common::LogicalTypeID> parameterTypes,
      common::LogicalTypeID returnType,
      function::scalar_func_exec_t scalarFunc) {
    addScalarFunction(name, function::UDF::getVectorizedFunction(
                                name, std::move(scalarFunc),
 ...
 private:
  ...
  std::unique_ptr<PreparedStatement> preparedStatementWithError(
  ...
  std::unique_ptr<QueryResult> executeWithParams(
    ...
    return clientContext->executeWithParams(preparedStatement,
                                            std::move(params), arg, args...);
  }

  void bindParametersNoLock(
      PreparedStatement* preparedStatement,
      const std::unordered_map<std::string, std::unique_ptr<common::Value>>&
          inputParams);

  std::unique_ptr<QueryResult> executeAndAutoCommitIfNecessaryNoLock(
      PreparedStatement* preparedStatement, uint32_t planIdx = 0u);

  KUZU_API void addScalarFunction(std::string name,
                                  function::function_set definitions);
  KUZU_API void removeScalarFunction(std::string name);

  std::unique_ptr<QueryResult> queryWithID(std::string_view query,
                                           uint64_t queryID);

  std::unique_ptr<QueryResult> executeWithParamsWithID(
      PreparedStatement* preparedStatement,
      std::unordered_map<std::string, std::unique_ptr<common::Value>>
          inputParams,
      uint64_t queryID);
