...
#include "neug/storages/metadata/local_file_metadata_store.h"
...
Result<bool> LocalFileMetadataStore::DeleteMeta(const meta_kind_t& meta_kind,
  ...
  if (!std::filesystem::remove(meta_file)) {
    return Status(StatusCode::ERR_IO_ERROR, "Failed to delete meta");
  }
  return true;
}
...
Result<bool> LocalFileMetadataStore::dump_file(
  ...
  if (!out_file.is_open()) {
    return Result<bool>(gs::StatusCode::ERR_IO_ERROR, false);
  }
  out_file << content;
  out_file.close();
...
Result<LocalFileMetadataStore::meta_value_t> LocalFileMetadataStore::read_file(
  ...
  if (!in_file.is_open()) {
    return Result<LocalFileMetadataStore::meta_value_t>(
        gs::StatusCode::ERR_IO_ERROR, "Failed to open file");
  }
...
Result<bool> LocalFileMetadataStore::create_directory(
  ...
  if (!std::filesystem::exists(dir)) {
    if (!std::filesystem::create_directory(dir)) {
      return Result<bool>(gs::StatusCode::ERR_IO_ERROR,
