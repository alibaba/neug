...
// In-place removal of invalidated edges within a single vertex's adjacency
// list; live entries keep their relative order and their timestamps are
// reset. Returns the new degree.
template <typename NBR_T>
int normalize_mutable_adjacency_list(NBR_T* data, int degree) {
  NBR_T* read_ptr = data;
  NBR_T* read_end = data + degree;
  NBR_T* write_ptr = data;
  int removed = 0;
...
// Parallel in-place normalization of every vertex's adjacency list. Each
// vertex owns a disjoint capacity buffer, so per-vertex work is independent;
// only the live-edge count is reduced across threads. Returns the total
// number of live edges.
template <typename NBR_T>
size_t parallel_normalize_mutable_adjacency_lists(NBR_T** adj_lists,
                                                  std::atomic<int>* degrees,
                                                  size_t vnum) {
  std::atomic<size_t> live_edges{0};
  csr_parallel::parallel_for_normalize_ranges(
      vnum, degrees,
      [adj_lists, degrees, &live_edges](size_t first, size_t last) {
        size_t local_live = 0;
        for (size_t i = first; i < last; ++i) {
          int degree = degrees[i].load(std::memory_order_relaxed);
          NBR_T* data = adj_lists[i];
          if (data == nullptr) {
            continue;
          }
          int new_degree = normalize_mutable_adjacency_list(data, degree);
          degrees[i].store(new_degree, std::memory_order_relaxed);
          local_live += static_cast<size_t>(new_degree);
        }
        if (local_live != 0) {
          live_edges.fetch_add(local_live, std::memory_order_relaxed);
        }
      });
  return live_edges.load(std::memory_order_relaxed);
}
...
void MutableCsr<EDATA_T>::Dump(Checkpoint& ckp, CheckpointManifest& meta,
  ...
  // Normalize in parallel first; the checksum/rewrite pass below must stay
  // serial because MD5 chaining requires the segments in vertex order.
  size_t live_edges =
      parallel_normalize_mutable_adjacency_lists(adj_lists, degrees, vnum);
  csr_dump::validate_edge_count(edge_num_.load(), live_edges);
  for (size_t i = 0; i < vnum; ++i) {
    nbr_file.AddSegment(reinterpret_cast<const char*>(adj_lists[i]),
...
void MutableCsr<EDATA_T>::compact() {
  auto** adj_lists = reinterpret_cast<nbr_t**>(adj_list_buffer_->GetData());
  auto* degrees = reinterpret_cast<std::atomic<int>*>(degree_list_->GetData());
  size_t live_edges = parallel_normalize_mutable_adjacency_lists(
      adj_lists, degrees, vertex_capacity());
  csr_dump::validate_edge_count(edge_num_.load(), live_edges);
}
...
void MutableCsr<EDATA_T>::batch_sort_by_edge_data(timestamp_t ts) {
  if (adj_list_buffer_ != nullptr) {
    ...
    auto** buf_arr = reinterpret_cast<nbr_t**>(adj_list_buffer_->GetData());
    auto* sz_arr = reinterpret_cast<std::atomic<int>*>(degree_list_->GetData());
    // Each vertex owns a disjoint adjacency list, so sorting can be
    // parallelized across vertex ranges without synchronization.
    csr_parallel::parallel_for_sort_ranges(
        vnum, sz_arr, [buf_arr, sz_arr](size_t first, size_t last) {
          for (size_t i = first; i < last; ++i) {
