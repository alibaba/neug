...
template <typename T>
std::unique_ptr<ColumnBase> MakeArrayColumn(std::unique_ptr<VecColumn<T>> vec) {
  auto array_type = vec->array_type();
  auto size = vec->size();
  auto buffer = vec->TakeBuffer();
  return ArrayColumn::FromBuffer<T>(array_type, std::move(buffer), size);
}

}  // namespace

template <typename T>
const ColumnBase* ReplaceVecColumnInternal(
    Table* table, size_t col, std::unique_ptr<VecColumn<T>> column) {
  if (!table) {
    THROW_RUNTIME_ERROR(
        "VertexTable::ReplaceVecColumn: table is not initialized");
  }
  if (!column) {
    THROW_INVALID_ARGUMENT_EXCEPTION(
        "VertexTable::ReplaceVecColumn: column must not be null");
  }
  auto* result = column.get();
  table->SetColumn(static_cast<int>(col), std::move(column));
  return result;
}

const ColumnBase* VertexTable::ReplaceVecColumn(
    size_t col, std::unique_ptr<VecColumn<float>> column) {
  return ReplaceVecColumnInternal(table_.get(), col, std::move(column));
}

const ColumnBase* VertexTable::ReplaceVecColumn(
    size_t col, std::unique_ptr<VecColumn<double>> column) {
  return ReplaceVecColumnInternal(table_.get(), col, std::move(column));
}

const ColumnBase* VertexTable::DegradeVecColumn(size_t col) {
  if (!table_) {
    THROW_RUNTIME_ERROR(
        "VertexTable::DegradeVecColumn: table is not initialized");
  }
  auto* column = table_->get_column_by_id(col);
  if (!column) {
  ...
  std::unique_ptr<ColumnBase> array;
  if (auto* vec = dynamic_cast<VecColumn<float>*>(owned.get())) {
    owned.release();
    array = MakeArrayColumn(std::unique_ptr<VecColumn<float>>(vec));
  } else if (auto* vec = dynamic_cast<VecColumn<double>*>(owned.get())) {
    owned.release();
    array = MakeArrayColumn(std::unique_ptr<VecColumn<double>>(vec));
  } else {
    table_->SetColumn(static_cast<int>(col), std::move(owned));
    THROW_INVALID_ARGUMENT_EXCEPTION(
        "VertexTable::DegradeVecColumn: target is not a VecColumn");
  }
  auto* result = array.get();
  table_->SetColumn(static_cast<int>(col), std::move(array));
  return result;
}

const ColumnBase* VertexTable::DegradeVecColumn(
  ...
  if (col < 0) {
    THROW_INVALID_ARGUMENT_EXCEPTION(
        "VertexTable::DegradeVecColumn: property not found: " + property_name);
  }
  return DegradeVecColumn(static_cast<size_t>(col));
}
