...
bool VertexTable::get_index(const columnar::Value& oid, vid_t& lid,
...
bool VertexTable::AddVertex(const columnar::Value& id,
...
bool VertexTable::UpdateProperty(vid_t vid, int32_t prop_id,
                                 const columnar::Value& value, timestamp_t ts) {
...
columnar::Value VertexTable::GetOid(vid_t lid, timestamp_t ts) const {
...
void VertexTable::DeleteVertex(const columnar::Value& id, timestamp_t ts) {
  vid_t vid;
...
vid_t VertexTable::insert_vertex_pk(const columnar::Value& id, timestamp_t ts,
                                    bool insert_safe) {
