...
std::string Schema::get_edge_strategy(label_t src_label, label_t dst_label,
  ...
  // kNone means that CSR side is absent (ONLY_OUT / ONLY_IN). Treat the missing
  // side as kMultiple so dump can emit a relation string; original SINGLE on
  // that side (if any) is not recovered.
