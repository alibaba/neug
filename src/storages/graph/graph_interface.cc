...
neug::result<StorageIndex*> StorageAPUpdateInterface::CreateIndex(
  ...
  if (IsHNSWIndex(*meta)) {
    ...
    GS_AUTO(existing_indexes, index_manager_.GetIndexesContainingProperty(
...
Status StorageAPUpdateInterface::DropIndex(const std::string& name) {
  ...
  if (IsHNSWIndex(meta)) {
    ...
    auto pending_indexes =
        index_manager_.GetPendingIndex(meta.schema.label_id, {property_name});
    if (!pending_indexes) {
