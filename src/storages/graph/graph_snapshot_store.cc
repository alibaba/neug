...
void GraphSnapshotStore::ReplaceCurrentSnapshot(
    std::shared_ptr<PropertyGraph> new_pg) {
  CHECK(new_pg != nullptr);
  GraphView new_view(*new_pg);
  int slot_index = cur_slot_index_.load(std::memory_order_acquire);
  slots_[slot_index].storage_ = std::move(new_pg);
  slots_[slot_index].view_ = std::move(new_view);
}
