...
std::shared_ptr<IColumn> SLVertexColumn::shuffle(
...
std::shared_ptr<IColumn> SLVertexColumn::optional_shuffle(
    const sel_vec_t& offsets) const {
  MSVertexColumnBuilder builder(label_);
  builder.reserve(offsets.size());
  for (auto offset : offsets) {
...
std::pair<std::shared_ptr<IColumn>, vector_t<sel_vec_t>>
...
std::shared_ptr<IColumn> SLVertexColumn::union_col(
    std::shared_ptr<IColumn> other) const {
  CHECK(other->column_type() == ColumnKind::kVertex);
...
std::shared_ptr<IColumn> MSVertexColumn::shuffle(
    const sel_vec_t& offsets) const {
...
std::shared_ptr<IColumn> MSVertexColumn::optional_shuffle(
    const sel_vec_t& offsets) const {
  MLVertexColumnBuilderOpt builder(this->get_labels_set());
  builder.reserve(offsets.size());
  for (auto offset : offsets) {
...
std::shared_ptr<IColumn> MSVertexColumnBuilder::finish() {
  if (!cur_list_.empty()) {
...
std::shared_ptr<IColumn> MLVertexColumn::shuffle(
    const sel_vec_t& offsets) const {
  MLVertexColumnBuilderOpt builder(this->get_labels_set());
  builder.reserve(offsets.size());
  for (auto offset : offsets) {
...
std::shared_ptr<IColumn> MLVertexColumn::optional_shuffle(
    const sel_vec_t& offsets) const {
  MLVertexColumnBuilderOpt builder(this->get_labels_set());
  builder.reserve(offsets.size());
  for (auto offset : offsets) {
...
std::shared_ptr<IColumn> MLVertexColumnBuilder::finish() {
