...
std::shared_ptr<IColumn> SDSLEdgeColumn::shuffle(
...
std::shared_ptr<IColumn> SDSLEdgeColumn::optional_shuffle(
    const sel_vec_t& offsets) const {
  SDSLEdgeColumnBuilder builder(dir_, label_);
  builder.reserve(offsets.size());
  for (auto offset : offsets) {
...
std::shared_ptr<IColumn> SDSLEdgeColumnBuilder::finish() {
...
std::shared_ptr<IColumn> MSEdgeColumn::shuffle(const sel_vec_t& offsets) const {
  if (labels_.size() == 1) {
...
std::shared_ptr<IColumn> MSEdgeColumn::optional_shuffle(
    const sel_vec_t& offsets) const {
  if (labels_.size() == 1) {
    BDSLEdgeColumnBuilder builder(labels_[0]);
    builder.reserve(offsets.size());
...
std::shared_ptr<IColumn> BDSLEdgeColumn::shuffle(
    const sel_vec_t& offsets) const {
...
std::shared_ptr<IColumn> BDSLEdgeColumn::optional_shuffle(
    const sel_vec_t& offsets) const {
  BDSLEdgeColumnBuilder builder(label_);
  builder.reserve(offsets.size());
  for (auto offset : offsets) {
...
std::shared_ptr<IColumn> SDMLEdgeColumn::shuffle(
    const sel_vec_t& offsets) const {
...
std::shared_ptr<IColumn> SDMLEdgeColumn::optional_shuffle(
    const sel_vec_t& offsets) const {
  SDMLEdgeColumnBuilder builder(dir_, labels_);
  builder.reserve(offsets.size());
  for (auto offset : offsets) {
...
std::shared_ptr<IColumn> BDMLEdgeColumn::shuffle(
    const sel_vec_t& offsets) const {
  BDMLEdgeColumnBuilder builder(labels_);
  builder.reserve(offsets.size());
  if (is_optional_) {
...
std::shared_ptr<IColumn> BDMLEdgeColumn::optional_shuffle(
    const sel_vec_t& offsets) const {
  BDMLEdgeColumnBuilder builder(labels_);
  builder.reserve(offsets.size());
  for (auto offset : offsets) {
