...
Value Value::FromJson(const rapidjson::Value& json_value,
  ...
  case DataTypeId::kBoolean: {
    ...
    if (json_value.IsInt()) {
      return columnar::Value::BOOLEAN(json_value.GetInt() != 0);
    }
    return columnar::Value::BOOLEAN(json_value.GetBool());
  }
  case DataTypeId::kDate: {
    if (json_value.IsInt64()) {
      return columnar::Value::DATE(Date(json_value.GetInt64()));
    } else if (json_value.IsString()) {
      return columnar::Value::DATE(Date(json_value.GetString()));
    } else {
  ...
  case DataTypeId::kDouble: {
    return columnar::Value::DOUBLE(json_value.GetDouble());
  }
  case DataTypeId::kFloat: {
    return columnar::Value::FLOAT(json_value.GetFloat());
  }
  case DataTypeId::kInt32: {
    return columnar::Value::INT32(json_value.GetInt());
  }
  case DataTypeId::kInt64: {
    return columnar::Value::INT64(json_value.GetInt64());
  }
  case DataTypeId::kUInt32: {
    return columnar::Value::UINT32(json_value.GetUint());
  }
  case DataTypeId::kUInt64: {
    return columnar::Value::UINT64(json_value.GetUint64());
  }
  case DataTypeId::kVarchar: {
    return columnar::Value::STRING(json_value.GetString());
  }
  case DataTypeId::kTimestampMs: {
    if (json_value.IsInt64()) {
      return columnar::Value::TIMESTAMPMS(
          columnar::timestamp_ms_t(json_value.GetInt64()));
    } else if (json_value.IsString()) {
      return columnar::Value::TIMESTAMPMS(
          columnar::timestamp_ms_t(json_value.GetString()));
    } else {
      THROW_INVALID_ARGUMENT_EXCEPTION(
  ...
  case DataTypeId::kList: {
    std::vector<columnar::Value> values;
    if (!json_value.IsArray()) {
      return columnar::Value::LIST(DataType::UNKNOWN, std::move(values));
    }
    ...
    return columnar::Value::LIST(child_type, std::move(values));
  }
  default:
...
#undef TYPE_DISPATCHER
  case neug::DataTypeId::kList: {
    ...
    const auto& list = columnar::ListValue::GetChildren(value);
...
InArchive& operator<<(InArchive& in_archive, const columnar::Value& value) {
  auto type_id = value.type().id();
  if (value.IsNull()) {
    ...
    in_archive << type_id << columnar::StringValue::Get(value);
  } else if (type_id == DataTypeId::kDate) {
    in_archive << type_id << value.GetValue<columnar::date_t>().to_u32();
  } else if (type_id == DataTypeId::kTimestampMs) {
    in_archive << type_id
               << value.GetValue<columnar::timestamp_ms_t>().milli_second;
  } else if (type_id == DataTypeId::kInterval) {
    auto interval = value.GetValue<columnar::interval_t>();
...
OutArchive& operator>>(OutArchive& out_archive, columnar::Value& value) {
  DataTypeId type_id;
  out_archive >> type_id;
  if (type_id == DataTypeId::kEmpty) {
    ...
    bool tmp;
    out_archive >> tmp;
    value = columnar::Value::BOOLEAN(tmp);
  } else if (type_id == DataTypeId::kInt32) {
    int32_t tmp;
    out_archive >> tmp;
    value = columnar::Value::INT32(tmp);
  } else if (type_id == DataTypeId::kUInt32) {
    uint32_t tmp;
    out_archive >> tmp;
    value = columnar::Value::UINT32(tmp);
  } else if (type_id == DataTypeId::kInt64) {
    int64_t tmp;
    out_archive >> tmp;
    value = columnar::Value::INT64(tmp);
  } else if (type_id == DataTypeId::kUInt64) {
    uint64_t tmp;
    out_archive >> tmp;
    value = columnar::Value::UINT64(tmp);
  } else if (type_id == DataTypeId::kFloat) {
    float tmp;
    out_archive >> tmp;
    value = columnar::Value::FLOAT(tmp);
  } else if (type_id == DataTypeId::kDouble) {
    double tmp;
    out_archive >> tmp;
    value = columnar::Value::DOUBLE(tmp);
  } else if (type_id == DataTypeId::kVarchar) {
    std::string_view tmp;
    out_archive >> tmp;
    value = columnar::Value::STRING(std::string(tmp));
  } else if (type_id == DataTypeId::kDate) {
    uint32_t date_val;
    out_archive >> date_val;
    Date d;
    d.from_u32(date_val);
    value = columnar::Value::DATE(d);
  } else if (type_id == DataTypeId::kTimestampMs) {
    int64_t dt_val;
    out_archive >> dt_val;
    value = columnar::Value::TIMESTAMPMS(DateTime(dt_val));
  } else if (type_id == DataTypeId::kInterval) {
    Interval interval;
    out_archive >> interval.months >> interval.days >> interval.micros;
    value = columnar::Value::INTERVAL(interval);
  } else {
    THROW_NOT_SUPPORTED_EXCEPTION(
