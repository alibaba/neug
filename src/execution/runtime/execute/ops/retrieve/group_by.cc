...
#include "neug/execution/runtime/common/operators/retrieve/group_by.h"
#include "neug/execution/runtime/common/operators/retrieve/project.h"
...
std::unique_ptr<ReducerBase> make_reducer(const GraphReadInterface& graph,
  ...
  if (var_.type() == RTAnyType::kI32Value) {
    ...
    return make_reducer<TimeStamp>(graph, ctx, std::move(var_), kind, alias);
  } else {
