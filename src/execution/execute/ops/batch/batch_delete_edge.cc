             ...
             timer](ContextChunk&& chunk) -> result<ContextChunk> {
                  ...
                  dynamic_cast<StorageUpdateInterface&>(graph_interface);
              {
                ...
                for (size_t i = 0; i < binding_size; i++) {
                  ...
                  auto edge_column =
                      std::dynamic_pointer_cast<IEdgeColumn>(chunk.get(alias));
                  if (edge_triplets.size() == 1) {
                    ...
                    auto edge_prop_types = graph.schema().get_edge_properties(
                        src_v_label, dst_v_label, edge_label);
                    std::vector<std::pair<vid_t, int32_t>> oe_to_delete,
                        ie_to_delete;
                    ...
                    for (size_t j = 0; j < edge_size; j++) {
                      ...
                      if (record.label == request_triplet) {
                        auto offset_pair = record_to_csr_offset_pair(
                            oe_view, ie_view, record, edge_prop_types);
                        oe_to_delete.emplace_back(record.src,
                                                  offset_pair.first);
                        ie_to_delete.emplace_back(record.dst,
                                                  offset_pair.second);
                      }
                    }
                    RETURN_STATUS_ERROR_IF_NOT_OK(graph.BatchDeleteEdges(
                        src_v_label, dst_v_label, edge_label, oe_to_delete,
                        ie_to_delete));
                  } else {
                  ...
                  sel_vec_t offsets;
                  chunk.reshuffle(offsets);  // reshuffle with empty offsets to
                                             // remove all data
                }
                return chunk;
              }
